...
/* <AR0009UD8A modified by leijianfeng/lwx468900 20180118 begin */
#ifdef CONFIG_HW_ZEROHUNG
static int report_zrhung_ID;
static int zrhung_WPhungtask_enable;
#endif
/* AR0009UD8A modified by leijianfeng/lwx468900 20180118 end> */
		...
		if (((p->state == TASK_RUNNING) || (p->state & state_filter))
			...
			if (unlikely(p->flags & PF_FROZEN)) {
				if (taskitem) {
					...
					pr_err("hungtask:name=%s,PID=%d,tgid=%d a new FROZEN task"
						",la:%llu/lq:%llu.\n",
						p->comm, p->pid, p->tgid,
						p->sched_info.last_arrival,
						p->sched_info.last_queued);
				...
				if (taskitem) {
					pr_err("hungtask:name=%s,PID=%d,tgid=%d,type=%d blocked %ds,"
						"la:%llu/lq:%llu.\n", taskitem->name,
						taskitem->pid, p->tgid, taskitem->task_type,
						taskitem->time_in_D_state * HEARTBEAT_TIME,
						p->sched_info.last_arrival, p->sched_info.last_queued);
				} else {
					pr_err("hungtask:name=%s,PID=%d,tgid=%d,la:%llu/lq:%llu.\n",
						p->comm, p->pid, p->tgid,
						p->sched_info.last_arrival, p->sched_info.last_queued);
				}
				sched_show_task(p);
...
extern void show_free_areas(unsigned int filter);
...
static void hungtask_report_zrhung(int event)
{
	...
	if (event & HUNGTASK_EVENT_LAZYWATCHDOG) {
		...
		zrhung_send_event(ZRHUNG_WP_HUNGTASK, "n=system_server", "hungtask: vmreboot watchdog not scheduled for 39s.");
	}
	...
	if (report_load) {
		//show_state_filter_ext(TASK_UNINTERRUPTIBLE);//AR0009LUCQ deleted by leijianfeng lwx468900 20171213 begin
...
#ifdef CONFIG_HW_ZEROHUNG
			zrhung_send_event(ZRHUNG_WP_HUNGTASK, "S,n=system_server", "hungtask: vmreboot watchdog not scheduled for 120s.");
#endif
			watchdog_nosched_cnt = 0;
			pr_err("hungtask: print all running cpu stack and"
				" D state stack due to systemserver watchdog"
				" thread not schedule\n");
			/* < AR0009LUCQ deleted by leijianfeng lwx468900 20171213 begin */
			//show_state_filter_ext(TASK_UNINTERRUPTIBLE);
			//do_panic();
			/* AR0009LUCQ deleted by leijianfeng lwx468900 20171213 end > */
		}
	}
