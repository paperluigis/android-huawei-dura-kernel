...
extern struct pinctrl_state *pins_output0, *pins_output1;
...
#define MAX_ONTIM_LED_DUTY 10000
...
#define BREATH_DURATION_SET_SIZE       (16)
static const unsigned int breath_stage_duration_set1[BREATH_DURATION_SET_SIZE] =
{123, 338, 523, 707, 926, 1107, 1291, 1507, 1691, 1876, 2091, 2276, 2460, 2676, 2860, 3075};
static const unsigned int breath_stage_duration_set2[BREATH_DURATION_SET_SIZE] =
{246, 677, 1046, 1417, 1845, 2214, 2583, 3014, 3383, 3752, 4183, 4552, 4921, 5351, 5720, 6151};
static unsigned int find_closest_index_from_array(const unsigned int array[], unsigned int value, unsigned int array_size)
{
    unsigned int i = 0;

    for (i = 0; i < array_size - 1; i++)
    {
        if (value < (array[i] + array[i + 1]) / 2)//choose the average value for compare
        {
            return i;
        }
    }
...
static bool red_led_timer_init_flag = false;
static bool should_stop_timer_flag = false;
static bool red_led_is_on = false;
static ktime_t time_on;
static struct hrtimer red_led_blink_timer;
static enum hrtimer_restart red_led_blink_func(struct hrtimer *timer)
{
	if (!should_stop_timer_flag)
	{
		if (!red_led_is_on)
		{
			pinctrl_select_state(pinctrled, pins_output1);
			red_led_is_on = true;
			hrtimer_start(&red_led_blink_timer, time_on, HRTIMER_MODE_REL);
		}
		else
		{
			pinctrl_select_state(pinctrled, pins_output0);
			red_led_is_on = false;
			hrtimer_start(&red_led_blink_timer, time_on, HRTIMER_MODE_REL);
		}
	}
...
static void red_led_timer_init(void)
{
	time_on = ktime_set(2, 0);//on and off about 2s
	hrtimer_init(&red_led_blink_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	red_led_blink_timer.function = red_led_blink_func;

	red_led_timer_init_flag = true;
}
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	int time_index = 0;
	int duty = 0;
	int ontim_led_duty =0;
    unsigned int oncode = 0, offcode = 0;
    unsigned int raise_code = 0, fall_code = 0, raise_stage1_code = 0, raise_stage2_code = 0, fall_stage1_code = 0, fall_stage2_code = 0;
	if (!led)
		return -1;
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
	{
		if (!red_led_timer_init_flag)
			red_led_timer_init();
	}
    /*genarate code for registers controling the time duration of each breathing stage*/
    oncode = find_closest_index_from_array(breath_stage_duration_set1, led->blink_on_time, BREATH_DURATION_SET_SIZE);
    offcode = find_closest_index_from_array(breath_stage_duration_set2, led->blink_off_time, BREATH_DURATION_SET_SIZE);

    if ((0 == led->blink_raise_duration) && (0 == led->blink_fall_duration))
    {
        if (500 == led->blink_on_time)//set 500ms
        {
            raise_stage1_code = ISINK_BREATH_STAGE_TIME2;
            raise_stage2_code = ISINK_BREATH_STAGE_TIME2;
            fall_stage1_code = ISINK_BREATH_STAGE_TIME2;
            fall_stage2_code = ISINK_BREATH_STAGE_TIME2;
			LEDS_DEBUG("%s:%d oncode %d offcode %d\n",  __func__, __LINE__, oncode, offcode);
        }
        else if (125 == led->blink_on_time)//set 125ms
        {
            raise_stage1_code = ISINK_BREATH_STAGE_TIME1;
            raise_stage2_code = ISINK_BREATH_STAGE_TIME0;
            fall_stage1_code = ISINK_BREATH_STAGE_TIME1;
            fall_stage2_code = ISINK_BREATH_STAGE_TIME0;
			LEDS_DEBUG("%s:%d oncode %d offcode %d\n",  __func__, __LINE__, oncode, offcode);
        }
        else
        {
            LEDS_DEBUG("led_blink_pmic: on_time == %d\n", led->blink_on_time);
        }
    }
    else
    {
        raise_code = find_closest_index_from_array(breath_stage_duration_set2, led->blink_raise_duration, BREATH_DURATION_SET_SIZE);
        raise_stage1_code = raise_code;
        raise_stage2_code = raise_code;

        fall_code = find_closest_index_from_array(breath_stage_duration_set2, led->blink_fall_duration, BREATH_DURATION_SET_SIZE);
        fall_stage1_code = fall_code;
        fall_stage2_code = fall_code;
		LEDS_DEBUG("%s:%d oncode %d offcode %d\n",  __func__, __LINE__, oncode, offcode);
    }
	...
	pmic_set_register_value(PMIC_CHRIND_MODE, ISINK_BREATH_MODE);
	switch (pmic_type) {
...
#else
	case MT65XX_LED_PMIC_NLED_ISINK0:
		should_stop_timer_flag = false;
		pinctrl_select_state(pinctrled, pins_output1);
		red_led_is_on = true;
		hrtimer_start(&red_led_blink_timer, time_on, HRTIMER_MODE_REL);
		LEDS_DEBUG("%s:%d red_led_is_on %d\n",  __func__, __LINE__, red_led_is_on);
		break;
#endif
	case MT65XX_LED_PMIC_NLED_ISINK1:
		should_stop_timer_flag = true;
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN, 0);
		pmic_set_register_value(PMIC_ISINK_CH1_MODE, ISINK_BREATH_MODE);/*kp*/
		pmic_set_register_value(PMIC_ISINK_CH1_STEP, ISINK_3);	/* 16mA */
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF1_SEL, fall_stage1_code);
        pmic_set_register_value(PMIC_ISINK_BREATH1_TF2_SEL, fall_stage2_code);
        pmic_set_register_value(PMIC_ISINK_BREATH1_TR1_SEL, raise_stage1_code);
        pmic_set_register_value(PMIC_ISINK_BREATH1_TR2_SEL, raise_stage2_code);
        pmic_set_register_value(PMIC_ISINK_BREATH1_TON_SEL, oncode);
        pmic_set_register_value(PMIC_ISINK_BREATH1_TOFF_SEL, offcode);
		pmic_set_register_value(PMIC_ISINK_CH1_EN, NLED_ON);
		break;
	default:
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK0) {
		should_stop_timer_flag = true;
		LEDS_DEBUG("%s:%d red_led_is_on %d should_stop_timer_flag %d\n",  __func__, __LINE__, red_led_is_on, should_stop_timer_flag);
		if (level)
		{
			LEDS_DEBUG("%s %d", __func__, __LINE__);
			pinctrl_select_state(pinctrled, pins_output1);
		}
		else
		{
		...
		pmic_set_register_value(PMIC_CHRIND_MODE, ISINK_PWM_MODE);
		pmic_set_register_value(PMIC_RG_DRV_128K_CK_PDN, 0x0);	/* Disable power down */
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN, 0);
			...
			unsigned long *delay_on, unsigned long *delay_off)
{
	...
	if (!led_cdev || !led_data || !delay_on || !delay_off)
		return -1;
	/* only allow software blink when delay_on or delay_off changed */
	LEDS_DEBUG("%s:%d delay_on %d delay_off %d\n",  __func__, __LINE__, *delay_on, *delay_off);
	if (*delay_on != led_data->delay_on
        || *delay_off != led_data->delay_off
        || led_cdev->blink_raise_duration != led_data->raise_duration
        || led_cdev->blink_fall_duration != led_data->fall_duration) {
            led_data->delay_on = *delay_on;
            led_data->delay_off = *delay_off;
            led_data->raise_duration = led_cdev->blink_raise_duration;
            led_data->fall_duration = led_cdev->blink_fall_duration;
		if (led_data->delay_on && led_data->delay_off) {	/* enable blink */
			...
			if (led_data->cust.mode == MT65XX_LED_MODE_PWM) {
				...
				nled_tmp_setting.blink_on_time =
				    led_data->delay_on;
                nled_tmp_setting.blink_raise_duration =
                    led_data->raise_duration;
                nled_tmp_setting.blink_fall_duration =
                    led_data->fall_duration;
