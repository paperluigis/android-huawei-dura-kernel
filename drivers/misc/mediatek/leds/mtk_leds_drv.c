...
struct pinctrl_state *pins_output0, *pins_output1;
int mt_led_get_gpio_info(struct platform_device *pdev)
{
	int ret;
	...
	if (IS_ERR(pinctrled)) 
	{
		...
		dev_err(&pdev->dev, "Cannot find leds pinctrl1!\n");
		return ret;
	}
	pins_output0 = pinctrl_lookup_state(pinctrled, "led_output0");
	if (IS_ERR(pins_output0)) 
	{
		ret = PTR_ERR(pins_output0);
		dev_err(&pdev->dev, "Cannot find leds pinctrl1!\n");
		return ret;
	}
	pins_output1 = pinctrl_lookup_state(pinctrled, "led_output1");
	if (IS_ERR(pins_output1)) 
	{
		ret = PTR_ERR(pins_output1);
		dev_err(&pdev->dev, "Cannot find leds pinctrl1!\n");
		return ret;
	}
	pr_err("%s %d\n", __func__, __LINE__);
...
static const struct of_device_id mt65xx_led_simple[]  = {
{ .compatible = "mt65xx_leds", },
{}
};
...
static struct platform_driver mt65xx_leds_driver = {
	.driver = {
		   .name = "leds-mt65xx",
		   .owner = THIS_MODULE,
		   .of_match_table = mt65xx_led_simple,
		   },
