...
#define  STEP_C_ERR STEP_C_PR_ERR
	...
	else
	{
		printk("kx126_sc_enable_nodata2\n");
		 buf[1] = buf[0] & (~0x02); 
    		...
    		err = kx126_i2c_write_step(0x1a, &buf[1], 0x01);
		if (err) {
			STEP_C_ERR("KX126_1063_write Data error: %d %d\n", err,buf[1]);
       		return err;	
   		 }
	}
...
static int kx126_floor_c_enable_nodata(int en)
{
       STEP_C_LOG("kx126_floor_c_enable_nodata\n");
       return 0;
}
static int kx126_get_data_floor_c(uint32_t *value, int *status)
{
    return 0;
}
...
static int kx126_step_c_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}
static int kx126_floor_c_flush(void)
{
    int err = 0;
...
static int kx126_floor_c_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}
...
static int kx126_sc_attr_create(void)
{
    ...
    ctl.step_c_batch = kx126_step_c_batch;
    ctl.floor_c_flush = kx126_floor_c_flush;
    ctl.floor_c_batch = kx126_floor_c_batch;
    ctl.enable_floor_c = kx126_floor_c_enable_nodata;
    ...
    data.get_data_floor_c = kx126_get_data_floor_c;
