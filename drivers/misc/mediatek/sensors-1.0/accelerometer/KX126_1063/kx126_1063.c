...
static int KX126_1063_suspend(struct i2c_client *client, pm_message_t msg);
static int KX126_1063_resume(struct i2c_client *client);
...
#define KX126_BUF_SIZE    256
#define KX126_RETCODE_SUCCESS                 (0)
static char    acc_file_path[KX126_BUF_SIZE]        = "/bak/kx126-calib.txt";
static char    backup_file_path[KX126_BUF_SIZE] = "/dev/block/platform/bootdevice/by-name/proinfo";
static int kx126_cali(int temp);
static int    s_CaliLoadEnable = false;  //modify by hzb
static int kx126_audo_cali_enable=0;   //0--disable auto calib; 1--enable auto calib
static int kx126_cali_enable=0;   // 0--stop calib; 1--start calib; 2--calib runing;
...
static char version[]="kx126_mtk_1.0";
static char vendor_name[20]="Kionix-kx126";
    DEV_ATTR_DECLARE(gsensor)
    DEV_ATTR_DEFINE("version",version)
    DEV_ATTR_DEFINE("vendor",vendor_name)
#ifdef ONTIM_CALI
    DEV_ATTR_EXEC_DEFINE("gsensor_cali",&kx126_cali)
#endif	
...
static int kx126_cali(int temp)
{
        int loop=0;
        kx126_cali_enable=1;
        while((loop<100) && (kx126_cali_enable))
        {
                msleep(20);
                loop++;
        }
...
static int kx126_write_cali_file(int raw_data[3])
{
    ...
    if (fd_file == NULL)
    {
        GSE_LOG("kx126_write_log_data fail to open\n");
...
static int KX126_auto_cali_condition_confirm(int x, int y, int z, int ave_xyz[3])
{
    int    max = 0, min = 0;
    int    i;
...
static void    kx126_load_cali(struct i2c_client *client);
static int KX126_do_audo_cali(int data[3])
{
    int ret=0;
    static int save_count=0;
    int cali_data[3]={0};
    if (KX126_auto_cali_condition_confirm(data[0], data[1], data[2], cali_data)==0)
    {
        if (kx126_write_cali_file(cali_data)==0)
        {
            s_CaliLoadEnable = true;
            s_nIsCaliLoaded=false;
            kx126_audo_cali_enable=3;
            kx126_cali_enable=0;
            save_count = 0;
        }
...
static int kx126_read_cali_file(struct i2c_client *client)
{
    int cali_data[3] = {0};
    int err = 0;
...
static void    kx126_load_cali(struct i2c_client *client)
{
    ...
    if ((false == s_nIsCaliLoaded) && (s_CaliLoadEnable))  //modify by hzb
    {
        ...
        if (KX126_RETCODE_SUCCESS == kx126_read_cali_file(client))
        {
...
#if !defined(CONFIG_HAS_EARLYSUSPEND) || !defined(USE_EARLY_SUSPEND)
    .suspend = KX126_1063_suspend,
    .resume = KX126_1063_resume,
#endif
...
{
    struct KX126_1063_i2c_data *priv = NULL;
    int err = 0;
...
static int KX126_1063_WriteCalibration(struct i2c_client *client, int dat[KX126_1063_AXES_NUM])
{
    struct KX126_1063_i2c_data *obj = i2c_get_clientdata(client);
    int err;
    int cali[KX126_1063_AXES_NUM] = {0};
    int raw[KX126_1063_AXES_NUM] = {0};
#ifdef CUSTOM_KERNEL_SENSORHUB
    SCP_SENSOR_HUB_DATA data;
    KX126_1063_CUST_DATA *pCustData;
    unsigned int len;
...
{
    struct KX126_1063_i2c_data *obj = NULL;
    u8 databuf[20];
	...
	obj = (struct KX126_1063_i2c_data *)i2c_get_clientdata(client);
	if (NULL == obj) {
		GSE_ERR("obj is NULL\n");
		return -1;
	}
    if (atomic_read(&obj->suspend))
        return 0;
#ifdef ONTIM_CALI
                kx126_load_cali(client);
                if (((s_nIsCaliLoaded==false)&&(kx126_audo_cali_enable==1))||(kx126_cali_enable==1))
                {
            for(i=0;i<3;i++)
                cali_offset[i]=0;
                        kx126_audo_cali_enable=2;  //auto calib runing
                        kx126_cali_enable =2;
                        s_CaliLoadEnable = false;
                        mdelay(50);
                }
...
#ifdef ONTIM_CALI
        if (kx126_audo_cali_enable==2)
        {
                KX126_do_audo_cali(acc);
        }
#endif
    return 0;
}
...
static void KX126_1063_early_suspend(struct early_suspend *h)
{
    ...
    GSE_FUN();
    printk("KX126_1063_early_suspend\n");
...
static int kx126_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}
static int kx126_flush(void)
{
    int err = 0;
...
//-add by hzb for ontim debug
	...
	ctl.batch = kx126_batch;
    ctl.flush = kx126_flush;
#ifdef CUSTOM_KERNEL_SENSORHUB
    ...
    if (obj->hw->calib_enable)
        {
        kx126_audo_cali_enable = (obj->hw->auto_calib_enable)?1:0;   //  0--disable auto clib;  1--enable auto clib; 2--auto calib runing; 3--auto calib OK
        s_CaliLoadEnable  = true;
        }
#endif
//+add by hzb for ontim debug
...
static int KX126_1063_i2c_remove(struct i2c_client *client)
{
    ...
    //err = misc_deregister(&KX126_1063_device);
   // if (0 != err)
...
{
    const char *name = "mediatek,KX126_1063";
    ...
    i2c_register_board_info(hw->i2c_num, &i2c_KX126_1063, 1);
