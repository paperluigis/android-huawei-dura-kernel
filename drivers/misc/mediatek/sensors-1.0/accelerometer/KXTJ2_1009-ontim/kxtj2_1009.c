/*****************************************************************************
 *
 * Kionix KXTJ2_1009 accelerometer driver for Mediatek
 *
...
#define KX_BUF_SIZE    256
#define KX_RETCODE_SUCCESS                 (0)

static char    acc_file_path[KX_BUF_SIZE]        = "/bak/kx-calib.txt";
static char    backup_file_path[KX_BUF_SIZE] = "/dev/block/platform/bootdevice/by-name/proinfo";
static int kx_cali(int temp);
static int    s_CaliLoadEnable = false;  //modify by hzb
static int kx_audo_cali_enable=0;   //0--disable auto calib; 1--enable auto calib
static int kx_cali_enable=0;   // 0--stop calib; 1--start calib; 2--calib runing;
...
static char kx_prox_version[]="kxtj3_mtk_1.0";
static char kx_prox_vendor_name[20]="KXTJ3-1058";
    DEV_ATTR_DECLARE(gsensor)
    DEV_ATTR_DEFINE("version",kx_prox_version)
    DEV_ATTR_DEFINE("vendor",kx_prox_vendor_name)
#ifdef ONTIM_CALI
    DEV_ATTR_EXEC_DEFINE("gsensor_cali",&kx_cali)
#endif
...
static int kx_write_cali_file(int raw_data[3])
{
    ...
    if (fd_file == NULL)
    {
        GSE_LOG("kx_write_log_data fail to open\n");
...
static int KX_auto_cali_condition_confirm(int x, int y, int z, int ave_xyz[3])
{
    int    max = 0, min = 0;
    int    i;
...
static void    kx_load_cali(struct i2c_client *client);
static int KX_do_audo_cali(int data[3])
{
    int ret=0;
    ...
    if (KX_auto_cali_condition_confirm(data[0], data[1], data[2], cali_data)==0)
    {
        if (kx_write_cali_file(cali_data)==0)
        {
            s_CaliLoadEnable = true;
            s_nIsCaliLoaded=false;
            kx_audo_cali_enable=3;
            kx_cali_enable=0;
            //kx_load_cali(kx_i2c_client);
            save_count = 0;
        }
 ...
 *** kx_read_cali_file
 *****************************************/
static int kx_read_cali_file(struct i2c_client *client)
{
    int cali_data[3] = {0};
    int err = 0;
...
static void    kx_load_cali(struct i2c_client *client)
{
    ...
    if ((false == s_nIsCaliLoaded) && (s_CaliLoadEnable))  //modify by hzb
    {
        ...
        if (KX_RETCODE_SUCCESS == kx_read_cali_file(client))
        {
...
{
// TODO - temporary disable. verify usage in MTK platform
#if 0
...
{
    struct kxtj2_1009_i2c_data *priv = NULL;
    int err = 0;
...
static int KXTJ2_1009_WriteCalibration(struct i2c_client *client, int dat[KXTJ2_1009_AXES_NUM])
{
    struct kxtj2_1009_i2c_data *obj = i2c_get_clientdata(client);
    int err;
    int cali[KXTJ2_1009_AXES_NUM] = {0};
    int raw[KXTJ2_1009_AXES_NUM] = {0};
#ifdef SW_CALIBRATION
#else
...
{
    struct kxtj2_1009_i2c_data *obj = NULL;
    u8 databuf[20];
...
#ifdef ONTIM_CALI
                kx_load_cali(client);
                if (((s_nIsCaliLoaded==false)&&(kx_audo_cali_enable==1))||(kx_cali_enable==1))
                {
            for(i=0;i<3;i++)
                cali_offset[i]=0;
                        kx_audo_cali_enable=2;  //auto calib runing
                        kx_cali_enable =2;
                        s_CaliLoadEnable = false;
                        mdelay(50);
                }
...
static ssize_t kx_poll_test(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    sscanf(buf, "%d", &poll_delay_ms);
    printk("kx_poll_test - poll_delay_ms = %d \n", poll_delay_ms);
       ...
       unsigned long arg)
{
    ...
    case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
        ...
        err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_SENSORDATA, (unsigned long)compat_ptr(arg));
        if (err){
...
static int kxt_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}
static int kxt_flush(void)
{
    int err = 0;
...
//-add by hzb for ontim debug
...
#ifdef KIONIX_TEST
    if(0 != (err = kxtj2_1009_create_attr(kxtj2_1009_i2c_client->dev.driver)))
#else
    ...
    if (obj->hw->calib_enable)
        {
        kx_audo_cali_enable = (obj->hw->auto_calib_enable)?1:0;   //  0--disable auto clib;  1--enable auto clib; 2--auto calib runing; 3--auto calib OK
        s_CaliLoadEnable  = true;
        }
#endif
//+add by hzb for ontim debug
...
static int __init kxtj2_1009_init(void)
{
    ...
    i2c_register_board_info(ihw->i2c_num, &i2c_kxtj2_1009, 1);
    GSE_LOG("%s: i2c_number=%d\n", __func__, ihw->i2c_num);
...
/*----------------------------------------------------------------------------*/
MODULE_DESCRIPTION("KXTJ2_1009 G-Sensor Driver");
