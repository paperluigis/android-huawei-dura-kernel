...
static struct trim_data_t bma4_mag_trim;
/* check for Mag auto/manual mode */
static uint8_t V_bmm150_manual_auto_condition;
/***************************************************************************/
 ...
 *
 */
static void unpack_acc_frm(struct bma4_accel *acc, uint16_t *idx, uint16_t *acc_idx, uint8_t frm, const struct bma4_dev *dev);
 ...
 *
 */
static uint16_t unpack_mag_frm(struct bma4_mag *data, uint16_t *idx, uint16_t *mag_idx, uint8_t frm, const struct bma4_dev *dev);
 ...
 *
 */
static void comp_for_grvty(uint16_t lsb_per_g, const int32_t g_val[3], const struct bma4_accel *data, struct offset_delta *comp_data);
/*!
...
uint16_t bma455_init(struct bma4_dev *dev)
{
	...
	if ((dev == NULL) || (dev->bus_read == NULL) || (dev->bus_write == NULL)) {
		...
		rslt |= bma455_read_regs(BMA4_CHIP_ID_ADDR, &data, 1, dev);
...
uint16_t bma455_write_config_file(struct bma4_dev *dev)
{
	...
	rslt |= bma455_write_regs(BMA4_POWER_CONF_ADDR, &adv_power_save, 1, dev);
	...
	if (rslt == BMA4_OK) {
		...
		rslt |= bma455_write_regs(BMA4_INIT_CTRL_ADDR, &config_load, 1, dev);
		...
		rslt |= bma455_read_regs(BMA4_INTERNAL_STAT, &config_stream_status, 1, dev);
 ...
 */
uint16_t bma455_write_regs(uint8_t addr, uint8_t *data, uint16_t len, struct bma4_dev *dev)
{
	uint8_t i;
	uint8_t loop_count;
	uint8_t overflow;
	...
	if (addr == BMA4_FEATURE_CONFIG_ADDR) {
		if (dev->read_write_len < dev->feature_len) {
			/* Calculate the no of writes to be performed according
			to the read/write length */
			loop_count = dev->feature_len / dev->read_write_len;
			overflow = dev->feature_len % dev->read_write_len;
			index = 0;
			...
			rslt = write_regs(BMA4_FEATURE_CONFIG_ADDR, data, dev->feature_len, dev);
		}
	} else {
 ...
 */
uint16_t bma455_read_regs(uint8_t addr, uint8_t *data, uint16_t len, struct bma4_dev *dev)
{
	uint8_t i;
	uint8_t loop_count;
	uint8_t overflow;
	...
	if (addr == BMA4_FEATURE_CONFIG_ADDR) {
		if (dev->read_write_len < dev->feature_len) {
			...
			rslt = read_regs(BMA4_FEATURE_CONFIG_ADDR, data, dev->feature_len, dev);
		}
	} else {
 ...
 */
uint16_t bma455_get_error_status(struct bma4_err_reg *err_reg, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_ERROR_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK) {
			/* Fatal error*/
			err_reg->fatal_err = BMA4_GET_BITSLICE(data, BMA4_FATAL_ERR);
			/* Cmd error*/
 ...
 */
uint16_t bma455_get_status(uint8_t *status, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		/* Read the error codes*/
		rslt |= bma455_read_regs(BMA4_STATUS_ADDR, &data, 1, dev);
 ...
 */
uint16_t bma455_read_accel_xyz(struct bma4_accel *accel, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint16_t lsb = 0;
	uint16_t msb = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_DATA_8_ADDR, data, BMA4_ACCEL_DATA_LENGTH, dev);
 ...
 */
uint16_t bma455_get_sensor_time(uint32_t *sensor_time, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_SENSORTIME_0_ADDR, data, BMA4_SENSOR_TIME_LENGTH, dev);
 ...
 */
uint16_t bma455_get_temperature(int32_t *temp, uint8_t temp_unit, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_TEMPERATURE_ADDR, data, BMA4_TEMP_DATA_SIZE, dev);
 ...
 *	and Range of accel.
 */
uint16_t bma455_get_accel_config(struct bma4_accel_config *accel, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data[2] = {0};
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_ACCEL_CONFIG_ADDR, data, BMA4_ACCEL_CONFIG_LENGTH, dev);

		if (rslt == BMA4_OK) {
			/* To get the ODR */
			accel->odr =  BMA4_GET_BITSLICE(data[0], BMA4_ACCEL_ODR);
			...
			accel->range = BMA4_GET_BITSLICE(data[1], BMA4_ACCEL_RANGE);
		}
	}
 ...
 *	and range of Accel.
 */
uint16_t bma455_set_accel_config(const struct bma4_accel_config *accel, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
		...
		if (rslt == BMA4_OK) {
			...
			rslt |= validate_odr(accel->odr);
				...
				if (rslt == BMA4_OK) {
					...
					rslt |= bma455_write_regs(BMA4_ACCEL_CONFIG_ADDR, &accel_config_data[0], 1, dev);
					dev->delay(BMA4_GEN_READ_WRITE_DELAY);
					rslt |= bma455_write_regs((BMA4_ACCEL_CONFIG_ADDR + 1), &accel_config_data[1], 1, dev);
				}
		}
	}
 ...
 */
uint16_t bma455_set_advance_power_save(uint8_t adv_pwr_save, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_POWER_CONF_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK) {
			data = BMA4_SET_BITSLICE(data, BMA4_ADVANCE_POWER_SAVE, adv_pwr_save);
			rslt |= bma455_write_regs(BMA4_POWER_CONF_ADDR, &data, 1, dev);
		}
	}
 ...
 *	from the sensor.
 */
uint16_t bma455_get_advance_power_save(uint8_t *adv_pwr_save, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (rslt == BMA4_OK)
			*adv_pwr_save = BMA4_GET_BITSLICE(data, BMA4_ADVANCE_POWER_SAVE);
	}
 ...
 */
uint16_t bma455_set_fifo_self_wakeup(uint8_t fifo_self_wakeup, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 *	the sensor.
 */
uint16_t bma455_get_fifo_self_wakeup(uint8_t *fifo_self_wake_up, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_set_accel_enable(uint8_t accel_en, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_POWER_CTRL_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK) {
			data = BMA4_SET_BITSLICE(data, BMA4_ACCEL_ENABLE, accel_en);
			rslt |= bma455_write_regs(BMA4_POWER_CTRL_ADDR, &data, 1, dev);
		}
	}
 ...
 */
uint16_t bma455_get_accel_enable(uint8_t *accel_en, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 *	in the sensor.
 */
uint16_t bma455_set_mag_enable(uint8_t mag_en, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (rslt == BMA4_OK) {
			data = BMA4_SET_BITSLICE(data, BMA4_MAG_ENABLE, mag_en);
			rslt |= bma455_write_regs(BMA4_POWER_CTRL_ADDR, &data, 1, dev);
		}
	}
 ...
 */
uint16_t bma455_get_mag_enable(uint8_t *mag_en, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (rslt == BMA4_OK)
			*mag_en = BMA4_GET_BITSLICE(data, BMA4_MAG_ENABLE);
	}
 ...
 *	interface.
 */
uint16_t bma455_get_spi_interface(uint8_t *spi, struct bma4_dev *dev)
{
	uint16_t rslt  = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		/* Read SPI mode */
		rslt |= bma455_read_regs(BMA4_IF_CONFIG_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK)
			*spi = BMA4_GET_BITSLICE(data, BMA4_CONFIG_SPI3);
	}
 ...
 */
uint16_t bma455_set_spi_interface(uint8_t spi, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (spi <= BMA4_MAX_VALUE_SPI3) {
			...
			if (rslt == BMA4_OK) {
				data = BMA4_SET_BITSLICE(data, BMA4_CONFIG_SPI3, spi);
				rslt |= bma455_write_regs(BMA4_IF_CONFIG_ADDR, &data, 1, dev);
			}
		} else {
			rslt |= BMA4_E_OUT_OF_RANGE;
		}
 ...
 *	to the sensor.
 */
uint16_t bma455_set_command_register(uint8_t command_reg, struct bma4_dev *dev)
{
	uint16_t rslt  = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_write_regs(BMA4_CMD_ADDR, &command_reg, 1, dev);
	}
 ...
 */
uint16_t bma455_set_i2c_device_addr(struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_AUX_DEV_ID_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK) {
			dev_id = BMA4_SET_BITSLICE(data, BMA4_I2C_DEVICE_ADDR, dev->aux_config.aux_dev_addr);
			rslt |= bma455_write_regs(BMA4_AUX_DEV_ID_ADDR, &dev_id, 1, dev);
		}
	}
 ...
 */
uint16_t bma455_set_mag_manual_enable(uint8_t mag_manual, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	/* Check the bma4 structure as NULL */
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_AUX_IF_CONF_ADDR, &data, 1, dev);
		dev->delay(BMA4_GEN_READ_WRITE_DELAY);

		if (rslt == BMA4_OK) {
			/* Set the bit of Mag manual enable */
			data = BMA4_SET_BITSLICE(data, BMA4_MAG_MANUAL_ENABLE, mag_manual);
			rslt |= bma455_write_regs(BMA4_AUX_IF_CONF_ADDR, &data, 1, dev);
 ...
 */
uint16_t bma455_get_mag_manual_enable(uint8_t *mag_manual, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 *	for auxiliary Mag.
 */
uint16_t bma455_set_aux_if_mode(uint8_t if_mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_get_mag_read_addr(uint8_t *mag_read_addr, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_AUX_RD_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK)
			*mag_read_addr = BMA4_GET_BITSLICE(data, BMA4_READ_ADDR);
		}
 ...
 *	where the data to be read.
 */
uint16_t bma455_set_mag_read_addr(uint8_t mag_read_addr, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_write_regs(BMA4_AUX_RD_ADDR, &mag_read_addr, 1, dev);
	}
 ...
 */
uint16_t bma455_get_mag_write_addr(uint8_t *mag_write_addr, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_AUX_WR_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK)
			*mag_write_addr = BMA4_GET_BITSLICE(data, BMA4_WRITE_ADDR);
	}
 ...
 *	Mag write address is where the Mag data will be written.
 */
uint16_t bma455_set_mag_write_addr(uint8_t mag_write_addr, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL)
		...
		rslt |= bma455_write_regs(BMA4_AUX_WR_ADDR, &mag_write_addr, 1, dev);
 ...
 *	Mag.
 */
uint16_t bma455_get_mag_write_data(uint8_t *mag_write_data, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_AUX_WR_DATA_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK)
			*mag_write_data = BMA4_GET_BITSLICE(data, BMA4_WRITE_DATA);
	}
 ...
 *	be written to Mag.
 */
uint16_t bma455_set_mag_write_data(uint8_t mag_write_data, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL)
		...
		rslt |= bma455_write_regs(BMA4_AUX_WR_DATA_ADDR, &mag_write_data, 1, dev);
 ...
 */
uint16_t bma455_read_mag_xyzr(struct bma4_mag_xyzr *mag, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint16_t msb = 0;
	uint16_t lsb = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_DATA_0_ADDR, data, BMA4_MAG_XYZR_DATA_LENGTH, dev);
 ...
 *	Mag sensor.
 */
uint16_t bma455_set_mag_burst(uint8_t mag_burst, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (rslt == BMA4_OK) {
			data = BMA4_SET_BITSLICE(data, BMA4_MAG_BURST, mag_burst);
			rslt |= bma455_write_regs(BMA4_AUX_IF_CONF_ADDR, &data, 1, dev);
		}
	}
 ...
 */
uint16_t bma455_get_mag_burst(uint8_t *mag_burst, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (rslt == BMA4_OK)
			*mag_burst = BMA4_GET_BITSLICE(data, BMA4_MAG_BURST);
	}
 ...
 */
uint16_t bma455_read_fifo_data(struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL || dev->fifo == NULL || dev->fifo->data == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_FIFO_CONFIG_1_ADDR, &data, 1, dev);
 ...
 */
uint16_t bma455_extract_accel(struct bma4_accel *accel_data, uint16_t *accel_length, const struct bma4_dev *dev)
{
	uint16_t rslt = 0;
 ...
 */
uint16_t bma455_extract_mag(struct bma4_mag *mag_data, uint16_t *mag_length, const struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint16_t data_index = 0;
 ...
 *	in the sensor.
 */
uint16_t bma455_get_fifo_wm(uint16_t *fifo_wm, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data[2] = {0, 0};
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_FIFO_WTM_0_ADDR, data, BMA4_FIFO_WM_LENGTH, dev);
 ...
 */
uint16_t bma455_set_fifo_wm(uint16_t fifo_wm, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data[2] = {0, 0};
	...
	if (dev == NULL) {
		...
		rslt |= bma455_write_regs(BMA4_FIFO_WTM_0_ADDR, &data[0], 1, dev);
		dev->delay(BMA4_GEN_READ_WRITE_DELAY);
		rslt |= bma455_write_regs((BMA4_FIFO_WTM_0_ADDR + 1), &data[1], 1, dev);
	}
 ...
 *	or unfiltered mode.
 */
uint16_t bma455_get_accel_fifo_filter_data(uint8_t *accel_fifo_filter, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
		...
		if (dev == NULL) {
			...
			rslt |= bma455_read_regs(BMA4_FIFO_DOWN_ADDR, &data, 1, dev);
 ...
 */
uint16_t bma455_set_accel_fifo_filter_data(uint8_t accel_fifo_filter, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (accel_fifo_filter <= BMA4_MAX_VALUE_FIFO_FILTER) {
			...
			if (rslt == BMA4_OK) {
				...
				rslt |= bma455_write_regs(BMA4_FIFO_DOWN_ADDR, &data, 1, dev);
			}
		} else {
			rslt |= BMA4_E_OUT_OF_RANGE;
		}
 ...
 *	for Accel FIFO data.
 */
uint16_t bma455_get_fifo_down_accel(uint8_t *fifo_down, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_set_fifo_down_accel(uint8_t fifo_down, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_get_fifo_length(uint16_t *fifo_length, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t index = 0;
	uint8_t data[BMA4_FIFO_DATA_LENGTH] = {0, 0};
		...
		if (dev == NULL) {
			...
			/* Read FIFO length*/
			rslt |= bma455_read_regs(BMA4_FIFO_LENGTH_0_ADDR, data, BMA4_FIFO_DATA_LENGTH, dev);

			if (rslt == BMA4_OK) {
				index = BMA4_FIFO_LENGTH_MSB_BYTE;
				data[index] = BMA4_GET_BITSLICE(data[index], BMA4_FIFO_BYTE_COUNTER_MSB);
 ...
 *	sensor.
 */
uint16_t bma455_second_if_mag_compensate_xyz(struct bma4_mag_fifo_data mag_fifo_data,
						uint8_t mag_second_if, struct bma4_mag *compensated_mag_data)
{
	...
	case BMA4_SEC_IF_BMM150:
		...
		compensated_mag_data->x = bma455_bmm150_mag_compensate_X(mag_x, mag_r);

		/* Compensated Mag y data */
		compensated_mag_data->y = bma455_bmm150_mag_compensate_Y(mag_y, mag_r);

		/* Compensated Mag z data */
		compensated_mag_data->z = bma455_bmm150_mag_compensate_Z(mag_z, mag_r);
		break;
#endif
 ...
 *	AKM9916 sensor
 */
uint16_t bma455_read_mag_xyz(struct bma4_mag *mag, uint8_t sensor_select, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		case BMA4_SEC_IF_BMM150:
			rslt |= bma455_read_regs(BMA4_DATA_0_ADDR, data, BMA4_MAG_XYZ_DATA_LENGTH, dev);
 ...
 */
uint16_t bma455_get_if_mode(uint8_t *if_mode, struct bma4_dev *dev)
{
	uint16_t rslt  = 0;
	uint8_t data = 0;
 ...
 *	sensor.
 */
uint16_t bma455_set_if_mode(uint8_t if_mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_get_accel_data_rdy(uint8_t *data_rdy, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_get_mag_data_rdy(uint8_t *data_rdy, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 */
uint16_t bma455_get_asic_status(struct bma4_asic_status *asic_status, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_INTERNAL_ERROR, &data, 1, dev);
 ...
 */
uint16_t bma455_set_offset_comp(uint8_t offset_en, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_NV_CONFIG_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK) {
			/* Write Accel FIFO filter data */
			data = BMA4_SET_BITSLICE(data, BMA4_NV_ACCEL_OFFSET, offset_en);
			rslt |= bma455_write_regs(BMA4_NV_CONFIG_ADDR, &data, 1, dev);
		}
	}
 ...
 */
uint16_t bma455_get_offset_comp(uint8_t *offset_en, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
 ...
 *	multiples of 1000000.
 */
uint16_t bma455_perform_accel_foc(const int32_t accel_g_value[3], struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (BMA4_OK ==  rslt) {
		...
		if (BMA4_OK == rslt) {
			...
			for (i = 0; i < 10; i++) {
				dev->delay(20);
				rslt |= bma455_read_accel_xyz(&accel_value[i], dev);
			...
			if (BMA4_OK == rslt) {
				...
				rslt |= bma455_write_regs(BMA4_OFFSET_0_ADDR, (uint8_t *)&offset.x, 1, dev);
				rslt |= bma455_write_regs(BMA4_OFFSET_1_ADDR, (uint8_t *)&offset.y, 1, dev);
				rslt |= bma455_write_regs(BMA4_OFFSET_2_ADDR, (uint8_t *)&offset.z, 1, dev);

				/* Enable offset compensation */
				rslt |= bma455_set_offset_comp(BMA4_ENABLE, dev);

				/* Set accel config, accel enable and advance power save */
				rslt |= bma455_set_accel_config(&acc_conf, dev);
				rslt |= bma455_set_accel_enable(accel_en, dev);
				rslt |= bma455_set_advance_power_save(adv_pwr_save, dev);
			} else {
 ...
 *	is working or not.
 */
uint16_t bma455_perform_accel_selftest(uint8_t *result, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	*result = BMA4_SELFTEST_FAIL;
	...
	if (dev == NULL) {
		...
		dev->delay(20);
		rslt |= bma455_selftest_config(BMA4_ENABLE, dev);

		if (rslt == BMA4_OK) {

			dev->delay(100);
			rslt |= bma455_read_accel_xyz(&positive, dev);
			rslt |= bma455_selftest_config(BMA4_DISABLE, dev);

			if (rslt == BMA4_OK) {

				dev->delay(100);
				rslt |= bma455_read_accel_xyz(&negative, dev);
				...
				rslt |= bma455_set_command_register(0xB6, dev);
				dev->delay(200);
			}
		}
	}
 ...
 */
uint16_t bma455_selftest_config(uint8_t sign, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
...
uint16_t bma455_map_interrupt(uint8_t int_line, uint16_t int_map, uint8_t enable, struct bma4_dev *dev)
{
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_INT_MAP_1_ADDR, data, 3, dev);
		...
		if (enable == TRUE) {
			...
			rslt |= bma455_write_regs(index[int_line], &data[int_line], 1, dev);
			rslt |= bma455_write_regs(BMA4_INT_MAP_DATA_ADDR,	&data[2], 1, dev);
 ...
 */
uint16_t bma455_set_interrupt_mode(uint8_t mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		if (mode == BMA4_NON_LATCH_MODE || mode == BMA4_LATCH_MODE)
			rslt |= bma455_write_regs(BMA4_INTR_LATCH_ADDR, &mode, 1, dev);
		else
			rslt |= BMA4_E_OUT_OF_RANGE;
	}
 ...
 */
uint16_t bma455_get_interrupt_mode(uint8_t *mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_INTR_LATCH_ADDR, &data, 1, dev);
		*mode = data;
	}
 ...
 *	rate and offset.
 */
uint16_t bma455_set_aux_mag_config(const struct bma4_aux_mag_config *aux_mag, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		&& ((aux_mag->offset & BMA4_MAG_CONFIG_OFFSET_MSK) == 0x00)) {
			...
			rslt |= bma455_write_regs(BMA4_AUX_CONFIG_ADDR, &data, 1, dev);
		} else {
			rslt |= BMA4_E_OUT_OF_RANGE;
		}
	}
 ...
 *	rate and offset.
 */
uint16_t bma455_get_aux_mag_config(struct bma4_aux_mag_config *aux_mag, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_AUX_CONFIG_ADDR, &data, 1, dev);
 ...
 */
uint16_t bma455_set_fifo_config(uint8_t config, uint8_t enable, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data[2] = {0, 0};
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_FIFO_CONFIG_0_ADDR, data, BMA4_FIFO_CONFIG_LENGTH, dev);
		...
		if (rslt == BMA4_OK) {
			...
			separate write is used with delay of 1 ms*/
			rslt |= bma455_write_regs(BMA4_FIFO_CONFIG_0_ADDR, &data[0], 1, dev);
			dev->delay(BMA4_GEN_READ_WRITE_DELAY);
			rslt |= bma455_write_regs((BMA4_FIFO_CONFIG_0_ADDR + 1), &data[1], 1, dev);
		}
	}
 ...
 */
uint16_t bma455_get_fifo_config(uint8_t *fifo_config, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data[2] = {0, 0};
 ...
 *	pin2 in the sensor.
 */
uint16_t bma455_set_int_pin_config(const struct bma4_int_pin_config *int_pin_config, uint8_t int_line, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev == NULL) {
		...
		if (int_line <= 1) {
			...
			rslt |= bma455_write_regs(interrupt_address_array[int_line],	&data, 1, dev);
		} else {
 ...
 *	from the sensor.
 */
uint16_t bma455_get_int_pin_config(struct bma4_int_pin_config *int_pin_config, uint8_t int_line, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t interrupt_address_array[2] = {BMA4_INT1_IO_CTRL_ADDR, BMA4_INT2_IO_CTRL_ADDR};
	uint8_t data = 0;
	...
	if (dev == NULL) {
		...
		if (int_line <= 1) {
			rslt |= bma455_read_regs(interrupt_address_array[int_line], &data, 1, dev);
...
uint16_t bma455_read_int_status(uint16_t *int_status, struct bma4_dev *dev)
{
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_INT_STAT_0_ADDR, data, 2, dev);
 ...
 * auxiliary sensor
 */
uint16_t bma455_aux_interface_init(struct bma4_dev *dev)
{
...
*/
uint16_t bma455_aux_read(uint8_t aux_reg_addr, uint8_t *aux_data, uint16_t len, struct bma4_dev *dev)
{
	/* Variable to return error codes */
	uint16_t rslt = BMA4_OK;
...
*/
uint16_t bma455_aux_write(uint8_t aux_reg_addr, uint8_t *aux_data, uint16_t len, struct bma4_dev *dev)
{
	...
	if (rslt == BMA4_OK) {
		...
		if (len > 0) {
			while (len--) {
				...
				rslt = bma455_write_regs(BMA4_AUX_WR_DATA_ADDR, aux_data, 1, dev);
				dev->delay(BMA4_AUX_COM_DELAY);
				if (rslt == BMA4_OK) {
					/* Then set address to write */
					rslt = bma455_write_regs(BMA4_AUX_WR_ADDR, &aux_reg_addr, 1, dev);
					dev->delay(BMA4_AUX_COM_DELAY);
...
static uint16_t  stream_transfer_write(const uint8_t *stream_data, uint16_t index, struct bma4_dev *dev)
{
	...
	rslt |= bma455_write_regs(BMA4_RESERVED_REG_5B_ADDR, &asic_lsb, 1, dev);

	if (rslt == BMA4_OK) {
		rslt |= bma455_write_regs(BMA4_RESERVED_REG_5C_ADDR, &asic_msb, 1, dev);
...
static uint16_t set_accel_selftest_enable(uint8_t accel_selftest_enable, struct bma4_dev *dev)
{
	...
	if (dev == NULL) {
		...
		rslt |= bma455_read_regs(BMA4_ACC_SELF_TEST_ADDR, &data, 1, dev);

		if (rslt == BMA4_OK) {
			data = BMA4_SET_BITSLICE(data, BMA4_ACCEL_SELFTEST_ENABLE, accel_selftest_enable);
			rslt |= bma455_write_regs(BMA4_ACC_SELF_TEST_ADDR, &data, 1, dev);
		}
	}
...
static uint16_t set_accel_selftest_config(struct bma4_dev *dev)
{
	...
	rslt |= bma455_set_accel_enable(BMA4_ENABLE, dev);
	rslt |= bma455_set_accel_config(&accel, dev);
...
static uint16_t unpack_mag_data(struct bma4_mag *mag_data, uint16_t start_idx, const struct bma4_dev *dev)
{
	...
	rslt |= bma455_second_if_mag_compensate_xyz(mag_fifo_data, dev->aux_sensor, mag_data);
...
static void unpack_sensortime_frame(uint16_t *data_index, const struct bma4_dev *dev)
{
	...
	if ((*data_index + BMA4_SENSOR_TIME_LENGTH) > dev->fifo->length) {
		...
		*data_index = dev->fifo->length;
		...
		sensor_time_byte1 = dev->fifo->data[(*data_index) + BMA4_SENSOR_TIME_LSB_BYTE];
		/* Sensor time */
...
static uint16_t validate_selftest(const struct selftest_delta_limit *accel_data_diff)
{
	...
	/* x axis limit 800mg, y axis limit 800mg and z axis limit 400mg */
	if (accel_data_diff->x > 800 && accel_data_diff->y > 800 && accel_data_diff->z > 400)
		rslt = BMA4_OK;
	else
...
static uint16_t foc_config(struct bma4_accel_config *acc_conf, uint8_t *acc_en, uint8_t *pwr_mode, struct bma4_dev *dev)
{
	...
	rslt |= bma455_get_accel_config(acc_conf, dev);
	rslt |= bma455_get_accel_enable(acc_en, dev);
	rslt |= bma455_get_advance_power_save(pwr_mode, dev);

	/* Disabling offset compensation that is in place*/
	rslt |= bma455_set_offset_comp(BMA4_DISABLE, dev);

	if (rslt == BMA4_OK) {
		/* Set Accel config to 50Hz, continuous filter mode,
		no under sampling */
		rslt |= bma455_write_regs(BMA4_ACCEL_CONFIG_ADDR, &accel_cnf, 1, dev);

		if (rslt == BMA4_OK) {
			/* Switch Accel to normal mode and
			advance power save to zero*/
			rslt |= bma455_set_accel_enable(BMA4_ENABLE, dev);
			rslt |= bma455_set_advance_power_save(BMA4_DISABLE, dev);
		}
	}
...
static uint16_t bma4_set_aux_interface_config(struct bma4_dev *dev)
{
	...
	if (rslt == BMA4_OK) {
		...
		rslt |= bma455_set_mag_enable(0x01, dev);
		dev->delay(BMA4_AUX_COM_DELAY);

		/* Disable advance power save */
		rslt |= bma455_set_advance_power_save(0x00, dev);
		dev->delay(BMA4_AUX_COM_DELAY);

		/* Set the I2C device address of auxiliary device */
		rslt |= bma455_set_i2c_device_addr(dev);
		dev->delay(BMA4_AUX_COM_DELAY);

		/* Set auxiliary interface to manual mode */
		rslt |= bma455_set_mag_manual_enable(dev->aux_config.manual_enable, dev);
		dev->delay(BMA4_AUX_COM_DELAY);

		/* Set the number of bytes for burst read */
		rslt |= bma455_set_mag_burst(dev->aux_config.burst_read_length, dev);
		dev->delay(BMA4_AUX_COM_DELAY);

		/* Switch on the the auxiliary interface mode */
		rslt |= bma455_set_if_mode(dev->aux_config.if_mode, dev);
		dev->delay(BMA4_AUX_COM_DELAY);
...
static uint16_t bma4_extract_aux_data(uint8_t aux_reg_addr, uint8_t *aux_data, uint16_t len, struct bma4_dev *dev)
{
	...
	for (burst_count = 0; burst_count < len; burst_count += burst_len) {
		...
		rslt |= bma455_set_mag_read_addr(aux_reg_addr, dev);
		dev->delay(BMA4_AUX_COM_DELAY);
		...
		if (rslt == BMA4_OK) {
			...
			if (len > 0) {
				...
				rslt |= bma455_read_regs(aux_read_addr, data, (uint8_t)burst_len, dev);
				dev->delay(BMA4_AUX_COM_DELAY);
 ...
 */
uint16_t bma455_bst_akm_mag_interface_init(uint8_t akm_i2c_address, uint8_t *akm_chip_id, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t v_data = 0;
	...
	rslt |=  bma455_set_accel_enable(0x01, dev);
	dev->delay(BMA4_AUX_COM_DELAY);
	...
	rslt |= bma455_set_mag_manual_enable(BMA4_MANUAL_ENABLE, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	rslt |= bma455_get_mag_manual_enable(&v_data, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	/*Enable the MAG interface */
	rslt |= bma455_set_if_mode(BMA4_ENABLE_MAG_IF_MODE, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	rslt |= bma455_get_if_mode(&v_data, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	/* read the device id of the AKM sensor
	if device id is 0x09 - AKM09916*/
	rslt |= bma455_set_mag_read_addr(AKM_CHIP_ID_REG, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, akm_chip_id, 1, dev);

	/* Set value power down mode mode*/
	rslt |= bma455_set_mag_write_data(AKM_POWER_DOWN_MODE_DATA, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	/* AKM mode address is 0x31*/
	rslt |= bma455_set_mag_write_addr(AKM_POWER_MODE_REG, dev);
	dev->delay(BMA4_AUX_COM_DELAY);

	/* Set AKM Force mode*/
	rslt |= bma455_set_mag_write_data(AKM_CONTINUOUS_MEASUREMENT_MODE1, dev);
	dev->delay(BMA4_AUX_COM_DELAY);
	...
	rslt |= bma455_set_mag_read_addr(AKM_DATA_REGISTER, dev);

	/* Enable Mag interface to auto mode*/
	rslt |= bma455_set_mag_manual_enable(BMA4_MANUAL_DISABLE, dev);
	dev->delay(BMA4_AUX_COM_DELAY);
 ...
 */
uint16_t bma455_bst_akm_set_powermode(uint8_t akm_pow_mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	...
	if (dev->aux_sensor == AKM9916_SENSOR) {
		...
		if (BMA4_OK == rslt) {
			...
			case AKM_SINGLE_MEAS_MODE:
				...
				rslt |= bma455_set_mag_write_data(AKM_SINGLE_MEASUREMENT_MODE, dev);
				dev->delay(BMA4_GEN_READ_WRITE_DELAY);
				rslt |= bma455_set_mag_write_addr(AKM_POWER_MODE_REG, dev);
				dev->delay(BMA4_AUX_IF_DELAY);
				rslt |= bma455_set_mag_read_addr(AKM_DATA_REGISTER, dev);
 ...
 *	sensor
 */
uint16_t bma455_set_bst_akm_and_secondary_if_powermode(uint8_t mag_sec_if_pow_mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;

	if (dev->aux_sensor == AKM9916_SENSOR) {
		/* Accel operation mode to normal*/
		rslt = bma455_set_command_register(ACCEL_MODE_NORMAL, dev);
		dev->delay(BMA4_GEN_READ_WRITE_DELAY);
		...
		if (BMA4_OK == rslt) {
			...
			if (BMA4_OK == rslt) {
				...
				case BMA4_MAG_FORCE_MODE:
					...
					single measurement mode*/
					rslt |= bma455_bst_akm_set_powermode(AKM_SINGLE_MEAS_MODE, dev);
					dev->delay(BMA4_AUX_IF_DELAY);
					rslt |= bma455_set_mag_read_addr(AKM_DATA_REGISTER, dev);
					dev->delay(BMA4_GEN_READ_WRITE_DELAY);
					break;
				case BMA4_MAG_SUSPEND_MODE:
					/* set the akm power mode as
					power down mode*/
					rslt |= bma455_bst_akm_set_powermode(AKM_POWER_DOWN_MODE, dev);
					dev->delay(BMA4_AUX_IF_DELAY);
					/* set the secondary mag
					power mode as SUSPEND*/
					rslt |= bma455_set_command_register(MAG_MODE_SUSPEND,	dev);
					dev->delay(BMA4_AUX_IF_DELAY);
					break;
				default:
					rslt = BMA4_E_OUT_OF_RANGE;
 ...
 *	its chip-id.
 */
uint16_t bma455_bmm150_mag_interface_init(uint8_t *chip_id, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	uint8_t data = 0;
	...
	rslt |= bma455_set_mag_burst(0x03, dev);
	dev->delay(10);

	rslt |= bma455_set_aux_if_mode(0x01, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_write_data(0x01, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_write_addr(BMA4_BMM150_POWER_CONTROL_REG, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_write_data(0x02, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_write_addr(BMA4_BMM150_POWER_MODE_REG, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_read_addr(BMA4_BMM150_CHIP_ID, dev);
	dev->delay(10);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data, 1, dev);

	if (BMA4_OK == rslt)
		*chip_id = data;

	/* set the power mode register*/
	rslt |= bma455_set_mag_write_data(BMA4_BMM_POWER_MODE_REG, dev);
	...
	rslt |= bma455_set_bmm150_mag_presetmode(BMA4_MAG_PRESETMODE_REGULAR, dev);

	/* To avoid the auto mode enable when manual mode operation running*/
	V_bmm150_manual_auto_condition = BMA4_MANUAL_DISABLE;

	/* Set the power mode of Mag to force mode*/
	rslt |= bma455_set_mag_write_data(BMA4_BMM150_FORCE_MODE, dev);
	dev->delay(10);
	...
	rslt |= bma455_set_mag_read_addr(BMA4_BMM150_DATA_REG, dev);
	dev->delay(10);
	...
	rslt |= bma455_get_mag_manual_enable(&data, dev);
 ...
 */
uint16_t bma455_bmm150_mag_wakeup(struct bma4_dev *dev)
{
	...
	for (i = 0; i < try_times; i++) {
		rslt |=  bma455_set_mag_write_data(BMA4_BMM150_POWER_ON, dev);
		...
		rslt |= bma455_set_mag_read_addr(BMA4_BMM150_POWER_CONTROL_REG, dev);
		dev->delay(BMA4_BMM150_WAKEUP_DELAY3);

		rslt |= bma455_read_regs(BMA4_DATA_0_ADDR, &power_control_bit, 1, dev);
 ...
 *	the read data.
 */
uint16_t bma455_bmm150_mag_compensate_xyz(struct bma4_mag *mag_comp_xyz, struct bma4_dev *dev)
{

	uint16_t rslt = 0;
	struct bma4_mag_xyzr mag_xyzr;

	rslt |=  bma455_read_mag_xyzr(&mag_xyzr, dev);
	if (BMA4_OK == rslt) {
		/* Compensation for X axis */
		mag_comp_xyz->x = bma455_bmm150_mag_compensate_X(mag_xyzr.x, mag_xyzr.r);
		/* Compensation for Y axis */
		mag_comp_xyz->y = bma455_bmm150_mag_compensate_Y(mag_xyzr.y, mag_xyzr.r);
		/* Compensation for Z axis */
		mag_comp_xyz->z = bma455_bmm150_mag_compensate_Z(mag_xyzr.z, mag_xyzr.r);
	}
 ...
 *	user.
 */
int32_t bma455_bmm150_mag_compensate_X(int16_t mag_data_x, uint16_t data_r)
{
 ...
 *	user.
 */
int32_t bma455_bmm150_mag_compensate_Y(int16_t mag_data_y, uint16_t data_r)
{
	int32_t inter_rslt = 0;
	int32_t trim_data1 = 0;
	int32_t trim_data2 = 0;
 ...
 *	user.
 */
int32_t bma455_bmm150_mag_compensate_Z(int16_t mag_data_z, uint16_t data_r)
{
	int32_t compensated_data = 0;
	int32_t trim_data1 = 0;
	int32_t trim_data2 = 0;
 ...
 */
uint16_t bma455_set_bmm150_mag_and_secondary_if_power_mode(uint8_t mag_sec_if_pow_mode, struct bma4_dev *dev)
{
	...
	if (BMA4_OK == rslt) {
		...
		case BMA4_MAG_FORCE_MODE:
			...
			rslt |= bma455_bmm150_mag_set_power_mode(FORCE_MODE, dev);
			dev->delay(BMA4_GEN_READ_WRITE_DELAY);
			break;
		case BMA4_MAG_SUSPEND_MODE:
			/* set the Mag power mode as SUSPEND mode*/
			rslt |= bma455_bmm150_mag_set_power_mode(SUSPEND_MODE, dev);
			dev->delay(BMA4_GEN_READ_WRITE_DELAY);
 ...
 *	x,y,z repetitions
 */
uint16_t bma455_set_bmm150_mag_presetmode(uint8_t mode, struct bma4_dev *dev)
{
	uint16_t rslt = 0;
 ...
 */
uint16_t bma455_bmm150_mag_set_power_mode(uint8_t mag_pow_mode, struct bma4_dev *dev)
{
	...
	if (BMA4_OK == rslt) {
		...
		case FORCE_MODE:
			...
			rslt |=  bma455_bmm150_mag_wakeup(dev);
		...
		case SUSPEND_MODE:
			...
			rslt |=  bma455_set_mag_write_data(BMA4_BMM150_POWER_OFF, dev);
			dev->delay(BMA4_GEN_READ_WRITE_DELAY);
...
static uint16_t read_bmm150_mag_trim(struct bma4_dev *dev)
{
	...
	/* read dig_x1 value */
	rslt |=  bma455_set_mag_read_addr(BMA4_MAG_DIG_X1, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_X1], 1, dev);
	dev->delay(1);
	bma4_mag_trim.dig_x1 = (int8_t)data[BMA4_BMM150_DIG_X1];

	/* read dig_y1 value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Y1, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Y1], 1, dev);
	dev->delay(1);
	bma4_mag_trim.dig_y1 = (int8_t)data[BMA4_BMM150_DIG_Y1];

	/* read dig_x2 value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_X2, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_X2], 1, dev);
	dev->delay(1);
	bma4_mag_trim.dig_x2 = (int8_t)data[BMA4_BMM150_DIG_X2];

	/* read dig_y2 value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Y2, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Y3], 1, dev);
	dev->delay(1);
	bma4_mag_trim.dig_y2 = (int8_t)data[BMA4_BMM150_DIG_Y3];

	/* read dig_xy1 value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_XY1, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_XY1], 1, dev);
	dev->delay(1);
	bma4_mag_trim.dig_xy1 = data[BMA4_BMM150_DIG_XY1];

	/* read dig_xy2 value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_XY2, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_XY2], 1, dev);
	dev->delay(1);
	bma4_mag_trim.dig_xy2 = (int8_t)data[BMA4_BMM150_DIG_XY2];

	/* read dig_z1 LSB value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z1_LSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z1_LSB], 1, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z1_MSB], 1, dev);
	dev->delay(1);
	...
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z2_LSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z2_LSB], 1, dev);
	dev->delay(1);

	/* read dig_z2 MSB value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z2_MSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z2_MSB], 1, dev);
	dev->delay(1);
	...
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z3_LSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z3_LSB], 1, dev);
	dev->delay(1);

	/* read dig_z3 MSB value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z3_MSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z3_MSB], 1, dev);
	dev->delay(1);
	...
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z4_LSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z4_LSB], 1, dev);
	dev->delay(1);

	/* read dig_z4 MSB value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_Z4_MSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_Z4_MSB], 1, dev);
	dev->delay(1);
	...
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_XYZ1_LSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_XYZ1_LSB], 1, dev);
	dev->delay(1);

	/* read dig_xyz1 MSB value */
	rslt |= bma455_set_mag_read_addr(BMA4_MAG_DIG_XYZ1_MSB, dev);
	dev->delay(1);

	rslt |= bma455_read_regs(BMA4_MAG_DATA_READ_REG, &data[BMA4_BMM150_DIG_XYZ1_MSB], 1, dev);
	dev->delay(1);
...
static uint16_t bmm150_preset_lowpower(struct bma4_dev *dev)
{
	...
	rslt |=  bma455_set_mag_write_data(BMA4_MAG_LOWPOWER_REPXY, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_write_addr(BMA4_BMM150_XY_REP, dev);
	dev->delay(10);

	/* write the Z repetitions*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_LOWPOWER_REPZ, dev);
	dev->delay(10);

	rslt |= bma455_set_mag_write_addr(BMA4_BMM150_Z_REP, dev);
	dev->delay(10);

	/* set the Mag data rate as 10 in the register 0x4C*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_LOWPOWER_DR, dev);
	dev->delay(10);
...
static uint16_t bmm150_preset_regular(struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	/* write the XY and Z repetitions*/
	rslt |=  bma455_set_mag_write_data(BMA4_MAG_REGULAR_REPXY, dev);
	dev->delay(10);
	...
	/* write the Z repetitions*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_REGULAR_REPZ, dev);
	dev->delay(10);
	...
	/* set the Mag data rate as 10 in the register 0x4C*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_REGULAR_DR, dev);
	dev->delay(10);
...
static uint16_t bmm150_preset_highaccuracy(struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	/* write the XY and Z repetitions*/
	rslt |=  bma455_set_mag_write_data(BMA4_MAG_HIGHACCURACY_REPXY, dev);
	dev->delay(10);
	...
	/* write the Z repetitions*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_HIGHACCURACY_REPZ,	dev);
	dev->delay(10);
	...
	rslt |= bma455_set_mag_write_data(BMA4_MAG_HIGHACCURACY_DR, dev);
	dev->delay(10);
...
static uint16_t bmm150_preset_enhanced(struct bma4_dev *dev)
{
	uint16_t rslt = 0;
	/* write the XY and Z repetitions*/
	rslt |=  bma455_set_mag_write_data(BMA4_MAG_ENHANCED_REPXY, dev);
	dev->delay(BMA4_GEN_READ_WRITE_DELAY);
	...
	/* write the Z repetitions*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_ENHANCED_REPZ, dev);
	dev->delay(BMA4_GEN_READ_WRITE_DELAY);
	...
	/* set the Mag data rate as 10 in the register 0x4C*/
	rslt |= bma455_set_mag_write_data(BMA4_MAG_ENHANCED_DR, dev);
	dev->delay(BMA4_GEN_READ_WRITE_DELAY);
...
uint16_t bma455_bmm150_mag_compensate_xyz_raw(
struct bma4_mag *mag_comp_xyz, struct bma4_mag_xyzr mag_xyzr)
{
	/* variable used for return the status of communication result*/
	uint16_t rslt = 0;
	/* Compensation for X axis */
	mag_comp_xyz->x = bma455_bmm150_mag_compensate_X(
	mag_xyzr.x, mag_xyzr.r);

	/* Compensation for Y axis */
	mag_comp_xyz->y = bma455_bmm150_mag_compensate_Y(
	mag_xyzr.y, mag_xyzr.r);

	/* Compensation for Z axis */
	mag_comp_xyz->z = bma455_bmm150_mag_compensate_Z(
