...
#include <linux/rtc.h>
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.5.2 20180123"
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,19))
    #include <linux/of.h>
...
static int coeff_duntai[6] = {120,286,265,226,202,147};   //coefficient of TP_duntai influence on als
static int coeff_huiding[6] = {120,241,213,196,168,148};  //coefficient of TP_huiding influence on als
static int coeff_mstar[6] = {120,218,205,173,159,130};    //coefficient of TP_mstar influence on als
static int *coeff_als_for_TP =coeff_duntai;
static int lightSour_dist_thr[5] ={10000,20000,30000,40000,50000}; //light source distinction threshold for calculating als
static int coefficient_15_to_13 = 860;  // coefficient of making als 15lux value modify reducing to 13lux
static int coefficient_20_to_16 = 782;  // coefficient of making als 20lux value modify reducing to 16lux
...
static unsigned int ps_detection = -1;        //ps detecte initial value 
#define  MAX_ADC_PROX_VALUE  65535   //max ps raw data
...
#ifdef CONFIG_OF
 static  struct alsps_hw stk3x13_hw;
#endif
...
static  int stk3x1x_update_ps_threshold_value(struct stk3x1x_priv *obj, u16 ps);
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
    ...
    int32_t tmp = 0;
    u32 als_data_u32 = 0;
	int irs_max_Num = 60000;  // 60000 is max number of irs_index cycles
    //int32_t als_comperator1,als_comperator_led1;
	int coeff_count = 10;  //coefficient of making als data being integer
    struct timeval tv = { 0 };
    struct rtc_time tm_android;
    struct timeval tv_android = { 0 };
    struct stk3x1x_priv *obj = NULL;

	uint32_t CWF_comperator = 0;
	uint32_t D_comperator = 0;
	uint32_t A_comperator_1 = 0;
	uint32_t A_comperator_2 = 0;
	uint32_t A_comperator_3 = 0;
	uint32_t A_comperator_4 = 0;
	uint32_t A_comperator_5 = 0;
    if(NULL == client)
    {
        APS_ERR("client is NULl!\n");
        return -EINVAL;
    }
...
als_default:
	...
	if(obj->irs_index < irs_max_Num)
		obj->irs_index ++;
	else
		obj->irs_index = 0;

	if((obj->irs_index % 10) == 0)  // read one ir data every 10 times, reduce to join eint_work func number
	{
	   	CWF_comperator = als_data;
		D_comperator = (als_data * coeff_als_for_TP[0]) / coeff_count;
		A_comperator_1 = (als_data * coeff_als_for_TP[1]) / coeff_count;
		A_comperator_2 = (als_data * coeff_als_for_TP[2]) / coeff_count;
		A_comperator_3 = (als_data * coeff_als_for_TP[3]) / coeff_count;
		A_comperator_4 = (als_data * coeff_als_for_TP[4]) / coeff_count;
		A_comperator_5 = (als_data * coeff_als_for_TP[5]) / coeff_count;

		ir_data = stk3x1x_get_ir_value(obj, STK_IRS_IT_REDUCE);

		APS_LOG("%s: Sensortek als=%d, ir_data=%d current_tp=%d \n", __func__, als_data, ir_data,current_tp);

		if(ir_data < CWF_comperator)
			current_color_temp = CWF_TEMP;
		else if(ir_data < D_comperator)
		{
			current_color_temp = D65_TEMP;
		}
		else
		{
			if(ir_data < lightSour_dist_thr[0])
			{
				if(ir_data > A_comperator_1)
					current_color_temp = A_TEMP;
				else
					current_color_temp = D65_TEMP;
			}
			else if(ir_data < lightSour_dist_thr[1])
			{
				if(ir_data > A_comperator_2)
					current_color_temp = A_TEMP;
				else
					current_color_temp = D65_TEMP;
			}
			else if(ir_data < lightSour_dist_thr[2])
			{
				if(ir_data > A_comperator_3)
					current_color_temp = A_TEMP;
				else
					current_color_temp = D65_TEMP;
			}
			else if(ir_data < lightSour_dist_thr[3])
			{
				if(ir_data > A_comperator_4)
					current_color_temp = A_TEMP;
				else
					current_color_temp = D65_TEMP;
			}
			else if(ir_data < lightSour_dist_thr[4])
			{
				if(ir_data > A_comperator_5)
					current_color_temp = A_TEMP;
				else
					current_color_temp = D65_TEMP;
			}
    ...
    *data = (u16)als_data_u32;
    ...
    if(1 == atomic_read(&als_sample_en))
    {
        ...
        als_sample[sample_count].ratio = CWF_comperator;
        als_sample[sample_count].ratio1 = D_comperator;
	als_sample[sample_count].ratio2= A_comperator_1;
        als_sample[sample_count].ratio3 = A_comperator_2;
	als_sample[sample_count].ratio4= A_comperator_3;
        als_sample[sample_count].ratio5 = A_comperator_4;
	als_sample[sample_count].ratio6 = A_comperator_5;
...
static int stk3x1x_enable_als(struct i2c_client *client, int enable)
{
    ...
    {
			stk3x1x_master_recv(client, obj->addr.data1_als, buf, 0x02);  //clear als data ready
#ifdef STK_ALS_FIR
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if(ret < 0)
	 ...
	 APS_LOG(" %s stk3x1x_read_flag  flag: 0x%x \n",__func__, flag);
	if((err = stk3x1x_clear_intr(obj->client, flag,  (STK_FLG_ALSINT_MASK | STK_FLG_PSINT_MASK | STK_FLG_IR_RDY_MASK))))
	{
		APS_ERR("%s  stk3x1x_clear_intr  fail: %d\n",__func__, err);
	}
	...
	if(enable)
	{
		...
		stk3x1x_read_cali_file(obj->client);
		...
		{
			pwindows_value = obj->hw->ps_threshold_high;
			pwave_value = obj->hw->ps_threshold_low;
			threshold_value = threshold_value_default;
		}

		APS_LOG("stk3x1x enable min_proximity = %d, pwindows_value = %d, pwave_value = %d,threshold_value=%d\n",
					min_proximity, pwindows_value, pwave_value,threshold_value);
    ...
    if(enable)
    {
       if((err = stk3x1x_write_wait(obj->client, obj->wait_val)))
		...
		if (err = stk3x1x_write_ps_high_thd(obj->client, ps_thd_val_high_set))
		{
		        APS_ERR("%s write high thd error: %d\n", __func__,err);

		}
		if (err = stk3x1x_write_ps_low_thd(obj->client, ps_thd_val_low_set))
		{
		        APS_ERR("%s write low thd error: %d\n", __func__,err);

		}
		APS_LOG("%s: set ps thd high =%d, ps thd low=%d,obj->ps=%d\n", __func__, ps_thd_val_high_set, ps_thd_val_low_set,obj->ps);
...
*/
	if (((ps_data + pwave_value) < min_proximity) && (ps_data >= 0)) {
		...
		ps_thd_val_low_set = ps_threshold_l;
		ps_thd_val_high_set = ps_threshold_h;
		APS_DBG("stk3x1x go into schedule min_proximity = %d, shold_l = %d, shold_h = %d,ps_data=%d\n",
			min_proximity, ps_threshold_l, ps_threshold_h,ps_data);
	}
	if (ps_data >= ps_threshold_h) {
		...
		ps_thd_val_low_set = ps_threshold_l;
		//min_proximity =  obj->hw->ps_threshold_low;
		APS_DBG("stk3x1x ps near!pdata=%d!td_l=%d,td_h=%d \n", ps_data, ps_threshold_l, ps_threshold_h);
		APS_DBG("stk3x1x set_l(%d),set_h(%d) \n", ps_thd_val_low_set,ps_thd_val_high_set);
	} else if (ps_data <= ps_threshold_l) {
		ps_detection = 1;
		ps_thd_val_high_set = ps_threshold_h;
		ps_thd_val_low_set = 0;
		APS_DBG("stk3x1x ps far! pdata=%d!td_l(%d),td_h(%d)\n", ps_data, ps_threshold_l, ps_threshold_h);
		APS_DBG("stk3x1x set_l = %d , set_h=%d \n", ps_thd_val_low_set,ps_thd_val_high_set);
	} else {
		APS_DBG("stk3x1x ps between!, ps_data=%d, min_pro= %d, td_l = %d, td_h = %d\n", ps_data , min_proximity, ps_threshold_l, ps_threshold_h);
	}
	return ps_detection;
}
...
static void stk3x1x_eint_work(struct work_struct *work)
{
   ...
   u8 flag;
    ...
    if(flag_reg & STK_FLG_IR_RDY_MASK)
    {
	disable_flag |= STK_FLG_IR_RDY_MASK;
	is_ir_data_ready = 1;
    }
...
*/
	...
	ps_value = stk3x1x_update_ps_threshold_value(obj,obj->ps);
	...
	APS_LOG("%s: set ps_thd high =%d, ps_thd low=%d,obj->ps=%d\n", __func__, ps_thd_val_high_set, ps_thd_val_low_set,obj->ps);
    ...
    if(disable_flag)
    {
        APS_LOG("stk3x1x_clear_intr: %s  %d  disable_flag=%d\n", __func__,__LINE__,disable_flag);
...
static ssize_t stk3x1x_show_ir(struct device_driver *ddri, char *buf)
{
    ...
    reading = stk3x1x_obj->last_ir_code; //stk3x1x_get_ir_value(stk3x1x_obj, STK_IRS_IT_REDUCE);
    if(reading < 0)
        return scnprintf(buf, PAGE_SIZE, "ERROR: %d\n", reading);

    //stk3x1x_obj->ir_code = reading;
    return scnprintf(buf, PAGE_SIZE, "0x%04X\n", stk3x1x_obj->last_ir_code);
}
...
{
        return scnprintf(buf, PAGE_SIZE, "als_sample_en:%d \n", atomic_read(&als_sample_en));
}
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
    ...
    {
        APS_ERR("exceed range  C_CUST_ALS_LEVEL =%d \n",C_CUST_ALS_LEVEL);
...
*/
    ...
    do
    {
        ...
        retry++;
    }while(retry < 10 && ((flag&STK_FLG_IR_RDY_MASK) == 0) && (is_ir_data_ready==0));
	...
	//reset als & clear als data ready
	stk3x1x_master_recv(obj->client, STK_STATE_REG, &w_reg, 0x01);  //read reg STK_STATE_REG data,reset als IC inside state
	i2c_smbus_write_byte_data(obj->client, STK_STATE_REG, w_reg);    
	stk3x1x_master_recv(obj->client, obj->addr.data1_als, buf, 0x02);   //read als raw data,clear als data ready
...
#ifdef CONFIG_COMPAT
static long stk3x1x_compat_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	int err = 0;
...
static void lsample_work(struct work_struct *work)
{
    ...
    if(1 == sscanf(sample_path, "%s", path))
        ...
        strcat(file_path, full_path);
            ...
            sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",als_sample[i].ir,als_sample[i].ratio,als_sample[i].ratio1,als_sample[i].ratio2,als_sample[i].ratio3,
				als_sample[i].ratio4,als_sample[i].ratio5,als_sample[i].ratio6,als_sample[i].als_value,als_sample[i].als_level);
...
static int als_get_data(int* value, int* status)
{
    ...
    else
    {
        ...
        */
		...
		if(obj->als < 3)    // 3 is als sensor raw data value
			*value = 0;
		else if(obj->als <= 7)  // 7 is als sensor raw data value
			*value = obj->als;
		else if(obj->als <= 15)  // 15 is als sensor raw data value
			*value = ((obj->als * coefficient_15_to_13) + 500) / 1000;    // 500 and 1000 is make data 4 discard or 5 increase
		else if(obj->als <= 20)  // 20 is als sensor raw data value
			*value = ((obj->als * coefficient_20_to_16) + 500) / 1000;    // 500 and 1000 is make data 4 discard or 5 increase
		else
			*value = stk3x1x_get_als_value(obj, obj->als);
...
static int stk_als_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}
static int stk_als_flush(void)
{
    int err = 0;
...
static int stk_ps_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}
static int stk_ps_flush(void)
{
    int err = 0;
...
//-add by hzb for ontim debug
    ...
    als_ctl.batch = stk_als_batch;
    als_ctl.flush = stk_als_flush;
#ifdef CUSTOM_KERNEL_SENSORHUB
    ...
    ps_ctl.batch = stk_ps_batch;
    ps_ctl.flush = stk_ps_flush;
#ifdef CUSTOM_KERNEL_SENSORHUB
...
static int __init stk3x1x_init(void)
{
    ...
    APS_FUN();
    if (get_alsps_dts_func(node, &stk3x13_hw))
    {
        printk(KERN_ERR "%s:get dts info fail\n",__func__);
        return -1;
    }
    i2c_register_board_info(stk3x13_hw.i2c_num, &i2c_stk3x1x, 1);
    APS_LOG("%s: i2c_number=%d\n", __func__,stk3x13_hw.i2c_num);
