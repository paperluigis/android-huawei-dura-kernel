...
/* add by gwx528023 gaoatao 20180109 begin*/
...
static struct dsm_dev dsm_flash = {
    .name = "dsm_flash",
    .fops = NULL,
    .buff_size = DSM_FLASH_BUF_MAX,
    };
static struct dsm_client *flash_dclient = NULL;
struct dsm_client*flash_dsm_get_client(void)
{
    return flash_dclient;
}
int flash_report_dsm_err( int type, int err_value,int add_value)
{
    struct dsm_client *flash_dclient = flash_dsm_get_client();

    printk("flash_DSM %s: entry! type:%d\n", __func__, type);

    if( NULL == flash_dclient )
    {
        printk("flash_DSM %s: there is not flash_dclient!\n", __func__);
        return -1;
    }

    /* try to get permission to use the buffer */
    if(dsm_client_ocuppy(flash_dclient))
    {
        /* buffer is busy */
        printk("flash_DSM %s: buffer is busy!\n", __func__);
        return -1;
    }

    /* flash report err according to err type */
    switch(type)
    {

        case DSM_FLASH_I2C_ERROR_NO:
            dsm_client_record(flash_dclient, "flashlight i2c ERROR\n");
            break;
        default:
            break;
    }

    dsm_client_notify(flash_dclient, type);

    return 0;
}
#endif
/* add by gwx528023 gaoatao 20180109 end*/
/* duty current */
...
typedef struct {
    int flash_id;
    int mode;
} FLASHLIGHT_INFO_STRUCT;

extern int aw3643_enable_ch1(void);
extern int aw3643_enable_ch2(void);
extern int aw3643_disable_ch1(void);
extern int aw3643_disable_ch2(void);
extern int aw3643_is_torch(int level);
static FLASHLIGHT_INFO_STRUCT flashInfo;

//mode: 0 close, 1 flash mode, 2 torch mode
static int subsys_rear_flashlight_node_read(struct seq_file *m, void *v)
{
   //pr_info("RunningTest rear flashlight,mode=%d \n",flashInfo.mode);
   seq_printf(m, "%d\n",flashInfo.mode);
   return 0;
};

static int proc_rear_flashlight_open(struct inode *inode, struct file *file)
{
    return single_open(file, subsys_rear_flashlight_node_read, NULL);
};

static int subsys_sub_flashlight_node_read(struct seq_file *m, void *v)
{
   //pr_info("RunningTest rear flashlight,mode=%d \n",flashInfo.mode);
   seq_printf(m, "%d\n",flashInfo.mode);
   return 0;
...
static int proc_sub_flashlight_open(struct inode *inode, struct file *file)
{
    return single_open(file, subsys_sub_flashlight_node_read, NULL);
};
...
extern struct hrtimer aw3643_timer_ch1;
extern struct hrtimer aw3643_timer_ch2;
static unsigned int aw3643_timeoutms[2] = {100, 100};
extern int aw3643_set_driver(int set);
extern int aw3643_set_level_ch1(int level);
extern int aw3643_set_level_ch2(int level);
//extern int aw3643_pinctrl_set(int pin, int state);

/*******************************************************************************
  * Used for rear flashlight register read/write by proc file
  ********************************************************************************/
static ssize_t  flashlight_node_write(struct file *file, const char *buffer, size_t count,
                         loff_t *data)
{
    char regBuf[64] = {'\0'};
    ...
    if (copy_from_user(regBuf, buffer, u4CopyBufSize))
        return -EFAULT;
    if (sscanf(regBuf, "%x %x",  &flashInfo.flash_id, &flashInfo.mode) == 2)
    {
        //pr_info("[%s] flash_id= %d,mode = %d.", __FUNCTION__,flashInfo.flash_id, flashInfo.mode);
        if(flashInfo.flash_id & 0x001) //rear first flash light
        {
            if(flashInfo.mode == 0x2)//torch mode
            {
                aw3643_is_torch(3);//#define AW3643_LEVEL_TORCH 7
                aw3643_enable_ch1();
            }
            else if(flashInfo.mode == 0x1)//flash mode
            {
                    aw3643_set_driver(1);
                    aw3643_set_level_ch1(3);//#define AW3643_LEVEL_NUM 26
                    ktime = ktime_set(aw3643_timeoutms[AW3643_CHANNEL_CH1] / 1000,
                        (aw3643_timeoutms[AW3643_CHANNEL_CH1] % 1000) * 1000000);
        ...
        else if(flashInfo.flash_id == 0x002)
        {
            pr_info("[%s] unsupport rear second flashlight.", __FUNCTION__);
        }
        else
        {
            pr_info("[%s] unsupport rear flashlight id.", __FUNCTION__);
        }
    }
    else if (sscanf(regBuf, "%x", &flashInfo.flash_id) == 1)
    {
        pr_info("[%s] unsupport rear flashlight cmd.", __FUNCTION__);
    }
...
/*******************************************************************************
  * Used for front flashlight register read/write by proc file
  ********************************************************************************/
static ssize_t sub_flashlight_node_write(struct file *file, const char *buffer, size_t count,
                         loff_t *data)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
    ...
    {
        //pr_info("[%s] flash_id= %d,mode = %d.", __FUNCTION__,flashInfo.flash_id, flashInfo.mode);
        if(flashInfo.flash_id == 0x04)//front first flash light
        {
            if(flashInfo.mode == 0x2)//torch mode
            {
                aw3643_is_torch(3);//#define AW3643_LEVEL_TORCH 7
            ...
            {
                aw3643_set_driver(1);
                aw3643_set_level_ch2(2);//#define AW3643_LEVEL_NUM 26
                ktime = ktime_set(aw3643_timeoutms[AW3643_CHANNEL_CH2] / 1000,
                        (aw3643_timeoutms[AW3643_CHANNEL_CH2] % 1000) * 1000000);
        ...
        else if(flashInfo.flash_id == 0x08)
        {
            pr_info("[%s] unsupport rear second flashlight.", __FUNCTION__);
        }
        else
        {
            pr_info("[%s] unsupport sub flashlight id.", __FUNCTION__);
        }
    }
    else if (sscanf(regBuf, "%x", &flashInfo.flash_id) == 1)
    {
        pr_info("[%s] unsupport sub flashlight cmd.", __FUNCTION__);
    }

    return count;

}
static  struct file_operations flashlight_proc_fops = {
    .open  = proc_rear_flashlight_open,
    .read  = seq_read,
    .write = flashlight_node_write
};
static  struct file_operations sub_flashlight_proc_fops = {
    .open  = proc_sub_flashlight_open,
    .read  = seq_read,
    .write = sub_flashlight_node_write
};
...
/* add by gwx528023 gaoatao 20180109 start*/
#ifdef CONFIG_HUAWEI_CAMERA_DSM//flash
    if(!flash_dclient)
    {
        flash_dclient = dsm_register_client(&dsm_flash);
    }
    if(!flash_dclient)
    {
        printk("flash_DSM flash dsm client register error\n");
    }
    else
    {
        printk("flash_DSM flash dsm client register success\n");
    }
#endif
/* add by gwx528023 gaoatao 20180109 end*/
    pr_debug("Probe done\n");
