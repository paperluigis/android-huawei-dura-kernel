...
#define AW3643_TORCH_LEVEL_NUM 8
#define SY7806_TORCH_LEVEL_NUM 8
...
extern int flash_report_dsm_err( int type, int err_value,int add_value);
#endif
static unsigned char *torch_level_for_moduleTP = NULL;
...
int aw3643_pinctrl_set(int pin, int state)
{
    int ret = 0;
...
static const unsigned char aw3643_torch_level[AW3643_TORCH_LEVEL_NUM] = {
    0x05, 0x0F, 0x12, 0x14, 0x17, 0x1D ,0x25, 0x1F
};
static const unsigned char aw3643_white_tp_torch_level[AW3643_TORCH_LEVEL_NUM] = {
    0x01, 0x05, 0x06, 0x07, 0x08, 0x0A, 0x0D, 0x1F
};
static const unsigned char sy7806_torch_level[SY7806_TORCH_LEVEL_NUM] = {
    0x0B, 0x20, 0x26, 0x2A, 0x31, 0x3D, 0x4E, 0x41
};
static const unsigned char sy7806_white_tp_torch_level[AW3643_TORCH_LEVEL_NUM] = {
    0x03, 0x0B, 0x0D, 0x0F, 0x11, 0x16, 0x1C, 0x41
};
...
static int aw3643_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
    ...
    if (ret < 0)
    {
        ...
        #ifdef CONFIG_HUAWEI_CAMERA_DSM
        flash_report_dsm_err(DSM_FLASH_I2C_ERROR_NO,0,0);
        #endif
    }
...
int aw3643_disable(int channel)
{
    if (channel == AW3643_CHANNEL_CH1)
        aw3643_disable_ch1();
    else if (channel == AW3643_CHANNEL_CH2)
...
int aw3643_set_level_ch1(int level)
{
    int ret;
    unsigned char reg, val;
    ...
    if(torch_level_for_moduleTP == NULL)
    {
        pr_err("Set torch_level error!\n");
        return -1;
    }
    val = torch_level_for_moduleTP[level];
...
static int get_torch_level_by_tp_info(int chipinfo)
{
    struct ts_module_info tp_info;
    int ret = 0;
    ret = ts_get_tp_module_info(&tp_info);

    if(chipinfo ==  AW3643_DEVICE_ID && (strstr(tp_info.panel_color,"white") != NULL))
    {
        torch_level_for_moduleTP = aw3643_white_tp_torch_level;
    }
    else if (chipinfo ==  AW3643_DEVICE_ID && (strstr(tp_info.panel_color,"black") != NULL))
    {
        torch_level_for_moduleTP = aw3643_torch_level;
    }
    else if(chipinfo ==  SY7806_DEVICE_ID && (strstr(tp_info.panel_color,"white") != NULL))
    {
        torch_level_for_moduleTP = sy7806_white_tp_torch_level;
    }
    else if(chipinfo ==  SY7806_DEVICE_ID && (strstr(tp_info.panel_color,"black") != NULL))
    {
        torch_level_for_moduleTP = sy7806_torch_level;
    }
    else
    {
        pr_err("Get tp_module_info fail!!");
    }
    return ret;
}

/* flashlight init for aw3643 or sy7806,aw3643 is the first batch of supply,sy7806 is second batch of supply*/
int aw3643_init(void)
{
    ...
    msleep(20);
    aw3643_write_reg(aw3643_i2c_client, FLASHLIGHT_REG_FLASH_LEVEL_LED2, FLASHLIGHT_SET_LED);
    aw3643_write_reg(aw3643_i2c_client, FLASHLIGHT_REG_TORCH_LEVEL_LED2, FLASHLIGHT_SET_LED);
 ...
 *****************************************************************************/
struct hrtimer aw3643_timer_ch1;
struct hrtimer aw3643_timer_ch2;
