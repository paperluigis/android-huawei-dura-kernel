...
int mtk_memcfg_get_memory_layout_info(int type, const char *name, unsigned long
		*start, unsigned long *size)
{
	struct mtk_memcfg_layout_info *info = NULL;
	int count = 0;
	...
	for(count =0; count < mtk_memcfg_layout_phy_count; count++)
	{
		info = &mtk_memcfg_layout_info_phy[count];
		if (info)
		{
			tmp = strnstr(info->name, name, sizeof(info->name));
			if(tmp)
			{
				*start = info->start;
				*size = info->size;
	...
	pr_err("layout_phy can't find %s\n",name);
	return -1;
}
