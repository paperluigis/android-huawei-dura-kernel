...
#endif
#define POWER_OFF 1     /* Power down mode */
#define POWER_ON  0
#ifdef CONFIG_HUAWEI_CAMERA_DSM
extern int cameralens_report_dsm_err( int type, int err_value,int add_value);
#endif
...
static int s4AF_WriteReg(u16 a_u2Data,u16 flag)
{
	int i4RetValue = 0;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
        ...
        s4AF_WriteReg((unsigned short)InitPos,POWER_ON);
    ...
    if (s4AF_WriteReg((unsigned short)g_u4TargetPosition,POWER_ON) == 0) {
		spin_lock(g_pAF_SpinLock);
		g_u4CurrPosition = (unsigned long)g_u4TargetPosition;
		spin_unlock(g_pAF_SpinLock);
	} else {
        #ifdef CONFIG_HUAWEI_CAMERA_DSM
        cameralens_report_dsm_err(DSM_CAMERA_ACTUATOR_MOVE_FAIL,0,0);
        #endif
...
int DW9714AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
    ...
    if (*g_pAF_Opened == 2) {
        s4AF_WriteReg((u16)(g_u4CurrPosition >> 1),POWER_ON);
        msleep(2);
        s4AF_WriteReg((u16)(g_u4CurrPosition >> 2),POWER_ON);
        msleep(2);
        s4AF_WriteReg((u16)(g_u4CurrPosition >> 3),POWER_ON);
        msleep(2);
        s4AF_WriteReg(0,POWER_ON);
        //LOG_INF("g_u4CurrPosition %d \n",g_u4CurrPosition);
        s4AF_WriteReg(0x80,POWER_OFF); /* Power down mode */
    }
