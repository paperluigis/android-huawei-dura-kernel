...
/* add by gwx528023 gaoatao 20180119 begin*/
#ifdef CONFIG_HUAWEI_CAMERA_DSM
#include <dsm/dsm_pub.h>
#include <linux/delay.h>

static struct dsm_dev dsm_cameralens = {
    .name = "dsm_camera",
    .fops = NULL,
    .buff_size = DSM_CAMERA_LENS_BUF_MAX,
    };
static struct dsm_client *cameralens_dclient = NULL;
struct dsm_client*cameralens_dsm_get_client(void)
{
    return cameralens_dclient;
}
int cameralens_report_dsm_err( int type, int err_value,int add_value)
{
    struct dsm_client *cameralens_dclient = cameralens_dsm_get_client();

    //printk("cameralens_DSM %s: entry! type:%d\n", __func__, type);

    if( NULL == cameralens_dclient )
    {
        printk("cameralens_DSM %s: there is not cameralens_dclient!\n", __func__);
        return -1;
    }
    /* try to get permission to use the buffer */
    if(dsm_client_ocuppy(cameralens_dclient))
    {
        /* buffer is busy */
        printk("cameralens_DSM %s: buffer is busy!\n", __func__);
        return -1;
    }

    /* cameralens report err according to err type */
    switch(type)
    {
        case DSM_CAMERA_ACTUATOR_MOVE_FAIL:
            dsm_client_record(cameralens_dclient, "LENS i2c ERROR\n");
            break;
    }

    dsm_client_notify(cameralens_dclient, type);

    return 0;
}
#endif
/* add by gwx528023 gaoatao 20180119 end*/
...
static inline int Register_AF_CharDrv(void)
{
	...
	if (lens_device ==  NULL)
		return -EIO;
/* add by gwx528023 gaoatao 20180119 start*/
#ifdef CONFIG_HUAWEI_CAMERA_DSM//cameralens
    if(!cameralens_dclient){
    cameralens_dclient = dsm_register_client(&dsm_cameralens);
    }
    if(!cameralens_dclient)
        printk("cameralens_DSM cameralens dsm client register error\n");
    else
        printk("cameralens_DSM cameralens dsm client register success\n");
#endif
/* add by gwx528023 gaoatao 20180119 end*/
