...
static int regulator_status[REGULATOR_TYPE_MAX_NUM] = {0}; //0 regulator_put, 1 regulator_get
...
static enum IMGSENSOR_RETURN regulator_init(void *pinstance)
{
	...
	for (i = 0; i < REGULATOR_TYPE_MAX_NUM; i++, pregulator_ctrl++) {
		...
		regulator_status[i] = 1; //add by mtk
	}
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	{
		reg_type_offset = REGULATOR_TYPE_MAIN_VCAMA;
...
{
	struct REGULATOR_CTRL    *pregulator_ctrl = regulator_control;
	pregulator_ctrl = pregulator_ctrl + index;
	if(regulator_status[index] == 0)
	{
		preg->pregulator[index] = regulator_get(pdevice, pregulator_ctrl->pregulator_type);
