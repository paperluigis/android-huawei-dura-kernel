...
bool S5K3L6_read_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
    ...
    LOG_INF("PDAF : S5K3L6 start read PDAF data begin\n");

    memset(S5K3L6_eeprom_data, 0xFF, DATA_SIZE);

    for(i = 0; i < PD1_SIZE; i++){
      S5K3L6_eeprom_data[i] = read_cmos_sensor_byte(proc1_addr+i);
      LOG_INF("add = 0x%x,\tvalue = 0x%x",i, S5K3L6_eeprom_data[i]);
    }

    for(i = 0; i < PD2_SIZE; i++)
    {
        S5K3L6_eeprom_data[PD1_SIZE + i] = read_cmos_sensor_byte(proc2_addr + i);
        LOG_INF("add = 0x%x,\tvalue = 0x%x", PD1_SIZE + i, S5K3L6_eeprom_data[PD1_SIZE + i]);
    }

    memcpy(data, S5K3L6_eeprom_data, DATA_SIZE);

    LOG_INF("PDAF : S5K3L6 start read PDAF data end\n");
