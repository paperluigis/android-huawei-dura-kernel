...
static struct dsm_dev dsm_camera = {
    ...
    .fops = NULL,
    .buff_size = DSM_CAMERA_BUF_MAX,
    };
static struct dsm_client *camera_dclient = NULL;
struct dsm_client*camera_dsm_get_client(void)
{
    return camera_dclient;
}
#endif
...
/* add by gwx528023 gaoatao 20180109 begin*/
#ifdef CONFIG_HUAWEI_CAMERA_DSM//camera
int camerasensor_report_dsm_err( int type, int err_value,int add_value)
{
    struct dsm_client *camera_dclient = camera_dsm_get_client();

    printk("camera_DSM %s: entry! type:%d\n", __func__, type);

    if( NULL == camera_dclient )
    {
        printk("camera_DSM %s: there is not camera_dclient!\n", __func__);
        return -1;
    }

    /* try to get permission to use the buffer */
    if(dsm_client_ocuppy(camera_dclient))
    {
        /* buffer is busy */
        printk("camera_DSM %s: buffer is busy!\n", __func__);
        return -1;
    }

    /* camera report err according to err type */
    switch(type)
    {

        case DSM_CAMERA_BACK_OTP_ERR:
            dsm_client_record(camera_dclient, "back camera OTP check ERROR\n");
            break;
        case DSM_CAMERA_FRONT_OTP_ERR:
            dsm_client_record(camera_dclient, "front camera OTP check ERROR\n");
        default:
            break;
    }

    dsm_client_notify(camera_dclient, type);
...
#ifdef CONFIG_HUAWEI_CAMERA_DSM//camera
    if(!camera_dclient){
    camera_dclient = dsm_register_client(&dsm_camera);
    }
    if(!camera_dclient)
        printk("camera_DSM camera dsm client register error\n");
    else
        printk("camera_DSM camera dsm client register success\n");
#endif
/* add by gwx528023 gaoatao 20180109 end*/
	return 0;
}
