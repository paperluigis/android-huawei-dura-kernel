...
#include "gc8034amipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC8034A_camera_sensor"
#define LOG_1 LOG_INF("GC8034A,MIPI 4LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

#define      GC8034AMIPI_SENSOR_ID          GC8034A_SENSOR_ID
...
#endif
extern u8 back_camera_id;
extern bool back_camera_probe_ok;
extern u8 back_checksum;
static struct agc_params_struct GC8034A_AGC_Param[9] = {
    {
        64, /* 1.00x */
        {
            { 0xfe, 0x00 },
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC8034AMIPI_SENSOR_ID,       /*record sensor id defined in Kd_imgsensor.h*/
...
static struct gc8034a_otp gc8034a_otp_info = {0};
#ifdef GC8034AOTP_FOR_CUSTOMER
static kal_uint8 LSC_ADDR[4] = { 0x0e, 0x20, 0x1a, 0x88 };
#endif

static kal_uint8 gc8034a_read_otp(kal_uint8 page, kal_uint8 addr)
{
    //LOG_INF("E!\n");
...
static void gc8034a_read_otp_group(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, kal_uint8 size)
{
    if(buff != NULL && (size <= MAX_WIDTH) ){
        kal_uint8 i;
...
static void gc8034a_gcore_read_otp_info(void)
{
    kal_uint8  flagdd = 0;
    kal_uint16 i = 0, j = 0;
    kal_uint8  temp = 0;
    ...
    kal_uint8  info[INFO_WIDTH] = { 0 };
    kal_uint8  wb[WB_WIDTH] = { 0 };
    kal_uint8  golden[GOLDEN_WIDTH] = { 0 };
    kal_uint8  af[AF_WIDTH] = { 0 };
#endif
    memset(&gc8034a_otp_info, 0, sizeof(gc8034a_otp_info));

    /* Static Defective Pixel */
    flagdd = gc8034a_read_otp(0, 0x0b);
    //LOG_INF("GC8034_OTP_DD : flag_dd = 0x%x\n", flagdd);

    switch (flagdd & 0x03) {
    case 0x00:
        LOG_INF("GC8034A_OTP_DD is Empty !!\n");
        gc8034a_otp_info.dd_flag = 0x00;
        break;
    case 0x01:
        //LOG_INF("GC8034_OTP_DD is Valid!!\n");
        gc8034a_otp_info.dd_cnt = gc8034a_read_otp(0, 0x0c) + gc8034a_read_otp(0, 0x0d);
        gc8034a_otp_info.dd_flag = 0x01;
        //LOG_INF("GC8034_OTP : total_number = %d\n", gc8034_otp_info.dd_cnt);
        break;
    case 0x02:
    case 0x03:
        LOG_INF("GC8034A_OTP_DD is Invalid !!\n");
        gc8034a_otp_info.dd_flag = 0x02;
        break;
    default:
        break;
    }
...
#ifdef GC8034AOTP_FOR_CUSTOMER
    flag_module = gc8034a_read_otp(9, 0x6f);
    flag_wb = gc8034a_read_otp(9, 0x5e);
    flag_af = gc8034a_read_otp(3, 0x3a);
    ...
    for (index = 0; index < 2; index++) {
        ...
        case 0x00:
            LOG_INF("GC8034A_OTP_INFO group %d is Empty !!\n", index + 1);
            break;
        case 0x04:
            //LOG_INF("GC8034_OTP_INFO group %d is Valid !!\n", index + 1);
            check = 0;
            gc8034a_read_otp_group(9, INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
            for (i = 0; i < INFO_WIDTH - 1; i++)
                check += info[i];

            if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
                gc8034a_otp_info.module_id = info[0];
                gc8034a_otp_info.lens_id = info[1];
                gc8034a_otp_info.vcm_driver_id = info[2];
                gc8034a_otp_info.vcm_id = info[3];
                gc8034a_otp_info.year = info[4];
                gc8034a_otp_info.month = info[5];
                gc8034a_otp_info.day = info[6];
            }
            else
            {
                back_checksum &= 0x7e;
                LOG_INF("GC8034A_OTP_INFO Check sum %d Error !!\n", index + 1);
            }
            break;
        case 0x08:
        case 0x0c:
            LOG_INF("GC8034A_OTP_INFO group %d is Invalid !!\n", index + 1);
            break;
        default:
            break;
        }
        ...
        case 0x00:
            LOG_INF("GC8034A_OTP_WB group %d is Empty !!\n", index + 1);
            gc8034a_otp_info.wb_flag = gc8034a_otp_info.wb_flag | 0x00;
            break;
        case 0x04:
            //LOG_INF("GC8034_OTP_WB group %d is Valid !!\n", index + 1);
            check = 0;
            gc8034a_read_otp_group(9, WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
            for (i = 0; i < WB_WIDTH - 1; i++)
                check += wb[i];

            if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
                gc8034a_otp_info.rg_gain = (wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
                    (wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
                gc8034a_otp_info.bg_gain = (((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
                    (((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
                gc8034a_otp_info.wb_flag = gc8034a_otp_info.wb_flag | 0x01;
            }
            else
            {
                back_checksum &= 0x7d;
                LOG_INF("GC8034A_OTP_WB Check sum %d Error !!\n", index + 1);
            }
            break;
        case 0x08:
        case 0x0c:
            LOG_INF("GC8034A_OTP_WB group %d is Invalid !!\n", index + 1);
            gc8034a_otp_info.wb_flag = gc8034a_otp_info.wb_flag | 0x02;
            break;
        default:
            break;
        }
        ...
        case 0x00:
            LOG_INF("GC8034A_OTP_GOLDEN group %d is Empty !!\n", index + 1);
            gc8034a_otp_info.golden_flag = gc8034a_otp_info.golden_flag | 0x00;
            break;
        case 0x40:
            //LOG_INF("GC8034_OTP_GOLDEN group %d is Valid !!\n", index + 1);
            check = 0;
            gc8034a_read_otp_group(9, GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
            for (i = 0; i < GOLDEN_WIDTH - 1; i++)
                check += golden[i];

            if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
                gc8034a_otp_info.golden_rg = (golden[0] | ((golden[1] & 0xf0) << 4)) > 0 ?
                    (golden[0] | ((golden[1] & 0xf0) << 4)) : RG_TYPICAL;
                gc8034a_otp_info.golden_bg = (((golden[1] & 0x0f) << 8) | golden[2]) > 0 ?
                    (((golden[1] & 0x0f) << 8) | golden[2]) : BG_TYPICAL;
                gc8034a_otp_info.golden_flag = gc8034a_otp_info.golden_flag | 0x01;
            } else
                LOG_INF("GC8034A_OTP_GOLDEN Check sum %d Error !!\n", index + 1);

            break;
        case 0x80:
        case 0xc0:
            LOG_INF("GC8034A_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
            gc8034a_otp_info.golden_flag = gc8034a_otp_info.golden_flag | 0x02;
            break;
        default:
            break;
        }
        ...
        case 0x00:
            LOG_INF("GC8034A_OTP_AF group %d is Empty !!\n", index + 1);
            gc8034a_otp_info.af_flag = gc8034a_otp_info.af_flag | 0x00;
            break;
        case 0x04:
            //LOG_INF("GC8034_OTP_AF group %d is Valid !!\n", index + 1);
            check = 0;
            gc8034a_read_otp_group(3, AF_ROM_START + index * AF_WIDTH, &af[0], AF_WIDTH);
            for (i = 0; i < AF_WIDTH - 1; i++)
                check += af[i];

            if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
                gc8034a_otp_info.af_infinity = ((af[0] << 4) & 0x0f00) + af[1];
                gc8034a_otp_info.af_macro = ((af[0] << 8) & 0x0f00) + af[2];
                gc8034a_otp_info.af_flag = gc8034a_otp_info.af_flag | 0x01;
            }
            else
            {
                back_checksum &= 0x7b;  //af
                LOG_INF("GC8034A_OTP_AF Check sum %d Error !!\n", index + 1);
            }
            break;
        case 0x08:
        case 0x0c:
            LOG_INF("GC8034A_OTP_AF group %d is Invalid !!\n", index + 1);
            gc8034a_otp_info.af_flag = gc8034a_otp_info.af_flag | 0x02;
            break;
        default:
            break;
        }
    ...
    #ifdef CONFIG_HUAWEI_CAMERA_DSM
    if(back_checksum != 0xff)
    {
        camerasensor_report_dsm_err(DSM_CAMERA_BACK_OTP_ERR,0,0);
        LOG_INF("camerasensor_report_dsm_err DSM_CAMERA_BACK_OTP_ERR \n");
    }
    #endif
    /* LSC */
    flag_lsc = gc8034a_read_otp(3, 0x43);
    //LOG_INF("flag_lsc = 0x%x\n", flag_lsc);
    gc8034a_otp_info.lsc_flag = 2;
    for (index = 0; index < 2; index++) {
        switch ((flag_lsc << (2 * index)) & 0x0c) {
        case 0x00:
            LOG_INF("GC8034A_OTP_LSC group %d is Empty !\n", index + 1);
            break;
        case 0x04:
            //LOG_INF("GC8034_OTP_LSC group %d is Valid !\n", index + 1);
            gc8034a_otp_info.lsc_flag = index;
            break;
        case 0x08:
        case 0x0c:
            LOG_INF("GC8034A_OTP_LSC group %d is Invalid !!\n", index + 1);
            break;
        default:
            break;
        }
    ...
    /* chip regs */
    gc8034a_otp_info.reg_flag = gc8034a_read_otp(2, 0x4e);

    if (gc8034a_otp_info.reg_flag == 1)
        for (i = 0; i < 5; i++) {
            temp = gc8034a_read_otp(2, 0x4f + 5 * i);
            for (j = 0; j < 2; j++)
                if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
                    gc8034a_otp_info.reg_page[gc8034a_otp_info.reg_num] = (temp >> (4 * j)) & 0x03;
                    gc8034a_otp_info.reg_addr[gc8034a_otp_info.reg_num] =
                        gc8034a_read_otp(2, 0x50 + 5 * i + 2 * j);
                    gc8034a_otp_info.reg_value[gc8034a_otp_info.reg_num] =
                        gc8034a_read_otp(2, 0x50 + 5 * i + 2 * j + 1);
                    gc8034a_otp_info.reg_num++;
                }
        }
}

static void gc8034a_gcore_check_prsel(void)
{
    kal_uint8 product_level = 0;

    product_level = gc8034a_read_otp(2, 0x68) & 0x07;
...
static void gc8034a_gcore_update_dd(void)
{
    kal_uint8 state = 0;

    if (0x01 == gc8034a_otp_info.dd_flag) {
        //LOG_INF("GC8034_OTP_AUTO_DD start !\n");
        write_cmos_sensor(0xfe, 0x00);
        write_cmos_sensor(0x79, 0x2e);
        write_cmos_sensor(0x7b, gc8034a_otp_info.dd_cnt);
...
#ifdef GC8034AOTP_FOR_CUSTOMER
static int gc8034a_gcore_update_wb(void)
{
    ...
    if (0x01 == (gc8034a_otp_info.golden_flag & 0x01)) {
        rg_typical = gc8034a_otp_info.golden_rg;
        bg_typical = gc8034a_otp_info.golden_bg;
    ...
    if(gc8034a_otp_info.rg_gain == 0 || gc8034a_otp_info.bg_gain == 0)
        return ERROR_NONE;
    if (0x01 == (gc8034a_otp_info.wb_flag & 0x01)) {
        r_gain_current = 2048 * rg_typical / gc8034a_otp_info.rg_gain;
        b_gain_current = 2048 * bg_typical / gc8034a_otp_info.bg_gain;
        g_gain_current = 2048;
...
static void gc8034a_gcore_update_lsc(void)
{
    kal_uint8 state = 0;

    if (0x02 != gc8034a_otp_info.lsc_flag) {
        write_cmos_sensor(0xfe, 0x00);
        write_cmos_sensor(0x78, 0x9b);
        write_cmos_sensor(0x79, 0x0d);
        write_cmos_sensor(0x7a, LSC_NUM);
        write_cmos_sensor(0x7c, LSC_ADDR[gc8034a_otp_info.lsc_flag * 2]);
        write_cmos_sensor(0x7d, LSC_ADDR[gc8034a_otp_info.lsc_flag * 2 + 1]);
        write_cmos_sensor(0x6e, 0x01);
        write_cmos_sensor(0xfe, 0x01);
...
static void gc8034a_gcore_update_chipversion(void)
{
    kal_uint8 i = 0;

    //LOG_INF("GC8034_OTP_UPDATE_CHIPVERSION:reg_num = %d\n", gc8034_otp_info.reg_num);

    if (gc8034a_otp_info.reg_flag)
        for (i = 0; i < gc8034a_otp_info.reg_num; i++) {
            write_cmos_sensor(0xfe, gc8034a_otp_info.reg_page[i]);
            write_cmos_sensor(gc8034a_otp_info.reg_addr[i], gc8034a_otp_info.reg_value[i]);
...
static void gc8034a_gcore_update_otp(void)
{
    gc8034a_gcore_update_dd();
    gc8034a_gcore_check_prsel();
#ifdef GC8034AOTP_FOR_CUSTOMER
    gc8034a_gcore_update_wb();
    gc8034a_gcore_update_lsc();
#endif
    gc8034a_gcore_update_chipversion();
}

static void gc8034a_gcore_enable_otp(kal_uint8 state)
{
...
static void gc8034a_gcore_identify_otp(void)
{
    gc8034a_gcore_enable_otp(otp_open);
    gc8034a_gcore_read_otp_info();
    gc8034a_gcore_update_otp();
    gc8034a_gcore_enable_otp(otp_close);
}
...
static kal_uint32 return_sensor_id(void)
{
    ...
    if(sensor_id == GC8034_SENSOR_ID )
    {
        ...
        gc8034a_gcore_enable_otp(otp_open);
        mode_id1 = gc8034a_read_otp(9,0x70);
        mode_id2 = gc8034a_read_otp(9,0x78);
        //LOG_INF("mode_id1 = %d\n", mode_id1);
        //LOG_INF("mode_id2 = %d\n", mode_id2);
        gc8034a_gcore_enable_otp(otp_close);
        if(mode_id1 == SANGLAISHI_MOD_ID || mode_id2 == SANGLAISHI_MOD_ID)//sanyingxing  0x02    sanglaishi 0x0c
        {
            sensor_id = GC8034A_SENSOR_ID;
...
static int set_max_framerate(kal_uint16 framerate, kal_bool min_framelength_en)
{
    kal_uint32 frame_length = imgsensor.frame_length;
    if(framerate == 0 || imgsensor.line_length == 0)
        return ERROR_NONE;
...
static kal_uint16 set_shutter(kal_uint16 shutter)
{
    unsigned long flags;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    for (gain_index = 8; gain_index >= 0; gain_index--)
        if (real_gain >= GC8034A_AGC_Param[gain_index].gain_level) {
            write_cmos_sensor(0xb6, gain_index);
            temp_gain = 256 * real_gain / GC8034A_AGC_Param[gain_index].gain_level;
            write_cmos_sensor(0xb1, temp_gain >> 8);
            write_cmos_sensor(0xb2, (temp_gain & 0xff));
            for (i = 0; i < AGC_REG_NUM; i++)
                write_cmos_sensor(GC8034A_AGC_Param[gain_index].agc_register[i].addr,
                    GC8034A_AGC_Param[gain_index].agc_register[i].value);
            break;
        }
    write_cmos_sensor(0xfe, 0x00);
...
static void set_af_pewer_down_mode(void)
{
            ...
            do {
                *sensor_id = return_sensor_id();
                if (*sensor_id == imgsensor_info.sensor_id) {
                    ...
                    set_af_pewer_down_mode();
                    back_camera_id = 6;
                    back_camera_probe_ok = 1;
                    return ERROR_NONE;
                }
...
{
    if(image_window != NULL && sensor_config_data != NULL){
        LOG_INF("E\n");
        spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        if(imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
            if(framerate == 0 ||  imgsensor_info.cap1.linelength == 0)
                return ERROR_NONE;
...
UINT32 GC8034AMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* Check Sensor status here */
    if (pfFunc != NULL)
        *pfFunc = &sensor_func;
    //LOG_INF("shuxinlei add sensor = %s",__FUNCTION__);
    return ERROR_NONE;
}
