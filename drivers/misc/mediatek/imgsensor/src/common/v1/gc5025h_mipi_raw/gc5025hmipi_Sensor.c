...
#include "gc5025hmipi_Sensor.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025H_camera_sensor"
#define LOG_1 LOG_INF("GC5025H,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define GC5025HOTP_FOR_CUSTOMER
extern void kdSetI2CSpeed(u16 i2cSpeed);
extern u8 front_camera_id;
extern bool front_camera_probe_ok;
extern u8 front_checksum;
static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC5025HDuringTestPattern = KAL_FALSE;
...
//sanyingxing  0x02    sanglaishi 0x0c

#define SANYINGXING_MOD_ID    0x02
#define SANGLAISHI_MOD_ID    0x0C

static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025H_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
       ...
       .normal_video = {
           ...
           .starty = 0,//record different mode's starty of grabwindow
           .grabwindow_width = 2592,//record different mode's width of grabwindow //1296
           .grabwindow_height = 1944,//record different mode's width of grabwindow //1296
           .mipi_data_lp2hs_settle_dc = 85,//unit , ns
           .max_framerate = 300,//normal_video max framerate is 30fps
    },
       .hs_video = {
           .pclk = 288000000,                //record different mode's pclk
           .linelength = 4800,                //record different mode's linelength
           .framelength = 2000,//record different mode's framelength
           .startx = 0,//record different mode's starty of grabwindow
           .starty = 0,//record different mode's starty of grabwindow
           .grabwindow_width = 2592,//record different mode's width of grabwindow //1296
           .grabwindow_height = 1944,//record different mode's width of grabwindow //1296
           .mipi_data_lp2hs_settle_dc = 85,//unit , ns
           .max_framerate = 300,//hs_video max framerate is 30fps
    },
        .slim_video = {
          .pclk = 288000000,                //record different mode's pclk
          .linelength = 4800,                //record different mode's linelength
          ...
          .grabwindow_height = 1944,//record different mode's width of grabwindow //1296
          .mipi_data_lp2hs_settle_dc = 85,//unit , ns
          .max_framerate = 300,//slim_video max framerate is 30fps
    },
        ...
        .i2c_addr_table = {0x6e, 0x6c, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
typedef struct otp_gc5025h
{
    ...
    kal_bool checksum_ok;
#endif

}gc5025h_otp;

static gc5025h_otp gc5025h_otp_info;
...
static void gc5025h_select_page_otp(otp_page otp_select_page)
{
    kal_uint8 page;
...
#define MOD_NUM 0xFF //mod for checksum
    ...
    kal_uint8 golden[5];
#endif
    memset(&gc5025h_otp_info,0,sizeof(gc5025h_otp));

    /*TODO*/
    gc5025h_select_page_otp(otp_page0);
    flagdd = gc5025h_read_otp(0x00);
    //LOG_INF("GC5025H_OTP_DD : flag_dd = 0x%x\n",flagdd);
    ...
    {
    case 0x00:
        LOG_INF("GC5025H_OTP_DD is Empty !!\n");
        gc5025h_otp_info.dd_flag = 0x00;
        break;
    case 0x01:
        //LOG_INF("GC5025H_OTP_DD is Valid!!\n");
        if(0x04 == (flagdd&0x0c)) {
            ddoffsetX = 252;
            ddoffsetY = 253;
        }
        else {
            ddoffsetX = 0;  //dd offset in width
            ddoffsetY = 0; //dd offset in hight
        }
        total_number = gc5025h_read_otp(0x01) + gc5025h_read_otp(0x02);
        //LOG_INF("GC5025H_OTP : total_number = %d\n",total_number);

        if (total_number <= 31)
        {
            gc5025h_read_otp_group(0x03, &ddtempbuff[0], total_number * 4);
        }
        else
        {
            gc5025h_read_otp_group(0x03, &ddtempbuff[0], 31 * 4);
            gc5025h_select_page_otp(otp_page1);
            gc5025h_read_otp_group(0x29, &ddtempbuff[31 * 4], (total_number - 31) * 4);
        }

        /*DD check sum*/
        gc5025h_select_page_otp(otp_page1);
        ddchecksum = gc5025h_read_otp(0x61);
        check = total_number;
        ...
        {
        LOG_INF("GC5025H_OTP_DD : DD check sum correct  type 1! checksum = 0x%x\n", ddchecksum);
        }
        else if(((check - total_number) % 255+1) == ddchecksum)
        {
            LOG_INF("GC5025H_OTP_DD : DD check sum correct  type 2! checksum = 0x%x\n", ddchecksum);
        }
        else if(((check - total_number) % 256) == ddchecksum)
        {
            LOG_INF("GC5025H_OTP_DD : DD check sum correct  type 3! checksum = 0x%x\n", ddchecksum);
        }
        else
        {
            LOG_INF("GC5025H_OTP_DD : DD check sum error! otpchecksum = 0x%x, sum = 0x%x\n", ddchecksum, (check % 255 + 1));
        }
        ...
        for(i=0; i<total_number; i++)
        {
            ...
            if (ddtempbuff[4 * i + 3] & 0x10)
            {
                ...
                case 3:
                    for (j = 0; j < 4; j++)
                    {
                        gc5025h_otp_info.dd_param_x[cnt] = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i] - ddoffsetX;
                        gc5025h_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) + j - ddoffsetY;
                        gc5025h_otp_info.dd_param_type[cnt++] = 2;
                    }
                    break;
                case 4:
                    for (j = 0; j < 2; j++)
                ...
                default:
                    gc5025h_otp_info.dd_param_x[cnt] = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i] - ddoffsetX;
                    gc5025h_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) - ddoffsetY;
                    gc5025h_otp_info.dd_param_type[cnt++] = ddtempbuff[4 * i + 3] & 0x0f;
                    break;
                }
            }
            else
            {
                LOG_INF("GC5025H_OTP_DD:check_id[%d] = %x,checkid error!!\n", i, ddtempbuff[4 * i + 3] & 0xf0);
            }
        }

        gc5025h_otp_info.dd_cnt = cnt;
        gc5025h_otp_info.dd_flag = 0x01;
        break;
    case 0x02:
    case 0x03:
        LOG_INF("GC5025H_OTP_DD is Invalid !!\n");
        gc5025h_otp_info.dd_flag = 0x02;
        break;
    default :
        break;
    }
...
/*For Chip Version*/
    //LOG_INF("GC5025H_OTP_CHIPVESION : flag_chipversion = 0x%x\n",flag_chipversion);

    switch((flag_chipversion>>4)&0x03)
    {
    case 0x00:
        LOG_INF("GC5025H_OTP_CHIPVERSION is Empty !!\n");
        break;
    case 0x01:
        //LOG_INF("GC5025H_OTP_CHIPVERSION is Valid !!\n");
        gc5025h_select_page_otp(otp_page1);
        i = 0;
        do{
            gc5025h_otp_info.reg_addr[i] = gc5025h_read_otp(REG_ROM_START + i*2 ) ;
            gc5025h_otp_info.reg_value[i] = gc5025h_read_otp(REG_ROM_START + i*2 + 1 ) ;
            //LOG_INF("GC5025H_OTP_CHIPVERSION reg_addr[%d] = 0x%x,reg_value[%d] = 0x%x\n",i,gc5025h_otp_info.reg_addr[i],i,gc5025h_otp_info.reg_value[i]);
            i++;
        }while((gc5025h_otp_info.reg_addr[i-1]!=0)&&(i<10));
        gc5025h_otp_info.reg_num = i - 1;
        break;
    case 0x02:
        LOG_INF("GC5025H_OTP_CHIPVERSION is Invalid !!\n");
        break;
    default :
        break;
    }
    ...
    front_checksum = 0xff;
    //LOG_INF("GC5025H_OTP : flag1 = 0x%x , flag_golden = 0x%x\n",flag1,flag_golden);
...
//INFO&WB
    for(index=0;index<2;index++)
    {
        ...
        {
        case 0x00:
            LOG_INF("GC5025H_OTP_INFO group %d is Empty !!\n", index + 1);
            break;
        case 0x01:
            //LOG_INF("GC5025H_OTP_INFO group %d is Valid !!\n", index + 1);
            check = 0;
            gc5025h_read_otp_group(INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
            for (i = 0; i < INFO_WIDTH - 1; i++)
            {
                check += info[i];
            }
            gc5025h_otp_info.module_id = info[0];
            gc5025h_otp_info.lens_id = info[1];
            gc5025h_otp_info.vcm_driver_id = info[2];
            gc5025h_otp_info.vcm_id = info[3];
            gc5025h_otp_info.year = info[4];
            gc5025h_otp_info.month = info[5];
            gc5025h_otp_info.day = info[6];
            break;
        case 0x02:
        case 0x03:
            LOG_INF("GC5025H_OTP_INFO group %d is Invalid !!\n", index + 1);
            break;
        default :
            break;
        }
        ...
        {
        case 0x00:
            //LOG_INF("GC5025H_OTP_WB group %d is Empty !!\n", index + 1);
            gc5025h_otp_info.wb_flag = gc5025h_otp_info.wb_flag|0x00;
            break;
        case 0x01:
            LOG_INF("GC5025H_OTP_WB group %d is Valid !!\n", index + 1);
            //check = 0;
            gc5025h_read_otp_group(WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
            for (i = 0; i < WB_WIDTH - 1; i++)
            {
                check += wb[i];
            }
            gc5025h_otp_info.rg_gain = (((wb[0]<<8)&0xff00)|wb[1]) > 0 ? (((wb[0]<<8)&0xff00)|wb[1]) : 0x400;
            gc5025h_otp_info.bg_gain = (((wb[2]<<8)&0xff00)|wb[3]) > 0 ? (((wb[2]<<8)&0xff00)|wb[3]) : 0x400;
            gc5025h_otp_info.wb_flag = gc5025h_otp_info.wb_flag|0x01;

            break;
        case 0x02:
        case 0x03:
            LOG_INF("GC5025H_OTP_WB group %d is Invalid !!\n", index + 1);
            gc5025h_otp_info.wb_flag = gc5025h_otp_info.wb_flag|0x02;
            break;
        default :
            break;
        }
        ...
        {
        case 0x00:
            LOG_INF("GC5025H_OTP_GOLDEN group %d is Empty !!\n", index + 1);
            gc5025h_otp_info.golden_flag = gc5025h_otp_info.golden_flag|0x00;
            break;
        case 0x01:
            //LOG_INF("GC5025H_OTP_GOLDEN group %d is Valid !!\n", index + 1);
            //check = 0;
            gc5025h_read_otp_group(GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
            for (i = 0; i < GOLDEN_WIDTH - 1; i++)
            {
                check += golden[i];
            }
            check_sum_all = golden[GOLDEN_WIDTH - 1];
            gc5025h_otp_info.golden_rg = (((golden[0]<<8)&0xff00)|golden[1]) > 0 ? (((golden[0]<<8)&0xff00)|golden[1]) : RG_TYPICAL;
            gc5025h_otp_info.golden_bg = (((golden[2]<<8)&0xff00)|golden[3]) > 0 ? (((golden[2]<<8)&0xff00)|golden[3]) : BG_TYPICAL;
            gc5025h_otp_info.golden_flag = gc5025h_otp_info.golden_flag|0x01;
            break;
        case 0x02:
        case 0x03:
            LOG_INF("GC5025H_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
            gc5025h_otp_info.golden_flag = gc5025h_otp_info.golden_flag|0x02;
            break;
        default :
            break;
        }
    ...
    if ((check % MOD_NUM + 1) == check_sum_all){
        gc5025h_otp_info.checksum_ok = KAL_TRUE;
        front_checksum = 0xff;
        //LOG_INF("GC5025H_OTP_INFO checksum ok :cal_checksum=0x%x, otp_checksum =0x%x\n",(check % MOD_NUM + 1), check_sum_all);
        //LOG_INF("GC5025H_OTP_INFO:module_id=0x%x\n",gc5025h_otp_info.module_id);
        //LOG_INF("GC5025H_OTP_INFO:lens_id=0x%x\n",gc5025h_otp_info.lens_id);
        //LOG_INF("GC5025H_OTP_INFO:vcm_id=0x%x\n",gc5025h_otp_info.vcm_id);
        //LOG_INF("GC5025H_OTP_INFO:vcm_driver_id=0x%x\n",gc5025h_otp_info.vcm_driver_id);
        //LOG_INF("GC5025H_OTP_INFO:data=%d-%d-%d\n",gc5025h_otp_info.year,gc5025h_otp_info.month,gc5025h_otp_info.day);
        //LOG_INF("GC5025H_OTP_WB:r/g=0x%x\n",gc5025h_otp_info.rg_gain);
        //LOG_INF("GC5025H_OTP_WB:b/g=0x%x\n",gc5025h_otp_info.bg_gain);
        //LOG_INF("GC5025H_OTP_GOLDEN:golden_rg=0x%x\n",gc5025h_otp_info.golden_rg);
        //LOG_INF("GC5025H_OTP_GOLDEN:golden_bg=0x%x\n",gc5025h_otp_info.golden_bg);
    }
    else{
        gc5025h_otp_info.checksum_ok = KAL_FALSE;
        front_checksum = 0x7d;
        //LOG_INF("GC5025H_OTP_INFO checksum error :cal_checksum=0x%x, otp_checksum =0x%x\n",(check % MOD_NUM + 1), check_sum_all);
    }
    #ifdef CONFIG_HUAWEI_CAMERA_DSM
...
static void gc5025h_gcore_update_dd(void)
{
    ...
    if(0x01 ==gc5025h_otp_info.dd_flag)
    {
#if defined(IMAGE_NORMAL_MIRROR)
    //do nothing
#elif defined(IMAGE_H_MIRROR)
    for(i=0; i<gc5025h_otp_info.dd_cnt; i++)
    {
        if(gc5025h_otp_info.dd_param_type[i]==0)
        {   gc5025h_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5025h_otp_info.dd_param_x[i]+1;    }
        else if(gc5025h_otp_info.dd_param_type[i]==1)
        {   gc5025h_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5025h_otp_info.dd_param_x[i]-1;    }
        else
        {   gc5025h_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5025h_otp_info.dd_param_x[i] ; }
    }
#elif defined(IMAGE_V_MIRROR)
    for(i=0; i<gc5025h_otp_info.dd_cnt; i++){
        gc5025h_otp_info.dd_param_y[i]= WINDOW_HEIGHT - gc5025h_otp_info.dd_param_y[i] + 1;
    }
    ...
    for(i=0; i<gc5025h_otp_info.dd_cnt; i++)
        ...
        //y
        for(i=0 ; i< gc5025h_otp_info.dd_cnt-1; i++)
        {
            for(j = i+1; j < gc5025h_otp_info.dd_cnt; j++)
            {
                if(gc5025h_otp_info.dd_param_y[i] > gc5025h_otp_info.dd_param_y[j])
                {
                    temp_x = gc5025h_otp_info.dd_param_x[i] ; gc5025h_otp_info.dd_param_x[i] = gc5025h_otp_info.dd_param_x[j] ;  gc5025h_otp_info.dd_param_x[j] = temp_x;
                    temp_y = gc5025h_otp_info.dd_param_y[i] ; gc5025h_otp_info.dd_param_y[i] = gc5025h_otp_info.dd_param_y[j] ;  gc5025h_otp_info.dd_param_y[j] = temp_y;
                    temp_type = gc5025h_otp_info.dd_param_type[i] ; gc5025h_otp_info.dd_param_type[i] = gc5025h_otp_info.dd_param_type[j]; gc5025h_otp_info.dd_param_type[j]= temp_type;
                }
            }
        ...
        for(i=0; i<gc5025h_otp_info.dd_cnt; i++)
        {
            if(gc5025h_otp_info.dd_param_y[i]==gc5025h_otp_info.dd_param_y[i+1])
            {
                s=i++;
                while((gc5025h_otp_info.dd_param_y[s] == gc5025h_otp_info.dd_param_y[i+1])&&(i<gc5025h_otp_info.dd_cnt-1))
                    i++;
                e=i;
                ...
                for(n=s; n<e; n++)
                {
                    for(m=n+1; m<e+1; m++)
                    {
                        if(gc5025h_otp_info.dd_param_x[n] > gc5025h_otp_info.dd_param_x[m])
                        {
                            temp_x = gc5025h_otp_info.dd_param_x[n] ; gc5025h_otp_info.dd_param_x[n] = gc5025h_otp_info.dd_param_x[m] ;  gc5025h_otp_info.dd_param_x[m] = temp_x;
                            temp_y = gc5025h_otp_info.dd_param_y[n] ; gc5025h_otp_info.dd_param_y[n] = gc5025h_otp_info.dd_param_y[m] ;  gc5025h_otp_info.dd_param_y[m] = temp_y;
                            temp_type = gc5025h_otp_info.dd_param_type[n] ; gc5025h_otp_info.dd_param_type[n] = gc5025h_otp_info.dd_param_type[m]; gc5025h_otp_info.dd_param_type[m]= temp_type;
                        }
                    }
                }
        ...
        for(i=0; i<gc5025h_otp_info.dd_cnt; i++)
        {
            temp_val0 = gc5025h_otp_info.dd_param_x[i]& 0x00ff;
            temp_val1 = (((gc5025h_otp_info.dd_param_y[i])<<4)& 0x00f0) + ((gc5025h_otp_info.dd_param_x[i]>>8)&0X000f);
            temp_val2 = (gc5025h_otp_info.dd_param_y[i]>>4) & 0xff;
            write_cmos_sensor(0xaa,i);
            write_cmos_sensor(0xac,temp_val0);
            write_cmos_sensor(0xac,temp_val1);
            write_cmos_sensor(0xac,temp_val2);
            while((i < gc5025h_otp_info.dd_cnt - 1) && (gc5025h_otp_info.dd_param_x[i]==gc5025h_otp_info.dd_param_x[i+1]) && (gc5025h_otp_info.dd_param_y[i]==gc5025h_otp_info.dd_param_y[i+1]))
                {
                    flag = 1;
                    i++;
                }
            if(flag)
                ...
                write_cmos_sensor(0xac,gc5025h_otp_info.dd_param_type[i]);

            //LOG_INF("GC5025H_OTP_GC val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",temp_val0,temp_val1,temp_val2);
            //LOG_INF("GC5025H_OTP_GC x = %d , y = %d \n",((temp_val1&0x0f)<<8) + temp_val0,(temp_val2<<4) + ((temp_val1&0xf0)>>4));
        }
...
static void gc5025h_gcore_update_wb(void)
{
    ...
    if(0x02==gc5025h_otp_info.golden_flag)
    {
        return;
    }
    if(0x00==(gc5025h_otp_info.golden_flag&0x01))
    {
        rg_typical=RG_TYPICAL;
        bg_typical=BG_TYPICAL;
    }
    if(0x01==(gc5025h_otp_info.golden_flag&0x01))
    {
        rg_typical=gc5025h_otp_info.golden_rg;
        bg_typical=gc5025h_otp_info.golden_bg;
        //LOG_INF("GC5025H_OTP_UPDATE_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);
    }

    if(0x01==(gc5025h_otp_info.wb_flag&0x01))
    {
        r_gain_current = 1024 * rg_typical/gc5025h_otp_info.rg_gain;
        b_gain_current = 1024 * bg_typical/gc5025h_otp_info.bg_gain;
        g_gain_current = 1024;
...
static void gc5025h_gcore_update_chipversion(void)
{
    ...
    //LOG_INF("GC5025H_OTP_UPDATE_CHIPVERSION:reg_num = %d\n",gc5025h_otp_info.reg_num);

    write_cmos_sensor(0xfe,0x00);

    for(i=0;i<gc5025h_otp_info.reg_num;i++)
    {
        write_cmos_sensor(gc5025h_otp_info.reg_addr[i] ,gc5025h_otp_info.reg_value[i]);
    }
...
#ifdef GC5025HOTP_FOR_CUSTOMER
    if(gc5025h_otp_info.checksum_ok == KAL_TRUE){
...
static void gc5025h_gcore_enable_otp(otp_state state)
{
    ...
    if(state)
    {
        ...
        //LOG_INF("GC5025H_OTP: Enable OTP!\n");
    }
    else
    {
        otp_en = otp_en & 0x7f;
        ...
        //LOG_INF("GC5025H_OTP: Disable OTP!\n");
    }
...
static int gc5025h_gcore_check_version(void)
{
    kal_uint8 flag_GC5025H = 0;

    gc5025h_gcore_initial_otp();
    gc5025h_gcore_enable_otp(otp_open);

    gc5025h_select_page_otp(otp_page1);
    flag_GC5025H = gc5025h_read_otp(0x27);

    gc5025h_gcore_enable_otp(otp_close);

    if((flag_GC5025H & 0x01) == 0x01) {
        //LOG_INF("GC5025H sensor! \n");
        return 0;
    }
    else {
        LOG_INF("error! \n");
...
{
    kal_uint32 read_sensor_id = 0;
    int mode_id = 0,mode_id2 = 0;
    read_sensor_id = (read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1);
    if(read_sensor_id == GC5025_SENSOR_ID )
    {
        gc5025h_gcore_initial_otp();
        gc5025h_gcore_enable_otp(otp_open);
        gc5025h_select_page_otp(otp_page1);
        ...
        //LOG_INF("mode_id2 = %d\n", mode_id2);
        gc5025h_gcore_enable_otp(otp_close);
        if (mode_id == SANYINGXING_MOD_ID || mode_id2 == SANYINGXING_MOD_ID)
        {
            read_sensor_id = GC5025H_SENSOR_ID;
        }
        else
        {
            read_sensor_id = 0;
...
static void set_shutter(kal_uint16 shutter)
{
    ...
    Dgain_ratio = (shutter << 8) / cal_shutter;
    if(cal_shutter <= 10)
    {
        write_cmos_sensor(0xfe, 0x00);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
    {
        ...
        temp = (iReg*Dgain_ratio) >> 8;//gain convert
        write_cmos_sensor(0xb1, temp>>6);
        write_cmos_sensor(0xb2, (temp<<2) & 0xfc);
        //LOG_INF("GC5025HMIPI analogic gain 1x, GC5025HMIPI add pregain = %d\n", temp);
    }
    else if((ANALOG_GAIN_2 <= iReg) && (iReg < ANALOG_GAIN_3))
    {
        write_cmos_sensor(0xfe,  0x00);
        write_cmos_sensor(0xb6,  0x01);
        temp = (iReg<< 6)/ANALOG_GAIN_2;
        temp = (temp*Dgain_ratio) >> 8;//gain convert
        write_cmos_sensor(0xb1, temp>>6);
        write_cmos_sensor(0xb2, (temp<<2)&0xfc);
        //LOG_INF("GC5025HMIPI analogic gain 1.4x, GC5025HMIPI add pregain = %d\n", temp);
    }
    else
    {
        write_cmos_sensor(0xfe,  0x00);
        write_cmos_sensor(0xb6,  0x02);
        temp = (iReg<< 6)/ANALOG_GAIN_3;
        temp = (temp*Dgain_ratio) >> 8;//gain convert
        write_cmos_sensor(0xb1, temp>>6);
        write_cmos_sensor(0xb2, (temp<<2)&0xfc);
        //LOG_INF("GC5025HMIPI analogic gain 2x, GC5025HMIPI add pregain = %d\n", temp);
    }
...
static kal_uint32 open(void)
{
   ...
   /*Don't Remove!!*/
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC5025HDuringTestPattern = KAL_FALSE;
