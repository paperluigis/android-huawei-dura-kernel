...
extern u8 back_camera_probe_ok;
...
static int read_otp(struct otp_struct *otp_ptr)
{
    if(otp_ptr != NULL){
        ...
        for(i=0;i<7;i++) {
            (* otp_ptr).awb[i]=read_cmos_sensor(addr + i);
            awb_checksum += (* otp_ptr).awb[i];
            //LOG_INF("awb_checksum : 0x%x\n",awb_checksum);
        }
            ...
            //LOG_INF("production_year_month = %d,production_day = %d,rg_ratio = %d, bg_ratio = %d, rg_ratio_golden =%d, bg_ratio_golden  =%d\n", (*otp_ptr).production_year_month,(*otp_ptr).production_day,(*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio, (*otp_ptr).rg_ratio_golden, (*otp_ptr).bg_ratio_golden);
        }
        else {
        ...
        if((otp_flag & 0xc0) == 0x40) {
            addr = 0x7020; // base address of VCM Calibration group 1
        }
        ...
        {
            back_checksum &= 0x7b;
            LOG_INF(" vcm group1 group2 all error \n");
            return 1;
        }
        if(addr != 0) {
        ...
        for(i=0;i<3;i++) {
            (* otp_ptr).vcm[i]=read_cmos_sensor(addr + i);
            vcm_checksum += (* otp_ptr).vcm[i];
        }
        vcm_checksum = (vcm_checksum)%255 +1;
        (*otp_ptr).vcm_checksum = read_cmos_sensor(addr + 3);
        //LOG_INF("vcm_checksum = %x,(*otp_ptr).vcm_checksum = %x\n", vcm_checksum, (*otp_ptr).vcm_checksum);
        if((*otp_ptr).vcm_checksum == vcm_checksum){
            (*otp_ptr).flag |= 0x10;
            //LOG_INF("Read VCM OTP Successful\n");
        }
        else{
            back_checksum &= 0x7b;
            LOG_INF("Read VCM OTP Check sum failed\n");
        }
        // OTP Lenc Calibration
        otp_flag = read_cmos_sensor(0x7028);
        addr = 0;
        //LOG_INF("lenc_otp_flag = %d, \n", otp_flag);
        if((otp_flag & 0xc0) == 0x40) {
        ...
        if(addr != 0) {
            for(i=0;i<240;i++) {
                ...
                //LOG_INF("lenc[%d] = %x,\n", i, (*otp_ptr).lenc[i]);
                lsc_checksum += (* otp_ptr).lenc[i];
            }
            checksum = (awb_checksum+lsc_checksum)%255 +1;
            (*otp_ptr).checksum = read_cmos_sensor(addr + 240);
            //LOG_INF("checksum = %x,(*otp_ptr).checksum = %x\n", checksum, (*otp_ptr).checksum);
...
static int set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
...
static kal_uint16 set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
    unsigned long flags;
    ...
    if (imgsensor.autoflicker_en) {
        if(imgsensor.line_length == 0 || imgsensor.line_length ==0)
            return ERROR_NONE;
