...
#include "gc5025amipi_Sensor.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025A_camera_sensor"
#define LOG_1 LOG_INF("GC5025A,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define GC5025AOTP_FOR_CUSTOMER
extern void kdSetI2CSpeed(u16 i2cSpeed);
extern u8 front_camera_id;
extern bool front_camera_probe_ok;
extern u8 front_checksum;
static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC5025ADuringTestPattern = KAL_FALSE;
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025A_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint8 gc5025a_read_otp(kal_uint8 addr)
{
    kal_uint8 value;
    kal_uint8 regd4;
...
static void gc5025a_read_otp_group(kal_uint8 addr,kal_uint8* buff,int size)
{
    kal_uint8 i;
    kal_uint8 regd4;
    kal_uint16 realaddr = addr * 8;
...
static void gc5025a_select_page_otp(otp_page otp_select_page)
{
    kal_uint8 page;
...
static void gc5025a_gcore_read_otp_info(void)
{
#define MOD_NUM 0xFF //mod for checksum
    ...
    /*TODO*/
    gc5025a_select_page_otp(otp_page0);
    flagdd = gc5025a_read_otp(0x00);
    //LOG_INF("GC5025A_OTP_DD : flag_dd = 0x%x\n",flagdd);
    flag_chipversion= gc5025a_read_otp(0x7f);

    //DD
    switch(flagdd&0x03)
    {
    case 0x00:
        LOG_INF("GC5025A_OTP_DD is Empty !!\n");
        gc5025a_otp_info.dd_flag = 0x00;
        break;
    case 0x01:
        //LOG_INF("GC5025A_OTP_DD is Valid!!\n");
        if(0x04 == (flagdd&0x0c)) {
            ddoffsetX = 252;
            ddoffsetY = 253;
        }
        ...
        total_number = gc5025a_read_otp(0x01) + gc5025a_read_otp(0x02);
        //LOG_INF("GC5025A_OTP : total_number = %d\n",total_number);

        if (total_number <= 31)
        {
            gc5025a_read_otp_group(0x03, &ddtempbuff[0], total_number * 4);
        }
        else
        {
            gc5025a_read_otp_group(0x03, &ddtempbuff[0], 31 * 4);
            gc5025a_select_page_otp(otp_page1);
            gc5025a_read_otp_group(0x29, &ddtempbuff[31 * 4], (total_number - 31) * 4);
        }

        /*DD check sum*/
        gc5025a_select_page_otp(otp_page1);
        ddchecksum = gc5025a_read_otp(0x61);
        check = total_number;
        ...
        {
        LOG_INF("GC5025A_OTP_DD : DD check sum correct  type 1! checksum = 0x%x\n", ddchecksum);
        }
        else if(((check - total_number) % 255+1) == ddchecksum)
        {
            LOG_INF("GC5025A_OTP_DD : DD check sum correct  type 2! checksum = 0x%x\n", ddchecksum);
        }
        else if(((check - total_number) % 256) == ddchecksum)
        {
            LOG_INF("GC5025A_OTP_DD : DD check sum correct  type 3! checksum = 0x%x\n", ddchecksum);
        }
        else
        {
            LOG_INF("GC5025A_OTP_DD : DD check sum error! otpchecksum = 0x%x, sum = 0x%x\n", ddchecksum, (check % 255 + 1));
        }
        ...
        for(i=0; i<total_number; i++)
        {
            ...
            if (ddtempbuff[4 * i + 3] & 0x10)
            {
                ...
                case 3:
                    for (j = 0; j < 4; j++)
                    {
                        gc5025a_otp_info.dd_param_x[cnt] = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i] - ddoffsetX;
                        gc5025a_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) + j - ddoffsetY;
                ...
                default:
                    gc5025a_otp_info.dd_param_x[cnt] = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i] - ddoffsetX;
                    gc5025a_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) - ddoffsetY;
            ...
            {
                LOG_INF("GC5025A_OTP_DD:check_id[%d] = %x,checkid error!!\n", i, ddtempbuff[4 * i + 3] & 0xf0);
            }
        }
    ...
    case 0x02:
    case 0x03:
        LOG_INF("GC5025A_OTP_DD is Invalid !!\n");
...
/*For Chip Version*/
    //LOG_INF("GC5025A_OTP_CHIPVESION : flag_chipversion = 0x%x\n",flag_chipversion);

    switch((flag_chipversion>>4)&0x03)
    {
    case 0x00:
        LOG_INF("GC5025A_OTP_CHIPVERSION is Empty !!\n");
        break;
    case 0x01:
        //LOG_INF("GC5025A_OTP_CHIPVERSION is Valid !!\n");
        gc5025a_select_page_otp(otp_page1);
        i = 0;
        do{
            gc5025a_otp_info.reg_addr[i] = gc5025a_read_otp(REG_ROM_START + i*2 ) ;
            gc5025a_otp_info.reg_value[i] = gc5025a_read_otp(REG_ROM_START + i*2 + 1 ) ;
            LOG_INF("GC5025A_OTP_CHIPVERSION reg_addr[%d] = 0x%x,reg_value[%d] = 0x%x\n",i,gc5025a_otp_info.reg_addr[i],i,gc5025a_otp_info.reg_value[i]);
            i++;
        }while((gc5025a_otp_info.reg_addr[i-1]!=0)&&(i<10));
        gc5025a_otp_info.reg_num = i - 1;
        break;
    case 0x02:
        LOG_INF("GC5025A_OTP_CHIPVERSION is Invalid !!\n");
        break;
    default :
        break;
    }
...
#ifdef GC5025AOTP_FOR_CUSTOMER
    gc5025a_select_page_otp(otp_page1);
    flag1 = gc5025a_read_otp(0x00);
    flag_golden = gc5025a_read_otp(0x1b);
    front_checksum = 0xff;
    //LOG_INF("GC5025A_OTP : flag1 = 0x%x , flag_golden = 0x%x\n",flag1,flag_golden);
...
//INFO&WB
    for(index=0;index<2;index++)
    {
        ...
        {
        case 0x00:
            LOG_INF("GC5025A_OTP_INFO group %d is Empty !!\n", index + 1);
            break;
        case 0x01:
            //LOG_INF("GC5025A_OTP_INFO group %d is Valid !!\n", index + 1);
            check = 0;
            gc5025a_read_otp_group(INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
            for (i = 0; i < INFO_WIDTH - 1; i++)
            {
                ...
                gc5025a_otp_info.vcm_driver_id = info[2];
                gc5025a_otp_info.vcm_id = info[3];
                gc5025a_otp_info.year = info[4];
                gc5025a_otp_info.month = info[5];
                gc5025a_otp_info.day = info[6];
            break;
        case 0x02:
        case 0x03:
            LOG_INF("GC5025A_OTP_INFO group %d is Invalid !!\n", index + 1);
            break;
        default :
            break;
        }
        ...
        {
        case 0x00:
            LOG_INF("GC5025A_OTP_WB group %d is Empty !!\n", index + 1);
            gc5025a_otp_info.wb_flag = gc5025a_otp_info.wb_flag|0x00;
            break;
        case 0x01:
            //LOG_INF("GC5025A_OTP_WB group %d is Valid !!\n", index + 1);
            //check = 0;
            gc5025a_read_otp_group(WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
            for (i = 0; i < WB_WIDTH - 1; i++)
            {
                check += wb[i];
            }
                gc5025a_otp_info.rg_gain = (((wb[0]<<8)&0xff00)|wb[1]) > 0 ? (((wb[0]<<8)&0xff00)|wb[1]) : 0x400;
                gc5025a_otp_info.bg_gain = (((wb[2]<<8)&0xff00)|wb[3]) > 0 ? (((wb[2]<<8)&0xff00)|wb[3]) : 0x400;
                gc5025a_otp_info.wb_flag = gc5025a_otp_info.wb_flag|0x01;
            break;
        case 0x02:
        case 0x03:
            LOG_INF("GC5025A_OTP_WB group %d is Invalid !!\n", index + 1);
            gc5025a_otp_info.wb_flag = gc5025a_otp_info.wb_flag|0x02;
            break;
        default :
            break;
        }
        ...
        {
        case 0x00:
            LOG_INF("GC5025A_OTP_GOLDEN group %d is Empty !!\n", index + 1);
            gc5025a_otp_info.golden_flag = gc5025a_otp_info.golden_flag|0x00;
            break;
        case 0x01:
            //LOG_INF("GC5025A_OTP_GOLDEN group %d is Valid !!\n", index + 1);
            //check = 0;
            gc5025a_read_otp_group(GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
            for (i = 0; i < GOLDEN_WIDTH - 1; i++)
            {
                check += golden[i];
            }
                check_sum_all = golden[GOLDEN_WIDTH - 1];
                gc5025a_otp_info.golden_rg = (((golden[0]<<8)&0xff00)|golden[1]) > 0 ? (((golden[0]<<8)&0xff00)|golden[1]) : RG_TYPICAL;
                gc5025a_otp_info.golden_bg = (((golden[2]<<8)&0xff00)|golden[3]) > 0 ? (((golden[2]<<8)&0xff00)|golden[3]) : BG_TYPICAL;
                gc5025a_otp_info.golden_flag = gc5025a_otp_info.golden_flag|0x01;
            break;
        case 0x02:
        case 0x03:
            LOG_INF("GC5025A_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
            gc5025a_otp_info.golden_flag = gc5025a_otp_info.golden_flag|0x02;
            break;
        default :
            break;
        }
    ...
    if ((check % MOD_NUM + 1) == check_sum_all){
        gc5025a_otp_info.checksum_ok = KAL_TRUE;
        front_checksum = 0xff;
        //LOG_INF("GC5025H_OTP_INFO checksum ok :cal_checksum=0x%x, otp_checksum =0x%x\n",(check % MOD_NUM + 1), check_sum_all);
        //LOG_INF("GC5025A_OTP_INFO:module_id=0x%x\n",gc5025a_otp_info.module_id);
        //LOG_INF("GC5025A_OTP_INFO:lens_id=0x%x\n",gc5025a_otp_info.lens_id);
        //LOG_INF("GC5025A_OTP_INFO:vcm_id=0x%x\n",gc5025a_otp_info.vcm_id);
        //LOG_INF("GC5025A_OTP_INFO:vcm_driver_id=0x%x\n",gc5025a_otp_info.vcm_driver_id);
        //LOG_INF("GC5025A_OTP_INFO:data=%d-%d-%d\n",gc5025a_otp_info.year,gc5025a_otp_info.month,gc5025a_otp_info.day);
        //LOG_INF("GC5025A_OTP_WB:r/g=0x%x\n",gc5025a_otp_info.rg_gain);
        //LOG_INF("GC5025A_OTP_WB:b/g=0x%x\n",gc5025a_otp_info.bg_gain);
        //LOG_INF("GC5025A_OTP_GOLDEN:golden_rg=0x%x\n",gc5025a_otp_info.golden_rg);
        //LOG_INF("GC5025A_OTP_GOLDEN:golden_bg=0x%x\n",gc5025a_otp_info.golden_bg);
    }
    else{
        gc5025a_otp_info.checksum_ok = KAL_FALSE;
        front_checksum = 0x7d;
        LOG_INF("GC5025A_OTP_INFO checksum error :cal_checksum=0x%x, otp_checksum =0x%x\n",(check % MOD_NUM + 1), check_sum_all);
  }
  #ifdef CONFIG_HUAWEI_CAMERA_DSM
...
static void gc5025a_gcore_update_dd(void)
{
    ...
    for(i=0; i<gc5025a_otp_info.dd_cnt; i++)
        ...
        for(i=0; i<gc5025a_otp_info.dd_cnt; i++)
        {
            ...
            if(flag)
                ...
                write_cmos_sensor(0xac,gc5025a_otp_info.dd_param_type[i]);

            //LOG_INF("GC5025A_OTP_GC val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",temp_val0,temp_val1,temp_val2);
            //LOG_INF("GC5025A_OTP_GC x = %d , y = %d \n",((temp_val1&0x0f)<<8) + temp_val0,(temp_val2<<4) + ((temp_val1&0xf0)>>4));
        }
...
#ifdef GC5025AOTP_FOR_CUSTOMER
static void gc5025a_gcore_update_wb(void)
{
    ...
    if(0x02==gc5025a_otp_info.golden_flag)
    {
        return;
    }
    if(0x00==(gc5025a_otp_info.golden_flag&0x01))
    {
        rg_typical=RG_TYPICAL;
        bg_typical=BG_TYPICAL;
    }
    if(0x01==(gc5025a_otp_info.golden_flag&0x01))
    {
        rg_typical=gc5025a_otp_info.golden_rg;
        bg_typical=gc5025a_otp_info.golden_bg;
        //LOG_INF("GC5025A_OTP_UPDATE_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);
    }

    if(0x01==(gc5025a_otp_info.wb_flag&0x01))
    {
...
static void gc5025a_gcore_update_chipversion(void)
{
    kal_uint8 i;

    //LOG_INF("GC5025A_OTP_UPDATE_CHIPVERSION:reg_num = %d\n",gc5025a_otp_info.reg_num);

    write_cmos_sensor(0xfe,0x00);

    for(i=0;i<gc5025a_otp_info.reg_num;i++)
    {
        write_cmos_sensor(gc5025a_otp_info.reg_addr[i] ,gc5025a_otp_info.reg_value[i]);
    }
...
static void gc5025a_gcore_update_otp(void)
{
    gc5025a_gcore_update_dd();
#ifdef GC5025AOTP_FOR_CUSTOMER
    if(gc5025a_otp_info.checksum_ok == KAL_TRUE){
        gc5025a_gcore_update_wb();
    }
#endif
    gc5025a_gcore_update_chipversion();
}


static void gc5025a_gcore_enable_otp(otp_state state)
{
    ...
    if(state)
    {
        ...
        //LOG_INF("GC5025A_OTP: Enable OTP!\n");
    }
    else
    {
        otp_en = otp_en & 0x7f;
        ...
        LOG_INF("GC5025A_OTP: Disable OTP!\n");
    }

}


static void gc5025a_gcore_initial_otp(void)
{
    write_cmos_sensor(0xfe,0x00);
    write_cmos_sensor(0xfe,0x00);
    write_cmos_sensor(0xfe,0x00);
...
static void gc5025a_gcore_identify_otp(void)
{
    gc5025a_gcore_initial_otp();
    gc5025a_gcore_enable_otp(otp_open);
    gc5025a_gcore_read_otp_info();
    gc5025a_gcore_enable_otp(otp_close);
}


static int gc5025a_gcore_check_version(void)
{
    kal_uint8 flag_GC5025A = 0;

    gc5025a_gcore_initial_otp();
    gc5025a_gcore_enable_otp(otp_open);

    gc5025a_select_page_otp(otp_page1);
    flag_GC5025A = gc5025a_read_otp(0x27);
...
static kal_uint32 return_sensor_id(void)
{
    ...
    if(read_sensor_id == GC5025_SENSOR_ID )
    {
        ...
        gc5025a_gcore_enable_otp(otp_open);
        gc5025a_select_page_otp(otp_page1);
        mode_id = gc5025a_read_otp(0x01);
        mode_id2 = gc5025a_read_otp(0x09);;
        //LOG_INF("mode_id1 = %d\n", mode_id);
        //LOG_INF("mode_id2 = %d\n", mode_id2);
        gc5025a_gcore_enable_otp(otp_close);
        if(mode_id == SANGLAISHI_MOD_ID || mode_id2 == SANGLAISHI_MOD_ID) //sanyingxing  0x02    sanglaishi 0x0c
        {
            read_sensor_id = GC5025A_SENSOR_ID;
        }
        else
        {
            read_sensor_id = 0;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
    {
        ...
        //LOG_INF("GC5025AMIPI analogic gain 1x, GC5025AMIPI add pregain = %d\n", temp);
    }
    else if((ANALOG_GAIN_2 <= iReg) && (iReg < ANALOG_GAIN_3))
    {
        ...
        write_cmos_sensor(0xb1, temp>>6);
        write_cmos_sensor(0xb2, (temp<<2)&0xfc);
        //LOG_INF("GC5025AMIPI analogic gain 1.4x, GC5025AMIPI add pregain = %d\n", temp);
    }
    else
    {
        write_cmos_sensor(0xfe,  0x00);
        ...
        write_cmos_sensor(0xb1, temp>>6);
        write_cmos_sensor(0xb2, (temp<<2)&0xfc);
        //LOG_INF("GC5025AMIPI analogic gain 2x, GC5025AMIPI add pregain = %d\n", temp);
    }
...
static kal_uint32 open(void)
{
    ...
    if(gc5025a_gcore_check_version())
        return ERROR_SENSOR_CONNECT_FAIL;

   /*Don't Remove!!*/
    gc5025a_gcore_identify_otp();

    /* initail sequence write in  */
    sensor_init();

    /*write registers from sram*/
    gc5025a_gcore_update_otp();
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC5025ADuringTestPattern = KAL_FALSE;
...
UINT32 GC5025AMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC5025AMIPI_RAW_SensorInit    */
