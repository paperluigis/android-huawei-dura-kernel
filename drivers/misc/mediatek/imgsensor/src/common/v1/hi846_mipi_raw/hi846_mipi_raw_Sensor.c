...
#define LSC_TABLE_SIZE_LSB   0x4C
#define LSC_TABLE_SIZE_MSB   0x07
#define INFO_START_ADDR      0x0CC1
#define INFO_END_ADDR        0x0CD0
#define INFO_START_ADDR1     0X1430
#define INFO_END_ADDR1       0x1440
#define AWB_START_ADDR       0x0CD2
#define AWB_END_ADDR         0x0CDC
#define AWB_START_ADDR1      0x1441
#define AWB_END_ADDR1        0x144C
#define LSC_START_ADDR       0x0CE0
#define LSC_END_ADDR         0x142B
#define LSC_START_ADDR1      0x144F
#define LSC_END_ADDR1        0x1B9C
#define AF_START_ADDR        0x1C01
#define AF_START_ADDR1       0x1C07
#ifdef HI846_OTP_FUNCTION
int hi846_otp_read_flag = 0;
#endif
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,    //zhaozhensen@wind-mobi.com 20161017
    .mclk = 24,
...
static imgsensor_struct imgsensor = {
    .mirror = IMAGE_H_MIRROR,//mirrorflip information //zhaozhensen@wind-mobi.com 20161017 IMAGE_V_MIRROR IMAGE_NORMAL
...
static kal_uint16 HI846_OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
    kal_uint16 data;
...
u8 hi846_otp_data[10] = {0}; //zy

static int HI846_otp_read(void)
{
    kal_uint16 i = 0, addr = 0,addr_end = 0, wb_start_addr = 0,wb_end_addr = 0,af_start_addr = 0,lsc_start_addr = 0,lsc_end_addr = 0;
    kal_uint32  checksum = 0;//wb_data[28];


    //INFO
    HI846_otp.Base_Info_Flag = HI846_OTP_read_cmos_sensor(0x0CC0);

    if (HI846_otp.Base_Info_Flag == 0x01)   //Base Info Group0 valid
    {
        addr = INFO_START_ADDR;
        addr_end = INFO_END_ADDR;
        HI846_otp.checksum = HI846_OTP_read_cmos_sensor(0x142f);
    }
    else if (HI846_otp.Base_Info_Flag == 0x07)  //Base Info Group1 valid
    {
        addr = INFO_START_ADDR1 ;
        addr_end = INFO_END_ADDR1 ;
        HI846_otp.checksum = HI846_OTP_read_cmos_sensor(0x1b9d);
    }
    else
    {
        addr = 0;
        HI846_otp.checksum = 0;
    }
    ...
    if (addr == 0) {
        ...
        HI846_otp.prodyction_year = HI846_OTP_read_cmos_sensor(addr);
        HI846_otp.production_month = HI846_OTP_read_cmos_sensor(addr + 1);
        HI846_otp.production_day = HI846_OTP_read_cmos_sensor(addr + 2);
        HI846_otp.module_integrator_id = HI846_OTP_read_cmos_sensor(addr + 3);
        HI846_otp.lens_id = HI846_OTP_read_cmos_sensor(addr + 4);
        HI846_otp.vcm_id = HI846_OTP_read_cmos_sensor(addr + 5);
        HI846_otp.Driver_ic_id = HI846_OTP_read_cmos_sensor(addr + 6);


    }

    //AWB
    HI846_otp.WB_FLAG = HI846_OTP_read_cmos_sensor(0x0CD1);
    if (HI846_otp.WB_FLAG == 0x01)  //AWB group0 valid
    {
        wb_start_addr = AWB_START_ADDR;
        wb_end_addr = AWB_END_ADDR ;
    }
    else if (HI846_otp.WB_FLAG == 0x7)   //AWB group1 valid
    {
        wb_start_addr = AWB_START_ADDR1;
        wb_end_addr = AWB_END_ADDR1;
    }
    else
        LOG_INF("HI846 WB data empty or invalid \n");
    ...
    //AF
    HI846_otp.AF_FLAG0 = HI846_OTP_read_cmos_sensor(0x1C00);
    if (HI846_otp.AF_FLAG0 == 0x01)
        af_start_addr = AF_START_ADDR;
     else
        LOG_INF("HI846 AF group0 data empty or invalid \n");
    HI846_otp.AF_FLAG1 = HI846_OTP_read_cmos_sensor(0x1C06);

    if (HI846_otp.AF_FLAG1 == 0x01)
        af_start_addr = AF_START_ADDR1;  //zy
    else
        LOG_INF("HI846 AF group1 data empty or invalid \n");
    ...
    if(af_start_addr != 0)
   {
     HI846_otp.af_infinity = (HI846_otp.af_data[0] << 8) | (HI846_otp.af_data[1]) ;
     HI846_otp.af_macro = (HI846_otp.af_data[2] << 8) | (HI846_otp.af_data[3]);
     HI846_otp.afcheck = HI846_otp.af_data[4];
     //LOG_INF("HI846_otp_apply:  HI846_otp.af_infinity= 0x%x,HI846_otp.af_macro= 0x%x\n ",HI846_otp.af_infinity ,HI846_otp.af_macro);
   }
   else
   {
     HI846_otp.af_infinity = 0;
     HI846_otp.af_macro = 0;
     HI846_otp.afcheck = 0;
    }
    checksum = (HI846_otp.af_data[0] + HI846_otp.af_data[1] + HI846_otp.af_data[2] + HI846_otp.af_data[3]) % 0xFF + 1 ;
    if (checksum == HI846_otp.afcheck)
    {
        LOG_INF("HI846_Sensor: AF checksum PASS\n ");
    }
    else
    {
        back_checksum &= 0x7b;
        LOG_INF("HI846_Sensor: AF checksum Fail\n ");
    }
    //LSC
    HI846_otp.LSC_FLAG = HI846_OTP_read_cmos_sensor(0x0CDD);
    if (HI846_otp.LSC_FLAG == 0x01)
    {
        lsc_start_addr = LSC_START_ADDR;
        lsc_end_addr = LSC_END_ADDR;
    }

    else if (HI846_otp.LSC_FLAG == 0x7)
    {
        lsc_start_addr = LSC_START_ADDR1;
        lsc_end_addr = LSC_END_ADDR1;
    }

    else
        LOG_INF("HI846 LSC data empty or invalid \n");
    ...
    if (lsc_start_addr != 0) {
        ...
        HI846_OTP_write_cmos_sensor(0x702, 0x01);   //single mode
        for (i = 0; i < 1868; i++) {
            HI846_otp.lsc_data[i] = read_cmos_sensor(0x708);    //otp data read

        }
    }
    /*add by gaoatao gwx528023 21071230 start*/
    checksum= 0;
    for(i = addr;i<= addr_end;i++) //info
    {
        checksum +=HI846_OTP_read_cmos_sensor(i);
    }
    for(i = wb_start_addr;i<= wb_end_addr;i++)  //awb
    {
        checksum +=HI846_OTP_read_cmos_sensor(i);
    }
    for(i = lsc_start_addr;i<= lsc_end_addr;i++)  //lsc
    {
        checksum +=HI846_OTP_read_cmos_sensor(i);
    }

    checksum = (checksum + LSC_TABLE_SIZE_LSB + LSC_TABLE_SIZE_MSB)%0xff + 1;
    if (checksum == HI846_otp.checksum)
    {
        LOG_INF("HI846_Sensor: all checksum PASS\n ");
    }
    else
    {
        back_checksum &= 0x74;
        LOG_INF("HI846_Sensor: all checksum Fail\n ");
    }
    /*add by gaoatao gwx528023 21071230 end*/
    HI846_OTP_write_cmos_sensor(0x0a00, 0x00); //sleep On
    mdelay(10);
    HI846_OTP_write_cmos_sensor(0x003e, 0x00); //OTP mode off
    HI846_OTP_write_cmos_sensor(0x0a00, 0x01); //sleep Off
    //LOG_INF("HI846 HI846_otp_read exit\n");
    #ifdef CONFIG_HUAWEI_CAMERA_DSM
...
static int HI846_otp_apply(void)
{
    ...
    RG_ratio_unit = ((HI846_otp.wb_data[0] << 8) | HI846_otp.wb_data[1]) & 0x03FF;
    BG_ratio_unit = ((HI846_otp.wb_data[2] << 8) | HI846_otp.wb_data[3]) & 0x03FF;
    RG_ratio_golden = ((HI846_otp.wb_data[6] << 8) | HI846_otp.wb_data[7]) & 0x03FF;
    BG_ratio_golden = ((HI846_otp.wb_data[8] << 8) | HI846_otp.wb_data[9]) & 0x03FF;
    wbcheck = HI846_otp.wb_data[29];

    //wbcheck = HI846_otp.wb_data[15];
    //checksum = (HI846_otp.wb_data[0] + HI846_otp.wb_data[1] + HI846_otp.wb_data[2] + HI846_otp.wb_data[3] + HI846_otp.wb_data[4] + HI846_otp.wb_data[5] + HI846_otp.wb_data[6] + HI846_otp.wb_data[7] + HI846_otp.wb_data[8] + HI846_otp.wb_data[9] + HI846_otp.wb_data[10] + HI846_otp.wb_data[11] ) % 0x100;
    for(i=0;i<29;i++)
    {
...
LOG_INF("HI846 RG_ratio_unit = 0x%x, BG_ratio_unit = 0x%x, RG_ratio_golden = 0x%x, BG_ratio_golden = 0x%x, wbcheck = 0x%x,checksum =0x%x \n",\
        RG_ratio_unit, BG_ratio_unit, RG_ratio_golden, BG_ratio_golden, wbcheck,checksum);
    if(RG_ratio_golden==0&&BG_ratio_golden==0)
        {
            RG_ratio_golden = 0x101;
            BG_ratio_golden = 0x120;
        }
...
LOG_INF("HI846 after apply otp G_gain = 0x%x, R_gain = 0x%x, B_gain = 0x%x \n",\
    (read_cmos_sensor(0x0078) << 8) | (read_cmos_sensor(0x0079) & 0xFFFF), (read_cmos_sensor(0x007c) << 8) | (read_cmos_sensor(0x007d) & 0xFFFF), (read_cmos_sensor(0x007e) << 8) | (read_cmos_sensor(0x007f) & 0xFFFF));

    return HI846_otp.WB_FLAG;
}
#endif
static void HI846_otp_cali(void)
{
...
write_cmos_sensor(0x021C, 0x0001);//0X0003
...
write_cmos_sensor(0x0A04, 0x014A);//0x015A
...
write_cmos_sensor(0x0A04, 0x016A);//0x017A
write_cmos_sensor(0x0418, 0x0000);
write_cmos_sensor(0x0B02, 0xE04D);
                ...
                do {
                    *sensor_id =return_sensor_id();
                    ...
                    if (*sensor_id == imgsensor_info.sensor_id) {
                        //LOG_INF("hi846 i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                        set_af_pewer_down_mode();
                        if(hi846_otp_read_flag == 0){
                            sensor_init();
                            HI846_otp_cali();
                        }
                        back_camera_id = 4;
                    ...
                    //LOG_INF("hi846 get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                    retry--;
                } while(retry > 0);
                i++;
                retry = 2;
    ...
    switch (scenario_id) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
         ...
         //   set_dummy();
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            if(framerate == 0 || imgsensor_info.normal_video.linelength){
                return ERROR_NONE;
            }
