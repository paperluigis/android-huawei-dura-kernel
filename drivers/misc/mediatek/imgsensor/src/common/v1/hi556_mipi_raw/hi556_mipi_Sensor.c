...
static imgsensor_info_struct imgsensor_info = {
    ...
    .hs_video = {
        ...
        .grabwindow_height = 480 ,//record different mode's hight of grabwindow
    ...
    .slim_video = {
        ...
        .grabwindow_height = 720,//record different mode's hight of grabwindow
        .mipi_data_lp2hs_settle_dc = 14,//unit , ns
        .max_framerate = 300,
    },
...
void HI556_OTPSetting(void)
{
    ...
    HI556_OTP_write_cmos_sensor(0x071A, 0x01); //CP TRI_H
    HI556_OTP_write_cmos_sensor(0x071B, 0x09); //IPGM TRIM_H
    ...
    HI556_OTP_write_cmos_sensor(0x003E, 0x10); //OTP R/W
...
static kal_uint16 OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
    ...
    HI556_OTP_write_cmos_sensor(0x10a, (otp_addr & 0xFF00) >> 8 ); //start address H
    HI556_OTP_write_cmos_sensor(0x10b, otp_addr & 0xFF); //start address L
    HI556_OTP_write_cmos_sensor(0x102, 0x01); //single read
        //mDELAY(10);
    data = read_cmos_sensor(0x108); //OTP data read
    return data;
}
u8 hi556_otp_data[10] = {0}; //zy

static int HI556_otp_read(void)
{
    int i = 0, addr = 0, wb_start_addr = 0;//,af_start_addr = 0;
    ...
    if (addr == 0) {
        ...
        HI556_otp.module_integrator_id = OTP_read_cmos_sensor(addr);
        HI556_otp.AF_FLAG = OTP_read_cmos_sensor(addr + 1);
        HI556_otp.prodyction_year = OTP_read_cmos_sensor(addr + 2);
        HI556_otp.production_month = OTP_read_cmos_sensor(addr + 3);
        HI556_otp.production_day = OTP_read_cmos_sensor(addr + 4);
        HI556_otp.sensor_id = OTP_read_cmos_sensor(addr + 5);
        HI556_otp.lens_id = OTP_read_cmos_sensor(addr + 6);
        HI556_otp.vcm_id = OTP_read_cmos_sensor(addr + 7);
        HI556_otp.Driver_ic_id = OTP_read_cmos_sensor(addr + 8);
        HI556_otp.F_num_id = OTP_read_cmos_sensor(addr + 9);
        HI556_otp.infocheck = OTP_read_cmos_sensor(addr + 16);
    }
    //LOG_INF("HI556_Sensor: Module information HI556_otp.infocheck = 0x%x\n ",HI556_otp.infocheck);
    checksum = ((HI556_otp.module_integrator_id + HI556_otp.AF_FLAG + HI556_otp.prodyction_year + HI556_otp.production_month + \
    HI556_otp.production_day + HI556_otp.sensor_id + HI556_otp.lens_id + HI556_otp.vcm_id + HI556_otp.Driver_ic_id + HI556_otp.F_num_id) % 0xff) + 1;
   // LOG_INF("HI556_Sensor: Module information checksum = 0x%x\n ",checksum);
    ...
    #endif
    /*LOG_INF("HI556 module_integrator_id = 0x%x, AF_FLAG = 0x%x, prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id = 0x%x, infocheck = 0x%x \n",\
        HI556_otp.module_integrator_id,HI556_otp.AF_FLAG, HI556_otp.prodyction_year, HI556_otp.production_month, HI556_otp.production_day, HI556_otp.sensor_id, HI556_otp.lens_id, HI556_otp.infocheck);*/

    HI556_otp.WB_FLAG = OTP_read_cmos_sensor(0x0435);
    if (HI556_otp.WB_FLAG == 0x01)
        wb_start_addr = 0x0436;
    else if (HI556_otp.WB_FLAG == 0x13)
        wb_start_addr = 0x0454;
    else if (HI556_otp.WB_FLAG == 0x37)
        wb_start_addr = 0x0472;
    else
    ...
    if (wb_start_addr != 0) {
        HI556_OTP_write_cmos_sensor(0x10a, (wb_start_addr >> 8) & 0xff);//start addr H
        HI556_OTP_write_cmos_sensor(0x10b, wb_start_addr & 0xff);//start addr L
        HI556_OTP_write_cmos_sensor(0x102, 0x01);//single mode
        for (i = 0; i < 30; i++) {
            HI556_otp.wb_data[i] = read_cmos_sensor(0x108);//otp data read
...
static int HI556_otp_apply(void)
{
    ...
    wbcheck = HI556_otp.wb_data[29];

    //wbcheck = HI556_otp.wb_data[15];
    //checksum = (HI556_otp.wb_data[0] + HI556_otp.wb_data[1] + HI556_otp.wb_data[2] + HI556_otp.wb_data[3] + HI556_otp.wb_data[4] + HI556_otp.wb_data[5] + HI556_otp.wb_data[6] + HI556_otp.wb_data[7] + HI556_otp.wb_data[8] + HI556_otp.wb_data[9] + HI556_otp.wb_data[10] + HI556_otp.wb_data[11] ) % 0x100;
    for(i=0;i<29;i++){
    ...
    /*LOG_INF("HI556 RG_ratio_unit = 0x%x, BG_ratio_unit = 0x%x, RG_ratio_golden = 0x%x, BG_ratio_golden = 0x%x, wbcheck = 0x%x,checksum =0x%x \n",\
        RG_ratio_unit, BG_ratio_unit, RG_ratio_golden, BG_ratio_golden, wbcheck,checksum);*/
    if(RG_ratio_golden==0&&BG_ratio_golden==0){
        RG_ratio_golden = 0x103;
        BG_ratio_golden = 0x129;
    }
    ...
    HI556_OTP_write_cmos_sensor(0x0a00, 0x00); //sleep On
    mdelay(10);
    HI556_OTP_write_cmos_sensor(0x003e, 0x00); //OTP mode off
    HI556_OTP_write_cmos_sensor(0x0a00, 0x01); //sleep Off
...
static kal_uint32 open(void)
{
	...
	LOG_INF("E\n");
    //LOG_INF("[open]: PLATFORM:MT6739,MIPI 2LANE\n");
