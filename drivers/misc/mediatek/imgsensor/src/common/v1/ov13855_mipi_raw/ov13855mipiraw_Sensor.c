...
static void OTP_checksum(void)
{
    ...
    //lsc&moduleinfo&AWB
    chksum = 0;
    for(i = 0x0001; i < 0x016; i++){
      chksum += read_eeprom_data(i);
    }
    for(i = 0x0041; i < 0x78d; i++){
      chksum += read_eeprom_data(i);
    }
    readsum = read_eeprom_data(0x78d);
    //LOG_INF("otpcheck lsc&moduleinfo addr = 0x%x,chksum = 0x%x,readsum=0x%x\n",i,chksum,readsum);
    if((chksum%0xff+1) != readsum)
    {
        back_checksum &=0x74;
        LOG_INF("otpcheck lsc fail \n");
    }
    //pdaf
    chksum = 0;
    for(i = 0x0791; i < 0xced; i++){
        chksum += read_eeprom_data(i);
    }
    ...
    {
        back_checksum &=0x6f;
        LOG_INF("otpcheck pdaf fail \n");
    }
    //AF
    chksum = 0;
    for(i = 0x037; i < 0x03f; i++){
      chksum += read_eeprom_data(i);
    }
    readsum = read_eeprom_data(0x03f);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                //LOG_INF("ov13855 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                set_af_pewer_down_mode();
                back_camera_id = 3;  //set as third camera
                back_camera_probe_ok = 1;  // 1 means ok
                return ERROR_NONE;
            }
            //LOG_INF("ov13855 Read sensor id fail:0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    sensor_info->PDAF_Support = PDAF_SUPPORT_RAW_LEGACY; /*0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode*/
