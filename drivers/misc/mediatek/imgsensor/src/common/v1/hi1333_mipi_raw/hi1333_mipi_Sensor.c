...
static kal_uint8 checksum_flag = TURE;
unsigned char bTh = 0;
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .checksum_value = 0xabaa55c3,       //0x6d01485c // Auto Test Mode 眠饶..
    .pre = {
...
#endif
/*add by gaoatao gwx528023 for OTP checksum start 2017 12 29*/
extern u8 back_checksum;
static kal_uint16 read_eeprom_data(kal_uint16 addr)
{
...
#define MODULEINFO_CHECKSUM_START  0x01
#define MODULEINFO_CHECKSUM_END    0x0C
#define AWB_CHECKSUM_START         0x0D
#define AWB_CHECKSUM_END           0x16
#define LSC_CHECKSUM_START         0x17
#define LSC_CHECKSUM_END           0x763
static void OTP_checksum(void)
{
    kal_uint16 i = 0;
    kal_uint32  chksum = 0,readsum = 0;
    //moduleinfo&AWB&lsc
    chksum = 0;

    for(i = MODULEINFO_CHECKSUM_START; i < MODULEINFO_CHECKSUM_END; i++){
      chksum += read_eeprom_data(i);
    }
    for(i = AWB_CHECKSUM_START; i < AWB_CHECKSUM_END; i++){
      chksum += read_eeprom_data(i);
    }
    for(i = LSC_CHECKSUM_START; i < LSC_CHECKSUM_END; i++){
      chksum += read_eeprom_data(i);
    }


    readsum = read_eeprom_data(0x763);
    //LOG_INF("otpcheck moduleinfo&AWB&lsc addr = 0x%x,chksum = 0x%x,readsum=0x%x\n",i,chksum,readsum);
    if((chksum%0xff) != readsum)
    {
        back_checksum &=0x74;
        LOG_INF("otpcheck moduleinfo&AWB&lsc fail \n");
    }
    //pdaf
    chksum = 0;
    for(i = 0x0765; i < 0x955; i++){
      chksum += read_eeprom_data(i);
    }
	readsum = read_eeprom_data(0x955);
    //LOG_INF("otpcheck pdaf addr = 0x%x,chksum = 0x%x,readsum=0x%x\n",i,chksum,readsum);
	if((chksum%0xff+1) != readsum)
    {
        back_checksum &=0x6f;
        LOG_INF("otpcheck pdaf proc1 fail \n");
    }
    //af
    chksum = 0;
    for(i = 0xce7; i < 0xced; i++){
        chksum += read_eeprom_data(i);
    }
    readsum = read_eeprom_data(0xced);
    //LOG_INF("otpcheck af addr = 0x%x,chksum = 0x%x,readsum=0x%x\n",i,chksum,readsum);
    if((chksum%0xff+1) != readsum)
    {
        back_checksum &=0x7b;
        LOG_INF("otpcheck af fail \n");
    }
    #ifdef CONFIG_HUAWEI_CAMERA_DSM
...
write_cmos_sensor(0x0600, 0x0D3e); //pdpc_ctl0 - PDAF TYPE-3 : Dyn-ON  & PD-OFF  //TYPE3 //pd dpc off
...
*/
write_cmos_sensor(0x0f38, 0x037b); //pll_cfg_mipi1
...
write_cmos_sensor(0x0912, 0x00a0); //mipi_hblank_delay
            ...
            do {
                *sensor_id = ((read_cmos_sensor(0x0F17) << 8) | read_cmos_sensor(0x0F16));
                ...
                if (*sensor_id == imgsensor_info.sensor_id) {
                    //LOG_INF("HI1333 i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                    //sprintf(Back_Camera_Name,"HI1333:E_welly:1300");
                    set_af_pewer_down_mode();
                    back_camera_id = 1;  //set as first camera
                    back_camera_probe_ok = 1;  // 1 means ok
                    return ERROR_NONE;
            }
                //LOG_INF("hi1333 get_imgsensor_id Read sensor id fail, id: 0x%x 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                retry--;
            } while(retry > 0);
            i++;
            retry = 2;
