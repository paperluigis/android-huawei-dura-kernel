			    ...
			    unsigned int aad_size, u8 *keysize)
{
	...
	do {
		...
		do {
			...
			pr_cont("test %u (%d bit key, %d byte blocks): ",
					i, *keysize * 8, *b_size);
...
static void test_available(void)
{
	...
	while (*name) {
	pr_cont("alg %s %s", *name, crypto_has_alg(*name, 0, 0) ? "found\n" : "not found\n");
		name++;
	}
}
...
#ifdef TCRYPT_FS_PROC
int tcrypt_select(int mode)
{
	int err = -ENOMEM;
	int i;
	...
	if (err) {
		pr_cont("tcrypt: one or more tests failed!\n");
		goto err_free_tv;
	}
...
static ssize_t tcrypt_write(struct file *file, const char __user *buffer,
...
static int __init tcrypt_mod_init(void)
{
	...
	entry = proc_create("tcrypt", S_IRUGO | S_IWUGO, NULL, &tcrypt_proc_fops); /* Also for userload version. */
	if (!entry)
		return -ENOMEM;
	return 0;
}
