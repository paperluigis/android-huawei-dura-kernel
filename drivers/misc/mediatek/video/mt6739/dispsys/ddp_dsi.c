...
long lcd_enp_bias_setting(unsigned int value)
{
	...
	if (value)
	{
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENP_ON);
		lcm_mdelay(5);
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENN_ON);
	}
	else
	{
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENN_OFF);
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENP_OFF);
	}
	return ret;
}
...
//	printk("enter ddp_dsi_build_cmdq\n");
	if (module == DISP_MODULE_DSIDUAL)
		dsi_i = 0;
	else
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			DISPDBG("[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     ...
			     read_data0.byte3);
			DISPDBG("[DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x,para_list[3]=0x%x\n",
...
//				printk("packet_type=0x%x,recv_data_cnt = %d\n", packet_type, recv_data_cnt);
				if(recv_data_cnt > RT_MAX_NUM)
					{
//						printk("DSI read long packet data exceeds 10 bytes \n");
...
//					printk("buffer[%d]=0x%x\n", j, buffer[j]);
					...
					result= 1;
					DISPERR("[ESD]CMP i %d return value 0x%x,para_list[%d]=0x%x\n", i,
...
//					printk("packet_type=0x%x,recv_data_cnt = %d\n", packet_type, recv_data_cnt);
					if(dsi_params->lcm_esd_check_table[i].cmd == 0x54)
						...
						if((0x24 != buffer[0]) && (0x2c != buffer[0]))
							{
								result= 1;
								DISPERR("[ESD]CMP i= %d return value= 0x%x esd_check_cmd=0x%x\n", i,
									buffer[0], j, dsi_params->lcm_esd_check_table[i].cmd);
							}
						}
					else
						for (j = 0; j < recv_data_cnt; j++) {
...
int ddp_dsi_read_lcm_cmdq(enum DISP_MODULE_ENUM module,cmdqBackupSlotHandle *read_Slot, struct cmdqRecStruct *cmdq_trigger_handle,
		struct ddp_lcm_read_cmd_table *read_table)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (*read_Slot && dsi_i == 0) {
			DSI_BACKUPREG32(cmdq_trigger_handle, *read_Slot, i,
