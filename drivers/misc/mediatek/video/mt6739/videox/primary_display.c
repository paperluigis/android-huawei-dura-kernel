...
extern int disp_lcm_set_cabc(struct disp_lcm_handle *plcm, void *handle, unsigned int enable);
int _set_cabc_by_cmdq(unsigned int mode)
{
       ...
       if (primary_display_is_video_mode())
       {
              ...
              disp_lcm_set_cabc(pgc->plcm, cmdq_handle_cabc, mode);
...
static unsigned int scan_mode_value = 0;
int primary_recognition_scan_mode(void)
{
       return scan_mode_value;
}
EXPORT_SYMBOL_GPL(primary_recognition_scan_mode);
extern int disp_lcm_set_scanmode(struct disp_lcm_handle *plcm, void *handle, unsigned int enable);
int _set_scanmode_by_cmdq(unsigned int mode)
{
       int ret = 0;
       struct cmdqRecStruct *cmdq_handle_scanmode = NULL;

       ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle_scanmode);
       DISPMSG("primary scan mode, handle=%p\n", cmdq_handle_scanmode);
       if (ret != 0)
       {
              DISPERR("fail to create primary cmdq handle for scan mode\n");
             return -1;
       }

       cmdqRecReset(cmdq_handle_scanmode);
       disp_lcm_set_scanmode(pgc->plcm, cmdq_handle_scanmode, mode);
       _cmdq_flush_config_handle_mira(cmdq_handle_scanmode, 1);
       cmdqRecDestroy(cmdq_handle_scanmode);
       cmdq_handle_scanmode = NULL;

       return ret;
}

int primary_display_setscanmode(unsigned int mode)
{
      static unsigned int last_enable;
       scan_mode_value = mode;
       if (last_enable == mode)
       return 0;

       _primary_path_switch_dst_lock();
       _primary_path_lock(__func__);
	   DISPERR("ycx: enter primary_display_setscanmode\n");

       if (pgc->state == DISP_SLEPT)
       {
              DISPERR("Sleep State set scan mode invald\n");
      }
       else
       {
             primary_display_idlemgr_kick((char *)__func__,0);
...
EXPORT_SYMBOL_GPL(primary_display_setscanmode);
#endif


#ifdef CONFIG_LCT_INVERSION_MODE_SUPPORT
static unsigned int inversion_mode_value = 0;
int primary_recognition_inversion_mode(void)
{
       return inversion_mode_value;
}
EXPORT_SYMBOL_GPL(primary_recognition_inversion_mode);
extern int disp_lcm_set_inversionmode(struct disp_lcm_handle *plcm, void *handle, unsigned int enable);
int _set_inversionmode_by_cmdq(unsigned int mode)
{
       int ret = 0;
       struct cmdqRecStruct *cmdq_handle_inversionmode = NULL;

       ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle_inversionmode);
       DISPMSG("primary inversion mode, handle=%p\n", cmdq_handle_inversionmode);
       if (ret != 0)
       {
              DISPERR("fail to create primary cmdq handle for inversion mode\n");
             return -1;
       }

       cmdqRecReset(cmdq_handle_inversionmode);
       disp_lcm_set_inversionmode(pgc->plcm, cmdq_handle_inversionmode, mode);
       _cmdq_flush_config_handle_mira(cmdq_handle_inversionmode, 1);
       cmdqRecDestroy(cmdq_handle_inversionmode);
       cmdq_handle_inversionmode = NULL;

       return ret;
}

int primary_display_setinversionmode(unsigned int mode)
{
       //int ret = 0;
      static unsigned int last_enable;
       inversion_mode_value = mode;
       if (last_enable == mode)
       return 0;

       _primary_path_switch_dst_lock();
       _primary_path_lock(__func__);
	   DISPERR("ycx: enter primary_display_setinversionmode\n");

       if (pgc->state == DISP_SLEPT)
       {
              DISPERR("Sleep State set inversion mode invald\n");
      }
       else
       {
             primary_display_idlemgr_kick((char *)__func__,0);
              if (primary_display_cmdq_enabled())
                     {
                         _set_inversionmode_by_cmdq(mode);
                     }
              last_enable = mode;
       }
       _primary_path_unlock(__func__);
...
EXPORT_SYMBOL_GPL(primary_display_setinversionmode);
#endif

int primary_display_lcd_state(void)
{
   if (pgc->state == DISP_SLEPT)
          return 0;   //lcd sleep
   else
          return 1;   //lcd running
}
EXPORT_SYMBOL_GPL(primary_display_lcd_state);
