...
int do_lcm_vdo_lp_read(struct ddp_lcm_read_cmd_table *read_table)
{
	...
	/* 2.stop dsi vdo mode */
	dpmgr_path_build_cmdq(primary_get_dpmgr_handle(), handle, CMDQ_STOP_VDO_MODE, 0);

	/* 3.read from lcm */
	ddp_dsi_read_lcm_cmdq(DISP_MODULE_DSI0, &read_Slot, handle, read_table);

	/* 4.start dsi vdo mode */
	dpmgr_path_build_cmdq(primary_get_dpmgr_handle(), handle, CMDQ_START_VDO_MODE, 0);
	...
	for (i = 0; i < 3; i++)
		cmdqBackupReadSlot(read_Slot, i, (uint32_t *)&read_table->data[i]);
		...
		if (esd_check_enable) {
			...
			if (ret == 1) {
				DISPERR("[ESD]after esd recovery %d times, still fail, disable esd check\n",
					esd_try_cnt);
#ifdef CONFIG_HUAWEI_DSM
					lcdkit_report_dsm_err(DSM_LCD_ESD_RECOVERY_NO, 0, 0, 0);
#endif
