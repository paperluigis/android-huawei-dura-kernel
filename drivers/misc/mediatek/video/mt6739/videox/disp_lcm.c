...
int disp_lcm_set_cabc(struct disp_lcm_handle *plcm, void *handle, unsigned int mode)
{
       LCM_DRIVER *lcm_drv = NULL;
       DISPFUNC();
       if (_is_lcm_inited(plcm))
...
int disp_lcm_set_scanmode(struct disp_lcm_handle *plcm, void *handle, unsigned int mode)
{
       LCM_DRIVER *lcm_drv = NULL;
       DISPFUNC();
       if (_is_lcm_inited(plcm))
       {
             lcm_drv = plcm->drv;
             if (lcm_drv->set_scanmode_cmdq)
              {
                     lcm_drv->set_scanmode_cmdq(handle, mode);
              }
              else
              {
                     DISPERR("FATAL ERROR, lcm_drv->set_scanmode is null\n");
                     return -1;
              }
              return 0;
       }
...
int disp_lcm_set_inversionmode(struct disp_lcm_handle *plcm, void *handle, unsigned int mode)
{
       LCM_DRIVER *lcm_drv = NULL;
       DISPFUNC();
       if (_is_lcm_inited(plcm))
       {
             lcm_drv = plcm->drv;
             if (lcm_drv->set_inversionmode_cmdq)
              {
                     lcm_drv->set_inversionmode_cmdq(handle, mode);
              }
              else
              {
                     DISPERR("FATAL ERROR, lcm_drv->set_inversionmode is null\n");
                     return -1;
              }
              return 0;
       }
