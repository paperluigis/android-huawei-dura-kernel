...
//#define EEPROM_DEBUG
#ifdef EEPROM_DEBUG
#define EEPROMDB pr_debug//pr_err
#else
#define EEPROMDB(x, ...)
#endif

/*#define GT24C32A_DURA_DRIVER_ON 0*/
#ifdef GT24C32A_DURA_DRIVER_ON
#define EEPROM_I2C_BUSNUM 2
static struct i2c_board_info kd_eeprom_dev __initdata = { I2C_BOARD_INFO("CAM_CAL_DRV", 0xA0 >> 1)};
...
#define S24CS64A_DEVICE_ID                          0xA0
...
static int iWriteEEPROM(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
    ...
    if (a_u4Bytes + 2 > 8) {
        EEPROMDB("[GT24c32a_dura] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
        return -1;
    }
    ...
    if (i4RetValue != (a_u4Bytes + 2)) {
        EEPROMDB("[GT24c32a_dura] I2C write  failed!!\n");
        return -1;
    }
...
static int iReadEEPROM(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
    ...
    if (ui4_length > 8) {
        EEPROMDB("[GT24c32a_dura] exceed I2c-mt65xx.c 8 bytes limitation\n");
        return -1;
    }
...
#define I2C_WR_FLAG             (0x1000)
    ...
    if (i4RetValue != ui4_length) {
        EEPROMDB("[GT24c32a_dura] I2C read data failed!!\n");
        return -1;
    }
    ...
    spin_unlock(&g_EEPROMLock); /* for SMP */
     ...
     EEPROMDB("[GT24c32a_dura] iReadEEPROM done!!\n"); */
    return 0;
}
...
/*
static u16 read_cmos_sensor_byte(u16 addr)
{
    u16 get_byte=0;
...
static bool read_eeprom( unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata){
    int i = 0;
    EEPROMDB("gggggggggggg:  start read data begin\n");

    //memset(tempdata, 0xFF, ui4_length);

    for(i = 0; i < ui4_length; i++){
      pinputdata[i] = read_cmos_sensor_byte(ui4_offset+i);
      EEPROMDB(" camcal addr = 0x%x,\tvalue = 0x%x \n",i+ui4_offset, pinputdata[i]);
    }



    EEPROMDB("gggggggggggg:  read_eeprom data end\n");
...
static bool byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
    ...
    //EEPROMDB("enter byteread_cmos_sensor \n");
    //kdSetI2CSpeed(EEPROM_I2C_SPEED);
    if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
        EEPROMDB("fail common_byteread_cmos_sensor addr =0x%x, data = 0x%x\n", addr, *data);
        return false;
    }
    /*EEPROMDB("selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr,
    *data,page,offset);*/
    EEPROMDB("byteread_cmos_sensor addr =0x%x, data = 0x%x\n", addr, *data);
    return true;

}

static int read_eeprom(u32 addr, u32 size, u8 *data)
{
    ...
    //EEPROMDB("Before byteread_cmos_sensor curAddr = 0x%x count = %d buffData = 0x%x\n", curAddr, size - size_to_read, *buff);
    EEPROMDB("start read_eeprom, curAddr = 0x%x, size_to_read = %d \n", curAddr, size_to_read);
    while (size_to_read > 0) {
        /*if(selective_read_byte(addr,(u8*)buff,COMM_DEVICE_ID)){*/
        if (byteread_cmos_sensor(0xA0, curAddr, buff)) {
            //EEPROMDB("after byteread_cmos_sensor curAddr =0x%x, count=%d, buffData=0x%x\n", curAddr, size - size_to_read, *buff);
            curAddr += 1;
            buff += 1;
            size_to_read -= 1;
            ret += 1;
        ...
        if (size_to_read > BUFF_SIZE) {
            EEPROMDB("offset =%x size %d\n", offset, BUFF_SIZE);
            if (iReadRegI2C(&offset, 1, (u8 *)buff, BUFF_SIZE, 0xA0 + (page << 1)) < 0)
                break;
            ret += BUFF_SIZE;
            buff += BUFF_SIZE;
            offset += BUFF_SIZE;
    ...
    EEPROMDB("end read_eeprom addr =0x%x, size = %d, readSize = %d\n", addr, size, ret);
    return ret;
}
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
    ...
    if (ui4_offset + ui4_length >= 0x2000) {
        EEPROMDB("[GT24c32a_dura] Read Error!! GT24c32a_dura not supprt address >= 0x2000!!\n");
        return -1;
    }
    ...
    do {
        if (i4ResidueDataLength >= 8) {
            ...
            if (i4RetValue != 0) {
                EEPROMDB("[GT24c32a_dura] I2C iReadData failed!!\n");
                return -1;
            }
            u4IncOffset += 8;
...
unsigned int gt24c32a_dura_selective_read_region(struct i2c_client *client, unsigned int addr,
    unsigned char *data, unsigned int size)
{
    EEPROMDB("[EEPROM] enter GT24c32a_dura_selective_read_region!!\n");
    g_pstI2Cclient = client;
    //if (iReadData(addr, size, data) == 0)
    if (read_eeprom(addr, size, data) > 0)
...
#endif
{
    ...
    if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
        ...
        if (pBuff == NULL) {
            EEPROMDB("[GT24c32a_dura] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
            if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
                ...
                kfree(pBuff);
                EEPROMDB("[GT24c32a_dura] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    EEPROMDB("[GT24c32a_dura] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff,
    (u32)a_u4Command);
    ...
    case CAM_CALIOC_S_WRITE:
        EEPROMDB("[GT24c32a_dura] Write CMD\n");
#ifdef EEPROMGETDLT_DEBUG
    ...
    case CAM_CALIOC_G_READ:
        EEPROMDB("[GT24c32a_dura] Read CMD\n");
#ifdef EEPROMGETDLT_DEBUG
        do_gettimeofday(&ktv1);
#endif
        EEPROMDB("[GT24c32a_dura] offset %x\n", ptempbuf->u4Offset);
        EEPROMDB("[GT24c32a_dura] length %x\n", ptempbuf->u4Length);
        EEPROMDB("[GT24c32a_dura] Before read Working buffer address 0x%8x\n", (u32)pWorkingBuff);
        ...
        if (ptempbuf->u4Offset == 0x0024C32a) {
            *(u32 *)pWorkingBuff = 0x0124C32a;
            ...
            while (readTryagain > 0) {
                i4RetValue =  iReadDataFromGT24c32a_dura((u16)ptempbuf->u4Offset, ptempbuf->u4Length,
                pWorkingBuff);
                EEPROMDB("[GT24c32a_dura] error (%d) Read retry (%d)\n", i4RetValue, readTryagain);
                if (i4RetValue != 0)
                    readTryagain--;
                else
                    readTryagain = 0;
        ...
        EEPROMDB("[GT24c32a_dura] After read Working buffer data  0x%4x\n", *pWorkingBuff);
    ...
    if (_IOC_READ & _IOC_DIR(a_u4Command)) {
        ...
        EEPROMDB("[GT24c32a_dura] to user length %d\n", ptempbuf->u4Length);
        EEPROMDB("[GT24c32a_dura] to user  Working buffer address 0x%8x\n", (u32)pWorkingBuff);
        if (copy_to_user((u8 __user *) ptempbuf->pu1Params, (u8 *)pWorkingBuff,
        ptempbuf->u4Length)) {
            kfree(pBuff);
            kfree(pWorkingBuff);
            EEPROMDB("[GT24c32a_dura] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
static int EEPROM_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
    ...
    EEPROMDB("[GT24c32a_dura] EEPROM_Open\n");
...
static inline int RegisterEEPROMCharDrv(void)
{
    ...
    if (alloc_chrdev_region(&g_EEPROMdevno, 0, 1, EEPROM_DRVNAME)) {
        EEPROMDB("[GT24c32a_dura] Allocate device no failed\n");

        return -EAGAIN;
    }
#else
    if (register_chrdev_region(g_EEPROMdevno, 1, EEPROM_DRVNAME)) {
        EEPROMDB("[GT24c32a_dura] Register device no failed\n");
    ...
    if (g_pEEPROM_CharDrv == NULL) {
        ...
        EEPROMDB("[GT24c32a_dura] Allocate mem for kobject failed\n");
    ...
    if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
        EEPROMDB("[GT24c32a_dura] Attatch file operation failed\n");
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    EEPROMDB("[GT24c32a_dura] Attach I2C\n");
    ...
    EEPROMDB("[GT24c32a_dura] g_pstI2Cclient->addr = 0x%8x\n", g_pstI2Cclient->addr);
    /* Register char driver */
    i4RetValue = RegisterEEPROMCharDrv();

    if (i4RetValue) {
        EEPROMDB("[GT24c32a_dura] register char device failed!\n");
        return i4RetValue;
    }


    EEPROMDB("[GT24c32a_dura] Attached!!\n");
    return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
    ...
    if (platform_driver_register(&g_stEEPROM_Driver)) {
        EEPROMDB("failed to register GT24c32a_dura driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stEEPROM_Device)) {
        EEPROMDB("failed to register GT24c32a_dura driver, 2nd time\n");
        return -ENODEV;
    }
