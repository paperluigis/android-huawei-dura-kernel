...
void set_usb_otg_eye_value(void)
{
}
...
void set_usb_otg_eye_value(void)
{
	DBG(0, "[OTG] USB_DRIVING_CAPABILITY value = 0x%x, USB20_ENHANCEMENT value = 0x%x\n",
			USBPHY_READ8(0x05), USBPHY_READ32(0x18));
	/* USB_DRIVING_CAPABILITY = RG_USB20_VRT_VREF_SEL + RG_USB20_TERM_VREF_SEL.
		RG_USB20_VRT_VREF_SEL is high 4 bit of 0x05 Register, and
		RG_USB20_TERM_VREF_SEL is low 4 bit of 0x05 Register. so if modified USB_DRIVING_CAPABILITY to 7,
		you requires setting RG_USB20_VRT_VREF_SEL to 3 and RG_USB20_TERM_VREF_SEL to 4 */
	//USBPHY_WRITE8(0x05, 0x33);

	/* USB20_ENHANCEMENT    : @value < 00 - 0x00 | 11 - 0x03 > */
	/* modify for USB20_ENHANCEMENT eye param. reference EngineerMode app start */
	USBPHY_CLR32(0x18, (0x3<<30));
	USBPHY_SET32(0x18, (0x0<<30));
	/* modify for USB20_ENHANCEMENT eye param. reference EngineerMode app end */

	udelay(5);  // wait 5 usec
	DBG(0, "[OTG] USB_DRIVING_CAPABILITY value = 0x%x, USB20_ENHANCEMENT value = 0x%x\n",
			USBPHY_READ8(0x05), USBPHY_READ32(0x18));
}
/* OTG reset end - mtk provide code */
...
void usb_phy_recover(void)
{
	...
	*/
	...
	/* Mediatek release code for Usb Eys Test. start */
	DBG(0, "-usb eye args0: 0x%x, 0x%x\n", USBPHY_READ8(0x05), USBPHY_READ32(0x18));
	/* USB_DRIVING_CAPABILITY = RG_USB20_VRT_VREF_SEL + RG_USB20_TERM_VREF_SEL.
		RG_USB20_VRT_VREF_SEL is high 4 bit of 0x05 Register, and
		RG_USB20_TERM_VREF_SEL is low 4 bit of 0x05 Register. so if modified USB_DRIVING_CAPABILITY to 10,
		you requires setting RG_USB20_VRT_VREF_SEL to 5 and RG_USB20_TERM_VREF_SEL to 5 */
	//USBPHY_WRITE8(0x05, 0x55);
	//USBPHY_SET8(0x15, USBPHY_READ8(0x15) | 3 << 4); // RG_USB20_HSTX_SRCTRL  : @value < 111 -> 0x70(bit4-bit6) >
	//USBPHY_SET8(0x00, USBPHY_READ8(0x00) | 1 << 5); // RG_USB20_INTR_EN      : @value < 0x00 - disable, 0x01 - enable >
	/* modify for USB20_ENHANCEMENT eye param. reference EngineerMode app start */
	USBPHY_CLR32(0x18, (0x3<<30));
	USBPHY_SET32(0x18, (0x3<<30));
	/* modify for USB20_ENHANCEMENT eye param. reference EngineerMode app end */
	udelay(5);  // wait 5 usec
	DBG(0, "-usb eye args1: 0x%x, 0x%x\n", USBPHY_READ8(0x05), USBPHY_READ32(0x18));
	/* Mediatek release code for Usb Eys Test. end */
