...
#include <linux/gpio.h>
#define OTG_ID_CHANNEL_NUM 12
struct pinctrl *adc_ctrl;
struct pinctrl_state *adc_pins_low,*adc_pins_high;
struct pinctrl_state *USB_ID_DET = NULL;
...
#define Delay_Kick_Dog_Time 10
void kickdog(struct work_struct *work);
static struct delayed_work otg_kick_charge;
static DECLARE_DELAYED_WORK(otg_kick_charge, kickdog);
void kickdog(struct work_struct *work)
{
    if (NULL == work)
    {
    ...
    if(usb20_check_vbus_on())
    {
        DBG(0,"%s will kick_vbus_dog!\n",__func__);
        charger_dev_kick_wdt(primary_charger);
        schedule_delayed_work(&otg_kick_charge,HZ*Delay_Kick_Dog_Time);
    }
}
...
void _set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on && !vbus_on) {
        ...
        cancel_delayed_work_sync(&otg_kick_charge);
#else
...
/*begin  test for usbid&battery_id*/
void cancel_mutex(struct work_struct *work);
static struct delayed_work cancel_mutex_work;
static DECLARE_DELAYED_WORK(cancel_mutex_work, cancel_mutex);
void cancel_mutex(struct work_struct *work)
{
...
static int hw_otg_id_adc_sampling(void )
{
    ...
    pinctrl_select_state(adc_ctrl, adc_pins_high);
    mdelay(ID_PIN_WORK_RECHECK_TIME);
    for(i = 0; i < VBATT_AVR_MAX_COUNT; i++)
    {
        udelay(AUXADC_RESAMPLE_TIME);
        IMM_GetOneChannelValue_Cali(OTG_ID_CHANNEL_NUM, &vol_value);
        if(vol_value < 0)
        {
            DBG(0,"%s The value from ADC is error", __func__);
            continue;
        }
        sum += vol_value;
        sample_cnt++;
    ...
    {
        avgvalue = sum/sample_cnt/UVTOMV;              //change the voltage form uv to mv;
    }
    DBG(0,"%s The average voltage of ADC is %d\n", __func__, avgvalue);
    pinctrl_select_state(adc_ctrl, adc_pins_low);
    schedule_delayed_work(&cancel_mutex_work, msecs_to_jiffies(ID_PIN_WORK_RECHECK_TIME));
    return avgvalue;
}
...
static void musb_host_work(struct work_struct *data)
{
    ...
    usb_eint = __gpio_get_value(GPIO_otg_eint);
    if(0 == usb_eint)
    {
        avgvalue = hw_otg_id_adc_sampling();
        if((avgvalue < 0) || (avgvalue > ADC_VOLTAGE_LIMIT))
        {
            DBG(0,"%s is not a correct adc %d,will return!! \n",__func__,avgvalue);
	...
	if ((mtk_musb->in_ipo_off)||(false == is_otg_has_inserted)) {
	...
	if (host_mode) {
		...
		musb_start(mtk_musb);
		/* OTG reset start - mtk provide code */
		DBG(0, "[OTG] reset Usb Eys Test value.\n");
		set_usb_otg_eye_value();
		/* OTG reset end - mtk provide code */
...
static int otg_iddig_probe(struct platform_device *pdev)
{
    ...
    adc_ctrl = devm_pinctrl_get(dev);
    if (IS_ERR(adc_ctrl))
    {
        ret = PTR_ERR(adc_ctrl);
        DBG(0,"adc_ctrl parse map error!!!!\n");
        return ret;
    }
    adc_pins_low = pinctrl_lookup_state(adc_ctrl, "adc_ctrl_low");
    adc_pins_high = pinctrl_lookup_state(adc_ctrl, "adc_ctrl_high");
    USB_ID_DET =  pinctrl_lookup_state(adc_ctrl, "USB_ID_DET");
    pinctrl_select_state(adc_ctrl, adc_pins_low);

    np = of_find_node_by_name(NULL,"otg_iddig");
    if(!np)
    {
        DBG(0,"%s find node otg_iddig fail\n",__func__);
        return -ENODEV;
    }
    GPIO_otg_eint = of_get_named_gpio(np,"usb,eint_gpio",0);
    if(GPIO_otg_eint<0)
    {
        DBG(0,"%s read usb,eint_gpio faile\n",__func__);
        return -ENODEV;
    }
