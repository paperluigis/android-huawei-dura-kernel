...
typedef struct {
	...
	void (*aod)(int enter);
	void  (*set_lcm_panel_support)(void);
#ifdef CONFIG_LCT_CABC_MODE_SUPPORT
	void (*set_cabc_cmdq)(void *handle, unsigned int mode);
#endif
#ifdef CONFIG_LCT_SCAN_MODE_SUPPORT
	void (*set_scanmode_cmdq)(void *handle, unsigned int mode);
#endif

#ifdef CONFIG_LCT_INVERSION_MODE_SUPPORT
	void (*set_inversionmode_cmdq)(void *handle, unsigned int mode);
#endif
