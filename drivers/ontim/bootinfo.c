...
static u8 back_camera_array[]="OuFeiGuang_ov8856_mipiraw";
static u8 back_camera_array1[]="SanYingXing_hi843_mipiraw";
static u8 front_camera[]="front_camera not found!";
static u8 front_camera_array[]="SiJiChun_gc2365_mipiraw";
static u8 front_camera_array1[]="SangLaiShi_hi259_mipiraw";
...
static ssize_t lcd_driving_mode_store(struct kobject *kobj, struct kobj_attribute *attr, const char * buf, size_t n)
{
        ...
        g_lcm_drv->lcd_set_driving_mode(val);
...
int get_hw_ver(void)
{
#define GPIO_HW_VER_PIN0    (GPIO94 | 0x80000000)
#define GPIO_HW_VER_PIN1    (GPIO95 | 0x80000000)
	int pin0_val = 0;
	int  pin1_val = 0;
	int hw_ver=0;
      ...
      mt_set_gpio_pull_select(GPIO_HW_VER_PIN1, GPIO_PULL_UP);
      mt_set_gpio_pull_enable(GPIO_HW_VER_PIN1, GPIO_PULL_ENABLE);
      mt_set_gpio_mode(GPIO_HW_VER_PIN1, GPIO_MODE_00);
      mt_set_gpio_dir(GPIO_HW_VER_PIN1, GPIO_DIR_IN);

	mdelay(20);
	
	pin0_val = mt_get_gpio_in(GPIO_HW_VER_PIN0);
	pin1_val = mt_get_gpio_in(GPIO_HW_VER_PIN1);
	hw_ver =( pin0_val?0:1);
	hw_ver |= (pin1_val?0:(1<<1)); 
	
	printk(KERN_ERR "%s: pin0 is %d , pin1 is %d, hw_ver=%d\n",__func__, pin0_val, pin1_val,hw_ver);
	
      mt_set_gpio_pull_enable(GPIO_HW_VER_PIN0, GPIO_PULL_DISABLE);
      mt_set_gpio_pull_enable(GPIO_HW_VER_PIN1, GPIO_PULL_DISABLE);
...
//+add by liujingchuan for check cust prj ver 
struct cust_ver_attr{
    int num;
    char* prj;
};

struct cust_ver_attr cust_ver[] = {{0, "CRO_L03"}, 
								   {1, "CRO_L02"}, 
								   {2, "CRO_L23"}, 
								   {3, "CRO_L22"},
};
...
static void check_cust_ver(void){
	...
	size = sizeof(cust_ver)/sizeof(cust_ver[0]);
	if (prj >=size)
	{
		printk(KERN_ERR "%s: prj=%d\n",__func__, prj);
		return;
	}
	printk(KERN_ERR "%s: Build PRJ is %s, This PRJ is %s!!\n",__func__, PRJ_NAME, cust_ver[prj].prj);
	if((!strcmp(cust_ver[prj].prj, PRJ_NAME)) || (!strcmp(cust_ver[prj].prj, "NULL"))){
		printk(KERN_ERR "%s: Version Pass!!\n",__func__);
	}
	else{
		printk(KERN_ERR "%s: Version Error, hw_ver = %d!!\n",__func__, hw_ver);
		if(hw_ver>=2)
		{
...
# if 0
int ontim_reserve_mem=0;
static int __init reserve_mem_setup(char *str)
{
        int en;
        if(!get_option(&str, &en))
                return 0;
        ontim_reserve_mem = en;
        return 1;
}
int get_reserve_mem(void)
{
        printk(KERN_ERR "[kernel]:get_reserve_mem=%x.\n",ontim_reserve_mem);
        return ontim_reserve_mem;
}
__setup("reserve_mem=", reserve_mem_setup);
#endif
