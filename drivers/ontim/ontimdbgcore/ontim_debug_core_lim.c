...
static ssize_t ontim_core_debug_dev_attr_debug_trace_level_store(struct kobject *kobj, struct kobj_attribute *attr,
        const char *buf, size_t count)
{
    unsigned int dbg_trace_level = 0;
    int res;   
    char *s_name ;
    struct ontim_debug *ontim_debug;
    mutex_lock(&sysfs_lock);
...
//    kobj_attr=kzalloc(sizeof(struct kobj_attribute)*(ontim_debug->dev_attr_array_len+1),GFP_KERNEL);
    ...
    if(!strcmp(ontim_debug->dev_name,"touch_screen"))
    {
	tp_probe_ok=1;
    }
    else  if(!strcmp(ontim_debug->dev_name,"gsensor"))
    {
	gsensor_probe_ok=1;
    }
    else  if(!strcmp(ontim_debug->dev_name,"als_prox"))
    {
	proximity_probe_ok=1;
    }
    else  if(!strcmp(ontim_debug->dev_name,"charge_ic"))
    {
	charger_probe_ok=1;
    }
    else  if(!strcmp(ontim_debug->dev_name,"msensor"))
    {
	compass_probe_ok=1;
    }
    else  if(!strcmp(ontim_debug->dev_name,"fpsensor"))
    {
	fingerprint_probe_ok=1;
    }

#ifdef CONFIG_APP_INFO
    for (i = 0; i<ontim_debug->dev_attr_array_len; i++)
    {
        if (0 == strncmp("vendor", ontim_debug->dev_attr_array[i].dev_attr_name, 6))
        {
            if((NULL != ontim_debug->dev_attr_array[i].dev_attr_context)
                &&(ONTIM_DEV_ARTTR_TYPE_STR == ontim_debug->dev_attr_array[i].dev_attr_type))
            {
                app_info_set(ontim_debug->dev_name, (char *)(ontim_debug->dev_attr_array[i].dev_attr_context));
            }
        }
    }
#endif

    //kfree(ontim_core_dbg_item);//add this can't boot
    //kfree(kobj_attr);//add this can't boot
    kfree(properties_attrs);
    kfree(properties_attr_group);
    return 0;
...
/*+++++++++++++++++++++++++++++++++++++++++++*/
static unsigned int gpio_force_num=0;
