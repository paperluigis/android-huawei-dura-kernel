...
#define ABNS_SAVE_OFFSET (BFR_PARTITION_SIZE - 16 * LOG_HALF_PAGE)
int set_unclean_shutdown_flag(int flag)
{
    ...
    ret = abns_write_emmc_raw_part(RSV_PART_NAME_STR, ABNS_SAVE_OFFSET, (char*)&abns_btfg, sizeof(ABNS_BTFG_T));
    if ( 0 != ret )
    {
...
int get_unclean_shutdown_flag(void)
{
    ...
    ABNS_PRINT_ENTER();
    memset(&abns_btfg,0,sizeof(ABNS_BTFG_T));
    ret = abns_read_emmc_raw_part(RSV_PART_NAME_STR, ABNS_SAVE_OFFSET, (char*)&abns_btfg, sizeof(ABNS_BTFG_T));
    if (0 != ret)
    {
...
int abns_check(void)
{
    if(g_abns_type == ABNS_NORMAL)
    {
        ...
        BFMR_PRINT_ERR("This is ABNS exception:%d!",g_abns_type);
        start_save_abns_log();
    }
    return 0;
}
