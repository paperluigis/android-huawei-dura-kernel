...
#define BFM_TEXT_LOG_SEPRATOR_WITHOUT_FIRST_NEWLINE "================================\r\n"
...
#define BOOTFAIL_INJECT_FLAG (2)
#define BOOTFAIL_INJECT_FREE (6)
#define LOG_HALF_PAGE (512)
#define LOG_PAGE_NUM (4)
#define LOG_PAGE_SIZE (1024)
#define LOG_COUNT (10)
#define BOOTFAIL_OFFSET  ((BOOTFAIL_INJECT_FLAG+BOOTFAIL_INJECT_FREE)*LOG_HALF_PAGE)
#define LOG_SIZE (LOG_PAGE_NUM*LOG_PAGE_SIZE)
#define RECOVERY_OFFSET  (BOOTFAIL_OFFSET+(LOG_SIZE*LOG_COUNT))
#define BFR_PARTITION_SIZE (768*1024)
#define LOG_OFFSET_INDEX  (BFR_PARTITION_SIZE - LOG_HALF_PAGE)
#define BFR_PARTITION "bfr"

#define COUNT_NUM (10)

extern int bfmr_read_log_from_raw_part(char *raw_part_name, unsigned long long offset, char *buf, unsigned int buf_size);

static int bfm_wflag = 0;
...
struct hwbfm_log
{
	uint32 flag;
	uint32 hash_code;
	uint32 hash_code_rcv;
	uint32 is_using_this_rcv; /*0 not used ,1 used*/
	char buf_log[3000];
	char buf_log_rcv[4096-3000-4-4-4-4-sizeof(struct boot_log_struct)];
	struct boot_log_struct boot_log;
};

static uint32 hwboot_calculate_checksum(unsigned char *addr, int len)
{
  int i;
  uint8 *w = addr;
  uint32 sum = 0;
...
static int bfm_copy_bootfail_info_to_raw_part(char *raw_part_name,
                                   unsigned long long offset, char *buf, unsigned int log_len,bfm_process_bootfail_param_t *pparam)
{
    int ret = 0;
    int *pflag = NULL;
    char *pdata = NULL;
    ...
    //add a global data struct for save raw partition include error flag and checksum value
    struct hwbfm_log *buf_log_data = (struct hwbfm_log*)bfmr_malloc(sizeof(struct hwbfm_log));
    if (NULL == buf_log_data)
    {
        BFMR_PRINT_ERR("bfmr_malloc buf_log_data failed!\n");
        return -1;
    }
    memset(buf_log_data, 0, BFMR_TEMP_BUF_LEN);
    buf_log_data->flag = 0x12345678;
    buf_log_data->boot_log.rtc_time = pparam->bootfail_time;
    buf_log_data->boot_log.last_boot_error_no = (unsigned int)pparam->bootfail_errno;
    buf_log_data->boot_log.last_boot_stage = (unsigned int)pparam->boot_stage;
    buf_log_data->boot_log.last_isSystemRooted = pparam->is_system_rooted;
    buf_log_data->boot_log.last_isUserPerceptiable = pparam->is_user_sensible;
    memcpy(buf_log_data->buf_log, buf, strlen(buf));
    buf_log_data->hash_code = hwboot_calculate_checksum((unsigned char*)buf_log_data->buf_log, strlen(buf_log_data->buf_log));

    pflag = (char *)bfmr_malloc(BFMR_TEMP_BUF_LEN);
    if (NULL == pflag)
    {
        BFMR_PRINT_ERR("bfmr_malloc pflag failed!\n");
        return -1;
    }
    ...
    memset(pflag, 0, BFMR_SIZE_32);
    memset(pdata, 0, BFMR_TEMP_BUF_LEN);

    /*1. get the count flag */
    ret = bfmr_read_log_from_raw_part(raw_part_name, (unsigned long long)LOG_OFFSET_INDEX, (char*)pflag, BFMR_SIZE_32);
    if (ret)
    {
        ret = -1;
        BFMR_PRINT_ERR("get the count flag failed!\n");
        goto __out;
    }
    ...
    ret = bfmr_save_log_to_raw_part(raw_part_name, offset + bfm_wflag*LOG_SIZE, pdata, LOG_SIZE);
    if (ret)
    {
      ret = -1;
      BFMR_PRINT_ERR("clear the old data failed!\n");
      goto __out;
    }

    /*3. wirte the data we need*/
    ret = bfmr_save_log_to_raw_part(raw_part_name, offset + (unsigned long long)(bfm_wflag*LOG_SIZE), (char*)buf_log_data, sizeof(struct hwbfm_log));
    if (ret)
    {
      ret = -1;
      BFMR_PRINT_ERR("wirte the data we need failed!\n");
      goto __out;
    }

    /*4. update count flag number*/
    *pflag += 1;
    if(*pflag >= COUNT_NUM)
    {
        *pflag = 0;
    }
    ret = bfmr_save_log_to_raw_part(raw_part_name, (unsigned long long)LOG_OFFSET_INDEX, (char*)pflag, BFMR_SIZE_32);
    if (ret)
    {
      ret = -1;
      BFMR_PRINT_ERR("update count flag failed!\n");
      goto __out;
    }
...
static int bfm_save_bootfail_info_txt(bfmr_log_dst_t *pdst, bfmr_log_src_t *psrc, bfm_process_bootfail_param_t *pparam)
{
    ...
    if (0 == ret)
    {
        ...
        {
            result1 = (long long)(100 * statfsbuf.f_bavail);
            do_div(result1, (long long)statfsbuf.f_blocks);
        }
    }
    else
    {
    ...
    if (0 == ret)
    {
        ...
        {
            result2 = (long long)(100 * (statfsbuf.f_files - statfsbuf.f_ffree));
            do_div(result2, (long long)statfsbuf.f_files);
        }
    }
    else
    {
    ...
    result2,
        ...
        case DST_FILE:
            ...
            ret = bfmr_save_log(pparam->bootfail_log_dir, BFM_BFI_FILE_NAME, (void *)pdata, bytes_formatted, 0);
                ...
                bfm_copy_bootfail_info_to_raw_part(BFR_PARTITION, (unsigned long long)BOOTFAIL_OFFSET, pdata, strlen(pdata),pparam);
                break;
            }
        case DST_RAW_PART:
            {
    ...
    bfm_process_bootfail_param_t *pparam)
{
    ...
    if (file_existed && bfm_is_beta_version())
    {
        (void)snprintf(pparam->bootfail_log_path, sizeof(pparam->bootfail_log_path) - 1,
...
static int bfm_update_recovery_info_for_each_log(bfm_bootfail_log_info_t *pbootfail_log_info)
{
    ...
    pdata = bfmr_malloc(BFMR_SIZE_4K);
    if (NULL == pdata)
    {
        BFMR_PRINT_ERR("bfmr_malloc failed!\n");
        goto __out;
    }
    memset(pdata, 0, BFMR_SIZE_4K);
    ...
    for (i = 0; i < pbootfail_log_info->real_recovery_info.record_count; i++)
    {
        ...
        bytes_formatted += snprintf(pdata + bytes_formatted, BFMR_SIZE_4K - bytes_formatted, BFM_RCV_FILE_CONTENT_FORMAT "%s",
            ...
            (i != (pbootfail_log_info->real_recovery_info.record_count - 1)) ? BFM_TEXT_LOG_SEPRATOR_WITHOUT_FIRST_NEWLINE : (""));
    }
    ...
    if (0 != bfmr_save_log(pbootfail_log_info->bootfail_logs[0].log_dir, BFM_DONE_FILE_NAME, (void *)&c, sizeof(char), 0))
    {
        BFMR_PRINT_ERR("Failed to update DONE!\n");
    }
...
static void bfm_merge_bootfail_logs(bfm_bootfail_log_info_t *pbootfail_log_info)
{
    ...
    for (i = 1; i < pbootfail_log_info->log_dir_count; i++)
    {
        for (j = 0; j < log_count; j++)
        {
            bool dst_file_exists = false;
            ...
            /* allocate mem */
            pdata = (char *)bfmr_malloc(src_file_len + strlen(BFM_TEXT_LOG_SEPRATOR_WITH_FIRST_NEWLINE) + 1);
            if (NULL == pdata)
            {
                BFMR_PRINT_ERR("bfmr_malloc failed!\n");
                continue;
            }
            memset(pdata, 0, (src_file_len + strlen(BFM_TEXT_LOG_SEPRATOR_WITH_FIRST_NEWLINE) + 1));
            ...
            /* if the dst file exists, save seprator firstly */
            dst_file_exists = bfmr_is_file_existed(dst_file_path);
            if (dst_file_exists)
            {
                bfmr_save_log(pbootfail_log_info->bootfail_logs[0].log_dir, s_valid_log_name[j], BFM_TEXT_LOG_SEPRATOR_WITH_FIRST_NEWLINE,
                    strlen(BFM_TEXT_LOG_SEPRATOR_WITH_FIRST_NEWLINE), 1);
            }

            /* format data and save it to dst file */
            pseprator = (('\n' == pdata[src_file_len])  ? (BFM_TEXT_LOG_SEPRATOR_WITHOUT_FIRST_NEWLINE)
                : (BFM_TEXT_LOG_SEPRATOR_WITH_FIRST_NEWLINE));
            memcpy(pdata + src_file_len, pseprator, strlen(pseprator));
            bfmr_save_log(pbootfail_log_info->bootfail_logs[0].log_dir, s_valid_log_name[j], pdata,
                src_file_len + (dst_file_exists ? 0 : strlen(pseprator)), 1);
            bfmr_free(pdata);
        }
