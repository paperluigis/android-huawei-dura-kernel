...
void  hwboot_fail_init_struct(void)
{
    ...
    ret = mtk_memcfg_get_memory_layout_info(MTK_MEMCFG_MEMBLOCK_PHY, HWBOOT_FAIL_MEM_NAME, &start, &size);
    if(ret)
    {
        pr_err("get HWBOOT_FAIL_MEM_NAME fail\n");
        return;
    }
    boot_log_virt = ioremap_nocache(start,size);
...
int qcom_hwboot_fail_init(void)
{
    ...
    {
        pr_err("%s bootfail_inject 0x%x\n",__func__,KERNEL_AP_PANIC);
        panic("hwboot: inject KERNEL_AP_PANIC");
    }

    if(check_bootfail_inject(KERNEL_AP_WDT))
    {
        pr_err("%s bootfail_inject 0x%x\n",__func__,KERNEL_AP_WDT);
        local_fiq_disable();
        preempt_disable();
        local_irq_disable();
        while (1)
