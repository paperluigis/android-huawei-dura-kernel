...
char g_fts_vendor_name[50]="trully-ft5436";
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ret = fts_i2c_read_reg(client, FTS_REG_FW_VER, &fw_ver_in_ic);
    if (ret < 0) {
        FTS_ERROR("fw version in ic read failed");
    }
    else
    {
        memset(g_fts_vendor_name,0,sizeof(g_fts_vendor_name));
        sprintf(g_fts_vendor_name,"trully,ft5436,fw=0x%x",fw_ver_in_ic);
    }
#endif
