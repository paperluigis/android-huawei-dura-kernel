...
/*< AR0009UD88 added leijianfeng/lwx468900 20180208 begin */
#ifdef CONFIG_HW_ZEROHUNG
#include "chipset_common/hwzrhung/hung_wp_screen.h"
#endif
/* AR0009UD88 added leijianfeng/lwx468900 20180208 end >*/
...
void long_press_work_func(struct work_struct *work)
{
	kpd_print("bfm KERNEL_PRESS10S doing\n");
	boot_fail_err(KERNEL_PRESS10S, NO_SUGGESTION, NULL);
	return;
}

#define BOOT_LOG_DELAY_TIME_5S 5
static struct hrtimer boot_log_timer_5s;
static enum hrtimer_restart boot_log_timer_5s_func(struct hrtimer *timer)
{
	schedule_work(&long_press_work);
	return HRTIMER_NORESTART;
}
static void boot_log_long_press_handler(u16 pressed)
{
	static int starting_hrtimer_flag = 0;
	...
	if (pressed)
	{
		...
		hrtimer_start(&boot_log_timer_5s, ktime_set(BOOT_LOG_DELAY_TIME_5S, 0), HRTIMER_MODE_REL);
		starting_hrtimer_flag = 1;
		kpd_print("bfm KERNEL_PRESS10S start\n");
		return;
	}
	else
	{
		if (starting_hrtimer_flag)
		{
			hrtimer_cancel(&boot_log_timer_5s);
			kpd_print("bfm KERNEL_PRESS10S cancel\n");
			starting_hrtimer_flag = 0;
			return;
		}
		kpd_print("bfm KERNEL_PRESS10S not started\n");
	}
}
...
void kpd_pwrkey_pmic_handler(unsigned long pressed)
{
	...
	boot_log_long_press_handler(pressed);
	kpd_pmic_pwrkey_hal(pressed);
/*< AR0009UD88 added leijianfeng/lwx468900 20180208 begin */
#ifdef CONFIG_HW_ZEROHUNG
    hung_wp_key_handle(kpd_dts_data.kpd_sw_pwrkey, pressed);
#endif
/* AR0009UD88 added leijianfeng/lwx468900 20180208 end >*/
...
/*< AR0009UD88 added leijianfeng/lwx468900 20180208 begin */
#ifdef CONFIG_HW_ZEROHUNG
            hung_wp_key_handle(linux_keycode, pressed);
#endif
/* AR0009UD88 added leijianfeng/lwx468900 20180208 end >*/
...
/* This func use as mrdump now, if powerky use kpd eint it need to open another API */
	...
	INIT_WORK(&long_press_work,long_press_work_func);
	hrtimer_init(&boot_log_timer_5s, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	boot_log_timer_5s.function = boot_log_timer_5s_func;
