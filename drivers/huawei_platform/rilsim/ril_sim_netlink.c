...
static void kernel_ril_sim_receive(struct sk_buff *__skb)
{
    ...
    hwlog_info("%s: kernel_ril_sim_receive enter.\n", __func__);

    if (NULL == __skb) {
        hwlog_err("%s: Invalid parameter: zero pointer reference(__skb).\n", __func__);
        return;
    }
    ...
    if (skb->len >= NLMSG_HDRLEN) {
        ...
        if ((nlh->nlmsg_len >= sizeof(struct nlmsghdr)) &&
            ...
            hwlog_info("%s: receive message form user space, nlmsg_type = %d, nlmsg_pid = 0x%x.\n", __func__,
            ...
            else if (NETLINK_KNL_SIM_CLEAR_ALL_VARIFIED_FLG == nlh->nlmsg_type)
            {
                ...
                {
                    gKnlSimPinInfo[slotIdx].is_verified = KNL_FALSE;
                }
    ...
    hwlog_info("%s: kernel_ril_sim_receive exit, skb->len:%d, nlh->nlmsg_len:%d.\n", __func__, skb->len, nlh->nlmsg_len);
}
...
int ril_sim_send_get_pin_info_cnf(u32 pid, int slotId)
{
    ...
    nlh = nlmsg_put(skb, 0, 0, NETLINK_KNL_SIM_GET_PIN_INFO_CNF, size, 0);
    if (!nlh) {
    ...
    if (ret < 0)
    {
        hwlog_err("%s: netlink_unicast fail, pid = 0x%x.\n", __func__, pid);
        kfree_skb(skb);
        skb = NULL;
        ret = KNL_FAILED;
        goto end;
    ...
    hwlog_info("%s: send NETLINK_KNL_SIM_GET_PIN_INFO_CNF to RIL, pid = 0x%x.\n", __func__, pid);
    goto end;
...
int ril_sim_send_query_verified_cnf(u32 pid)
{
    ...
    hwlog_info("%s: send NETLINK_KNL_SIM_QUERY_VARIFIED_CNF to RIL, pid = 0x%x, is_verified: %d.\n", __func__, pid, is_verified);
    goto end;
...
{
    hwlog_info("%s: ril_sim_netlink_thread enter.\n", __func__);
    ...
    while (1) {
        ...
        hwlog_info("%s: get sema success, g_ril_sim_rcv_msg_type:%d.\n", __func__, g_ril_sim_rcv_msg_type);

        if (NETLINK_KNL_SIM_GET_PIN_INFO_REQ == g_ril_sim_rcv_msg_type) {

            hwlog_info("%s: return NETLINK_KNL_SIM_GET_PIN_INFO_CNF to RIL, PID = 0x%x, slot id = %d.\n", __func__,
        ...
        {
            hwlog_info("%s: return NETLINK_KNL_SIM_QUERY_VARIFIED_CNF to RIL, PID = 0x%x.\n", __func__,
                g_ril_sim_req_pid);
    ...
    hwlog_info("%s: ril_sim_netlink_thread end.\n", __func__);
    return 0;
}
...
static void ril_sim_netlink_init(void)
{
    ...
    hwlog_info("%s: ril_sim_netlink_init enter.\n", __func__);

    g_ril_sim_nlfd = netlink_kernel_create(&init_net,
        NETLINK_RIL_EVENT_SIM,
        &ril_nl_cfg);
    if (!g_ril_sim_nlfd) {
        hwlog_err("%s: netlink_kernel_create return fail!\n", __func__);
    }
    else {
        hwlog_info("%s: netlink_kernel_create return success!\n", __func__);
    }
    ...
    hwlog_info("%s: ril_sim_netlink_init end.\n", __func__);
}
