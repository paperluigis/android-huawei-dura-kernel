 ...
 *
 * $Id: dhd_pcie.c 646109 2016-06-28 09:58:46Z $
 */
...
#ifdef DEVICE_TX_STUCK_DETECT
	{"dev_tx_stuck_monitor", IOV_DEVICE_TX_STUCK_DETECT,	0,	IOVT_UINT32,	0 },
#endif /* DEVICE_TX_STUCK_DETECT */
#endif
	{NULL, 0, 0, 0, 0 }
};
	...
	volatile char *regs, volatile char *tcm, void *pci_dev)
{
	...
	do {
		...
		bus->device_tx_stuck_check = jiffies;
#endif /* DEVICE_TX_STUCK_DETECT */
...
#endif /* defined(CUSTOMER_HW_ZEN) && defined(CONFIG_ARCH_MSM) && defined(CONFIG_64BIT) */
...
#ifdef DEVICE_TX_STUCK_DETECT
extern int
dhd_prot_txstatus_wakelock_timeout(dhd_pub_t *dhd);
extern void
dhd_prot_txstuck_ring_dump(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node);
...
dhd_bus_device_tx_stuck_scan(dhd_bus_t *bus)
{
	...
	bool ring_empty;
	bool active;
	bool all_rings_are_consumed;

	DHD_INFO(("%s: enter\n", __func__));
	all_rings_are_consumed = true;
	DHD_FLOWRING_LIST_LOCK(bus->dhd->flowring_list_lock, list_lock_flags);

	for (item = dll_tail_p(&bus->const_flowring);
			!dll_end(&bus->const_flowring, item); item = prev) {
		...
		DHD_INFO(("%s: flowid: %d tx_cmpl: %u tx_cmpl_prev: %u stuck_count: %d\n",
			...
			switch (flow_ring_node->stuck_count) {
				case DEVICE_TX_STUCK_WARN_DURATION:
					...
					DHD_ERROR(("stuck warning for flowid: %d sent to app\n",
						...
						/* Move debug info here to avoid print too many in softap mode. */
					DHD_ERROR(("%s: flowid: %d tx_cmpl: %u tx_cmpl_prev: %u stuck_count: %d\n",
						__func__, flow_ring_node->flowid, tx_cmpl,
						flow_ring_node->tx_cmpl_prev, flow_ring_node->stuck_count));

					dhd_prot_txstuck_ring_dump(bus->dhd, flow_ring_node);
				...
				case DEVICE_TX_STUCK_DURATION:
					...
					"Notify Device Tx Stuck,flowid %d\n", flow_ring_node->flowid);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
					dhd_os_send_hang_message(bus->dhd);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27) && OEM_ANDROID */
...
#ifdef BCM_WAKELOCK_PATCH
	if (all_rings_are_consumed) {
		/* We check the txstatus wakelock timeout here. */
		dhd_prot_txstatus_wakelock_timeout(bus->dhd);
	}
#endif
...
dhd_bus_device_stuck_scan(dhd_bus_t *bus)
{
	...
	timeout = bus->device_tx_stuck_check +
		msecs_to_jiffies(DEVICE_TX_STUCK_CKECK_TIMEOUT);
...
bool dhd_bus_watchdog(dhd_pub_t *dhd)
{
	...
	dhd->dhd_bus_busy_state |= DHD_BUS_BUSY_IN_WD;
		...
		if (dhd_os_devwake_check_deepsleep(dhd)) {
			DHD_GENERAL_UNLOCK(dhd, flags);
			dhd_os_devwake_wait(dhd, &(dhd->deepsleep));
			DHD_TRACE(("%s: contine after dev wake\n", __FUNCTION__));
		} else
...
#if defined(DEVICE_TX_STUCK_DETECT) && defined(BCM_WAKELOCK_PATCH)
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	while ((len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
		...
		/* check if CR4 */
#ifndef HW_OPENTIME_SAVE
		if (si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
#else
		if (offset == 0 && si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
#endif
...
#ifdef HW_NVRAM_VARS_CALIBRATE
                if(len){
                        hw_cal_len = hw_calibrate_nvram_vars(bus, bufp, len);
                        if(hw_cal_len > 0) {
                                len=hw_cal_len;
                        }
                }
...
#ifdef MSM_PCIE_LINKDOWN_RECOVERY || CONFIG_ARCH_HISI
		bus->islinkdown = TRUE;
...
void dhd_read_console_ex(dhd_pub_t *dhd)
{
	int ret = 0;
	if (NULL != dhd) {
		ret = dhdpcie_bus_readconsole(dhd->bus);
		DHD_ERROR(("%s: read console, ret: %d\n", __FUNCTION__, ret));
	}
}
#endif
...
dhdpcie_bus_membytes(dhd_bus_t *bus, bool write, ulong address, uint8 *data, uint size)
{
	...
	bool little_endian;
#if (defined(CONFIG_ARCH_MSM) || (CONFIG_ARCH_HISI)) && defined(CONFIG_64BIT)
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	if (dhd_download_fw_on_driverload) {
		...
		if (flag == TRUE) { /* Turn off WLAN */
			...
			/* Hold spinlock to avoid race condition between watchdog thread
			...
			if (bus->dhd->busstate != DHD_BUS_DOWN) {
...
#ifdef HW_PCIE_STABILITY
				/*If pcie is linkdown not visit register avoid noc reset*/
				if(!bus->is_linkdown) {
#endif
					...
					DHD_ERROR(("%s: busstate_on to kirin_pcie_pm_control\n", __FUNCTION__));
					kirin_pcie_pm_control(POWEROFF_BUSON, 0);
#ifdef HW_PCIE_STABILITY
				} else {
					kirin_pcie_pm_control(POWEROFF_BUSDOWN, 0);
				}
#endif
#ifdef CONFIG_ARCH_MSM
				...
				if(!bus->is_linkdown) {
					...
					DHD_ERROR(("%s: busstate_off to kirin_pcie_pm_control\n",__FUNCTION__));
#ifdef HW_PCIE_STABILITY
				}
#endif
				kirin_pcie_pm_control(POWEROFF_BUSDOWN, 0);
...
#endif /* CONFIG_ARCH_MSM */
				bcmerror = kirin_pcie_pm_control(1, 0);
				if (bcmerror){
					DHD_ERROR(("%s: host pcie power on failed: %d\n",
						__FUNCTION__, bcmerror));
					goto done;
				}
	...
	if (bcmerror) {
		...
		bus->dhd->busstate = DHD_BUS_DOWN;
		DHD_GENERAL_UNLOCK(bus->dhd, flags);
		/* disable interrupt since failed */
	        dhdpcie_bus_intr_disable(bus);
...
dhdpcie_bus_write_vars(dhd_bus_t *bus)
{
	...
	if (bus->vars) {
		...
		/* Write the vars list */
		DHD_INFO(("%s: tcm: %p varaddr: 0x%x varsize: %d\n",
	...
	DHD_INFO(("%s: tcm: %p phys_size: 0x%x varsizew: %x\n",
...
#endif
#ifdef DHD_DEVWAKE_EARLY
extern int devwake_gpio_is_support(void);
#endif
int
...
#ifdef BRCM_RSDB
{
    ...
    int softap_mode;
    softap_mode = strstr(bus->fw_path, "apsta")? 1:0;
    DHD_ERROR(("%s: softap_mode %d  devwake_gpio_is_support() is %d\n", __FUNCTION__, \
    softap_mode, devwake_gpio_is_support()));
    //softap or devwake timeout or gpio is not configured disable deepsleep function
    if (bus->dhd->devwake_timeout || devwake_gpio_is_support() == -1 || softap_mode) {
	device_wake_opt = getvar(bus->vars, "device_wake_opt");
 		if (device_wake_opt) {
			/* move string ptr to head of item */
			device_wake_opt -= (strlen("device_wake_opt") + 1);
			/* rename to disable it */
			device_wake_opt[0] = 'x';
			DHD_ERROR(("%s: device_wake_opt renames as %s to remove it.\n",
				__FUNCTION__, device_wake_opt));
		}
    }

#endif
    rsdb_mode = getvar(bus->vars, "rsdb_mode");
    if(rsdb_mode) {
        bus->dhd->rsdb_mode = bcm_atoi(rsdb_mode);
        DHD_ERROR(("nvram rsdb_mode: %s => %d\n", rsdb_mode, bus->dhd->rsdb_mode));
    } else {
        bus->dhd->rsdb_mode = TRUE;
        DHD_ERROR(("No rsdb_mode in nvram => rsdb_mode set TRUE\n"));
    }
#ifdef DHD_DEVWAKE_EARLY
	device_wake_opt = getvar(bus->vars, "device_wake_opt");
	if (device_wake_opt) {
			bus->dhd->devwake_enable = TRUE;
			DHD_ERROR(("nvram device_wake_opt: %s => devwake_enable %d\n",
				device_wake_opt, bus->dhd->devwake_enable));
	} else {
...
dhd_bus_dpc(struct dhd_bus *bus)
{
	...
	dhd_tput_monitor_func((ulong)(bus->dhd->info));
#endif /* DHD_TPUT_MONITOR */
#ifdef BCM_WAKELOCK_PATCH
#ifdef DEVICE_TX_STUCK_DETECT
	if (bus->dev_tx_stuck_monitor == TRUE) {
		dhd_bus_device_stuck_scan(bus);
	}
#else /* DEVICE_TX_STUCK_DETECT */
	/* check active tx count */
        dhd_prot_txstatus_wakelock_timeout(bus->dhd);
#endif
#endif
...
dhdpcie_send_mb_data(dhd_bus_t *bus, uint32 h2d_mb_data)
{
	...
	/* Avoid to access PCIe after Wi-Fi off. */
#ifdef DHD_DEVWAKE_EARLY
	uint flags;
#endif
...
dhdpcie_readshared(dhd_bus_t *bus)
{
	...
	DHD_INFO(("%s: ram_base: 0x%x ramsize 0x%x tcm: %p shaddr: 0x%x nvram_csm: 0x%x\n",
...
int dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if (ret < 0) {
#ifdef HW_PCIE_STABILITY
		/*Pcie bus is error,avoid disable interrupt cause noc reset*/
		bus->is_linkdown = 1;
#endif
		DHD_ERROR(("%s :Shared area read failed \n", __FUNCTION__));
		return ret;
...
#ifdef PCIE_CLEANFLOW
		if (timer_pending(&flow_ring_node->resp_timer)) {
			DHD_ERROR(("%s : Flow %d, cancel previous response timer\n",
				__FUNCTION__, flow_ring_node->flowid));

			del_timer_sync(&flow_ring_node->resp_timer);
		}
		_timer = &flow_ring_node->resp_timer;
		INIT_FLOW_RINGS_TIMER(_timer, dhd_bus_flow_ring_response_expired, FLOW_RINGS_RESPONSE_TIME);
#endif
...
#ifdef PCIE_CLEANFLOW
	if (timer_pending(&flow_ring_node->resp_timer)) {
		DHD_ERROR(("%s :Flow %d, response received, delete timer\n",
			__FUNCTION__, flow_ring_node->flowid));

		del_timer_sync(&flow_ring_node->resp_timer);
    } else if(flow_ring_node->status != FLOW_RING_STATUS_DELETE_PENDING){
		DHD_ERROR(("%s :Flow %d, response received after expired, skip clean\n",
			__FUNCTION__, flow_ring_node->flowid));
...
#ifdef PCIE_CLEANFLOW
void
dhd_bus_flow_ring_response_expired(unsigned long data)
{
	flow_ring_node_t *flow_ring_node = (flow_ring_node_t *) data;
    dhd_pub_t *dhd = (dhd_pub_t *)(flow_ring_node->dhd);
    dhd_bus_t *bus = dhd->bus;
    DHD_TRACE(("%s: Enter\n", __FUNCTION__));

    if(bus && flow_ring_node->status == FLOW_RING_STATUS_DELETE_PENDING) {
      DHD_ERROR(("%s :Flow Delete Response expired, %d \n",
                   __FUNCTION__, flow_ring_node->status));
...
static int hw_load_Txcalibrate_data(char * calbufp, uint size)
{
	int ret = 0;
	memset(calbufp, 0x00, size);

#define TEL_HUAWEI_NV_WINVRAM_NAME               "WINVRAM"
#define TEL_HUAWEI_NV_WINVRAM_NUMBER             340
#define TEL_HUAWEI_NV_WINVRAM_LENGTH             104

	struct hisi_nve_info_user  info;
	memset(&info, 0, sizeof(info));
	info.nv_number  = TEL_HUAWEI_NV_WINVRAM_NUMBER;   //nve item
	info.valid_size = TEL_HUAWEI_NV_WINVRAM_LENGTH;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, TEL_HUAWEI_NV_WINVRAM_NAME, 7);
	ret = hisi_nve_direct_access( &info );
	if (!ret)
	{
		strncpy(calbufp, info.nv_data, size);
		HW_PRINT_HI("%s: calbufp =\"%s\"\n",  __FUNCTION__, calbufp);
		return (int)strlen(calbufp);

	}
	HW_PRINT_HI("%s: No data \n",  __FUNCTION__);
	return 0;
}

static int hw_load_Rxcalibrate_data(char * calbufp, size_t size)
{
	int ret = 0;
	memset(calbufp, 0x00, size);

#define TEL_HUAWEI_NV_WIRXNVRAM_NAME               "WIRXNV"
#define TEL_HUAWEI_NV_WIRXNVRAM_NUMBER             363
#define TEL_HUAWEI_NV_WIRXNVRAM_LENGTH             104

	struct hisi_nve_info_user  info;
	memset(&info, 0, sizeof(info));
	info.nv_number  = TEL_HUAWEI_NV_WIRXNVRAM_NUMBER;   //nve item
	info.valid_size = TEL_HUAWEI_NV_WIRXNVRAM_LENGTH;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, TEL_HUAWEI_NV_WIRXNVRAM_NAME, 6);
	ret = hisi_nve_direct_access( &info );
	if (!ret)
	{
		strncpy(calbufp, info.nv_data, size);
		DHD_ERROR(("%s: calbufp =\"%s\"\n",  __FUNCTION__, calbufp));
...
static int hw_load_MimoTxcckcalibrate_data(char * calbufp, size_t size)
{
	int ret = 0;
	memset(calbufp, 0x00, size);

#define TEL_HUAWEI_NV_WITXCCK_NAME               "WITXCCK"
#define TEL_HUAWEI_NV_WITXCCK_NUMBER             367
#define TEL_HUAWEI_NV_WITXCCK_LENGTH             104

	struct hisi_nve_info_user  info;
	memset(&info, 0, sizeof(info));
	info.nv_number  = TEL_HUAWEI_NV_WITXCCK_NUMBER;   //nve item
	info.valid_size = TEL_HUAWEI_NV_WITXCCK_LENGTH;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, TEL_HUAWEI_NV_WITXCCK_NAME, 7);
	ret = hisi_nve_direct_access( &info );
	if (!ret)
	{
		strncpy(calbufp, info.nv_data, size);
...
static int hw_load_MimoTxc1calibrate_data(char * calbufp, size_t size)
{
	int ret = 0;
	memset(calbufp, 0x00, size);

#define TEL_HUAWEI_NV_WITXC1_NAME               "WITXC1"
#define TEL_HUAWEI_NV_WITXC1_NUMBER             368
#define TEL_HUAWEI_NV_WITXC1_LENGTH             104

	struct hisi_nve_info_user  info;
	memset(&info, 0, sizeof(info));
	info.nv_number  = TEL_HUAWEI_NV_WITXC1_NUMBER;   //nve item
	info.valid_size = TEL_HUAWEI_NV_WITXC1_LENGTH;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, TEL_HUAWEI_NV_WITXC1_NAME, 6);
	ret = hisi_nve_direct_access( &info );
	if (!ret)
	{
		strncpy(calbufp, info.nv_data, size);
...
static int hw_load_MimoTxbw0calibrate_data(char * calbufp, size_t size)
{
	int ret = 0;
	memset(calbufp, 0x00, size);

#define TEL_HUAWEI_NV_WITXBW0_NAME               "WITXBW0"
#define TEL_HUAWEI_NV_WITXBW0_NUMBER             369
#define TEL_HUAWEI_NV_WITXBW0_LENGTH             104

	struct hisi_nve_info_user  info;
	memset(&info, 0, sizeof(info));
	info.nv_number  = TEL_HUAWEI_NV_WITXBW0_NUMBER;   //nve item
	info.valid_size = TEL_HUAWEI_NV_WITXBW0_LENGTH;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, TEL_HUAWEI_NV_WITXBW0_NAME, 7);
	ret = hisi_nve_direct_access( &info );
	if (!ret)
	{
		strncpy(calbufp, info.nv_data, size);
...
static int hw_load_MimoTxbw1calibrate_data(char * calbufp, size_t size)
{
	int ret = 0;
	memset(calbufp, 0x00, size);

#define TEL_HUAWEI_NV_WITXBW1_NAME               "WITXBW1"
#define TEL_HUAWEI_NV_WITXBW1_NUMBER             370
#define TEL_HUAWEI_NV_WITXBW1_LENGTH             104

	struct hisi_nve_info_user  info;
	memset(&info, 0, sizeof(info));
	info.nv_number  = TEL_HUAWEI_NV_WITXBW1_NUMBER;   //nve item
	info.valid_size = TEL_HUAWEI_NV_WITXBW1_LENGTH;
	info.nv_operation = NV_READ;
	strncpy(info.nv_name, TEL_HUAWEI_NV_WITXBW1_NAME, 7);
	ret = hisi_nve_direct_access( &info );
	if (!ret)
	{
		strncpy(calbufp, info.nv_data, size);
...
static char* hw_get_var_from_buff(int index, char *bufp, uint len)
{
	uint num =0;
	uint i = 0;
	...
	for(i=0; i<len; i++)
	{
		if(i+1<=len && bufp[i] == 0 && bufp[i+1] == 0) {
			continue;
		}
		if (bufp[i] == 0 ){
			num ++;
...
static uint hw_get_var_num_from_buff(char *bufp, uint len) {
	uint num =0;
	uint i=0;
	char *ptr = bufp;
...
static uint hw_get_var_name_length(char *bufp, uint len)
{
	uint i=0;
	for(i=0; i<len; i++)
	{
...
static uint hw_check_var(char *varbufp)
{
	...
	varlen = strlen(varbufp);
	varnamelen = hw_get_var_name_length(varbufp, varlen);
...
/*
  function: calibrate the wifi nvram vars
  the bufp formate is "var1=value1"0"var2=value3"0"var3=value3"
*/
#define CALIBRATE_SIZE 128
#ifdef SUPPORT_WT_WIFI_MIMO_CALIBRATE
#define MAX_CALIBRATE_SIZE 128*6
#else
#define MAX_CALIBRATE_SIZE 128*2
#endif
static int hw_calibrate_nvram_vars(struct dhd_bus *bus, char * bufp, uint len)
{
	char CalDateBuf[MAX_CALIBRATE_SIZE];
	int CalDateLen = 0;
	uint num_nvram_var = 0;
	uint num_caldata_ver = 0;
	char *calvarPtr = NULL;
	char *nvramvarPtr = NULL;
	uint namelen = 0;
	uint i=0, k=0;
	char * VarsBufp = NULL;
	uint VarsBufLen = 0;
	...
	CalDateLen = hw_load_Txcalibrate_data(CalDateBuf, CALIBRATE_SIZE);
	CalDateLen += hw_load_Rxcalibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE);
#ifdef SUPPORT_WT_WIFI_MIMO_CALIBRATE
	CalDateLen += hw_load_MimoTxcckcalibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE);
	CalDateLen += hw_load_MimoTxc1calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE);
	CalDateLen += hw_load_MimoTxbw0calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE);
	CalDateLen += hw_load_MimoTxbw1calibrate_data(CalDateBuf+(strlen(CalDateBuf)), CALIBRATE_SIZE);
#endif /* SUPPORT_WT_WIFI_MIMO_CALIBRATE */
	if (CalDateLen<=0) {
	...
	DHD_ERROR(("%s:nvram calibrate data:\"%s\"\n",  __FUNCTION__, CalDateBuf));

	/*alloc the memery buffer for nvram temp*/
	VarsBufp = MALLOC(bus->dhd->osh, MAX_NVRAMBUF_SIZE);
	if(VarsBufp == NULL) {
		DHD_ERROR(("%s:alloc memery failed \n",  __FUNCTION__));
		goto fail;
	}

	/*clean the temp buffer VarsBufp*/
	memset(VarsBufp, 0x00, MAX_NVRAMBUF_SIZE);
	VarsBufLen = 0;

	/*make the calibrate data to nvram var formate*/
	for (i=0; i<CalDateLen; i++)
	{
		if( (CalDateBuf[i] == ';') || (CalDateBuf[i] == '\n') || (CalDateBuf[i] == '\r')) {
			CalDateBuf[i] = 0;
		}
	}
	if(CalDateBuf[CalDateLen]!=0)
		CalDateBuf[CalDateLen] = 0;

	/*get the vars number of calibrate data and nvram*/
	num_caldata_ver = hw_get_var_num_from_buff(CalDateBuf, CalDateLen);
	num_nvram_var = hw_get_var_num_from_buff(bufp, len);
	DHD_ERROR(("%s: num_caldata_ver=%d\n",  __FUNCTION__, num_caldata_ver));
	DHD_ERROR(("%s: num_nvram_var=%d\n",  __FUNCTION__, num_nvram_var));
	if (num_caldata_ver <=0 || num_nvram_var<=0) {
		DHD_ERROR(("%s:get var number is not ok \n",  __FUNCTION__));
		goto fail;
	}

	/*apply the calibrate data vars*/
	for (i=0; i<num_nvram_var; i++)
	{
		nvramvarPtr = hw_get_var_from_buff(i, bufp, len);
		if(hw_check_var(nvramvarPtr) == 0) {
			DHD_ERROR(("%s:get nvram var fail, nvramvarPtr=%s \n",  __FUNCTION__, (nvramvarPtr==NULL)?"":nvramvarPtr));
			goto fail;
		}

		namelen= hw_get_var_name_length(nvramvarPtr, strlen(nvramvarPtr));
		if(namelen == 0) {
			DHD_ERROR(("%s:get nvram var name length fail \n",  __FUNCTION__));
			goto fail;
		}

		for(k=0; k<num_caldata_ver; k++) {
			calvarPtr = hw_get_var_from_buff(k, CalDateBuf, CalDateLen);
			if(hw_check_var(calvarPtr) == 0) {
				DHD_ERROR(("%s:get calibrate data var fail, calvarPtr=%s\n",  __FUNCTION__, (calvarPtr==NULL)?"":calvarPtr));
				goto fail;
			}
			if (calvarPtr[namelen]=='=' && memcmp(nvramvarPtr, calvarPtr, namelen) ==0 ) {
					memcpy(VarsBufp+VarsBufLen, calvarPtr, strlen(calvarPtr));
					VarsBufLen+= strlen(calvarPtr)+1;
					VarsBufp[VarsBufLen-1] = 0;
					DHD_ERROR(("%s: calibrate var from \"%s\" to \"%s\"\n",  __FUNCTION__, nvramvarPtr, calvarPtr));
					break;
			}
		}
		if (k >= num_caldata_ver) {
			memcpy(VarsBufp+VarsBufLen, nvramvarPtr, strlen(nvramvarPtr));
			VarsBufLen+= strlen(nvramvarPtr)+1;
			VarsBufp[VarsBufLen-1] = 0;
		}
	}
	/* check if have external nv ram */
	for (i=0; i<num_caldata_ver; i++)
	{
		calvarPtr = hw_get_var_from_buff(i, CalDateBuf, CalDateLen);
		if(hw_check_var(calvarPtr) == 0) {
			HW_PRINT_HI("%s:get nvram var fail, calvarPtr=%s \n",  __FUNCTION__, (calvarPtr==NULL)?"":calvarPtr);
			goto fail;
		}

		namelen= hw_get_var_name_length(calvarPtr, strlen(calvarPtr));
		if(namelen == 0) {
			HW_PRINT_HI("%s:get nvram var name length fail \n",  __FUNCTION__);
			goto fail;
		}

		for(k=0; k<num_nvram_var; k++) {
			nvramvarPtr = hw_get_var_from_buff(k, VarsBufp, VarsBufLen);
			if(hw_check_var(nvramvarPtr) == 0) {
				HW_PRINT_HI("%s:get calibrate data var fail, calvarPtr=%s\n",  __FUNCTION__, (calvarPtr==NULL)?"":calvarPtr);
				MFREE(bus->dhd->osh, VarsBufp, MAX_NVRAMBUF_SIZE);
				return -1;
			}
			if (nvramvarPtr[namelen]=='=' && memcmp(nvramvarPtr, calvarPtr, namelen) ==0 )
			{
				HW_PRINT_HI("%s: calibrate var exist %s \n",  __FUNCTION__, calvarPtr);
				break;
			}
		}
		if (k >= num_nvram_var) {
			HW_PRINT_HI("%s: calibrate var add %s \n",  __FUNCTION__, calvarPtr);
			memcpy(VarsBufp+VarsBufLen, calvarPtr, strlen(calvarPtr));
			VarsBufLen+= strlen(calvarPtr)+1;
			VarsBufp[VarsBufLen-1] = 0;
		}
	}
	/*check the calibrated nvram var number*/
	if (hw_get_var_num_from_buff(VarsBufp, VarsBufLen) >= num_nvram_var) {
		memset(bufp, 0x00, len);
		memcpy(bufp, VarsBufp, VarsBufLen);
		DHD_ERROR(("%s:calibrated nvram vars is ok\n",  __FUNCTION__));
		goto success;
	}
	else
	{
		DHD_ERROR(("%s:calibrated nvram vars is not ok, ignore the calibrate data\n",  __FUNCTION__));
		goto fail;
	}

fail:
	DHD_ERROR(("%s: calibrated nvram var failed!\n",  __FUNCTION__));
	VarsBufLen = 0;
success:
	/*free the memery buffer*/
	if (VarsBufp)
		MFREE(bus->dhd->osh, VarsBufp, MAX_NVRAMBUF_SIZE);
	return VarsBufLen;
}
