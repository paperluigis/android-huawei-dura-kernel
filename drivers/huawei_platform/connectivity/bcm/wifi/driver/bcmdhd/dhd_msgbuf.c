...
typedef struct dhd_prot {
	...
	unsigned long active_tx_last_jiffies;	/* The last active tx wakelock time */
#endif
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	 ...
	 */
#ifdef H2DRING_PREALLOC_SIZE
	if (dhd_dma_buf_alloc(dhd, &prot->flowrings_dma_buf, H2DRING_PREALLOC_SIZE)) {
		DHD_ERROR(("%s: H2DRING_PREALLOC_SIZE=%d alloc fail\n", __FUNCTION__, H2DRING_PREALLOC_SIZE));
	}
#endif /* H2DRING_PREALLOC_SIZE */
...
#ifdef H2DRING_PREALLOC_SIZE
		if (prot->flowrings_dma_buf.va) {
			dhd_dma_buf_free(dhd, &prot->flowrings_dma_buf);
		}
#endif
...
#ifdef BCM_WAKELOCK_PATCH
#define TX_WAKE_TIMEOUT_IN_JIFFIES msecs_to_jiffies(10000) /* 10s */

int BCMFASTPATH dhd_prot_txstatus_wakelock_timeout(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	unsigned long flags;
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	prot->active_tx_count++;
#ifdef BCM_WAKELOCK_PATCH
	prot->active_tx_last_jiffies = jiffies;
#endif
	...
	if (ioc->cmd != WLC_GET_MAGIC && ioc->cmd != WLC_GET_VERSION) {
		if (ioc->cmd == WLC_SET_VAR || ioc->cmd == WLC_GET_VAR) {
			...
			DHD_IOV_INFO(("%s: IOVAR_INFO name = %s set = %d\n",
				__FUNCTION__, (char *)buf, ioc->set));
			if ((dhd_msg_level & DHD_IOV_INFO_VAL) && ioc->set && buf) {
				prhex(NULL, buf + strlen(buf) + 1,
				len - strlen(buf) - 1);
			}
		} else {
...
#ifdef H2DRING_PREALLOC_SIZE
		if ((base_offset + dma_buf_len) > rsv_buf->len) {
			DHD_ERROR(("RING_ATTACH : End of flow dma(%x) is over prealloc size(%d)!\n",
				(base_offset + dma_buf_len), rsv_buf->len));
		}
#endif
...
#ifdef H2DRING_PREALLOC_SIZE
		if (PHYSADDRLO(dma_buf->pa) < PHYSADDRLO(rsv_buf->pa)) {
			DHD_ERROR(("RING_ATTACH : boundary across error!\n"));
		}
#endif
...
dhd_prot_get_read_addr(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint32 *available_len)
{
	...
	if (items >= ring->max_items) {
		...
		*available_len = 0;
		...
		/*If read is 0xffff,wifi recovery avoid pcie noc*/
		if (wr==0xffff) {
...
dhd_prot_txstuck_ring_dump(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	DHD_ERROR(("%s: %4d %2d %4d %17s %4d %4d %10u\n", __func__,
	...
	dhd_bus_cmn_readshared(dhd->bus, &rd, RING_RD_UPD, flow_ring->idx);
	dhd_bus_cmn_readshared(dhd->bus, &wr, RING_WR_UPD, flow_ring->idx);
	DHD_ERROR(("%s: RD %d WR %d BASE(VA) %p BASE(PA) %x:%x SIZE %d\n", __func__,
		rd, wr, flow_ring->dma_buf.va,
                ltoh32(flow_ring->base_addr.high_addr),
                ltoh32(flow_ring->base_addr.low_addr), dma_buf_len));

	DHD_ERROR(("%s: active_tx_count %d pktidmap_avail %d\n", __func__,
		dhd->prot->active_tx_count,
		DHD_PKTID_AVAIL(dhd->prot->pktid_map_handle)));
}
#endif
