...
static ssize_t stateinfo_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if(pdisc_data)
	{
		len = snprintf(buf, PAGE_SIZE, "irq-enable=[%s] kb_online[%d] start_detect=[%d] \n",pdisc_data->irq_enabled ? "enabled" : "disabled",pdisc_data->kb_online,pdisc_data->start_detect );
	}
	else
	{
		len = snprintf(buf, PAGE_SIZE, "=====stateinfo_show FAILED![data ERR]===== \n");
	}
