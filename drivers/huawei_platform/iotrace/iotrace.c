...
/* for apr DFX HI1861*/
static unsigned long scsi_io[64][2];
static unsigned int scsi_cnt = 0;
...
#define UFS_HI1861_FSR_DMD_CODE  914005000
#define UFS_HI1861_FSR_UPLOAD_INTER  (1 * 24 * 3600)
...
static int iotrace_ufs_fsr_get(void)
{
    ...
    unsigned char *mem = io_mem_mngt_alloc(io_trace_this->mem_mngt);
    static struct file *fsr_filp = NULL;
    //unsigned char *mem = kmalloc(IO_MAX_BUF_ENTRY,GFP_KERNEL);
    //mutex_lock(&output_mutex);
    if(mem == NULL){
    ...
    io_trace_print("add by somic, ptr : %d\n", *mem);
    ret = write_io_monlog_kernel(fsr_filp, (unsigned char *)mem, IO_MAX_BUF_ENTRY);
    //ret = sys_write(fsr_filp, (unsigned char *)mem, IO_MAX_BUF_ENTRY);

    if(ret < 0){
        io_trace_print("write log kernel failed !\n");
        goto exit;
    }
...
static struct file *open_log_file(char *name)
{
    ...
    filp = filp_open(name, O_CREAT| O_RDWR | O_TRUNC, 0664);
...
static int apr_work_thread_fn(void *data)
{
    ...
    while (1) {
        ...
        if (stop){
            io_trace_print("apr get thread exit!\n");
            break;
        }
        ...
        sprintf(file_name, "/data/log/log_%s", "hi1861_fsr");
		fsr_filp = open_log_file(file_name);
