...
#else
int  Process_KBChannelData(char *data, int count) { return -1;}
void Notify_KB_DoDetect(void) { }
int sw_get_statue(void) { return  -1 ; }
#endif
...
{
	hwlog_info("%s go!\n", __func__);
	//inputhub_route_read(ROUTE_KB_PORT, buf, count);
	return 0;
}
...
static long kbhb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	hwlog_err("%s  cmd : %d\n", __func__, cmd);
	//inputhub_route_cmd(ROUTE_KB_PORT, cmd, arg);
...
MODULE_AUTHOR("KBHub <smartphone@huawei.com>");
