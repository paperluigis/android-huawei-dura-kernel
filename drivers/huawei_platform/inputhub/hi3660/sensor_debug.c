...
static int rpc_test(int tag, int argv[], int argc)
{
	...
	if (cmd == CMD_RPC_UPDATE_REQ)
	 {
		pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
		pkg_ioctl.sub_cmd = cmd;
		pkg_ioctl.sarinfo = stat;
		pkg_ap.wr_buf = &pkg_ioctl;
		pkg_ap.wr_len = sizeof(pkg_ioctl);
		hwlog_info("command in rpc_test, cmd=%c\n", cmd);
	} else {
		hwlog_err("error command in rpc_test, cmd=%c\n", cmd);
		return -1;
	}
    ret = write_customize_cmd(&pkg_ap, &pkg_mcu);
    if (ret) {
            hwlog_err("send rpc cmd to mcu fail,ret=%c\n", ret);
            return ret;
    }
    if (pkg_mcu.errno != 0) {
            hwlog_err("set rpc cmd fail cmd=%c\n", cmd);
     } else {
            hwlog_info("set rpc cmd success cmd=%c\n",cmd);
     }
			    ...
			    const char *buf, size_t size)
{
	...
	if(val == 0 && data->tag == 34)
	{
		...
		hwlog_info("rpc_motion_request val = %d in %s \n", rpc_motion_request,__func__);
	}
	ret = write_customize_cmd(&pkg_ap, &pkg_mcu);
	if (ret != 0) {
...
static DEVICE_ATTR(rpc_sar_service_req,0660, NULL,
	          store_rpc_sar_service_req);
