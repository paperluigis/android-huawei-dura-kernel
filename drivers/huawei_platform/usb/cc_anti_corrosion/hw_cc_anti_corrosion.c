...
{
	if (!ops || !cc_corrosion_dev_p) {
		hw_usb_err("%s ops or cc_corrosion_dev_p is NULL\n", __func__);
		return;
	}
	cc_corrosion_dev_p->ops = ops;
}
static void hw_cc_corrosion_first_work(void)
{
	...
	if (!cc_corrosion_dev_p->ops) {
		hw_usb_err("%s get cc_corrosion_dev_p->ops fail\n", __func__);
		return;
	}
	...
	if (ret) {
		...
		if ( (NV_CGNF_OK == nv_value) && gpio_value) {
			...
			cc_corrosion_dev_p->ops->set_cc_mode(CC_MODE_UFP);
		}else {
...
static void hw_cc_corrosion_update_nv_work(struct work_struct *work)
{
	...
	if (!cc_corrosion_dev_p || !cc_corrosion_dev_p->ops) {
	...
	if (gpio_value) {
		...
		cc_corrosion_dev_p->ops->set_cc_mode(CC_MODE_DRP);
	}
...
static void hw_cc_corrosion_intb_work(struct work_struct *work)
{
	...
	if (gpio_value != gpio_get_value(cc_corrosion_dev_p->irq_gpio))
	{
		hw_usb_err("%s gpio changed to %d\n", __func__, gpio_get_value(cc_corrosion_dev_p->irq_gpio));
		goto err_bounce;
	}
