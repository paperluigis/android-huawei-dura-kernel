...
/*int ts_string_seprater(const char *buf,int *enable, int *x0, int *y0, int *x1, int *y1)
{
    ...
    tmp = kzalloc((strlen(buf) +1 )* sizeof(char), GFP_KERNEL);
    if(!tmp){
        TS_LOG_ERR("%s: kalloc fail !!\n", __func__);
        return error;
    }
...
static ssize_t ts_touch_window_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
    ...
    memset(&cmd, 0, sizeof(struct ts_cmd_node));
    TS_LOG_INFO("ts_touch_window_store called\n");
...
static ssize_t ts_register_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
    ...
    error = sscanf(buf, "%d 0x%4x %s %d %d", &flag, &addr, value, &num, &bit);
    if (error <= 0)
    {
        ...
        struct device_attribute* attr, const char* buf, size_t size)
{
    ...
    if ((TS_SLEEP == atomic_read(&g_ts_kit_platform_data.state)) || (TS_WORK_IN_SLEEP == atomic_read(&g_ts_kit_platform_data.state))
            ||(TS_RESUMING == atomic_read(&g_ts_kit_platform_data.state)))
    {
...
static ssize_t ts_tp_load_status_show(struct device* dev,
        struct device_attribute* attr, char* buf)
{
    struct ts_easy_wakeup_info* info = &g_ts_kit_platform_data.chip_data->easy_wakeup_info;
    ssize_t ret;
...
static ssize_t ts_logo_info_show(struct device* dev, struct device_attribute* attr, char* buf)
{
    int error = NO_ERR;
    u8 tp_logo = 0;

    TS_LOG_INFO("ts_logo_info_show called\n");
    ...
    TS_LOG_INFO("%s:ts_logo_info = 0x%x\n",__func__,g_ts_kit_platform_data.chip_data->ts_logo_info[0]);

    tp_logo = g_ts_kit_platform_data.chip_data->ts_logo_info[0];
    switch(tp_logo) {
    case LOGO_HUAWEI:
        error = snprintf(buf, sizeof("huawei"), "%s", "huawei");
        break;
    case LOGO_HONOR:
        error = snprintf(buf, sizeof("honor"), "%s", "honor");
        break;
    /* extern other logo info... */
    default:
        error = snprintf(buf, sizeof("unknow"), "%s", "unknow");
        TS_LOG_ERR("%s: TP logo can not detected\n");
        break;
    }
    TS_LOG_INFO("%s: TP logo is %s\n", __func__, buf);
out:
    TS_LOG_INFO("ts_logo_info_show done\n");
    return error;
}
...
static DEVICE_ATTR(tp_capacitance_test_type, (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH), ts_capacitance_test_type_show, ts_capacitance_test_type_store);
...
static DEVICE_ATTR(tp_load_status, (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH |S_IWGRP),ts_tp_load_status_show, NULL);
static DEVICE_ATTR(touch_logo_info, (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH), ts_logo_info_show, NULL);
static struct attribute* ts_attributes[] =
{
    ...
    &dev_attr_tp_load_status.attr,
    &dev_attr_touch_logo_info.attr,
    NULL
};
