...
struct ts_kit_device_data *g_ts_kit_device_data = NULL;
...
u8 *gpDMABuf_va = NULL;
u8 *gpDMABuf_pa = NULL;
struct of_device_id touch_of_match1[] = {
	...
	{ .compatible = "mediatek,mt6739-touch", },//add by zhangle for pandura.
	{},
};
...
static struct platform_driver tpd_driver = {
	...
	.driver = {
			...
			.pm = NULL,
			.owner = THIS_MODULE,
			.of_match_table = touch_of_match1,
	},
};
...
int ts_kit_update_firmware(void);
...
static DECLARE_DELAYED_WORK(ts_firmware_update_work, ts_kit_update_firmware);
...
void ts_firmware_update_again(void)
{
    schedule_delayed_work(&ts_firmware_update_work,5*HZ);
}
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gpDMABuf_va = (u8 *)dma_alloc_coherent(&client->dev, 8192, (dma_addr_t *)&gpDMABuf_pa, GFP_KERNEL);
	if(!gpDMABuf_va)
	{
		TS_LOG_ERR("Allocate DMA I2C Buffer failed\n");
		goto err_alloc_MTK_DMA_failed;
	}
	memset(gpDMABuf_va, 0, 8192);
	return 0;
	err_alloc_MTK_DMA_failed:
	if(gpDMABuf_va)
	{
		dma_free_coherent(&client->dev, 8192, gpDMABuf_va, (dma_addr_t)gpDMABuf_pa);
		gpDMABuf_va = NULL;
		gpDMABuf_pa = NULL;
		return -ENODEV;
	}
#else
    TS_LOG_INFO("enter [%s], do not use i2c dma\n",__func__);
#endif
	return 0;
}
static void rawdata_timeout_proc_fn(struct work_struct *work)
...
int ts_i2c_write(u8* addr, u8 addr_len,u8 *buffer, s32 len,u8 toRetry);
int ts_spi_write(u8* buf, u16 length);
int ts_i2c_read(u8* reg_addr, u16 reg_len, u8* buf, u16 len,u8 toRetry);
...
int ts_i2c_write(u8* addr, u8 addr_len,u8 *buffer, s32 len,u8 toRetry)
{
	...
	struct i2c_msg msg[]={
		{
		.addr = (g_ts_kit_platform_data.client->addr & I2C_MASK_FLAG),
		.ext_flag = (g_ts_kit_platform_data.client->ext_flag | I2C_ENEXT_FLAG | I2C_DMA_FLAG),
		.flags = 0,
		.buf = gpDMABuf_pa,
		.len = len+addr_len,
	...
	if(!pWriteData)
	{
		TS_LOG_ERR("dma_alloc_coherent failed!\n");//E
		mutex_unlock(&dma_mutex);
		return -1;
	}
	if(1 == addr_len)
	{
	  gpDMABuf_va[0] = addr[0];
	  memcpy(gpDMABuf_va+1, buffer, len);
	}
	else if(2 == addr_len)
	{
	  gpDMABuf_va[0] = addr[0];
	  gpDMABuf_va[1]	 = addr[1];
	  memcpy(gpDMABuf_va+2, buffer, len);
	}
	else
	{
	  TS_LOG_ERR("invalid address");
	  mutex_unlock(&dma_mutex);
	  return -1;
	}
	for (retry = 0; retry < toRetry; ++retry)
	{
		rc = i2c_transfer(g_ts_kit_platform_data.client->adapter, &msg[0], 1);
		if (rc < 0)
		{
			continue;
		}
...
{
    int rc=0,retry=0;
    u8 reg_addr[512] = {0};//512 for goodix send cfg & fw upgrade.

    struct i2c_msg msg[] = {
        {
            .addr = (g_ts_kit_platform_data.client->addr),
            .flags = 0,
            .buf = reg_addr,
            .len = len+addr_len,
        },
    ...
    /*Add for KASAN memory test */
    if((len+addr_len) >= sizeof(reg_addr)) 
    {
        TS_LOG_ERR("[%s]:invalid buffer size for write i2c data.\n",__func__);
        return -EINVAL;
    }

    if(1 == addr_len)//for u8 register address.
    {
        reg_addr[0] = addr[0];
        if(buffer != NULL && len != 0) {
            memcpy(reg_addr+1, buffer, len);
        }else {
            /* focal_write_default enter this branch */
            TS_LOG_DEBUG("invalid register value and length,do nothing!\n");
        }
    }
    else if(2 == addr_len)/* for u16 register address. */
    {
        reg_addr[0] = addr[0];
        reg_addr[1] = addr[1];
        memcpy(reg_addr+2, buffer, len);
    }
    else
    {
        TS_LOG_ERR("invalid address");
    ...
    for (retry = 0; retry < toRetry; ++retry)
    {
        ...
        if (rc < 0)
        {
            TS_LOG_ERR("i2c_transfer(write) error, ret=%d!!\n", rc);
            continue;
        }
        return 0;
    }
...
int ts_i2c_read(u8* reg_addr, u16 reg_len, u8* buf, u16 len,u8 toRetry)
{
	...
	mutex_unlock(&dma_mutex);
		...
		return -EIO;
            ...
            if(false == g_ts_kit_platform_data.chip_data->esd_checking) {
		if (!dsm_client_ocuppy(ts_dclient)) {
        dsm_client_record(ts_dclient, "I2C_status:%d.\n",g_ts_kit_platform_data.dsm_info.constraints_I2C_status);
		dsm_client_notify(ts_dclient, DSM_TP_I2C_RW_ERROR_NO);
	      }
...
int ts_i2c_read(u8* reg_addr, u16 reg_len, u8* buf, u16 len,u8 toRetry)
{
    ...
    for (retry = 0; retry < toRetry; ++retry)
    {
    	...
    	if (ret < 0)
    	{
            TS_LOG_ERR("i2c_transfer(read) error, ret=%d!!\n", ret);
            continue;
    	}
...
extern int charger_detect_register_client(struct notifier_block *nb);
extern int charger_detect_unregister_client(struct notifier_block *nb);
...
static void ts_charger_detect_register_client(void)
{
	int error;
	enum ts_charger_state charger_state = USB_PIUG_IN;
	g_ts_kit_platform_data.charger_detect_notify.notifier_call = charger_detect_notifier_callback;
	error = charger_detect_register_client(&g_ts_kit_platform_data.charger_detect_notify);
	if (error < 0) {
		...
		if(TS_CHARGER_TYPE_NONE == upmu_is_chr_det()){
			charger_state = USB_PIUG_OUT;
		}
		ts_charger_detect_cmd(charger_state);
	}
        ...
        case FB_BLANK_UNBLANK:
            ...
            switch (event)
            {
                ...
                case FB_EVENT_BLANK:
                    ...
                    {
                        ts_state = atomic_read(&g_ts_kit_platform_data.state);
                        if ((TS_SLEEP == ts_state) || (TS_WORK_IN_SLEEP == ts_state) || (TS_RESUMING == ts_state))
                        {
        ...
        default:
            ...
            switch (event)
            {
                case FB_EARLY_EVENT_BLANK:
                    ...
                    {
                        ts_state = atomic_read(&g_ts_kit_platform_data.state);
                        if ((TS_WORK == ts_state) || (TS_WORK_IN_SLEEP == ts_state) || (TS_RESUMING == ts_state))
                        {
                        ...
                        else if(TS_SLEEP == ts_state)
                        {
                            TS_LOG_INFO("ts has already in sleep status while suspend device in early blank,do nothing\n");
                            break;
                        }
                        else
                        {
                ...
                case FB_EVENT_BLANK:
                    while (1)
                    {
                        ...
                        {
                            TS_LOG_INFO("ts has already in sleep status,do nothing\n");
                            break;
                        }
                        else
                        {
...
#endif
void hw_ts_power_switch(s32 state)
{
	int ret;
	...
	switch(state){
	case SWITCH_ON:
			ret = regulator_set_voltage(g_ts_kit_platform_data.vdd, 2800000, 2800000);	/*set 2.8v*/
			if (ret)
				TS_LOG_ERR("regulator_set_voltage() failed!\n");
			ret = regulator_enable(g_ts_kit_platform_data.vdd);	/*enable regulator*/
			if (ret) {
				TS_LOG_ERR("regulator_enable() failed!\n");
			}
			else {
				TS_LOG_INFO("regulator_enable() success!\n");
			}
			break;
	case SWITCH_OFF:
			ret = regulator_disable(g_ts_kit_platform_data.vdd);	/*disable regulator*/
			if (ret) {
				TS_LOG_ERR("regulator_disable() failed!\n");
			}
			else {
				TS_LOG_INFO("regulator_disable() success!\n");
			}
			break;
	default:
			TS_LOG_ERR("Invalid power switch command!");
			break;
	}
}
 ...
 void tpd_gpio_output(int pin, int level)
{
	...
	TS_LOG_INFO("[tpd]tpd_gpio_output pin = %d, level = %d\n", pin, level);
	if (pin == 1) {
		if (level)
			pinctrl_select_state(g_ts_kit_platform_data.pinctrl, g_ts_kit_platform_data.pinctrl_state_int_high);
		else
			pinctrl_select_state(g_ts_kit_platform_data.pinctrl, g_ts_kit_platform_data.pinctrl_state_int_low);
	} else {
		if (level)
			pinctrl_select_state(g_ts_kit_platform_data.pinctrl, g_ts_kit_platform_data.pinctrl_state_active);
		else
			pinctrl_select_state(g_ts_kit_platform_data.pinctrl, g_ts_kit_platform_data.pinctrl_state_release);
	}
...
{
	mutex_lock(&tpd_set_gpio_mutex);
	TS_LOG_INFO("[tpd]tpd_gpio_as_int\n");
	if (pin == 1)
		pinctrl_select_state(g_ts_kit_platform_data.pinctrl, g_ts_kit_platform_data.pinctrl_state_as_int);
	mutex_unlock(&tpd_set_gpio_mutex);
}
static int get_ts_board_info(void)
{
    ...
    g_ts_kit_platform_data.node = of_find_matching_node(g_ts_kit_platform_data.node, touch_of_match1);
    ...
    rc = of_property_read_u32(g_ts_kit_platform_data.node, "charger_supported", &read_val);
    if (rc)
    {
		TS_LOG_ERR("charger_supported  read failed\n");
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.feature_info.charger_info.charger_supported = read_val;
    g_ts_kit_platform_data.pinctrl = devm_pinctrl_get(&(g_ts_kit_platform_data.ts_dev->dev));
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.pinctrl))
    {
		TS_LOG_ERR("Target does not use pinctrl %d\n");
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.pinctrl_state_active= pinctrl_lookup_state(g_ts_kit_platform_data.pinctrl,PINCTRL_STATE_ACTIVE);
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.pinctrl_state_active))
    {
		TS_LOG_ERR("Can not lookup %s pinstate \n",PINCTRL_STATE_ACTIVE);
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.pinctrl_state_release= pinctrl_lookup_state(g_ts_kit_platform_data.pinctrl,PINCTRL_STATE_RELEASE);
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.pinctrl_state_release))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n",PINCTRL_STATE_RELEASE);
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.pinctrl_state_as_int= pinctrl_lookup_state(g_ts_kit_platform_data.pinctrl,PINCTRL_STATE_AS_INT);
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.pinctrl_state_as_int))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n",PINCTRL_STATE_AS_INT);
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.pinctrl_state_int_high= pinctrl_lookup_state(g_ts_kit_platform_data.pinctrl,PINCTRL_STATE_INT_HIGH);
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.pinctrl_state_int_high))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n",PINCTRL_STATE_RELEASE);
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.pinctrl_state_int_low= pinctrl_lookup_state(g_ts_kit_platform_data.pinctrl,PINCTRL_STATE_INT_LOW);
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.pinctrl_state_int_low))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n", PINCTRL_STATE_RELEASE);
		error = -EINVAL;
		goto out;
    }
    g_ts_kit_platform_data.vdd = regulator_get(&(g_ts_kit_platform_data.ts_dev->dev),"vtouch");
    if (IS_ERR_OR_NULL(g_ts_kit_platform_data.vdd))
    {
	  TS_LOG_ERR("-Can not get vdd\n");
	  error = -EINVAL;
	  goto out;
	}
out:
...
#define HW_TS_DRIVER_NAME  "hw-ts"
#define HW_TS_I2C_ADDRESS 0xBA
static const struct i2c_device_id tpd_i2c_id[] = { {HW_TS_DRIVER_NAME, 0}, {} };
static unsigned short force[] = { 0, HW_TS_I2C_ADDRESS, I2C_CLIENT_END, I2C_CLIENT_END };
...
static struct i2c_driver tpd_i2c_driver = {
	...
	.remove = NULL,
	.detect = NULL,
	.driver.name = HW_TS_DRIVER_NAME,
	.driver = {
		   .name = HW_TS_DRIVER_NAME,
...
static int ts_creat_i2c_client(void)
{
 ...
 {
		TS_LOG_ERR("i2c_add_driver failed\n");
		return -1;
 }
#if defined (CONFIG_HUAWEI_DSM)
	if (!ts_dclient) {
...
static int ts_kit_parse_config(void)
{
    ...
    rc = of_property_read_u32(g_ts_kit_platform_data.node, "algo_id",
						&g_ts_kit_platform_data.chip_data->algo_id);
	if (rc) {
		TS_LOG_ERR("Get algo id failed");
		error = -EINVAL;
		goto out;
	}
	/*rc = of_property_read_u32(g_ts_kit_platform_data.node, "fw_upgrade_delay",
						&g_ts_kit_platform_data.chip_data->fw_upgrade_delay);
	if (rc) {
		TS_LOG_ERR("Get fw_upgrade_delay failed");
		error = -EINVAL;
		goto out;
	}*/
    ...
    TS_LOG_INFO("parse product name :%s\n", g_ts_kit_platform_data.product_name);
       ...
       rc = of_property_read_u32(g_ts_kit_platform_data.node, "x_max",
						&g_ts_kit_platform_data.chip_data->x_max);
	if (rc) {
		TS_LOG_ERR("Get x_max failed");
		error = -EINVAL;
		goto out;
	}
	rc = of_property_read_u32(g_ts_kit_platform_data.node, "y_max",
						&g_ts_kit_platform_data.chip_data->y_max);
	if (rc) {
		TS_LOG_ERR("Get y_max failed");
		error = -EINVAL;
		goto out;
	}
out:
...
static int ts_kit_create_sysfs(void)
{
    ...
    error = sysfs_create_link( &platform_bus.kobj, &g_ts_kit_platform_data.ts_dev->dev.kobj,"huawei_touch");
    if (error)
    {
        TS_LOG_ERR("%s: Fail create link huawei_touch error = %d\n", __func__, error);
        goto err_free_sysfs;
    }
    goto err_out;
...
#if defined(TS_KIT_PROXIMITY_SUPPORT)
			if (error == 0) {
				error = ts_kit_proximity_init();
			}
#endif
		}
	}
...
static int ts_kit_irq_init(void)
{
    ...
    if (error)
    {
        TS_LOG_ERR("debounce read failed\n");
        error = -EINVAL;
        goto err_out;
    }
    ...
    TS_LOG_INFO("ts_kit_irq_init success irq_id=%d\n",g_ts_kit_platform_data.irq_id);
err_out:
    return error;
}
...
int ts_kit_put_one_cmd_direct_sync(struct ts_cmd_node *cmd, int timeout)
{
	...
	if((atomic_read(&g_ts_kit_platform_data.state) == TS_SLEEP)
	     || (atomic_read(&g_ts_kit_platform_data.state) == TS_WORK_IN_SLEEP)
            || (atomic_read(&g_ts_kit_platform_data.state) == TS_RESUMING)){
...
static int ts_kit_init(void)
{
    ...
    error = ts_kit_parse_config();//product id
    if (error)
    {
...
/*     if (not_get_special_tp_node )
	...
	schedule_delayed_work(&ts_firmware_update_work, msecs_to_jiffies(g_ts_kit_platform_data.chip_data->fw_upgrade_delay));
...
#if defined(HUAWEI_CHARGER_FB)
//      ts_charger_detect_register_client();    //del by zhangle 20171121
#endif
...
//		charger_detect_unregister_client(&g_ts_kit_platform_data. charger_detect_notify);//del by zhangle 20171121
...
static int tpd_probe(struct platform_device *pdev)
{
   ...
   g_ts_kit_platform_data.ts_dev = pdev;//platform_device_alloc("huawei_touch", -1);
   ...
   error = get_ts_board_info();
    ...
    g_ts_kit_platform_data.irq_gpio = 1;
    ts_init_flag = 1;
...
int huawei_ts_module_init(void)
{
    TS_LOG_INFO("Huawei touch panel driver init\n");
    if (platform_driver_register(&tpd_driver) != 0) {
        TS_LOG_ERR("unable to register touch panel driver.\n");
        return -ENODEV;
    }
    return 0;
}
/* should never be called */
void  huawei_ts_module_exit(void)
{
	TS_LOG_INFO("Huawei touch panel driver exit\n");
