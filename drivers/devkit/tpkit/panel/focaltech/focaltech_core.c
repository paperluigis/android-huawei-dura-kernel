...
Filename      : focaltech_core.c
Author        : wWX231245
Creation time : 2016/7/13
Description   :
...
static char fts_version[]="ft5436 ver 1.0";
char g_fts_vendor_name[50]="truly-ft5436";
DEV_ATTR_DECLARE(touch_screen)
DEV_ATTR_DEFINE("version",fts_version)
DEV_ATTR_DEFINE("vendor",g_fts_vendor_name)
DEV_ATTR_DECLARE_END;
ONTIM_DEBUG_DECLARE_AND_INIT(touch_screen,touch_screen,8);

bool focal_suspend_flag = false;
...
#define NULL 0
#endif
#define TOUCH_IRQ_WAIT_RESET_TIMEOUT    3 /* 3 is for 3*100 ms */
...
#ifdef FTS_PROXIMITY_DETECTION_EN
int focal_proximity_enable(int mode);
int focal_ps_enable_nodata(int en, struct ts_proximity_info *info);
#endif
...
#ifdef FTS_PROXIMITY_DETECTION_EN
	.chip_proximity_enable = focal_ps_enable_nodata,
#endif
};
...
int focal_proximity_enable(int mode)
{
    ...
    while (i < 5) {
        if (mode && focal_suspend_flag) {
            rc = 4;
            TS_LOG_INFO("fts: tp suspend not write proximity enable cmd\n");
        }

        if (rc == 0) {
            mdelay(10);   // delay 10ms
            rc = focal_write_reg(buf_addr, buf_value);
            if (rc < 0) {
                TS_LOG_ERR("fts: Write proximity register(0xB0) fail!");
            } else {
                if (mode) {
                    TS_LOG_INFO("fts:enable proximity detection success\n");
                } else {
                    TS_LOG_INFO("fts:disable proximity detection success\n");
                }
                break;
            }
        }
...
int focal_read(u8 *addrs, u16 addr_size, u8 *values, u16 values_size)
{
	...
	ret = bops->bus_read(addrs, addr_size, values, values_size,3);
...
int focal_hardware_reset(int model)
{
    ...
    /*if(true == g_focal_dev_data->fw_update_flag){
        TS_LOG_INFO("%s:fw is in updating process,do not reset:%d\n", __func__,g_focal_dev_data->fw_update_flag);
        return 0;
    }*/

    if(true == g_focal_dev_data->need_wd_check_status){
        focal_esdcheck_set_reset_chip_flag(true);
    }

    /* set reset flag for fw update process */
    g_focal_dev_data->chip_in_reset = true;

    disable_irq_nosync(g_focal_dev_data->ts_platform_data->irq_id);
    tpd_gpio_output(GTP_RST_PORT, 1);// reset pin high
    msleep(10);
    ...
    enable_irq(g_focal_dev_data->ts_platform_data->irq_id);

    if(true == g_focal_dev_data->need_wd_check_status){
        focal_esdcheck_set_reset_chip_flag(false);
    }

    g_focal_dev_data->chip_in_reset = false;
...
static int focal_change_i2c_hid2std(void)
{
    ...
    if (ret < 0)
    {
        TS_LOG_ERR("%s:write cmd failed, ret=%d.\n",__func__, ret);
        return ret;
    }
    ...
    if (ret < 0)
    {
        TS_LOG_ERR("%s:read cmd failed, ret=%d.\n",__func__, ret);
        return ret;
    }
...
static int focal_esdcheck_tp_reset( void )
{
	...
	if(ret){
		...
		if (!dsm_client_ocuppy(ts_dclient)) {
			TS_LOG_INFO("%s: try to client record DSM_TP_ESD_ERROR_NO(%d) hardware reset\n", __func__, DSM_TP_ESD_ERROR_NO);
			dsm_client_record(ts_dclient, "try to client record DSM_TP_ESD_ERROR_NO(%d): focal ESD hardware reset.\n",
						DSM_TP_ESD_ERROR_NO);
...
#if defined (CONFIG_HUAWEI_DSM)
		if (!dsm_client_ocuppy(ts_dclient)) {
			TS_LOG_INFO("%s: try to client record DSM_TP_ESD_ERROR_NO(%d) esdcheck chip id\n", __func__, DSM_TP_ESD_ERROR_NO);
			dsm_client_record(ts_dclient, "try to client record DSM_TP_ESD_ERROR_NO(%d): focal esdcheck chip id.\n",
						DSM_TP_ESD_ERROR_NO);
			dsm_client_notify(ts_dclient, DSM_TP_ESD_ERROR_NO);
		}
#endif
...
static int focal_esdcheck_algorithm(void)
{
	...
	g_focal_pdata->focal_device_data->esd_checking = true;

	/* 0. In chip reset, can't check esd */
	if (true == fts_esdcheck_data.reset){
		TS_LOG_INFO("%s:[ESD]: In chip reset state, not check esd, return immediately! \n", __func__);
        g_focal_pdata->focal_device_data->esd_checking = false;
		return 0;
	}
...
static int focal_esdcheck_func(void)
{
	...
	if(true == g_focal_pdata->focal_device_data->is_in_cell){
		TS_LOG_DEBUG("%s:esdcheck not support INCELL ic", __func__);
		return 0;
	}
...
int focal_esdcheck_set_reset_chip_flag(u8 reset_chip)
{
	fts_esdcheck_data.reset = (bool)reset_chip;
	return 0;
}
...
#ifdef FTS_PROXIMITY_DETECTION_EN
static int focal_proximity_readdata(struct ts_fingers *info)
{
    int ret = 0;
    u8  regvalue = 0;
    struct ts_proximity_info *ps_info = &g_focal_dev_data->ts_platform_data->feature_info.proximity_info;
    ps_info->proximity_status = 0xFF;

    if (ps_info->proximity_enable == 0) {
        TS_LOG_DEBUG("%s can not read proximity state while proximity_enable is disable.\n",__func__);
        return -EPERM;
    }

    ret = focal_read_reg(0x01, &regvalue);
    if (ret < 0)
    {
        TS_LOG_ERR("%s read proximity state (0x01) fail \n",__func__);
        return RESULT_ERR;
    }
    ...
    TS_LOG_DEBUG("ft5436 proximity status = %d\n", ps_info->proximity_status);
	...
	struct ts_cmd_node *out_cmd)
{
    ...
    /* Wait until chip reset is completed,maximum 300 ms */
	while(g_focal_dev_data->chip_in_reset == true)
	{
		msleep(100);
		irq_touch_wait++;
		if(irq_touch_wait >TOUCH_IRQ_WAIT_RESET_TIMEOUT){
			break;
		}
	}

#ifdef FTS_PROXIMITY_DETECTION_EN
	ret = focal_proximity_readdata(info);
	if (!ret) {
		TS_LOG_DEBUG("focal_proximity_readdata is called and report proximity\n");
		return ret;
	}
#endif
...
static void focal_sleep_mode_in(void)
{
	...
	msleep(10);
	/* set rst pin to low before turn off power */
	tpd_gpio_output(GTP_RST_PORT, 0);
	msleep(10);
	return;
}
...
int focal_power_init(void)
{
    int retval;

    /*set TP volt*/
    retval = regulator_set_voltage(g_focal_dev_data->ts_platform_data->vdd, 2800000, 2800000);
    if (retval != 0)
    {
    ...
    retval = regulator_enable(g_focal_dev_data->ts_platform_data->vdd);
    if (retval != 0)
    {
...
void focal_regulator_power_switch(bool flag)
{
    s32 nRetVal = 0;

    TS_LOG_INFO("*** %s(%d) ***\n", __func__,flag);

    if (g_focal_dev_data->ts_platform_data->vdd != NULL) {
        if (flag == true) {
            nRetVal = regulator_enable(g_focal_dev_data->ts_platform_data->vdd);
            if (nRetVal) {
                TS_LOG_INFO("regulator_enable failed. nRetVal=%d\n", nRetVal);
            }
            mdelay(20);
        } else {
            nRetVal = regulator_disable(g_focal_dev_data->ts_platform_data->vdd);
            if (nRetVal) {
...
static int focal_suspend(void)
{
    TS_LOG_INFO("focal_suspend \n");
	if(true == g_focal_dev_data->need_wd_check_status){
		fts_esdcheck_data.suspend = true;
	}

#ifdef FTS_PROXIMITY_DETECTION_EN
    if (g_focal_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable == 1) {
        TS_LOG_INFO("[%s]:proximity enable mode = %d\n", __func__,g_focal_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable);
        goto out;
    }
#endif
	...
	case TS_POWER_OFF_MODE:
		...
		focal_sleep_mode_in();
		/* tp power off */
		focal_regulator_power_switch(false);
		break;
	case TS_GESTURE_MODE:
		if(g_focal_dev_data->ts_platform_data->feature_info.wakeup_gesture_enable_info.switch_value == true){
...
static void focal_release_all_finger(void)
{
    TS_LOG_INFO("%s: called\n", __func__);

    if(g_focal_dev_data->ts_platform_data->input_dev) {
        input_mt_sync (g_focal_dev_data->ts_platform_data->input_dev);
        input_report_key(g_focal_dev_data->ts_platform_data->input_dev, BTN_TOUCH, 0);
        input_sync(g_focal_dev_data->ts_platform_data->input_dev);
        TS_LOG_DEBUG("%s: input_dev->name = %s\n", __func__,g_focal_dev_data->ts_platform_data->input_dev->name);
    }
    TS_LOG_INFO("%s: end\n", __func__);
}
...
{
    /* tp power on */
    focal_regulator_power_switch(true);
    focal_sleep_mode_out();
    focal_suspend_flag = false;

#ifdef FTS_PROXIMITY_DETECTION_EN
    if (g_focal_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable == 1) {
        TS_LOG_INFO("[%s]:resume proximity enable mode = %d restore enable proximity\n",__func__,
            g_focal_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable);
        focal_proximity_enable(1);
    }
#endif
...
static int focal_get_lcd_module_name(struct focal_platform_data *focal_pdata)
{
    int i = 0;
    int lcm_info_len = strlen(mtkfb_lcm_name);

    // get lcm panel name from lcd
    strncpy(focal_pdata->lcd_panel_info, mtkfb_lcm_name, sizeof(focal_pdata->lcd_panel_info));
    if (lcm_info_len > (LCD_PANEL_INFO_MAX_LEN-1)) {
        lcm_info_len = LCD_PANEL_INFO_MAX_LEN - 1;
    }
    focal_pdata->lcd_panel_info[lcm_info_len] = '\0';
    TS_LOG_INFO("%s:lcmname = %s\n", __func__, focal_pdata->lcd_panel_info);

    // parse lcm module name
    for (i=0; i<LCD_MODULE_INFO_MAX_LEN-1; i++)
    {
        if (focal_pdata->lcd_panel_info[i] == '_') {
            break;
        }
        focal_pdata->lcd_module_name[i] = tolower(focal_pdata->lcd_panel_info[i]);
    }
...
//		memset(focal_pdata->project_id,0,FTS_PROJECT_ID_LEN);
	...
	strncpy(g_focal_dev_data->module_name,focal_pdata->vendor_name,MAX_STR_LEN-1);
...
static int focal_init_chip(void)
{
	...
	ret = focal_get_lcd_module_name(focal_pdata);
	if (ret) {
		TS_LOG_ERR("%s:read lcd module name fail, ret=%d\n", __func__, ret);
	}
...
static int __init focal_core_module_init(void)
{
	...
	g_ts_kit_device_data = g_focal_dev_data;
