	...
	size_t size)
{
	...
	ret = of_property_read_u32(np, FTS_NEED_DISTINGUISH_LCD, &g_focal_pdata->need_distinguish_lcd);
	if (ret) {
		TS_LOG_ERR("%s: no need_distinguish_lcd read\n", __func__);
	} else {
		TS_LOG_INFO("%s: need_distinguish_lcd = %d \n", __func__, g_focal_pdata->need_distinguish_lcd);
	}
	return 0;
}
	...
	struct ts_kit_device_data *dev_data)
{
	...
	focal_of_property_read_u32_default(np, FTS_FW_UPGRADE_DELAY,
		&dev_data->ts_platform_data->chip_data->fw_upgrade_delay, 100);

	TS_LOG_INFO("%s:%s=%d, %s=%d, %s=%d.\n", __func__,
		"reset_delay", dev_data->reset_delay,
		"slave_addr", dev_data->slave_addr,
		"fw_upgrade_delay", dev_data->ts_platform_data->chip_data->fw_upgrade_delay);
	return 0;
}
	...
	struct focal_platform_data *focal_pdata)
{
	...
	if (!strcmp(focal_pdata->lcd_panel_info, LCD_TXD_FT8006U_INCELL)) {
		focal_pdata->focal_device_data->is_in_cell = true;
	}
	...
	if(glove_info->glove_supported){
		focal_of_property_read_u32_default(np, FTS_GLOVE_SWITCH_ADDR, &dts_val, 0);
		glove_info->glove_switch_addr = (u16)dts_val;
	}
	...
	if(holster_info->holster_supported){
		focal_of_property_read_u32_default(np, FTS_HOSTLER_SWITCH_ADDR, &dts_val, 0);
		holster_info->holster_switch_addr = (u16)dts_val;
	}
	...
	if(roi_info->roi_supported){
		focal_of_property_read_u32_default(np, FTS_ROI_SWITCH_ADDR, &dts_val, 0);
		roi_info->roi_control_addr = (u16)dts_val;
	}
	...
	TS_LOG_INFO("%s:glove_supported =%d  glove_switch_addr = 0x%04x,holster_supported = %d holster_switch_addr=0x%04x,\n",__func__,
