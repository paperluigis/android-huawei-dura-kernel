	...
	struct ts_rawdata_info *info)
{
	...
	if(FOCAL_FT8716 == g_focal_dev_data->ic_type || FOCAL_FT8006U == g_focal_dev_data->ic_type)
	{
		/* open test */
	...
	size_t data_size)
{
	...
	if(data_size <BYTES_PER_TIME ){
		...
		for (i = 0; i < pkg_count; i++) {
			...
			ret = focal_read(cmd, 1, data + data_size + readed_count, pkg_size);
			if (ret) {
				TS_LOG_ERR("%s:read cb data fail, ret=%d\n",
					__func__, ret);
				return ret;
...
int focal_get_channel_num(u8 *channel_x, u8 *channel_y)
{
	...
	if (chl_y <= 0 || chl_y > ((FOCAL_FT8006U == g_focal_dev_data->ic_type)
			? RX_NUM_MAX_FT8006U : RX_NUM_MAX)) {
...
static int focal_get_scap_raw_data_format_5x46(int raw_data_type,int *data, size_t size)
{
    ...
    if (!dsm_client_ocuppy(ts_dclient)) {
        TS_LOG_INFO ("try to client record 926004002 for rawdata \n");
        dsm_client_record(ts_dclient, "device status for 926004002 is :%d\n", ret);
	...
	struct focal_test_result **result,char test_num)
{
	...
	for (i = 0; i < raw_data_size-chl_x; i++) {
...
static int focal_get_open_test_data(int *data, size_t size,unsigned int chl_x,unsigned int chl_y)
{
	...
	} else if (judge_ic_type == FTS_8006U_VALUE) {
...
static int focal_get_short_circuit_data(int *data, size_t size,unsigned int chl_x,unsigned int chl_y)
{
	...
	if (FOCAL_FT8006U == g_focal_dev_data->ic_type) {
		for (i = 0; i < size; i++) {
			adc_data_tmp = 252000 / (data[i] - 120) - 60;
			data[i] = adc_data_tmp;
		}
	} else {
		for (i = 0; i < size; i++) {
