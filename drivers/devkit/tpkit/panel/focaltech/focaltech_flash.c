 ...
 *                            ALL RIGHTS RESERVED
 *
 * Filename      : focaltech_flash.c
 * Author        : wWX231245
 * Creation time : 2016/6/30
 * Description   :
 *
 * Version       : 1.0
 */
...
extern char g_fts_vendor_name[50];
...
#define FW_UPDATE_WAIT_RESET_TIMEOUT 50 /* 50 is for 5 seconds */
	...
	u32 start_addr)
{
	...
	TS_LOG_INFO("%s:Write firmware to ic\n", __func__);
	TS_LOG_INFO("%s:fw write:data_length=%u, package_count=%u\n",
	...
	for (i = 0; i < package_count; i++) {
        ...
        if (true == focal_pdata->focal_device_data->is_in_cell) {
            start_addr_in_ic = (FOCAL_FT8006U == g_focal_dev_data->ic_type)
                    ? FTS_FW_WRITE_STATUS_ADDR_START : start_addr;
        } else {
            start_addr_in_ic = FTS_FW_WRITE_STATUS_ADDR_START;
        }
        ret = focal_wait_firmware_write_finish(focal_pdata, command,
                start_addr_in_ic, start_write_addr, data_size);
		if (ret) {
	...
	u32 fw_length)
{
    ...
    start_addr_in_ic = (FOCAL_FT8006U == g_focal_dev_data->ic_type)
            ? FTS_FW_IC_ADDR_START_FT8006U : FTS_FW_IC_ADDR_START;
    return focal_write_firmware_data(focal_pdata, fw_data, fw_length,
            FTS_CMD_WRITE_FLASH, start_addr_in_ic);
}
	...
	u8 *check_sum)
{
    ...
    packet_num = crc_length / FTS_MAX_LEN_ECC_CALC;
    remainder = crc_length % FTS_MAX_LEN_ECC_CALC;
    if (remainder) {
        packet_num++;
    }
    ...
    /* start checksum calc addr */
    cmd[0] = FTS_CMD_SET_CALC_ADDR;
    for (i = 0; i < packet_num; i++) {
        addr = start_addr + FTS_MAX_LEN_ECC_CALC * i;
        ...
        TS_LOG_DEBUG("ecc calc start addr: 0x%04x, len: %d", addr, packet_len);
...
static int focal_check_firmware_size_in_pram_model(u32 fw_len)
{
	...
	default:
		...
		max_fw_len = (FOCAL_FT8006U == g_focal_dev_data->ic_type)
				? FTS_FLASH_MAX_LEN_WINBOND_FT8006U : FTS_FLASH_MAX_LEN_WINBOND;
	}
	...
	u32 fw_len)
{
	...
	if (ret) {
		...
		TS_LOG_INFO("%s:enter upgrade mode success\n", __func__);
	}
    ...
    if (!dsm_client_ocuppy(ts_dclient)) {
        TS_LOG_INFO("try to client record 926004003 \n");
        dsm_client_record(ts_dclient, "device status for 926004003 is :%d\n", g_focal_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status);
	...
	u32 fw_len)
{
    ...
    start_addr_in_ic =  (FOCAL_FT8006U == g_focal_dev_data->ic_type)
            ? FTS_FW_IC_ADDR_START_FT8006U : FTS_FW_IC_ADDR_START;
    ret = focal_read_check_sum(focal_pdata, start_addr_in_ic, fw_len, &check_sum_in_ic);
	if (ret) {
		TS_LOG_ERR("%s:read check sum in ic fail, ret=%d\n",
			__func__, ret);
#if defined (CONFIG_HUAWEI_DSM)
	...
	u8 *fw_buf)
{
	...
	set_fs(KERNEL_DS);
	pos = 0;
    nread = vfs_read(pfile, fw_buf, fsize, &pos);
    if (nread < 0)
    {
        TS_LOG_ERR("%s:error in file read \n",  __func__);
    }
	filp_close(pfile, NULL);
	set_fs(old_fs);
	...
	size_t size)
{
	...
	if (focal_pdata->need_distinguish_lcd) { //TP fw depend on LCD module
		...
		if(FTS_FW_NAME_LEN > strlen(fw_name) + strlen(focal_pdata->lcd_module_name)) {
	...
	const struct firmware *fw)
{
	...
	/* only update fw to higher version */
	if (fw_ver_in_file > fw_ver_in_ic) {
		TS_LOG_INFO("%s:%s, update firmware\n",
			__func__, "firmware version is higher");
		return 1;
	} else {
	...
	const char *product_name)
{
	...
	/* Wait until chip reset is completed,maximum 5 seconds */
	while(g_focal_dev_data->chip_in_reset == true)
	{
		msleep(100);
		fw_count++;
		if(fw_count >FW_UPDATE_WAIT_RESET_TIMEOUT){
			break;
		}
	}
	...
	g_focal_dev_data->fw_update_flag = true;    /* set fw update flag,do not reset chip */
	...
	if (ret) {
		...
		g_focal_dev_data->fw_update_flag = false;
		return ret;
	}
...
release_fw:
    ...
    focal_read_reg(FTS_REG_FW_VER, &fw_ver);
    sprintf(g_fts_vendor_name,"%s,ft5436,fw=0x%x\n",g_focal_pdata->vendor_name,fw_ver);
...
/*
 * name: focal_read_tp_logo
 *
 * param: tp_logo - buffer to receive tp_logo
 *
 * param: size       - size of tp_logo
 *
 * return: if read tp logo success, return 0, else return error code
 *
 * notice: this function may made ic reset
 */
int focal_check_tp_logo(
	struct focal_platform_data *focal_pdata,
	char *tp_logo,
	size_t size)
{
	int ret = 0;
	int is_read_tp_logo_success = -EINVAL;
	...
	for (i = 0; i < focal_pdata->delay_time->upgrade_loop_times; i++) {
		...
		if (ret) {
			TS_LOG_ERR("%s:%s, ret=%d, retry=%d\n", __func__,
				"read tp logo fail", ret, i);
			msleep(FTS_SLEEP_TIME_5);
			continue;
		}
		if (0 == reg_val[0]) {
			TS_LOG_INFO("%s:read tp logo fail, retry=%d\n",
				__func__, i);
			msleep(FTS_SLEEP_TIME_5);
			continue;
		} else {
			reg_val[FTS_TP_COLOR_LEN-1] = '\0';
			tp_logo[0] = (u8)str_to_hex(reg_val);
			is_read_tp_logo_success = 0;
			TS_LOG_INFO("%s:read tp logo , retry=%d,tp_logo is 0x%x\n",
				__func__, i,tp_logo[0]);
			break;
		}
	}

	/* check tp logo */
	if (is_read_tp_logo_success == 0){
		TS_LOG_INFO("%s:read tp logo success\n", __func__);
		switch(tp_logo[0]) {
		case LOGO_HUAWEI:
		...
		default:
			...
			TS_LOG_ERR("%s: TP logo can not detected\n");
			break;
		}
		...
		TS_LOG_ERR("%s:read tp logo fail\n", __func__);
	}

	return is_read_tp_logo_success;
}
...
//	cmd[3] = FTS_BOOT_PROJ_CODE_ADDR2;
    ...
    memset(g_focal_dev_data->ts_logo_info , 0 , sizeof(g_focal_dev_data->ts_logo_info));
    ret = focal_check_tp_logo(focal_pdata,g_focal_dev_data->ts_logo_info, FTS_TP_COLOR_LEN - 1);
    if (ret) {
