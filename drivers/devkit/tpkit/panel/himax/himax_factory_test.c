...
#ifdef HMX852xE
extern uint8_t *mutual_data;
extern uint8_t *self_data;
...
int himax_bank_test(int step) //for Rawdara
{
	...
	else
	{
		...
		#ifdef CONFIG_HUAWEI_DSM
			hmx_tp_report_dsm_err(DSM_TP_RAWDATA_ERROR_NO , result);
		#endif
...
void himax_print_rawdata(int mode)
{
	...
	{
		info_test->buff[current_index++] = mutual_buff[index1];
	}
	/*for(index1=0;index1<self_num;index1++)
	{
		info_test->buff[current_index++] = self_buff[index1];
	}*/
}
...
void himax_get_rawdata(void)
{
	...
	/* start polling mode  */
	g_himax_ts_data->himax_wq = create_singlethread_workqueue("himax_touch");
	INIT_WORK(&g_himax_ts_data->work,himax_get_rawdata_work_func);
	hrtimer_init(&g_himax_ts_data->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	g_himax_ts_data->timer.function = himax_ts_timer_func;
	hrtimer_start(&g_himax_ts_data->timer, ktime_set(1/8, 0), HRTIMER_MODE_REL);
	TS_LOG_INFO("%s: polling mode enabled\n", __func__);
	//=============================== Get Bank value Start===============================
	diag_command = 0x03;
	command_F1h_bank[0] = 0xF1;
	command_F1h_bank[1] = 0x03;
	//i2c_himax_write(command_F1h_bank[0] ,&command_F1h_bank[1], 1, sizeof(command_F1h_bank),DEFAULT_RETRY_CNT);
	i2c_himax_master_write(&command_F1h_bank[0],2, sizeof(command_F1h_bank),DEFAULT_RETRY_CNT);
	msleep(1000);
	...
	/* get bank from polling(ts_work) */
	memcpy(mutual_dc,&mutual_data[0],mutual_num);
	memcpy(self_dc,&self_data[0],self_num);
	...
	msleep(10);
	//=============================== Get IIR value End===============================
	hrtimer_cancel(&g_himax_ts_data->timer);
	cancel_work_sync(&g_himax_ts_data->work);
}

uint8_t Self_Test_Bank(uint8_t *RB1H)
{
	uint8_t i;
	uint32_t bank_sum, m;
	uint8_t bank_avg;
	...
	uint8_t set_bnk_ulmt = 255;// = hx_criteria[bank_mutual_up];
	uint8_t set_bnk_dlmt = 0;// = hx_criteria[bank_mutual_down];
	uint8_t set_avg_bnk_ulmt = 255; // hx_criteria[]; need to add
	uint8_t set_avg_bnk_dlmt = 0; // hx_criteria[]; need to add
	uint8_t set_slf_bnk_ulmt = 255;// = hx_criteria[bank_self_up];
	uint8_t set_slf_bnk_dlmt = 0;// = hx_criteria[bank_self_down];
...
int himax_factory_start(struct himax_ts_data *ts,struct ts_rawdata_info *info_top)
{
	...
	//enable_irq(g_himax_ts_data->tskit_himax_data->ts_platform_data->irq_id);
	himax_HW_reset(HX_LOADCONFIG_EN,HX_INT_DISABLE);
	...
	msleep(120);
	/* step0: himax self test*/
	//hx_result_status[test0] = himax_self_test();//Self_Test_Bank(&RB1H[0]);//himax_self_test(); //0 :iq and self test pass  1: iq fail  or self test fail
	himax_get_rawdata();
	/* step1: cap rawdata */
	hx_result_status[test0] = himax_bank_test(1); //for Rawdata
	himax_fac_dump(fac_dump_step,mutual_num,self_num,mutual_bank,self_bank);
	fac_dump_step += 2;

	/* step2: TX RX Delta */
	hx_result_status[test1] = himax_self_delta_test(2); //for TRX delta
	fac_dump_step += 2;
	himax_fac_dump(fac_dump_step,mutual_num,self_num,mutual_dc,self_dc);
	fac_dump_step += 2;
	/* step3: Noise Delta */
	hx_result_status[test2] = himax_iir_test(3); //for Noise Delta
	himax_fac_dump(fac_dump_step,mutual_num,self_num,mutual_iir,self_iir);
...
static int himax_parse_threshold_file(void)
{
	...
	retval = request_firmware(&fw_entry, "CHOPIN_auo_threshold.csv", cd->dev);
	if (retval < 0) {
