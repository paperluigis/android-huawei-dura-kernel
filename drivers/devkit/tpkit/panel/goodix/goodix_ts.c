...
static char g_gtp_vendor[50]="shenyue-gt917d";
...
bool goodix_suspend_flag = false;
...
static inline int goodix_init_watchdog(void)
{
	...
	return goodix_i2c_write(0x8041, &value[0], 1);
}
...
int goodix_sleep_mode_out(void)
{
    ...
    goodix_suspend_flag = false;

#ifdef GTP_PROXIMITY_DETECTION_EN
    if (g_goodix_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable == 1) {
        TS_LOG_INFO("[%s]:resume proximity enable mode = %d restore enable proximity\n",__func__,
            g_goodix_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable);
        goodix_proximity_enable(1);
    }
#endif
    //enable_irq(ts->ts_platform_data->irq_id);
				...
				struct ts_fingers *info)
{
...
#ifdef GTP_PROXIMITY_DETECTION_EN
    u8 proximity_value = touch_data[0];
    struct ts_proximity_info *ps_info = &goodix_ts->goodix_device_data->ts_platform_data->feature_info.proximity_info;
    ps_info->proximity_status = 0xFF;

    if (ps_info->proximity_enable == 1) {
        TS_LOG_DEBUG("%s:REG INDEX[0x814E]:0x%02X\n", __func__,proximity_value);
        /* proximity or large touch detect,enable hwm_sensor. */
        if (proximity_value & 0x60) {
            /* close. need lcd off */
    ...
    TS_LOG_DEBUG("gt917D proximity status = %d\n", ps_info->proximity_status);
#endif
...
int goodix_read_version(struct goodix_hw_info * hw_info)
{
	...
	{
		strncpy(goodix_ts->hw_info.vendor_name, "Helitec", sizeof(goodix_ts->hw_info.vendor_name));
	}
	else
	{
		strncpy(goodix_ts->hw_info.vendor_name, "Unknow", sizeof(goodix_ts->hw_info.vendor_name));
	}
...
s32 gt1x_init_panel(void)
{
	s32 ret = 0;
	u8 cfg_len = 0,i=0;
	int tpd_resolution[2] = {720,1440};
	int  GTP_INT_TRIGGER = 0;
...
static int goodix_read_tp_color(void)
{
    int ret = 0;
    u8 tp_color_reg_value[3] = {0};

    ret = goodix_i2c_read(0x81AA, tp_color_reg_value, 2);
    if(ret<0)
    {
    	TS_LOG_ERR("Read goodix tp color reg error\n");
    }

    tp_color_reg_value[GTP_TP_COLOR_LEN-1] = '\0';
    cypress_ts_kit_color[0] = (u8)str_to_hex(tp_color_reg_value);

    TS_LOG_INFO("%s:read tp_color is 0x%x\n",__func__, cypress_ts_kit_color[0]);

    return ret;
}

static int goodix_check_tp_logo(void)
{
    int ret = 0;
    u8 tp_logo_reg_value = 0;
    u8 tp_logo_str[16] = {0};

    ret = goodix_i2c_read(0x81AC, &tp_logo_reg_value, 1);
    if(ret<0)
    {
        TS_LOG_ERR("Read goodix tp logo reg error\n");
        return ret;
    }

    g_goodix_dev_data->ts_logo_info[0] = (u8)tp_logo_reg_value;
    TS_LOG_INFO("%s:read tp_logo is 0x%x\n",__func__, g_goodix_dev_data->ts_logo_info[0]);

    /* check tp logo */
    switch(tp_logo_reg_value) {
    case LOGO_HUAWEI:
        snprintf(tp_logo_str, sizeof("HUAWEI"), "%s", "HUAWEI");
        break;
    case LOGO_HONOR:
        snprintf(tp_logo_str, sizeof("HONOR"), "%s", "HONOR");
        break;
    /* extern other logo info... */
    default:
        snprintf(tp_logo_str, sizeof("NOT_DEFINED"), "%s", "NOT_DEFINED");
        TS_LOG_ERR("%s: TP logo is not defined! \n");
        break;
    }

    TS_LOG_INFO("%s:read tp_logo_str is %s,current sw brand is %s\n",__func__, tp_logo_str,SW_BRAND);

    if(strcmp(tp_logo_str,"HUAWEI") == 0 && strcmp(SW_BRAND, "HONOR") == 0 ||
        strcmp(tp_logo_str,"HONOR") == 0 && strcmp(SW_BRAND, "HUAWEI") == 0) {
        TS_LOG_INFO("%s:TP logo and sw Brand mismatched!\n", __func__);
        arch_reset(0, "brand_mismatch");
    }
    else {
            TS_LOG_INFO("%s:TP logo and sw Brand is matched! \n", __func__);
    }
...
static int goodix_chip_init(void)
{
	...
	if(goodix_ts->support_get_tp_color) {
		ret = goodix_read_tp_color();
		if (ret) {
			TS_LOG_ERR("%s:read tp color fail, ret=%d\n", __func__, ret);
		}
	}

    ret = goodix_check_tp_logo();
    if (ret) {
        TS_LOG_ERR("%s:read tp logo fail, ret=%d\n", __func__, ret);
    }
...
static int goodix_chip_suspend(void)
{
    ...
    TS_LOG_INFO("goodix_suspend \n");

#ifdef GTP_PROXIMITY_DETECTION_EN
    if (g_goodix_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable == 1) {
        TS_LOG_INFO("[%s]:proximity enable mode = %d\n", __func__,g_goodix_dev_data->ts_platform_data->feature_info.proximity_info.proximity_enable);
        goto out;
    }
#endif
...
static int goodix_fw_update_boot(char *file_name)
{
	...
	sprintf(goodix_ts->firmware_name,"%s",AUTO_UPGRADE_FW_NAME);
...
static int goodix_fw_update_sd(void)
{
	...
	update_from_sd = true;
	memset(goodix_ts->firmware_name,0,64);
	sprintf(goodix_ts->firmware_name,"%s", MANUAL_UPGRADE_FW_NAME);
	ret = gup_update_proc(UPDATE_TYPE_HEADER);
	if (ret != SUCCESS) {
                 TS_LOG_ERR("%s:firmware update fail, ret=%d\n", __func__, ret);
#if defined (CONFIG_HUAWEI_DSM)
...
static int goodix_chip_get_info(struct ts_chip_info_param *info)
{
	...
	goodix_i2c_read(GTP_REG_CONFIG_DATA,&goodix_ts->hw_info.config_ver, 1);
	...
	goodix_i2c_read(GTP_REG_SENSOR_ID, &goodix_ts->hw_info.sensor_id, 1);
	sprintf(info->mod_vendor, "SensorID:%X",goodix_ts->hw_info.sensor_id);
...
static int goodix_esdcheck_tp_reset(void)
{
    ...
    ret = gt1x_send_cfg(gt1x_config, GTP_CONFIG_ORG_LENGTH);
	if(ret < 0){
...
#ifdef GTP_PROXIMITY_DETECTION_EN
int goodix_proximity_enable(int mode)
{
    u8	state;
    int rc = 0;
    u8 i = 0;
    ...
    if (mode) {
        state = 1;
        TS_LOG_INFO("[%s]:TPD proximity function to be on.",__func__);
    } else {
        state = 0;
        TS_LOG_INFO("[%s]:TPD proximity function to be off.",__func__);
    }

    while (i < 5) {
        if (mode && goodix_suspend_flag) {
            rc = 4;
        ...
        if (rc == 0) {
            ...
            rc = goodix_i2c_write(GTP_PROXIMITY_ENABLE_REG, &state, 1);
            if (rc < 0) {
                TS_LOG_ERR("goodix: Write proximity register(0x8042) fail!");
            } else {
                if (mode) {
                    TS_LOG_INFO("goodix:enable proximity detection success\n");
                } else {
                    TS_LOG_INFO("goodix:disable proximity detection success\n");
                }
                break;
            }
        }
    ...
    TS_LOG_INFO("[%s]:TPD proximity function %s success.", state ? "enable" : "disable",__func__);
    return rc;
}

// only enabled but not report inputEvent to HAL
int goodix_ps_enable_nodata(int en, struct ts_proximity_info *info)
{
    int ret = 0;

    ret = goodix_proximity_enable(en);
    if (ret == 0) {
...
#ifdef GTP_PROXIMITY_DETECTION_EN
	.chip_proximity_enable = goodix_ps_enable_nodata,
#endif
};
...
static int __init goodix_ts_module_init(void)
{
	...
	tpd_load_status = 1;
	g_ts_kit_device_data = g_goodix_dev_data;
