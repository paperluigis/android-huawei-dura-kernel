...
#include <asm/div64.h>   //add by zhangle for 64bit div.
...
static int gt9xx_get_testlimits_from_ini(struct goodix_ts_test *ts_test)
{
	...
	ts_test->test_params.test_types = test_types[0];
	retval = request_firmware(&tptest_limit, "ts/dura_goodix_rawdata_test.ini",&ts_test->ts->pdev->dev);
	if (retval < 0) {
...
static inline int goodix_tptest_finish(struct goodix_ts_test *ts_test)
{
    ...
    if(ret < 0){
        GTP_INFO("%s: goodix_chip_reset  fail\n",__func__);
    }

    ret = gt1x_init_panel();
    if (ret != 0) {
        GTP_ERROR("%s: failed to send normal config:%d\n", __func__, ret);
        return ret;
    } else {
        GTP_INFO("%s: send normal cfg success\n", __func__);
    }
				...
				struct ts_test_params *test_params)
{
	...
	for (i = 0; i < sc_data_num; i++) {
		...
		max_val /= rawdata_val;
		TS_LOG_INFO("deltacapacitance max_val=%d",max_val);
				...
				u16 short_code, u8 pos)
{
	...
	{
#if 0//mod by zhangle zwx450315 for 64bit div.
		r = (long long)(52662850)*10 / (short_code & (~0x8000)) - 40 * FLOAT_AMPLIFIER * 10;	//52662.85/code-40
#else
		a = (long long)(52662850)*10;
		b = (short_code & (~0x8000)) - 40 * FLOAT_AMPLIFIER * 10;
		mod = do_div(a,b);
		r = a;
#endif
	}
...
#else
		a = (long long)40 * 9 * 1024 * (avdd_value - 9) * FLOAT_AMPLIFIER;
		b = ((short_code & (~0x8000)) * 7) - 40 * FLOAT_AMPLIFIER * 10;
		mod = do_div(a,b);
		r = a;
#endif
	}
...
static int goodix_shortcircut_analysis(struct goodix_ts_test *ts_test)
{
	...
	length = (max_drvs + max_sens) * 2;
	ret = ts_test->ts->ops.i2c_read(0xA4A1, data_buf, length);
	if (ret < 0)
		goto exit_kfree;
...
exit_finish:
	strncpy(info->result, gts_test->test_result, sizeof(info->result));
