...
*********************************************************/
s32 gup_i2c_write(u8 *buffer,s32 len)
{
	struct ts_bus_info *bops = g_goodix_dev_data->ts_platform_data->bops;
...
static u8 gup_get_ic_msg(u16 addr, u8* msg, s32 len)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (goodix_i2c_read(addr, msg, len) == 0)
        {
            GTP_INFO("gup_get_ic_msg-->0 Read data from 0x%x len is %d,No.%d!", addr,len,i);

		GTP_DEBUG_ARRAY(&msg[0], len);
		  GTP_INFO("gup_get_ic_msg-->1 Read data from 0x%x len is %d,No.%d!", addr,len,i);
            break;
        }
    }
...
{
	//int irq_gpio = goodix_ts->ts_platform_data->irq_gpio;
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        if(update_from_sd == true)
        {
            GTP_INFO("Need enter update for auto test");
            update_from_sd=false;
            return SUCCESS;
        }
...
{
    u8  rd_buf[PACK_SIZE + GTP_ADDR_LENGTH] = { 0 };
    u8  ret = 0;
...
GTP_INFO("1111111111Hold ss51 & dsp confirm SUCCESS 0x%x 0x%x 0x%x 0x%x,,",hold_buf[0],hold_buf[1],hold_buf[2],hold_buf[3]);
#endif
...
s32 gup_update_proc(int type)
{
    ...
    if(FAIL == ret)
    {
       GTP_ERROR("[update_proc]check update file fail.");
			...
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_CHECK_FILE_FAIL;
	 #endif
        goto file_fail;
    }
    ...
    if(FAIL == ret)
    {
	  ...
	  #ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_GET_IC_FW_MSG_FAIL;
	  #endif
        goto file_fail;
    }
    ...
    {
		GTP_ERROR("[update_proc] don't need to update");
		update_ret = SUCCESS;
		goto file_fail;
    }
    ts->enter_update = 1;
    ...
    if(FAIL == ret)
    {
	  ...
	  #ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_ENTER_UPDATE_MODE_FAIL;
	  #endif
         goto update_fail;
    }
    ...
    while(retry++ < 5)
    {
        ...
        if(FAIL == ret)
        {
	     ...
	     #ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_DSP_ISP_FAIL;
	     #endif
            continue;
        }
	 ...
	 ret = gup_burn_fw_gwake();
		...
		#ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_SS51_ISP_FAIL;
		#endif
            continue;
        }
		...
		#ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_DSP_FW_FAIL;
		#endif
            continue;
        }
		...
		#ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_FW_BOOT_FAIL;
		#endif
            continue;
        }
        show_len = 60;
		...
		#ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_FW_BOOT_ISP_FAIL;
		#endif
            continue;
        }
		...
		#ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_FW_LINK_FAIL;
		#endif
            continue;
        }
		...
		#ifdef CONFIG_HUAWEI_DSM
			goodix_ts->goodix_device_data->ts_platform_data->dsm_info.constraints_UPDATE_status = GOODIX_BURN_FW_FINISH_FAIL;
		#endif
            continue;
        }
        show_len = 90;
