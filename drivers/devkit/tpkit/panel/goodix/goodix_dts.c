	...
	struct goodix_ts_data *goodix_pdata)
{
    ...
    /* get tp color flag */
	ret = of_property_read_u32(np,  "support_get_tp_color", &goodix_pdata->support_get_tp_color);
	if (ret) {
		TS_LOG_INFO("%s, get device support get tp color failed, will use default value: 0 \n ", __func__);
		goodix_pdata->support_get_tp_color = 0; //default 0: no need know tp color
	}
	TS_LOG_INFO("%s, support get tp color = %d \n", __func__, goodix_pdata->support_get_tp_color);

    gesture_info = &(goodix_pdata->ts_platform_data->feature_info.wakeup_gesture_enable_info);
    ret = of_property_read_u32(np, GTP_GESTURE_SUPPORTED, &dts_val);
    if (ret) {
        gesture_info->switch_value = 0;
        TS_LOG_ERR("gtp get gesture_supported failed, used default value : 0\n");
    } else {
        gesture_info->switch_value = (u8)dts_val;
        TS_LOG_INFO("gtp gesture_supported = %d\n", gesture_info->switch_value);
    }
