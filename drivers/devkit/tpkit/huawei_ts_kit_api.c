...
void ts_report_input(struct ts_cmd_node* in_cmd, struct ts_cmd_node* out_cmd)
{
    ...
    struct ts_proximity_info *ps_info = &g_ts_kit_platform_data.feature_info.proximity_info;
    int finger_num = 0;
    int id = 0;
...
#if defined(TS_KIT_PROXIMITY_SUPPORT)
    if ((ps_info->proximity_enable) && (ps_info->proximity_status != 0xFF)) {
        wake_lock_timeout(&g_ts_kit_platform_data.ts_wake_lock, HZ);
        ps_report_interrupt_data(ps_info->proximity_status);
        TS_LOG_INFO("ts proximity status = %d\n", ps_info->proximity_status);
    }
#endif
                     ...
                     struct ts_cmd_node* in_cmd, struct ts_cmd_node* out_cmd)
{
    ...
    struct ts_kit_device_data* dev = g_ts_kit_platform_data.chip_data;
    static enum ts_pm_type last_pm_type = TS_STATE_UNDEFINE;
   if (TS_RESUMING==atomic_read(&g_ts_kit_platform_data.state) && (TS_AFTER_RESUME !=pm_type))
   {
        ...
        switch (pm_type)
        {
            case TS_BEFORE_SUSPEND: 	/*do something before suspend*/
                if(!(g_ts_kit_platform_data.chip_data->is_parade_solution == 1
                    ...
                    if (0 == g_ts_kit_platform_data.feature_info.proximity_info.proximity_enable) {
                        disable_irq(irq_id);
                    }
                }
            ...
            case TS_RESUME_DEVICE:	 /*device power on or wakeup*/
                ...
                last_pm_type = TS_STATE_UNDEFINE;
                atomic_set(&g_ts_kit_platform_data.state, TS_RESUMING);
                break;
            case TS_AFTER_RESUME:		/*do something after resume*/
                ...
                if (last_pm_type != TS_STATE_UNDEFINE)
                {
                    ts_kit_power_control_notify(last_pm_type, NO_SYNC_TIMEOUT);
                    last_pm_type = TS_STATE_UNDEFINE;
                }
                break;
            default:
...
extern void ts_firmware_update_again(void);
int ts_fw_update_boot(struct ts_cmd_node* in_cmd, struct ts_cmd_node* out_cmd)
{
    ...
    if (dev->ops->chip_fw_update_boot)
    {
        ...
        if(error < 0)
        {
            TS_LOG_ERR("ZSK:FW update fail ,will update again");
            if(update_count < 6)
            {
                update_count ++;
                ts_firmware_update_again();
                TS_LOG_ERR(" %s try update time is  %d!!\n",__func__ ,update_count);
            }
        }
     }
    ts_start_wd_timer(&g_ts_kit_platform_data);
...
{
    if (!cd || !cd->chip_data ||!cd->chip_data->need_wd_check_status){
        TS_LOG_ERR("%s : the pointer is null or no need to check the status by watch dog\n", __func__);
        return;
    }
...
bool ts_cmd_need_process(struct ts_cmd_node* cmd)
{
    ...
    {
        if ((atomic_read(&g_ts_kit_platform_data.state) == TS_SLEEP)||(atomic_read(&g_ts_kit_platform_data.state) == TS_RESUMING))
        {
            switch (cmd->command)
            {
 ...
 int ts_get_calibration_info(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	if (error)
	{
		...
		if (!dsm_client_ocuppy(ts_dclient)) {
			...
			dsm_client_record(ts_dclient, "tp calibration crc is :%d\n", info->calibration_crc);
			dsm_client_notify(ts_dclient, DSM_TP_CALIBRATION_CRC_ERROR_NO);
		}
		#endif
	}
...
int ts_get_tp_module_name(char *name, u8 size)
{
    int ret = 0;
    u8 len = 0;

    // get tp module name
    if (NULL == g_ts_kit_device_data) {
        TS_LOG_ERR("none any chip registered success\n");
        return -EINVAL;
    }
    len = size < MAX_STR_LEN-1 ? size : MAX_STR_LEN-1;
    strncpy(name, g_ts_kit_device_data->module_name, len);
    return ret;
}
/*lint -restore*/
