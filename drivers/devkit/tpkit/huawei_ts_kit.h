...
#if defined(CONFIG_SW_BRAND_HUAWEI)
	#define SW_BRAND "HUAWEI"
#else
#if defined(CONFIG_SW_BRAND_HONOR)
	#define SW_BRAND "HONOR"
#else
	#define SW_BRAND "NOT_DEFINED"
#endif
#endif
...
#define LOGO_HUAWEI 0xaa
#define LOGO_HONOR 0x55
...
struct ts_proximity_info
{
    u8 proximity_enable;
    u8 proximity_status;   // 0: close 1: faraway
};
...
struct ts_feature_info
{
    ...
    struct ts_proximity_info proximity_info;
};
...
struct ts_device_ops
{
    ...
    int (*chip_proximity_enable)(int en, struct ts_proximity_info *info);
};
...
struct ts_kit_device_data
{
    ...
    char tp_test_type[TS_CAP_TEST_TYPE_LEN];
    char ts_logo_info[LOGO_INFO_LENGTH];
...
extern struct ts_kit_device_data *g_ts_kit_device_data;
