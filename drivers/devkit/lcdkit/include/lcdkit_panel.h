...
/*  Moved [sence_array] and [cmd_array] definition to lcdkit_panel.c file
*	for compiling report "error: 'cmd_array' defined but not used" */
...
struct lcdkit_panel_infos
{
	...
	int cabc_mode;
	char* model_name;
	unsigned char reg_for_check_lcm;
	int pram_num_lcm;
	unsigned char pram_expect_lcm[5];
	unsigned int panel_status_cmds;
	unsigned int inversion_mode_cmds;
	unsigned int scan_mode_cmds;
	unsigned char scan_mode_normal_data;
	unsigned char scan_mode_reverse_data;
...
struct lcdkit_panel_data
{
	...
	ssize_t (*lcdkit_panel_status_show)(char* buf);
...
#define HOST_OEM_CHIP_TYPE_RESERVED 6
#define HOST_OEM_CHIP_TYPE_LEN_RESERVED 5
