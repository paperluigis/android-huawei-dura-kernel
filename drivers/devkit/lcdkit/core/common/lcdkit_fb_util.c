...
/*function:  this function is used to get panel status
 *input:
 *@pdata: this void point is used to converte to fb data struct.
 *output:
 *@buf: get the panel status
*/
static ssize_t lcdkit_lcd_panel_status_show(struct device* dev,
        struct device_attribute* attr, char* buf)
{
    ssize_t ret = 0;
    struct lcdkit_panel_data* lcdkit_info;
    ...
    if (lcdkit_info->lcdkit_panel_status_show)
    {
        ret = lcdkit_info->lcdkit_panel_status_show(buf);
    }
        ...
        struct device_attribute* attr, const char* buf, size_t count)
{
    ...
    ret =  lcdkit_info->lcdkit_voltage_enable_store(NULL,  buf);
    return count;
}
        ...
        struct device_attribute* attr, char* buf)
{
    ...
    if (lcdkit_info->lcdkit_pcd_errflag_check && lcdkit_info->panel_infos.pcd_errflag_check_support)
    {
...
static ssize_t lcdkit_alpm_function_show(struct device* dev,
        struct device_attribute* attr, char* buf)
{
    return alpm_function_show(dev, attr, buf);
}
...
{
    ssize_t ret = 0;
    ret = alpm_function_store(dev, attr, buf, count);
    return count;
}
...
static DEVICE_ATTR(panel_status, 0600, lcdkit_lcd_panel_status_show, NULL);
