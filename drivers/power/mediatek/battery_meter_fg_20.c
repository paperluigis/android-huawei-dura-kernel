...
#define DUAL_85_TEMP_HIGH   59
#define DUAL_85_TEMP_LOW   11
#endif
...
extern int ontim_charger_hz_onoff;
...
void fgauge_get_profile_id(void)
{
	...
	if ((sizeof(g_battery_id_voltage) / sizeof(struct battery_id_voltage)) != TOTAL_BATTERY_NUMBER) {
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if (id_volt > g_battery_id_voltage[id].id_voltage_low&&id_volt < g_battery_id_voltage[id].id_voltage_high) {
			g_fg_battery_id = id;
			strncpy(battery_vendor_name,g_battery_id_voltage[id].vendor_name,20);
			break;
	...
	bm_info("[fgauge_get_profile_id]Battery id (%d) vendor_name=%s\n", g_fg_battery_id,battery_vendor_name);
}
...
#endif
#endif
int ontim_get_capacity_by_voltage(int voltage)
{
    int i = 0;

    if (batt_meter_table_cust_data.battery_profile_t2_size != 0)
    {
        for (i = 0; i < batt_meter_table_cust_data.battery_profile_t2_size - 1; i++)
        {
            if ( voltage >= batt_meter_table_cust_data.battery_profile_t2[i].voltage)
            {
                return 100 - batt_meter_table_cust_data.battery_profile_t2[i].percentage;
            }
        }
    }
...
int __batt_meter_init_cust_data_from_cust_header(struct platform_device *dev)
{
	...
	batt_meter_table_cust_data.battery_profile_t0_size = battery_profile_t0_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.battery_profile_t0,
        battery_profile_t0[g_fg_battery_id],
        battery_profile_t0_size[g_fg_battery_id]*sizeof(BATTERY_PROFILE_STRUCT));

    batt_meter_table_cust_data.battery_profile_t1_size = battery_profile_t1_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.battery_profile_t1,
        battery_profile_t1[g_fg_battery_id],
        battery_profile_t1_size[g_fg_battery_id]*sizeof(BATTERY_PROFILE_STRUCT));

    batt_meter_table_cust_data.battery_profile_t2_size = battery_profile_t2_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.battery_profile_t2,
        battery_profile_t2[g_fg_battery_id],
        battery_profile_t2_size[g_fg_battery_id]*sizeof(BATTERY_PROFILE_STRUCT));

    batt_meter_table_cust_data.battery_profile_t3_size = battery_profile_t3_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.battery_profile_t3,
        battery_profile_t3[g_fg_battery_id],
        battery_profile_t3_size[g_fg_battery_id]*sizeof(BATTERY_PROFILE_STRUCT));

    batt_meter_table_cust_data.r_profile_t0_size = r_profile_t0_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.r_profile_t0,
        r_profile_t0[g_fg_battery_id],
        r_profile_t0_size[g_fg_battery_id]*sizeof(R_PROFILE_STRUCT));

    batt_meter_table_cust_data.r_profile_t1_size = r_profile_t1_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.r_profile_t1,
        r_profile_t1[g_fg_battery_id],
        r_profile_t1_size[g_fg_battery_id]*sizeof(R_PROFILE_STRUCT));

    batt_meter_table_cust_data.r_profile_t2_size = r_profile_t2_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.r_profile_t2,
        r_profile_t2[g_fg_battery_id],
        r_profile_t2_size[g_fg_battery_id]*sizeof(R_PROFILE_STRUCT));

    batt_meter_table_cust_data.r_profile_t3_size = r_profile_t3_size[g_fg_battery_id];

    memcpy(&batt_meter_table_cust_data.r_profile_t3,
        r_profile_t3[g_fg_battery_id],
        r_profile_t3_size[g_fg_battery_id]*sizeof(R_PROFILE_STRUCT));
#else
				...
				const char *node_srting, int iSize, BATTERY_PROFILE_STRUCT_P profile_p)
{
	...
	idx = 0;
	bm_err("batt_meter_parse_table: %s, %d\n", node_srting, iSize);
	...
	while (idx < (iSize * 2)) {
		...
		#if 0
		msleep(20);
		bm_err("__batt_meter_parse_table>> %s[%d]: <%d, %d>\n",
				node_srting, (idx/2) - 1, profile_p->percentage, profile_p->voltage);
		#endif
	}
}

int __batt_meter_init_cust_data_from_dt_part(struct platform_device *dev)
{
	...
	if (of_property_read_u32(np, "batt_dts_enable", &val)) {
		bm_err("no battery customer dts and pass\n");
		return 0;
	}

	__batt_meter_parse_node(np, "q_max_pos_50", &batt_meter_cust_data.q_max_pos_50);
	__batt_meter_parse_node(np, "q_max_pos_25", &batt_meter_cust_data.q_max_pos_25);
	__batt_meter_parse_node(np, "q_max_pos_0", &batt_meter_cust_data.q_max_pos_0);
	__batt_meter_parse_node(np, "q_max_neg_10", &batt_meter_cust_data.q_max_neg_10);
	__batt_meter_parse_node(np, "q_max_pos_50_h_current", &batt_meter_cust_data.q_max_pos_50_h_current);
	__batt_meter_parse_node(np, "q_max_pos_25_h_current", &batt_meter_cust_data.q_max_pos_25_h_current);
	__batt_meter_parse_node(np, "q_max_pos_0_h_current", &batt_meter_cust_data.q_max_pos_0_h_current);
	__batt_meter_parse_node(np, "q_max_neg_10_h_current", &batt_meter_cust_data.q_max_neg_10_h_current);
	__batt_meter_parse_node(np, "car_tune_value", &batt_meter_cust_data.car_tune_value);
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
		Batt_Temperature_Table[0][idx / 2].BatteryTemp = addr;
		Batt_Temperature_Table[0][idx / 2].TemperatureR = val;
#else
	...
	__batt_meter_parse_node(np, "temperature_t0",
		&batt_meter_cust_data.temperature_t0);
	__batt_meter_parse_node(np, "battery_profile_t0_num",
	...
	__batt_meter_parse_node(np, "r_profile_t0_num",
			&batt_meter_table_cust_data.r_profile_t0_size);
	...
	__batt_meter_parse_node(np, "temperature_t1",
		&batt_meter_cust_data.temperature_t1);
	__batt_meter_parse_node(np, "battery_profile_t1_num",
	...
	__batt_meter_parse_node(np, "r_profile_t1_num",
			&batt_meter_table_cust_data.r_profile_t1_size);
	...
	__batt_meter_parse_node(np, "temperature_t2",
		&batt_meter_cust_data.temperature_t2);
	__batt_meter_parse_node(np, "battery_profile_t2_num",
	...
	__batt_meter_parse_node(np, "r_profile_t2_num",
			&batt_meter_table_cust_data.r_profile_t2_size);
	...
	__batt_meter_parse_node(np, "temperature_t3",
		&batt_meter_cust_data.temperature_t3);
	__batt_meter_parse_node(np, "battery_profile_t3_num",
	...
	__batt_meter_parse_node(np, "r_profile_t3_num",
			&batt_meter_table_cust_data.r_profile_t3_size);
...
#endif /*modified by luhongjiang */
int batt_meter_init_cust_data(struct platform_device *dev)
{
	/* #ifdef CONFIG_OF */
	/* return __batt_meter_init_cust_data_from_dt(dev); */
	/* #else */
	__batt_meter_init_cust_data_from_cust_header(dev);
#if defined(BATTERY_DTS_SUPPORT) && defined(CONFIG_OF)
	__batt_meter_init_cust_data_from_dt_part(dev);
#endif
	return 0;
	/* #endif */
}
...
int BattThermistorConverTemp(int Res)
{
	...
	BATT_TEMPERATURE *batt_temperature_table = (BATT_TEMPERATURE *)&Batt_Temperature_Table[g_fg_battery_id][0];
...
static int ontim_battery_on_flag = 0;
int get_ontim_battery_on_flag(void)
{
    return ontim_battery_on_flag;
}
void  set_ontim_battery_on_flag(int flag)
{
    ontim_battery_on_flag = flag;
}
int force_get_tbat(kal_bool update)
{
    ...
    if (bat_temperature_volt > 1750)
    {
        printk(KERN_ERR "at %s line %d bat_temperature_volt=%d\n", __func__, __LINE__, bat_temperature_volt);
        set_ontim_battery_on_flag(0);
        bat_temperature_val = 25;
    }
    else
    {
        set_ontim_battery_on_flag(1);
    }
#ifdef CONFIG_ONTIM_DUAL_85_TEST
    if(bat_temperature_val > DUAL_85_TEMP_HIGH)
    {
        bat_temperature_val = DUAL_85_TEMP_HIGH;
    }
    if(bat_temperature_val < DUAL_85_TEMP_LOW)
    {
        bat_temperature_val = DUAL_85_TEMP_LOW;
    }
#endif
...
kal_bool battery_meter_get_battery_current_sign(void)
{
    ...
    if (val)
    {
        if ((ontim_charger_hz_onoff == 1) || (cmd_discharging == 1) || BMT_status.charger_exist == KAL_FALSE)
            val = 0;
    }
	return val;
}
...
extern void pmic_ldo_tref_sw_en(int en_value);
	...
	case FG_DAEMON_CMD_SET_POWEROFF:
		{
			...
			printk(KERN_ERR "[fg_res] FG_DAEMON_CMD_SET_POWEROFF\n");
			kernel_power_off();
#endif
		}
		break;
...
static void nl_data_handler(struct sk_buff *skb)
{
	...
	if(fgd_msg->fgd_ret_data_len >= FGD_NL_MSG_MAX_LEN) {
		bm_notice("fgd_msg->fgd_ret_data_len=%d\n", fgd_msg->fgd_ret_data_len);
		return;
	}
...
char* ontim_get_battery_vendor_name(void)
{
	return battery_vendor_name;
}
#endif
