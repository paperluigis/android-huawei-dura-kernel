...
static int shutdown_event_handler(struct shutdown_controller *sdd)
{
	...
	int vbat = pmic_get_battery_voltage();
	int bat_temp = battery_get_bat_temperature();
	int shutdown_bat_vol = BAT_VOLTAGE_LOW_BOUND;
	now.tv_sec = 0;
	now.tv_nsec = 0;
	duraction.tv_sec = 0;
	duraction.tv_nsec = 0;
	...
	if (sdd->shutdown_status.is_under_shutdown_voltage) {
		...
		if (bat_temp < 0)
		{
			shutdown_bat_vol = BAT_VOLTAGE_LOW_BOUND_LOW_TEMP;
			if (g_fg_battery_id == 0)
				shutdown_bat_vol = BAT_VOLTAGE_LOW_BOUND_LOW_TEMP_XINWANGDA;
		}

		if (sdd->avgvbat < shutdown_bat_vol) {
