...
char battery_vendor_name[50]="NONSTANDARD";
...
int product_version;
EXPORT_SYMBOL(product_version);
#define DUAL85_BATTERY_CAPACITY_MIN 30
#define DUAL85_BATTERY_TEMP_HIGH 49
#define DUAL85_BATTERY_TEMP_LOW 11

#define NEGATIVE -1
#define BATTERY_CURRENT_DIVI 10
#define CAP_DENOM  100
/* ============================================================ */
/* define */
/* ============================================================ */
...
#define BAT_CYCLE_COUNT  5000

#define DURA_BATTERY_TYPICAL_CAPACITY (2920)
#define MAH_TO_UAH (1000)
#define DURA_BATTERY_VOLTAGE_MAX_DESIGN  4400000
#define BAASP_LEARNED_FCC_MAX_VAILED (33000)
#define BAASP_LEARNED_FCC_MIN_VAILED (17500)

#define BAT_MAX_LIMIT_TEMP  60
#define BAT_MAX_ACTUAL_TEMP 50
#define MAX_CONVERT_STR  10
...
extern void charge_ic_dump_register(char *buf);/**/
...
static int Enable_BATDRV_LOG = 8;	/* Todo: charging.h use it, should removed */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	int fgcurrent = 0;
	bool b_ischarging = 0;
    char dumpedRegOfChargeIc[REGISTER_BUF_SIZE] = {0};
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		{
			if(data->BAT_CAPACITY < DUAL85_BATTERY_CAPACITY_MIN)
			{
				val->intval = DUAL85_BATTERY_CAPACITY_MIN;
			}
		}
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = DURA_BATTERY_TYPICAL_CAPACITY * MAH_TO_UAH;
		break;
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = FG_status.ui_soc*bat_cap/CAP_DENOM;
	...
	case POWER_SUPPLY_PROP_brand:
		val->strval = battery_vendor_name;
		break;
#ifdef CONFIG_HUAWEI_CHARGER
...
/*POWER_SUPPLY_PROP_TEMP same as POWER_SUPPLY_PROP_batt_temp,for huawei_charger*/
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		val->intval = DURA_BATTERY_VOLTAGE_MAX_DESIGN;
		break;
#ifdef CONFIG_HUAWEI_CHARGER
	case POWER_SUPPLY_PROP_REGISTER_HEAD:
		/* add the charger headinfo,the head is chars */
        memset((char *)(val->strval), 0, CHARGELOG_SIZE);
		snprintf((char *)(val->strval), MAX_SIZE, "batt_soc full_design charge_full reg0 reg1 reg2 reg4 reg5 reg6 systemp_level    bch_en    icur_max    ");
		break;
	case POWER_SUPPLY_PROP_DUMP_REGISTER:
		/* add the charger headinfo,the head is chars */
        memset(dumpedRegOfChargeIc, 0, REGISTER_BUF_SIZE);
        charge_ic_dump_register(dumpedRegOfChargeIc);
        memset((char *)(val->strval), 0, CHARGELOG_SIZE);
		snprintf((char *)(val->strval), MAX_SIZE, "%d      %d      NULL    %s  NULL        NULL    NULL    ", FG_status.soc*bat_cap/CAP_DENOM, bat_cap, dumpedRegOfChargeIc);
		break;
#endif
...
static char proc_log[MAX_BAT_PROCLOG_LEN];
...
#if 0
struct pinctrl *pinctrlVolt;
struct pinctrl_state *Volt_output0,*Volt_output1;

static int mt_Volt_get_gpio_info(struct platform_device *pdev)
{
    int ret;
    if(pdev == NULL)
    {
    ...
    pinctrlVolt = devm_pinctrl_get(&pdev->dev);
    if(IS_ERR(pinctrlVolt))
    {
        ret = PTR_ERR(pinctrlVolt);
        dev_err(&pdev->dev,"Cannot find Volt_Cotrol\n");
        return ret;
    }
    Volt_output0 = pinctrl_lookup_state(pinctrlVolt, "Volt_output0");
    if(IS_ERR(Volt_output0))
    {
        ret = PTR_ERR(Volt_output0);
        dev_err(&pdev->dev,"Cannot find Volt_output0\n");
        return ret;
    }
    Volt_output1 = pinctrl_lookup_state(pinctrlVolt,"Volt_output1");
    if(IS_ERR(Volt_output1))
    {
        ret = PTR_ERR(Volt_output1);
        dev_err(&pdev->dev,"Cannot find Volt_output1\n");
        return ret;
    }
    return 0;
}
...
int g_fg_battery_id;
int bat_id_volt;
extern struct mutex auxadc_switch_lock;
#define NR_ID_VOLT_READ_CNT 10
#define MAX_ID_VOLT 1800000
static void mt_get_id_volt(int *id_volt)
{
	int id_volt_min = MAX_ID_VOLT;
	int id_volt_max = 0;
	int id_volt_sum = 0;
	int id_volt_tmp;
	int fail_cnt = 0;
	int ret = 0;
	int i;
    mutex_lock(&auxadc_switch_lock);
	for (i = 0; i < NR_ID_VOLT_READ_CNT; i++)
	{
		ret = IMM_GetOneChannelValue_Cali(BATTERY_ID_CHANNEL_NUM, &id_volt_tmp);
	    if (ret != 0)
		{
			bm_err("[mt_get_id_volt]id_volt read fail\n");
			fail_cnt++;
		}
	    else
	    {
	        bm_err("[mt_get_id_volt]id_volt = %d\n", id_volt_tmp);
			if (id_volt_tmp > id_volt_max)
				id_volt_max = id_volt_tmp;
			if (id_volt_tmp < id_volt_min)
				id_volt_min = id_volt_tmp;
			id_volt_sum += id_volt_tmp;
	    }
		mdelay(1);//delay 1ms per loop
	}
	id_volt_sum -= id_volt_min;
	id_volt_sum -= id_volt_max;
	*id_volt = id_volt_sum/(NR_ID_VOLT_READ_CNT - 2 - fail_cnt);
    mutex_unlock(&auxadc_switch_lock);
}
...
void fgauge_get_profile_id(void)
{
	...
	mt_get_id_volt(&id_volt);
	bm_err("[fgauge_get_profile_id]id_volt = %d\n", id_volt);
	bat_id_volt = id_volt;
	if ((sizeof(g_battery_id_voltage) / sizeof(g_battery_id_voltage[0])) != TOTAL_BATTERY_NUMBER) {
	...
	{
		if (id_volt > g_battery_id_voltage[id].id_voltage_low && id_volt < g_battery_id_voltage[id].id_voltage_high) 
		{
			g_fg_battery_id = id;
			strncpy(battery_vendor_name, g_battery_id_voltage[id].vendor_name, strlen(g_battery_id_voltage[id].vendor_name)+1);
			break;
		}
	}
	if (id == TOTAL_BATTERY_NUMBER)
		g_fg_battery_id = 0;

	bm_err("[fgauge_get_profile_id]Battery id (%d)\n", g_fg_battery_id);
}
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	...
	if (!of_property_read_u32(np, "Q_MAX_BATTERY_CAPACITY", &val)) {
		bat_cap = (int)val;
		bm_debug("Get Q_MAX_BATTERY_CAPACITY: %d\n",bat_cap);
	} else {
		bm_err("Get Q_MAX_BATTERY_CAPACITY failed\n");
	}
...
int force_get_tbat(bool update)
{
	...
	if (product_version == 1)
	{
		if (bat_temperature_val > DUAL85_BATTERY_TEMP_HIGH)
			bat_temperature_val = DUAL85_BATTERY_TEMP_HIGH;
		if (bat_temperature_val < DUAL85_BATTERY_TEMP_LOW)
			bat_temperature_val = DUAL85_BATTERY_TEMP_LOW;
	}
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_KERNEL_SOC:
	{
		...
		if (soc_type == 5) {//ALPS03701139 
			if((daemon_soc > BAASP_LEARNED_FCC_MIN_VAILED) && (daemon_soc < BAASP_LEARNED_FCC_MAX_VAILED)){
				bm_err("[fg_res] FG_DAEMON_CMD_SET_KERNEL_SOC qmax_t_aging = %d\n", daemon_soc);
				g_basp_learned_fcc = daemon_soc;
				sysfs_notify(g_sysfs_poll, NULL, "poll_basp_done_event");
			}else{
				bm_err("[fg_res] FG_DAEMON_CMD_SET_KERNEL_SOC get invailed qmax_t_aging = %d\n", daemon_soc);
			}
		}
	}
	break;
	...
	case FG_DAEMON_CMD_DUMP_LOG:
	{
		...
		memset(proc_log, 0, MAX_BAT_PROCLOG_LEN);

		if (strlen(&msg->fgd_data[0]) > MAX_BAT_PROCLOG_LEN)
			strncpy(proc_log, &msg->fgd_data[0], MAX_BAT_PROCLOG_LEN);
		else
...
void fg_drv_update_hw_status(void)
{
	...
	bm_err("car[%d,%ld,%ld,%ld,%ld] vbat:%d T=%d,CT:%d Ibat=%d c:%d %d vbus:%d soc:%d %d gm3:%d %d %d %d bat_id_volt %d car %d\n",
		fg_coulomb, coulomb_plus.end, coulomb_minus.end, soc_plus.end, soc_minus.end,bat_vol,
		battery_get_bat_temperature(),mt_get_charger_type(),curr_sign ? bat_current : NEGATIVE * bat_current,fg_current_state, fg_current, chr_vol,
		battery_get_bat_soc(), battery_get_bat_uisoc(),
		gDisableGM30, fg_cust_data.disable_nafg, ntc_disable_nafg, cmd_disable_nafg,
		bat_id_volt, fg_cust_data.car_tune_value);
					 ...
					 const char *buf, size_t size)
{
	...
	strncpy(desc, buf, len);
	desc[len] = '\0';
	if (buf != NULL && size != 0) {
		bm_debug("battery_temperature_debug buf is %s\n", desc);
		ret=kstrtoint(desc, MAX_CONVERT_STR, &battery_temperature_debug);
		if (ret < 0) {
			bm_err("can't Convert buffer\n");
			return 0;
		}
		if (battery_temperature_debug > BAT_MAX_LIMIT_TEMP)
			battery_temperature_debug = BAT_MAX_ACTUAL_TEMP;
...
extern void battery_wake_up_charger(void *info);
static bool ontim_manul_charger_onoff = false;
bool ontim_get_manul_charger_onoff(void)
{
	return ontim_manul_charger_onoff;
}

void ontim_set_manul_charger_onoff(bool flag )
{
...
{
	bm_err("ontim_manul_charger_onoff = %d\n", ontim_manul_charger_onoff);
	return sprintf(buf, "%d\n", ontim_manul_charger_onoff);
}
				  ...
				  const char *buf, size_t size)
{
	...
	void *info = NULL;
	int charger_onoff = 0;
	bm_debug("[ontim_manul_charger_onoff]\n");
	...
	if (buf != NULL && size != 0)
	{
		bm_err("ontim_manul_charger_onoff buf is %s\n", desc);
		if (pbat_consumer)
		{
			if ((info = pbat_consumer->cm) == NULL)
			{
				bm_err("too early to start manual charger off, charger not ready!\n");
				return 0;
			}
		}
		ret = kstrtoint(desc, MAX_CONVERT_STR, &charger_onoff);
		if (ret < 0)
		{
			bm_err("can't Convert buffer\n");
			return 0;
		}
		ontim_manul_charger_onoff = charger_onoff?true:false;
		if (!ontim_manul_charger_onoff)
			battery_wake_up_charger(info);
		bm_err("ontim_manul_charger_onoff=%d\n", ontim_manul_charger_onoff);
	}
	return size;
}
...
/* CHARGING FULL */
			cycle_count++;
			if(cycle_count > BAT_CYCLE_COUNT)
			{
				cycle_count = 0;
			}
			notify_fg_chr_full();
...
//		pmic_register_interrupt_callback(FG_BAT_PLUGOUT_NO, fg_bat_plugout_int_handler);
	...
	battery_debug_init();
	if (strstr(saved_command_line, "product.type=normal"))
		product_version = 0;
	else
		product_version = 1;
	bm_err("%s %d product_version = %d\n", __func__, __LINE__, product_version);
