...
extern kal_bool set_cv_flag;
#define BAT_CAPACITY_FCC_ONTIM 2900
#define VOL_MAX_ONTIM 4400
#ifdef CONFIG_HUAWEI_CHARGER
#define CHARGELOG_SIZE      2048
#define MAX_SIZE            1024
#define NAME_SIZE           50
static char reg_head[CHARGELOG_SIZE] = { 0 };
extern char* ontim_get_battery_vendor_name(void);
#endif
...
extern void set_dpm_proc(unsigned int vbat,u32 charger_enable);
static int dms_chg_monitor_report_err(int errno);
int bat_update_thread_dsm(void);
#endif

extern int fuel_guage_init(void);
extern int charge_init(void);
extern void mutex_init_of_hw_power_class(void);
...
#endif
#define CHAEGR_FULL 2900
#define CHAEGR_FULL_DESIGN 3000
...
static int debug_set_battery_capacity = -1;
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
int ontim_get_manul_charger_onoff(void);
void ontim_set_manul_charger_onoff(int flag);
...
kal_bool upmu_is_chr_det(void)
{
	...
	unsigned int tmp32;
#endif
       if(ontim_get_manul_charger_onoff()==1)
       {
 	      return KAL_FALSE;
       }
	if (!g_bat_init_flag) {
			    ...
			    enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPE:
		val->intval = BMT_status.charger_type;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = BMT_status.charger_vol;
		break;
	case POWER_SUPPLY_PROP_REGISTER_HEAD:
		memset((char *)(val->strval), 0, CHARGELOG_SIZE);
		/* add the charger headinfo,the head is chars */
		snprintf((char *)(val->strval), MAX_SIZE, "batt_soc	full_design	charge_full\n");
		break;
	case POWER_SUPPLY_PROP_BATTERY_TYPE:
		memset(reg_head, 0, NAME_SIZE);
		/* add the charger headinfo,the head is chars */
		snprintf(reg_head, NAME_SIZE, ontim_get_battery_vendor_name());
		val->strval = reg_head;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		val->intval = BMT_status.ZCV;
		break;
	case POWER_SUPPLY_PROP_DUMP_REGISTER:
		memset((char *)(val->strval), 0, CHARGELOG_SIZE);
		/* add the charger headinfo,the head is chars */
		snprintf((char *)(val->strval), MAX_SIZE, "NULL    NULL    NULL    ");
		break;
#endif
	default:
		ret = -EINVAL;
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
     if (data->BAT_batt_temp > 580)
         val->intval = POWER_SUPPLY_HEALTH_OVERHEAT;
     else if (data->BAT_batt_temp < -190)
	...
	case POWER_SUPPLY_PROP_CAPACITY:
        if (debug_set_battery_capacity == -1 || debug_set_battery_capacity > 100)
        {
        ...
        {
            val->intval = debug_set_battery_capacity;
        }
        break;
#ifdef CONFIG_HUAWEI_CHARGER
/*POWER_SUPPLY_PROP_VOLTAGE_NOW same as POWER_SUPPLY_PROP_batt_vol ,for huawei_charger*/
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
#endif
...
/*POWER_SUPPLY_PROP_TEMP same as POWER_SUPPLY_PROP_batt_temp,for huawei_charger*/
	...
	case POWER_SUPPLY_PROP_InstatVolt:
		...
		break;
#ifdef CONFIG_HUAWEI_CHARGER
/*POWER_SUPPLY_PROP_CURRENT_NOW same as POWER_SUPPLY_PROP_BatteryAverageCurrent,for huawei_charger*/
	...
	case POWER_SUPPLY_PROP_BatterySenseVoltage:
		val->intval = battery_meter_get_battery_voltage(KAL_TRUE);//data->BAT_BatterySenseVoltage;
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		val->intval = CHAEGR_FULL;
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = CHAEGR_FULL_DESIGN;
		break;
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		val->intval = data->CycleCount;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
	...
	case POWER_SUPPLY_PROP_capacity_fcc:
		val->intval = BAT_CAPACITY_FCC_ONTIM;
		break;
	case POWER_SUPPLY_PROP_capacity_rm:
		val->intval = (data->BAT_CAPACITY)*2900/100;
		break;
	case POWER_SUPPLY_PROP_voltage_max:
		val->intval = VOL_MAX_ONTIM;
		break;
#ifdef CONFIG_HUAWEI_CHARGER
	case POWER_SUPPLY_PROP_DUMP_REGISTER:
		memset((char *)(val->strval), 0, CHARGELOG_SIZE);
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		val->intval = (cmd_discharging==1?0:1);
		break;
	case POWER_SUPPLY_PROP_REGISTER_HEAD:
		memset((char *)(val->strval), 0, CHARGELOG_SIZE);
		/* add the charger headinfo,the head is chars */
		snprintf((char *)(val->strval), MAX_SIZE, "systemp_level    bch_en    icur_max    ");
		break;
#endif
	default:
		ret = -EINVAL;
...
static ssize_t show_battery_current(struct device *dev, struct device_attribute *attr, char *buf)
{
    int icurrent,icurrent_sign;
    icurrent = battery_meter_get_battery_current();
	icurrent_sign = battery_meter_get_battery_current_sign();
	icurrent *= (icurrent_sign ? 1 : (-1));
	return sprintf(buf, "%d\n", icurrent/10);
}
static ssize_t store_battery_current(struct device *dev, struct device_attribute *attr,
            const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "[battery_current] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(battery_current, 0664, show_battery_current, store_battery_current);
static ssize_t show_charging_current(struct device *dev, struct device_attribute *attr, char *buf)
{
	int charging_current_temp = 0;
	battery_log(BAT_LOG_CRTI, "BMT_status.ICharging = %d\n", BMT_status.ICharging);
	if (upmu_is_chr_det() == KAL_TRUE) {
		charging_current_temp = battery_meter_get_charging_current();
	}
	return sprintf(buf, "%d\n", BMT_status.ICharging/10);
}

static ssize_t store_charging_current(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "[ICharging_current] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(charging_current, 0664, show_charging_current, store_charging_current);


static int ontim_set_charging_current=0;
int ontim_get_charging_iin_current(void)
{
     return ontim_set_charging_current;
}

static ssize_t show_charging_iin_current(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "BMT_status.IBattery = %d\n", ontim_set_charging_current);
	return sprintf(buf, "%d\n", ontim_set_charging_current);
}

static ssize_t store_charging_iin_current(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	if (buf != NULL && size != 0) {
		battery_log(BAT_LOG_CRTI,"battery_temperature_debug buf is %s\n", buf);
		sscanf(buf, "%d", &ontim_set_charging_current);
		ontim_set_charging_current*=100;
		battery_log(BAT_LOG_CRTI,"battery_temperature_debug=%d\n", ontim_set_charging_current);
	}
	return size;
}
static DEVICE_ATTR(charging_iin_current, 0664, show_charging_iin_current, store_charging_iin_current);

static int ontim_manul_charger_onoff=0;
int ontim_get_manul_charger_onoff(void)
{
     return ontim_manul_charger_onoff;
}

void ontim_set_manul_charger_onoff(int flag )
{
      ontim_manul_charger_onoff=flag;
      return ;
}

static ssize_t show_manul_charger_onoff(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "BMT_status.IBattery = %d\n", ontim_manul_charger_onoff);
	return sprintf(buf, "%d\n", ontim_manul_charger_onoff);
}

static ssize_t store_manul_charger_onoff(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	if (buf != NULL && size != 0) {
		battery_log(BAT_LOG_CRTI,"ontim_manul_charger_onoff buf is %s\n", buf);
		sscanf(buf, "%d", &ontim_manul_charger_onoff);
		battery_log(BAT_LOG_CRTI,"ontim_manul_charger_onoff=%d\n", ontim_manul_charger_onoff);
	}
	return size;
}

static DEVICE_ATTR(manul_charger_onoff, 0664, show_manul_charger_onoff, store_manul_charger_onoff);

int ontim_charger_hz_onoff = -200;
int ontim_get_charger_hz_onoff(void)
{
    return ontim_charger_hz_onoff;
}
void ontim_set_charger_hz_onoff(int flag )
{
    ontim_charger_hz_onoff = flag;
    return ;
}

static ssize_t show_charger_hz_onoff(struct device* dev, struct device_attribute* attr, char* buf)
{
    battery_log(BAT_LOG_CRTI, "ontim_charger_hz_onoff = %d\n", ontim_charger_hz_onoff);
    return sprintf(buf, "%d\n", ontim_charger_hz_onoff);
}

static ssize_t store_charger_hz_onoff(struct device* dev, struct device_attribute* attr,
                                      const char* buf, size_t size)
{
    int  ontim_hz_mode_enable = 0 ;

    if (buf != NULL && size != 0)
    {
        battery_log(BAT_LOG_CRTI, "ontim_charger_hz_onoff buf is %s\n", buf);
        sscanf(buf, "%d", &ontim_charger_hz_onoff);
        battery_log(BAT_LOG_CRTI, "ontim_charger_hz_onoff=%d\n", ontim_charger_hz_onoff);

        if (ontim_charger_hz_onoff == 1)
        {
            ontim_hz_mode_enable = 1;
            battery_charging_control(CHARGING_CMD_SET_HZ_MODE, &ontim_hz_mode_enable);
        }
        else if (ontim_charger_hz_onoff == 0)
        {
            ontim_hz_mode_enable = 0;
            battery_charging_control(CHARGING_CMD_SET_HZ_MODE, &ontim_hz_mode_enable);
        }
...
static DEVICE_ATTR(charger_hz_onoff, 0664, show_charger_hz_onoff, store_charger_hz_onoff);
kal_bool usb_current_set_flag = KAL_FALSE;
static ssize_t chg_usb_current_get(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "[Battery] batt_cust_data.usb_charger_current: %x\n", batt_cust_data.usb_charger_current);

	return sprintf(buf, "%u\n", batt_cust_data.usb_charger_current/100);
}

static ssize_t chg_usb_current_set(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		ret = kstrtouint(buf, 10, &set_current);
		set_current *= 100;
		if ((set_current > 0) && (set_current <= 900*100))
		{
			batt_cust_data.usb_charger_current = set_current;
            usb_current_set_flag = KAL_TRUE;
		}
		else
		{
			batt_cust_data.usb_charger_current = USB_CHARGER_CURRENT;
            usb_current_set_flag = KAL_FALSE;
		}
...
static DEVICE_ATTR(set_usb_current, 0664, chg_usb_current_get, chg_usb_current_set);
...
static ssize_t show_battery_capacity(struct device *dev, struct device_attribute *attr, char *buf)
{
    battery_log(BAT_LOG_CRTI, "debug_set_battery_capacity = %d\n", debug_set_battery_capacity);
    return sprintf(buf, "%d\n", debug_set_battery_capacity);
}
static ssize_t store_battery_capacity(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    if (buf != NULL && size != 0)
    {
        battery_log(BAT_LOG_CRTI,"set battery capacity to %s\n", buf);
        sscanf(buf, "%d", &debug_set_battery_capacity);
        battery_log(BAT_LOG_CRTI,"debug_set_battery_capacity=%d\n", debug_set_battery_capacity);
    }
    return size;
}
static DEVICE_ATTR(debug_battery_capacity, 0664, show_battery_capacity, store_battery_capacity);
int runin_set_cv_flag = -1;
static ssize_t show_runin_cv_value(struct device *dev, struct device_attribute *attr, char *buf)
{
    battery_log(BAT_LOG_CRTI, "runin_set_cv_flag = %d\n", runin_set_cv_flag);
    return sprintf(buf, "%d\n", runin_set_cv_flag);
}
static ssize_t store_runin_cv_value(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    if (buf != NULL && size != 0)
    {
        battery_log(BAT_LOG_CRTI,"runin set cv to %s\n", buf);
        sscanf(buf, "%d", &runin_set_cv_flag);
        battery_log(BAT_LOG_CRTI,"runin_set_cv_flag = %d\n", runin_set_cv_flag);
    }
    return size;
}
static DEVICE_ATTR(runin_test_cv_flag, 0664, show_runin_cv_value, store_runin_cv_value);
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	bat_data->CycleCount = BMT_status.cycle_count;
	/* Dual battery */
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    if(BMT_status.charger_exist == KAL_TRUE)
    {
        if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD
            || (BMT_status.temperature > TEMP_POS_45_THRESHOLD && bat_data->BAT_batt_vol > JEITA_TEMP_POS_45_TO_POS_60_CV_VOLTAGE/1000))
        {
            bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_NOT_CHARGING;
            if (last_bat_vol != bat_data->BAT_batt_vol)
            {
                power_supply_changed(&bat_data->psy);
                last_bat_vol = bat_data->BAT_batt_vol;
            }
        }
    }
	mt_battery_update_EM(bat_data);
    if (cmd_discharging == 1 || ontim_charger_hz_onoff == 1)
    {
...
extern int ontim_get_battery_temperature_debug(void);
...
void mt_battery_GetBatteryData(void)
{
	...
	static unsigned int t_charge_type = CHARGER_UNKNOWN;
	if (batteryIndex == 0xff)
		batteryIndex = 0;

	if((BMT_status.charger_type != CHARGER_UNKNOWN)&&(t_charge_type == CHARGER_UNKNOWN))
		BMT_status.cycle_count++;
	t_charge_type = BMT_status.charger_type;
	if(BMT_status.cycle_count == MAX_CYCLE_COUNT)
		BMT_status.cycle_count = 0;
	...
	{
		BMT_status.temperature = ontim_get_battery_temperature_debug();
	}
	...
	battery_log(BAT_LOG_CRTI,
		"[kernel]AvgVbat %d,bat_vol %d, AvgI %d, I %d, VChr %d, AvgT %d, T %d, ZCV %d, CHR_Type %d, SOC %3d:%3d:%3d, bcct %d:%d, Ichg %d, IBat %d, car_tune_value %d\n",
		...
		BMT_status.IBattery / 10, batt_meter_cust_data.car_tune_value);
}
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	if (do_jeita_state_machine() == PMU_STATUS_FAIL) {
		...
		if(BMT_status.temperature < 0)
			dms_chg_monitor_report_err(ERROR_BATT_TEMP_LOW);
#endif
	}
#else
...
static void mt_battery_CheckBatteryStatus(void)
{
    ...
    {
        battery_log(BAT_LOG_CRTI,
                    "%s line %d cmd_discharging=(%d) ontim_charger_hz_onoff=(%d)\n", __func__, __LINE__,
                    cmd_discharging, ontim_charger_hz_onoff);
        BMT_status.bat_charging_state = CHR_ERROR;
        battery_charging_control(CHARGING_CMD_SET_ERROR_STATE, &cmd_discharging);
        return;
    }
    else if (cmd_discharging == 0 && (ontim_charger_hz_onoff == 0 || ontim_charger_hz_onoff == -200))
    {
        battery_log(BAT_LOG_CRTI,
                    "%s line %d cmd_discharging=(%d) ontim_charger_hz_onoff=(%d)\n", __func__, __LINE__,
                    cmd_discharging, ontim_charger_hz_onoff);
    ...
    if (ontim_charger_hz_onoff  == 0 && (cmd_discharging == 0 || cmd_discharging == -1))
    {
        int val = 0;
        battery_log(BAT_LOG_CRTI,
                    "%s line %d cmd_discharging=(%d) ontim_charger_hz_onoff=(%d)\n", __func__, __LINE__,
                    cmd_discharging, ontim_charger_hz_onoff);
        BMT_status.bat_charging_state = CHR_PRE;
        battery_charging_control(CHARGING_CMD_SET_ERROR_STATE, &val);
        ontim_charger_hz_onoff = -200;
    }
...
kal_bool chrg_type_mistake_flag = KAL_FALSE;
static int check_cnt = 0;
kal_bool mt_get_chrgtype_mistake_flag(void)
{
    return chrg_type_mistake_flag;
}
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
        ...
        if (BMT_status.charger_type == NONSTANDARD_CHARGER)
        {
            chrg_type_mistake_flag = KAL_TRUE;
        }
        else
        {
            chrg_type_mistake_flag = KAL_FALSE;
        }
    ...
    else if (BMT_status.charger_type == NONSTANDARD_CHARGER && check_cnt < 2)
    {
        printk(KERN_ERR "check for non-standard charger!\n");
        battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
        if (chrg_type_mistake_flag && check_cnt == 1)
        {
            chrg_type_mistake_flag = KAL_FALSE;
        }
        BMT_status.charger_type = CHR_Type_num;
...
void do_chrdet_int_task(void)
{
    ...
    ontim_set_manul_charger_onoff(0);
...
static int dms_chg_monitor_report_err(int errno)
{
	...
	int size = 0;
    signed int ICharging,temperature;
	...
	switch (errno)
	{
        case ERROR_BATT_NOT_EXIST:
            dsm_client_record(bq_power_dclient,"No battery exist\n");
            break;
        case ERROR_BATT_TEMP_LOW:
			dsm_client_record(bq_power_dclient,"Battery temperature too low. \n");
            break;
        case ERROR_BATT_VOLT_LOW:
            dsm_client_record(bq_power_dclient,"Battery voltage below 3.3V\n");
            break;
        case ERROR_BATT_VOLT_HIGH:
            dsm_client_record(bq_power_dclient,"Battery voltage over 4.52V\n");
            break;
        case ERROR_BATT_TERMINATE_TOO_EARLY:
            dsm_client_record(bq_power_dclient,"Battery cutoff early\n");
            break;
        case ERROR_BATT_NOT_TERMINATE:
            dsm_client_record(bq_power_dclient,"Battery  cutoff never\n");
            break;
        case ERROR_BATT_BAD_CURR_SENSOR:
            dsm_client_record(bq_power_dclient,"Battery current detect error\n");
            break;
        case ERROR_WATCHDOG_RESET:
            dsm_client_record(bq_power_dclient,"Watchdog over-time\n");
            break;
        case ERROR_CHARGE_CURR_OVERHIGH:
            dsm_client_record(bq_power_dclient,"Charger current > max-current 2A set\n");
            break;
        case ERROR_CHARGE_CURR_ZERO:
            dsm_client_record(bq_power_dclient,"Charger current zero\n");
            break;
        case ERROR_CHARGE_TEMP_FAULT:
            dsm_client_record(bq_power_dclient,"Error temp error but charing not stop\n");
            break;
        case ERROR_CHARGE_VBAT_OVP:
            dsm_client_record(bq_power_dclient,"Charger OVP happen\n");
            break;
        default:
            break;
    }
    size = dsm_client_record(bq_power_dclient, "[dsm]AvgVbat %d, charger_exit %d, I %d, VChr %d, AvgT %d, T %d, ZCV %d, CHR_Type %d, SOC %d:%d:%d, bcct %d:%d, Ichg %d, IBat %d\n",
        BMT_status.bat_vol, BMT_status.charger_exist, ICharging,
        BMT_status.charger_vol, BMT_status.temperature, temperature, BMT_status.ZCV,
        BMT_status.charger_type, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.UI_SOC2,
        g_bcct_flag, get_usb_current_unlimited(), get_bat_charging_current_level() / 100,
...
int bat_update_thread_dsm(void)
{
    unsigned int i_charge;
    i_charge = get_bat_charging_current_level() / 100;
	if(!BMT_status.bat_exist)
        dms_chg_monitor_report_err(ERROR_BATT_NOT_EXIST);
    if(BMT_status.bat_vol < 3300)
    {
        dms_chg_monitor_report_err(ERROR_BATT_VOLT_LOW);
    }
    if(BMT_status.bat_vol > 4520)
    {
        dms_chg_monitor_report_err(ERROR_BATT_VOLT_HIGH);
    }
    if((BMT_status.bat_vol > 4520) && (BMT_status.charger_exist == KAL_TRUE))
    {
        dms_chg_monitor_report_err(ERROR_BATT_NOT_TERMINATE);
    }
    if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.charger_type != CHARGER_UNKNOWN))
    {
        if((BMT_status.bat_charging_state != CHR_ERROR) && (i_charge == 0))
        {
            dms_chg_monitor_report_err(ERROR_CHARGE_CURR_ZERO);
        }
        if (BMT_status.charger_vol > 6500) // > 6V OVP occur
        {
           dms_chg_monitor_report_err(ERROR_CHARGE_VBAT_OVP);
        }
        if((BMT_status.temperature < 0)||(BMT_status.temperature > 60))
        {
            dms_chg_monitor_report_err(ERROR_CHARGE_TEMP_FAULT);
        }
    }
   return 0;
}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_battery_current);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_charging_current);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_charging_iin_current);
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_manul_charger_onoff);
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_charger_hz_onoff);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_set_usb_current);
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_debug_battery_capacity);
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_runin_test_cv_flag);
	}
...
static ssize_t current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &cmd_current_unlimited, &cmd_discharging) == 2) {
		...
		if (cmd_discharging == 1) {
            ...
            if (ontim_charger_hz_onoff == 0 || ontim_charger_hz_onoff == -200)
            {
...
static int __init battery_init(void)
{
	...
	if (ret)
		battery_log(BAT_LOG_CRTI, "[%s] failed to register PM notifier %d\n", __func__,
			    ret);
#ifdef	CONFIG_HUAWEI_CHARGER
	mutex_init_of_hw_power_class();
	if( charge_init() || fuel_guage_init() )
	{
		pr_err("fuel_guage_init or charge_init failed\n");
	}
#endif
