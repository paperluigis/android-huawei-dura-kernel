...
int ontim_charger_hz_onoff;
int ontim_set_charging_current = -1;

bool psydo_usb_flag = false;
#define BASP_CONSTANT_VOLTAGE_INVAILED -1
#define BASP_MAX_CURRENT_INVAILED -1
int g_basp_aging_cv = BASP_CONSTANT_VOLTAGE_INVAILED;
int g_basp_aging_max_current = BASP_MAX_CURRENT_INVAILED;
extern int g_basp_learned_fcc;
extern struct kobject *g_sysfs_poll;

#define BATTERY_CURRENT_DIVI 10
#define BATTERY_CURRENT_MUTI 100
#define CHARGER_FILE_PROPERTY 0664
#define UNLIMITED_INPUT_CURRENT 1050000
#define MA_TO_UA 1000
#define USE_FG_TIMER 1
#define BASP_CHARGE_MAX_CV 4400000
#define BASP_CHARGE_MIN_CV 3500000
#define T3_T4_MAX_VOL  4100

#ifdef  CONFIG_HUAWEI_DSM
#define BAT_VOLT_MIN_LIMINT   3300
#define BAT_VOLT_MAX_LIMINT   4520
#define BAT_TEMP_MAX_LIMINT   50
#define BAT_TEMP_MIN_LIMINT   0
#define VBUS_VOLT_MAX_LIMINT  6200
#define CHG_CUR_MAX_LIMINT  20500
...
static struct dsm_client *battery_dsm_dclient = NULL;
static int dms_chg_monitor_report_err(int errno);
int bat_update_thread_dsm(void);
#endif

extern void set_dpm_proc(struct charger_device *charger_dev, unsigned int vbat, bool charger_enable);

struct charger_manager *get_charger_manager_info(void)
{
    return pinfo;
}
...
void battery_wake_up_charger(void *info)
{
	struct charger_manager *chrg_info = (struct charger_manager *)info;
	psydo_usb_flag = true;
	_wake_up_charger(chrg_info);
	...
	int idx, int input_current)
{
	...
	if (info != NULL) {
		...
		if (chr_type == CHARGER_UNKNOWN || info->cmd_discharging == true || ontim_charger_hz_onoff == 1)
			return 0;
		...
		if (input_current != -1)
			...
			bcct_input_flag = 0;
		chr_err("%s: dev:%s idx:%d bcct_current_input:%d\n", __func__, dev_name(consumer->dev),
		idx, input_current);
	...
	int idx, int charging_current)
{
	...
	if (info != NULL) {
		...
		if (charging_current != -1)
			bcct_flag = 1;
		else
			bcct_flag = 0;
		chr_err("jeita %s: dev:%s idx:%d bcct_current:%d\n", __func__, dev_name(consumer->dev),
...
static int mtk_charger_plug_in(struct charger_manager *info, CHARGER_TYPE chr_type)
{
	...
	/*notify basp thread start charging*/
	if(NULL != g_sysfs_poll){
		sysfs_notify(g_sysfs_poll, NULL, "poll_charge_start_event");
		chr_err("[basp]mtk_switch_chr_full call sysfs_notify!\n");
	}else{
		chr_err("[basp]g_sysfs_poll = null can not mtk_switch_chr_full call sysfs_notify!\n");
	}

	if (info->plug_in != NULL)
		info->plug_in(info);
	if (psydo_usb_flag && (chr_type == STANDARD_HOST || chr_type == CHARGING_HOST))
		mt_usb_connect();
	charger_dev_plug_in(info->chg1_dev);
	return 0;
}
extern bool switch_cv_flag;
static int mtk_charger_plug_out(struct charger_manager *info)
{
	...
	set_dpm_proc(info->chg1_dev, battery_get_bat_voltage(), false);
	if (info->plug_out != NULL)
		info->plug_out(info);
	if (ontim_get_manul_charger_onoff())
		mt_usb_disconnect();
...
static void mtk_battery_notify_VBatTemp_check(struct charger_manager *info)
{
	...
	if((sw_jeita->sm == TEMP_T3_TO_T4)&&(info->bat_vol >= T3_T4_MAX_VOL)&&(mtk_is_charger_on(info)==true))
	{
		info->notify_code |= 0x0040;
		chr_err("[BATTERY] bat_temp(%d) Pressure limiting current limit\n",
			info->battery_temperature);
	}
	if (info->battery_temperature >= info->thermal.max_charge_temperature&&(mtk_is_charger_on(info)==true)) {
	...
	if (info->enable_sw_jeita == true) {
		...
		if (info->battery_temperature < info->thermal.min_charge_temperature&&(mtk_is_charger_on(info)==true)) {
			info->notify_code |= 0x0020;
			chr_err("[BATTERY] bat_temp(%d) out of range(too low)\n",
				info->battery_temperature);
		}
...
static void charger_check_status(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita == true) {
		...
		if (thermal->enable_min_charge_temperature) {
			if (temperature < thermal->min_charge_temperature) {
				...
				if (temperature >= thermal->min_charge_temperature) {
					chr_err("[BATTERY] Battery Temperature above %d, allow charging!!\n\r",
							thermal->min_charge_temperature );
	...
	if (info->cmd_discharging || ontim_charger_hz_onoff == 1)
		charging = false;
...
stop_charging:
	...
	chr_err("bat_temp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) charging_en:%d cmd_discharging:%d ontim_hz_en:%d s:%d ov:%d\n",
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, ontim_charger_hz_onoff, info->safety_timeout,
		info->vbusov_stat);
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		info->bat_vol = battery_get_bat_voltage();
		info->battery_temperature = battery_get_bat_temperature();
	#ifdef  CONFIG_HUAWEI_DSM
		bat_update_thread_dsm();
	#endif
		chr_err("avgVbat=%d,Ibat=%d,I=%d,VChr=%d,T=%d,Soc=%d:%d,CT:%d:%d hv:%d bcct %d:%d\n", info->bat_vol,
...
static ssize_t show_battery_current(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	icurrent = battery_get_bat_current();
	icurrent_sign = battery_get_bat_current_sign();
...
static ssize_t store_battery_current(struct device *dev, struct device_attribute *attr,
         const char *buf, size_t size)
{
	chr_err("[battery_current] Not Support Write Function\n");
	return size;
}
...
static ssize_t show_charging_current(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!buf)
		return 0;
	return sprintf(buf, "%d\n", pmic_get_charging_current());
}

static ssize_t store_charging_current(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	chr_err("[Charging_current] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(charging_current, CHARGER_FILE_PROPERTY, show_charging_current, store_charging_current);

static ssize_t show_charger_hz_onoff(struct device* dev, struct device_attribute* attr, char* buf)
{
	if (!buf)
		return 0;
	chr_err("ontim_charger_hz_onoff = %d\n", ontim_charger_hz_onoff);
	return sprintf(buf, "%d\n", ontim_charger_hz_onoff);
}
                                   ...
                                   const char* buf, size_t size)
{
	if (buf != NULL && size != 0)
	{
		chr_err("ontim_charger_hz_onoff buf is %s\n", buf);
		sscanf(buf, "%d", &ontim_charger_hz_onoff);
		chr_err("ontim_charger_hz_onoff=%d\n", ontim_charger_hz_onoff);
		if (pinfo != NULL && pinfo->chg1_dev != NULL)
			charger_dev_set_hz_mode(pinfo->chg1_dev, ontim_charger_hz_onoff);
		else
			chr_err("%s set failed\n", __func__);
	}
 
	return size;
 }

static DEVICE_ATTR(charger_hz_onoff, CHARGER_FILE_PROPERTY, show_charger_hz_onoff, store_charger_hz_onoff);

static ssize_t show_charging_iin_current(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!buf)
		return 0;
	chr_err("iin_current = %d\n", ontim_set_charging_current);
	return sprintf(buf, "%d\n", ontim_set_charging_current);
}
...
{	
	...
	if (buf != NULL && size != 0) 
	{
		chr_err("iin_current buf is %s\n", buf);
		sscanf(buf, "%d", &ontim_set_charging_current);
		if(ontim_set_charging_current < 0)
		{
			return size;
		}
		if (ontim_set_charging_current == 0 || ontim_set_charging_current == 1)
		{
			pdata->input_current_limit = UNLIMITED_INPUT_CURRENT;
			return size;
		}
		ontim_set_charging_current *= MA_TO_UA;
		chr_err("iin_current = %d\n", ontim_set_charging_current);
		if (ontim_set_charging_current < pdata->input_current_limit)
		{
			pdata->input_current_limit = ontim_set_charging_current;
			charger_dev_set_input_current(pinfo->chg1_dev, pdata->input_current_limit);
		}
	}
	return size;
}
static DEVICE_ATTR(charging_iin_current, CHARGER_FILE_PROPERTY, show_charging_iin_current, store_charging_iin_current);
...
const char *buf, size_t size)
{
	chr_err("[Charger_Type] Not Support Write Function\n");
	return size;
}
static DEVICE_ATTR(Charger_Type, CHARGER_FILE_PROPERTY, show_Charger_Type, store_Charger_Type);

static ssize_t show_basp_set_cv(struct device *dev, struct device_attribute *attr, char *buf)
{	
	if (!buf)
		return 0;
	chr_err("g_basp_aging_cv = %d\n", g_basp_aging_cv);
	return sprintf(buf, "%d\n", g_basp_aging_cv);
}
static ssize_t store_basp_set_cv(struct device *dev, struct device_attribute *attr,
const char *buf, size_t size)
{
	if (buf != NULL && size != 0) 
	{
		sscanf(buf, "%d", &g_basp_aging_cv);
		if (g_basp_aging_cv < BASP_CHARGE_MIN_CV || g_basp_aging_cv > BASP_CHARGE_MAX_CV){
			g_basp_aging_cv = BASP_CONSTANT_VOLTAGE_INVAILED;
		}
			
		chr_err("[basp]set aging_cv to = %d\n", g_basp_aging_cv);
	}
	return size;
}
static DEVICE_ATTR(aging_cv, CHARGER_FILE_PROPERTY, show_basp_set_cv, store_basp_set_cv);
static ssize_t show_basp_set_max_current(struct device *dev, struct device_attribute *attr, char *buf)
{	
	if (!buf)
		return 0;
	chr_err("aging_max_current = %d\n", g_basp_aging_max_current);
	return sprintf(buf, "%d\n", g_basp_aging_max_current);
}
static ssize_t store_basp_set_max_current(struct device *dev, struct device_attribute *attr,
const char *buf, size_t size)
{
	if (buf != NULL && size != 0) 
	{
		sscanf(buf, "%d", &g_basp_aging_max_current);
		chr_err("set aging_max_current to = %d\n", g_basp_aging_max_current);
	}
	return size;
}
static DEVICE_ATTR(aging_current, CHARGER_FILE_PROPERTY, show_basp_set_max_current, store_basp_set_max_current);
/* procfs */
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	if (ret)
		goto _out;
	ret = device_create_file(&(pdev->dev), &dev_attr_battery_current);
	ret = device_create_file(&(pdev->dev), &dev_attr_charging_current);
	ret = device_create_file(&(pdev->dev), &dev_attr_charger_hz_onoff);
	ret = device_create_file(&(pdev->dev), &dev_attr_charging_iin_current);
	ret = device_create_file(&(pdev->dev), &dev_attr_Charger_Type);
	ret = device_create_file(&(pdev->dev), &dev_attr_aging_current);
	ret = device_create_file(&(pdev->dev), &dev_attr_aging_cv);
...
static int dms_chg_monitor_report_err(int errno)
{
	...
	if(dsm_client_ocuppy(battery_dsm_dclient)){
		/* buffer is busy */
		chr_err("%s: buffer is busy!\n", __func__);
		return -EBUSY;
	}
	switch (errno)
	{
		case ERROR_BATT_NOT_EXIST:
			dsm_client_record(battery_dsm_dclient,"No battery exist\n");
			break;
		case ERROR_BATT_TEMP_LOW:
			dsm_client_record(battery_dsm_dclient,"Battery temperature too low. \n");
			break;
		case ERROR_BATT_VOLT_LOW:
			dsm_client_record(battery_dsm_dclient,"Battery voltage below 3.3V\n");
			break;
		case ERROR_BATT_VOLT_HIGH:
			dsm_client_record(battery_dsm_dclient,"Battery voltage over 4.52V\n");
			break;
		case ERROR_BATT_TERMINATE_TOO_EARLY:
			dsm_client_record(battery_dsm_dclient,"Battery cutoff early\n");
			break;
		case ERROR_BATT_NOT_TERMINATE:
			dsm_client_record(battery_dsm_dclient,"Battery  cutoff never\n");
			break;
		case ERROR_BATT_BAD_CURR_SENSOR:
			dsm_client_record(battery_dsm_dclient,"Battery current detect error\n");
			break;
		case ERROR_WATCHDOG_RESET:
			dsm_client_record(battery_dsm_dclient,"Watchdog over-time\n");
			break;
		case ERROR_CHARGE_CURR_OVERHIGH:
			dsm_client_record(battery_dsm_dclient,"Charger current > max-current 2A set\n");
			break;
		case ERROR_CHARGE_CURR_ZERO:
			dsm_client_record(battery_dsm_dclient,"Charger current zero\n");
			break;
		case ERROR_CHARGE_TEMP_FAULT:
			dsm_client_record(battery_dsm_dclient,"Error temp error but charing not stop\n");
			break;
		case ERROR_CHARGE_VBAT_OVP:
			dsm_client_record(battery_dsm_dclient,"Charger OVP happen\n");
			break;
		default:
			break;
	}
	size = dsm_client_record(battery_dsm_dclient,
	"VBUS:%d, VBAT:%d, capacity:%d, present:%d,,batt_temp:%d\n",battery_get_vbus(),
	pinfo->bat_vol, battery_get_bat_uisoc(), pmic_is_battery_exist(), pinfo->battery_temperature);

	/*if device is not probe successfully or client is null, don't notify dsm work func*/
	dsm_client_notify(battery_dsm_dclient, errno);
...
int bat_update_thread_dsm(void)
{
	...
	if(pinfo->bat_vol < BAT_VOLT_MIN_LIMINT)
	{
		dms_chg_monitor_report_err(ERROR_BATT_VOLT_LOW);
	}
	if(pinfo->bat_vol> BAT_VOLT_MAX_LIMINT)
	{
		dms_chg_monitor_report_err(ERROR_BATT_VOLT_HIGH);
	}
	if((pinfo->bat_vol > BAT_VOLT_MAX_LIMINT) && (ChgType!= CHARGER_UNKNOWN))
	{
		dms_chg_monitor_report_err(ERROR_BATT_NOT_TERMINATE);
	}
	if ((ChgType != CHARGER_UNKNOWN) && (vbus_volt> VBUS_VOLT_MAX_LIMINT)) // > 6V OVP  occur
	{
		dms_chg_monitor_report_err(ERROR_CHARGE_VBAT_OVP);
	}
	if((ChgType!= CHARGER_UNKNOWN) &&((pinfo->battery_temperature < BAT_TEMP_MIN_LIMINT)||(pinfo->battery_temperature > BAT_TEMP_MAX_LIMINT)))
	{
		dms_chg_monitor_report_err(ERROR_CHARGE_TEMP_FAULT);
	}
	if((ChgType!= CHARGER_UNKNOWN) && (abs(chg_current) > CHG_CUR_MAX_LIMINT)) //Current can't > 2000mA
	{
		dms_chg_monitor_report_err(ERROR_CHARGE_CURR_OVERHIGH);
	}
	return 0;
}
#endif
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	charger_ftm_init();
#ifdef  CONFIG_HUAWEI_DSM
	battery_dsm_dclient = dsm_register_client(&dsm_mt_charger);
	if(!battery_dsm_dclient)
	{
		chr_err("%s :register dsm_dclient failed!!, line = %d\n",__func__,__LINE__);
	}
#endif
	mutex_lock(&consumer_mutex);
	list_for_each(pos, phead) {
...
{
#ifdef  CONFIG_HUAWEI_DSM
	dsm_unregister_client(battery_dsm_dclient,&dsm_mt_charger);
#endif
	return 0;
}
