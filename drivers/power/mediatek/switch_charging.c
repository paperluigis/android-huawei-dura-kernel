...
extern int runin_set_cv_flag;
extern kal_bool usb_current_set_flag;
extern int ontim_charger_hz_onoff;
extern int cmd_discharging;
#define minx(x, y) ((x)>(y)?(y):(x))
kal_bool set_cv_flag = KAL_FALSE;
#if (CONFIG_MTK_GAUGE_VERSION == 20)
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
BATTERY_VOLTAGE_ENUM g_cv_voltage = JEITA_TEMP_POS_10_TO_POS_45_CV_VOLTAGE;
#else
...
int mtk_get_dynamic_cv(unsigned int *cv)
{
	...
	if (!g_enable_dynamic_cv) {
		if (batt_cust_data.high_battery_voltage_support)
			_cv = JEITA_TEMP_POS_10_TO_POS_45_CV_VOLTAGE / 1000;
		else
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (g_temp_status != TEMP_POS_10_TO_POS_45) {
        ...
        if (runin_set_cv_flag != -1)
        {
            min_cv = minx(cv_voltage, RUNIN_CV_VALUE);
            battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, (void *)&min_cv);
        }
        else
        {
            battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, (void *)&cv_voltage);
        }
...
static void set_jeita_charging_current(void)
{
	...
	if ((g_temp_status == TEMP_NEG_10_TO_POS_0) ||(g_temp_status == TEMP_ABOVE_POS_60)) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY1] JEITA set charging current : %d\r\n",
			    g_temp_CC_value);
	}
	else
	{
		if ((g_temp_status == TEMP_POS_0_TO_POS_10) ||(g_temp_status == TEMP_POS_45_TO_POS_60)){
			if (g_temp_CC_value > CHARGE_CURRENT_900_00_MA)
			{
				...
				battery_log(BAT_LOG_CRTI, "[BATTERY2] JEITA set charging current : %d; g_temp_status is %d \r\n",
...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	if (current_limit != -1) {
		...
		g_bcct_flag = 0;
    if ((ontim_charger_hz_onoff == 1) || (cmd_discharging == 1))
    {
        mutex_unlock(&g_ichg_access_mutex);
        return g_bcct_flag;
    }
    if (BMT_status.charger_exist == KAL_FALSE || BMT_status.charger_type == CHARGER_UNKNOWN)
    {
        mutex_unlock(&g_ichg_access_mutex);
        return g_bcct_flag;
    }
...
static unsigned int charging_full_check(void)
{
	...
	if (status == KAL_TRUE) {
#if  defined(ONTIM_RECHARGING_BY_SOC)
		if(BMT_status.charger_exist == KAL_TRUE)
		{
			battery_log(BAT_LOG_CRTI, " charging_full_check, BMT_status.SOC =%d !line:%d\r\n",BMT_status.SOC,__LINE__);
            bat_vol = battery_meter_get_battery_voltage(KAL_TRUE);
            if (bat_vol < 4300 && (g_temp_status == TEMP_POS_45_TO_POS_60 || BMT_status.temperature < 0))
            {
                battery_log(BAT_LOG_CRTI, " charging_full_check, BMT_status.SOC =%d BMT_status.UI_SOC =%d !line:%d\r\n",BMT_status.SOC,BMT_status.UI_SOC,__LINE__);
				g_full_check_count = 0;
				return KAL_FALSE;
            }
            if (runin_set_cv_flag != -1)
                return KAL_FALSE;
			if((BMT_status.SOC<=ONTIM_RECHARGING_BY_SOC) && (BMT_status.UI_SOC == 100))
			{
				battery_log(BAT_LOG_CRTI, " charging_full_check, BMT_status.SOC =%d BMT_status.UI_SOC =%d !line:%d\r\n",BMT_status.SOC,BMT_status.UI_SOC,__LINE__);
				g_full_check_count = 0;
				return KAL_FALSE;
...
extern int ontim_get_charging_iin_current(void);
...
void select_charging_current(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
            ...
            if (mt_get_chrgtype_mistake_flag() == KAL_TRUE)
            {
                g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
...
#endif
  {
          int tmp_charging_current=ontim_get_charging_iin_current();
          if( tmp_charging_current!=0)
          {
              if(tmp_charging_current<g_temp_CC_value&&tmp_charging_current<g_temp_input_CC_value)
              {
                g_temp_input_CC_value = tmp_charging_current;
                g_temp_CC_value = tmp_charging_current;
                printk(KERN_ERR " %s %d iin limit charging current\n",__func__,__LINE__);
              }
          }
  }
...
static void mtk_select_ichg_aicr(void)
{
    ...
    if (g_bcct_flag)
    {
        if (BMT_status.charger_type == STANDARD_HOST && usb_current_set_flag)
        {
            g_bcct_value = CHARGE_CURRENT_900_00_MA/100;//new strategy
        }//up to 900 when usb current set
        else
        {
            if (BMT_status.UI_SOC2 <= 10)
                g_bcct_value = CHARGE_CURRENT_900_00_MA/100;//new strategy
        }
    }
	/* Set Ichg, AICR */
...
static void mtk_select_cv(void)
{
    ...
    if (batt_cust_data.high_battery_voltage_support)
    {
        if (bat_vol < 4395 && set_cv_flag == KAL_FALSE)
            cv_voltage = BATTERY_VOLT_04_400000_V;
        else
        {
            cv_voltage = BATTERY_VOLT_04_380000_V;
            set_cv_flag = KAL_TRUE;
        }
    }
	else
		cv_voltage = BATTERY_VOLT_04_200000_V;
    ...
    {
        battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_min);
    }
#if (CONFIG_MTK_GAUGE_VERSION == 20)
	g_cv_voltage = cv_voltage;
#endif
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if (charging_full_check() == KAL_FALSE) {
		...
		int hz_mode_enable = 1;
		battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
		battery_charging_control(CHARGING_CMD_SET_HZ_MODE, &hz_mode_enable);
		charging_enable = 1;
		battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
