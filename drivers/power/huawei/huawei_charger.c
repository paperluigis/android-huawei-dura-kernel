...
struct dsm_client *dsm_chargemonitor_dclient = NULL;
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!buf)
		return 0;
	if (chip) {
		if ((strict_strtol(buf, STRTOL_DECIMAL_BASE, &val) < 0) || (val < INPUT_EVENT_ID_MIN) || (val > INPUT_EVENT_ID_MAX)) {
			return -EINVAL;
		}
...
static ssize_t get_poll_basp_done_event(struct device *dev,
...
static DEVICE_ATTR(poll_basp_done_event,(S_IWUSR | S_IRUGO),
		   get_poll_basp_done_event, NULL);

static ssize_t get_basp_learned_fcc(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
    if (!dev || !attr || !buf) {
...
static DEVICE_ATTR(basp_learned_fcc,(S_IWUSR | S_IRUGO),
		   get_basp_learned_fcc, NULL);
...
static int charge_event_poll_register(struct device *dev)
{
	...
	ret = sysfs_create_file(&dev->kobj, &dev_attr_poll_basp_done_event.attr);
	if (ret) {
		pr_err("fail to create poll node for %s\n", dev->kobj.name);
		return ret;
	}
	ret = sysfs_create_file(&dev->kobj, &dev_attr_basp_learned_fcc.attr);
	if (ret) {
		pr_err("fail to create poll node for %s\n", dev->kobj.name);
		return ret;
	}
...
static void smb_update_status(struct charge_device_info *di)
{
	...
	if (!di)
		return;
	charging_enabled = get_property_from_psy(di->bms_psy,
...
{
	struct charge_device_info *chip;
	if (!work)
		return;
                                 ...
                                 struct device_attribute *attr, char *buf)
{
	...
	case CHARGE_SYSFS_CHARGELOG:
		...
		online = get_loginfo_int(di->bms_psy, POWER_SUPPLY_PROP_ONLINE);
		in_type = get_loginfo_int(di->bms_psy, POWER_SUPPLY_PROP_CHARGE_TYPE);
		conver_usbtype(in_type, cType);
		usb_vol = get_loginfo_int(di->bms_psy, POWER_SUPPLY_PROP_CHARGE_VOLTAGE_NOW);
		ch_en = get_loginfo_int(di->bms_psy, POWER_SUPPLY_PROP_CHARGING_ENABLED);
		...
		cur = get_loginfo_int(di->bms_psy, POWER_SUPPLY_PROP_CHARGE_CURRENT_NOW);
		capacity = get_loginfo_int(di->batt_psy, POWER_SUPPLY_PROP_CAPACITY);
		ibus = get_loginfo_int(di->bms_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_NOW);
...
static int charge_probe(struct platform_device *pdev)
{
    ...
    np = pdev->dev.of_node;
    ret = of_property_read_string(np, "bms_power_supply",
            &bms_power_supply);

	bms_psy = power_supply_get_by_name(bms_power_supply);
	if (!bms_psy) {
