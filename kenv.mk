...
ifneq ($(strip $(TARGET_NO_KERNEL)),true)
  ...
  ifeq ($(wildcard $(TARGET_PREBUILT_KERNEL)),)
    ...
    KERNEL_CONFIG_FILE_SRC := $(KERNEL_DIR)/arch/$(TARGET_ARCH)/configs/$(KERNEL_DEFCONFIG)
    KERNEL_BOARD_CONFIG_SRC := $(KERNEL_DIR)/arch/$(TARGET_ARCH)/configs/$(KERNEL_BOARD_DEFCONFIG)
    KERNEL_CONFIG_FILE := $(KERNEL_OUT)/arch/$(TARGET_ARCH)/configs/$(KERNEL_DEFCONFIG)

    $(info $(shell mkdir -p $(KERNEL_OUT)/arch/$(TARGET_ARCH)/configs))
    $(info $(shell cp $(KERNEL_CONFIG_FILE_SRC) $(KERNEL_CONFIG_FILE)))
    $(info $(shell echo -e "\n########Append Board Config to Kernel Config##########\n" >>$(KERNEL_CONFIG_FILE) ))
    $(info $(shell cat $(KERNEL_BOARD_CONFIG_SRC) >> $(KERNEL_CONFIG_FILE)))
