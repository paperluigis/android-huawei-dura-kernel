...
static void __init print_sn_masked_cmdline(void)
{
    char *p = NULL;
    static char __initdata sn_masked_boot_command_line[COMMAND_LINE_SIZE] = {0};

    memcpy(sn_masked_boot_command_line, boot_command_line, COMMAND_LINE_SIZE);
    p = strstr(sn_masked_boot_command_line, "androidboot.serialno");
    if (p != NULL)
    {
        p += (20 + 1);/*20 is the len of string "androidboot.serialno" , 1 is for the symbol '='*/
        while (*p != ' ' && *p != '\0')
        {
            *p = '*';
            p++;
...
static int __init_or_module do_one_initcall_debug(initcall_t fn)
{
	...
	printk(KERN_CRIT "calling  %pF @ %i\n", fn, task_pid_nr(current));
	calltime = ktime_get();
	ret = fn();
	rettime = ktime_get();
	delta = ktime_sub(rettime, calltime);
	duration = (unsigned long long) ktime_to_ns(delta) >> 10;
	printk(KERN_CRIT "initcall %pF returned %d after %lld usecs\n",
		 fn, ret, duration);
...
#endif
	TIME_LOG_START();
	if (1)//(initcall_debug) print duration of each initcall temporarily
