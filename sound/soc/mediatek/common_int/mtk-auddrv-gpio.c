...
#define ONTIM_DEFAULT_MODE_RECIVER   8
#define ONTIM_SUPER_MODE_RECIVER   9
extern void get_board_ID(int *board_ID);
...
bool is_SGM3715_board_type(void )
{
	int board_ID = 0,i;
	get_board_ID(&board_ID);
	for(i = 0;i < sizeof(noswitch_board)/sizeof(noswitch_board[0]);i++){
		if(noswitch_board[i] == board_ID)
			return false;
	}
	return true;
}
static int extamp_mode;
void AudDrv_GPIO_Select_Super_Receiver(bool mode)
{
	if(true == is_SGM3715_board_type()) {
		if(true == mode){
			pr_err("%s,do super receiver mode 9!\n",__func__);
			AudDrv_GPIO_Select(GPIO_AUD_SWITCH_EN_HIGH);
			AudDrv_GPIO_Select(GPIO_AUD_RCV_EN_HIGH);
			AudDrv_GPIO_EXTAMP_Select(true,ONTIM_SUPER_MODE_RECIVER);
		}else if(false == mode){
			pr_err("%s ,do  default_mode !\n",__func__);
...
int AudDrv_GPIO_EXTAMP_Select(int bEnable, int mode)
{
	...
	mutex_lock(&gpio_request_mutex);
	if(true == is_SGM3715_board_type()){
		if(ONTIM_DEFAULT_MODE_SPEK == extamp_mode){
			pr_err("%s ,do extamp_mode 3!\n",__func__);
			AudDrv_GPIO_Select(GPIO_AUD_SWITCH_EN_HIGH);
			AudDrv_GPIO_Select(GPIO_AUD_RCV_EN_LOW);
		}else if(ONTIM_DEFAULT_MODE_RECIVER == extamp_mode){
			pr_err("%s ,do extamp_mode 9!\n",__func__);
			extamp_mode = ONTIM_SUPER_MODE_RECIVER;
			AudDrv_GPIO_Select(GPIO_AUD_SWITCH_EN_LOW);
		}
	}
	if (bEnable == 1){
