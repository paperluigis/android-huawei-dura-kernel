...
/*beging added for do super speaker*/
static int Super_Receiver_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	return 0;
}
static int Super_Receiver_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_err("%s :do super receiver mode %ld!\n ", __func__,ucontrol->value.integer.value[0]);
	AudDrv_GPIO_Select_Super_Receiver(ucontrol->value.integer.value[0]);
	return 0;
}
/*end added for do super speaker*/
...
#define ONTIM_DEFAULT_MODE_RECIVER   8
	if (enable) {
		...
		{
			AudDrv_GPIO_EXTAMP_Select(true, ONTIM_DEFAULT_MODE_SPEK);
			pr_debug("%s:mod is 3!\n ", __func__);
		}
#else
	if (enable) {
...
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_ENUM_EXT("Super_Receiver_Switch", Audio_DL_Enum[15],
		     Super_Receiver_Get, Super_Receiver_Set),
};
